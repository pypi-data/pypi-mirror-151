...
from libreflow.baseflow.file import TrackedFile, TrackedFolder
...
class EntityManager(flow.Object):
    '''
    This class manages the entity store and all global
    entity collections of this project.
    '''
...
class Project(flow.Object):
    ...
    films           = flow.Child(Films).ui(expanded=True, action_submenus=True, items_action_submenus=True)
    ...
    def _injection_provider(cls, slot_name, default_type):
        if slot_name == 'libreflow.baseflow.file.SyncStatutes':
            return SyncStatutes
        elif slot_name == 'libreflow.baseflow.file.Revisions':
            return Revisions
        elif slot_name == 'libreflow.baseflow.file.TrackedFolderRevisions':
            return TrackedFolderRevisions
        elif slot_name == 'libreflow.baseflow.file.HistorySyncStatutes':
            return HistorySyncStatutes
    ...
    def get_default_contextual_edits(self, context_name):
        if context_name == 'settings':
            return dict(
                path_format='{film}/{sequence}/{shot}/{department}/{file_mapped_name}/{revision}/{file_base_name}'
            )
