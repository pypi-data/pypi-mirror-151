...
class TQuarchUSB_IF:
    ...
    def OpenPort(self):
        if self.connection == None:
            logging.error("Open: Device not assigned")
        ...
        if self.deviceHandle is not None:
            ...
            try :
                ...
                logging.error(f"Problem claiming device interface : {e}")
                return 0
            ...
            if self.QCmdEPSize > 64:
                ...
                try:
                    ...
                    logging.error(f"Open: Lock write fail : {e}")
                    self.lastError = 'Open: Lock Write Fail'
                    return 0

                try:
                    retLineStr = self.FetchCmdReplyTOut(self.timeout)
                    # Attempting bug fix by sending out a new read stuff.
                    # retLineStr = self.deviceHandle.bulkRead( self.QCmdEP, self.QCmdEPSize, self.timeout )
                except Exception as e:
                    logging.error(f"Open: Lock read reply fail : {e}")
                    self.lastError = 'Open: Lock Read Reply Fail'
                    return 0

                try:
                    if format(retLineStr[2], '02x') != '01':
                        logging.error(f"Open: Failed to lock device (1)")
                    ...
                    if retLineStr[2] != '\x01':
                        logging.error(f"Open: Failed to lock device (2)")
                        self.lastError = 'Open: Failed to Lock Device'
                        return 0
                    else:
                        self.lastError = ''
            ...
            logging.error(f"Open: Get Device Handle Failed")
        ...
        if self.deviceHandle is not None:
            if not self.QCmdEPSize > 64:  # If this is and Pic32
                try:
                    ...
                    logging.error(f"Close: Unlock write fail : {e}")
                ...
                try:
                    ...
                    if sys.version[0] == '2':
                        if retLineStr[2] != '\x01':
                            logging.error(f"Close: Failed to unlock device (1)")
                            self.lastError = 'Close: Failed to Unlock Device'
                    else:
                        if format(retLineStr[2], '02x') != '01':
                            logging.error(f"Close: Failed to unlock device (2)")
                            self.lastError = 'Close: Failed to Unlock Device'

                except Exception as e:
                    logging.error(f"Close: Unlock read reply fail : {e}")
            ...
            try:
                ...
                logging.error(f"Close: Device close Failed : {e}")
            ...
            logging.warning("Close: Device was not open")
    ...
    def SendCommand( self, command ):
        logging.debug(f"Sending command over USB: {repr(command)}")
    ...
    def clean_and_flush_stuck_usb_comms (self):
        ...
        while True:
            ...
            try:
                ...
                logging.error(os.path.basename(__file__) + ".FetchCmdReplyTOut: bulkRead exception (" + str(ex) + ") during response read: " + retLineStr)
                break
    ...
    def FetchCmdReplyTOut( self, timeout ):
        ...
        logging.debug(f"Returning response from USB: {repr(response)}")
...
class USBConn:
    ...
    def close(self):
        ...
        if not self.Connection.ClosePort():
            logging.debug(f"Error occured on close: {self.Connection.lastError}")
            return False
        else:
            logging.debug(f"USB device {self._conn} closed successfully")
            if self.Connection.lastError:
                logging.debug(f"Successful close reported problem : {self.Connection.lastError}")
        return True
    
    def sendCommand(self, Command, expectedResponse = True):
        logging.debug(f"Sending command '{Command}' to {self._conn}" )
