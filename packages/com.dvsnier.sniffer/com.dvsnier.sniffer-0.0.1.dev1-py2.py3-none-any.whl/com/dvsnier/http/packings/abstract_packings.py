...
class AbstractPackings(object):
    ...
    def execute(self, _on_callback=None):
        ...
        if __common_object_model.get_debug():
            ...
            __common_object_model.set_debug(IDebug(), lambda it: logging.debug('the {} object configured successfully.'.format('debug')))
        if __common_object_model.get_eoe():
            ...
            __common_object_model.set_eoe(eoe, lambda it: logging.debug('the {} object configured successfully.'.format('eoe')))
        if __common_object_model.get_http():
            ...
            http = AbstractHttp()
            __common_object_model.set_http(http, lambda it: logging.debug('the {} object configured successfully.'.format('http')))
            __common_object_model.set_brs(http.brs, lambda it: logging.debug('the {} object configured successfully.'.format('brs')))
    ...
    def get_dom(self):
        ...
        if not self._dom:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('dom'))
        return self._dom

    def get_rma(self):
        'the get rma instance'
        if not self._rma:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('rma'))
        return self._rma
