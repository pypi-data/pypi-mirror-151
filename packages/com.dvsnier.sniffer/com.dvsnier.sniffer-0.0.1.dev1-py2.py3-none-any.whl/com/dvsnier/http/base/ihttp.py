    ...
    def get(self, url, timeout=60, _on_callback=None):
        ...
        try:
            ...
            if self._COUNT < self._MAX_COUNT:
                ...
                logging.warning('the current request timed out, sleep that is {:.3f}s and soon afterwards start {}th request...'.format(
                    random_value, self._COUNT))
            ...
            if response.status_code != codes.ok:
                ...
                __common_object_model.get_eoe().append({
                    'id': url,
                    'emsg': 'the current error code that is {}'.format(response.status_code),
                    'timestamp': datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                })
                if self._COUNT < self._MAX_COUNT:
                    self._COUNT += 1
