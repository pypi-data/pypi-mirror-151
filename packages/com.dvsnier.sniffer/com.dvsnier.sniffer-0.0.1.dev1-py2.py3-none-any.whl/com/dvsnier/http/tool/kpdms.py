...
class KpmDs(object):
    ...
    def set_quality(self, resource_quality):
        # private property
        __common_object_model = AnalysisFactory.get_com()
        return __common_object_model.get_attribute().set_quality(resource_quality,
                                                                 _on_resource_quality_callback=self._on_resource_quality_callback)
    ...
    def local_to_json(self, flags, _on_item_callback=None, _on_callback=None):
        ...
        '''
        if flags:
            self.__local_to_json(self._callback_with_file_list_with_single, _on_item_callback=_on_item_callback, _on_callback=_on_callback)
        else:
        ...
        for file in file_list:
            ...
            if markup_file and os.path.exists(markup_file):
                ...
                if file and isinstance(file, str):
                    dest_file_name = None
                    if __ext_dir:
                        dest_file_name = generate_file_name_only(dest_dir_name, '{}.json'.format(file.split(r'.', maxsplit=1)[0]))
                    else:
                        dest_file_name = base_file.generate_file_name_only(dest_dir_name, '{}.json'.format(file.split(r'.', maxsplit=1)[0]))
