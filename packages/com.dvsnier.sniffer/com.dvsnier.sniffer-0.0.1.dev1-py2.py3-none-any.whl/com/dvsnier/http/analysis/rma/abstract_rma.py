...
class AbstractRma(IRma, object):
    ...
    def __init__(self):
        ...
        self.__common_object_model.set_model(self._model,
                                             lambda it: logging.debug('the {} object configured successfully.'.format('model')))
        self.__common_object_model.set_meta_model(self._meta_model,
                                                  lambda it: logging.debug('the {} object configured successfully.'.format('meta_model')))
        self.__common_object_model.set_publisher_model(
            self._publisher_model, lambda it: logging.debug('the {} object configured successfully.'.format('publisher_model')))
        self.__common_object_model.set_respondent_model(
            self._respondent_model, lambda it: logging.debug('the {} object configured successfully.'.format('respondent_model')))
        self.__common_object_model.set_comment_model(
            self._comment_model, lambda it: logging.debug('the {} object configured successfully.'.format('comment_model')))
        self.__common_object_model.set_resolver(self._resolver,
                                                lambda it: logging.debug('the {} object configured successfully.'.format('resolver')))

    def get_adapter(self):
        'the get adapter instance'
        if not self._adapter:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('adapter'))
        return self._adapter

    def get_comment_model(self):
        'the get comment model instance'
        if not self._comment_model:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('comment_model'))
        return self._comment_model

    def get_model(self):
        'the get model instance'
        if not self._model:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('model'))
        return self._model

    def get_meta_model(self):
        'the get meta model instance'
        if not self._meta_model:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('meta_model'))
        return self._meta_model

    def get_publisher_model(self):
        'the get publisher model instance'
        if not self._publisher_model:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('publisher_model'))
        return self._publisher_model

    def get_respondent_model(self):
        'the get respondent model instance'
        if not self._respondent_model:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('respondent_model'))
        return self._respondent_model

    def get_resolver(self):
        'the get resolver instance'
        if not self._resolver:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('resolver'))
