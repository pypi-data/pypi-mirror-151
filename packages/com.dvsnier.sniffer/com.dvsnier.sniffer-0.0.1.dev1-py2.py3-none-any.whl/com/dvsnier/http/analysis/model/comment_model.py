...
class CommentModel(AbstractModel, object):
    ...
    def get_article_id_with_comments_params(self):
        ...
        if self._response and isinstance(self._response, Response):
            ...
            raise NameError('the Illegal operation. Please execute the get() or set_url() request before using this method')
        ...
        if li_result_set:
            ...
            for index_item, li_item in enumerate(li_result_set):  # the first comment page data
                if li_item and isinstance(li_item, Tag):
                    ...
                    kwargs_li_item_comment_key_with_union_id = kwargs_li_item_attrs.get('comment_key_with_union_id', str())
                    kwargs_li_item_comment_key_with_union_rid = kwargs_li_item_attrs.get('comment_key_with_union_rid', str())
                    ...
                    element_item['comment_key_with_union_id'] = li_item.get(kwargs_li_item_comment_key_with_union_id)  # union primary key
                    element_item['comment_key_with_union_rid'] = li_item.get(kwargs_li_item_comment_key_with_union_rid)  # union primary key
                                    ...
                                    for index, a_item in enumerate(a_result_set):
                                        if a_item and isinstance(a_item, Tag):
                                            ...
                                            if comment_content_value.find('：') >= 0:
                                                element_item['comment_content'] = span_item.text.strip().split(r'：', maxsplit=1)[1]
                                            else:
            ...
            element_nodes.extend(self.__get_more_comments(tag, len(element_nodes),
                                                          **kwargs_li_item_more_comment))  # the more comment page data list
                            ...
                            if response_with_dict and isinstance(response_with_dict, dict):
                                ...
                                if comment_lists and len(comment_lists) > 0:
                                    for index, comment_item in enumerate(comment_lists):
                                        element_item = dict()
                                        kwargs_comment_key_with_union_id = kwargs_attrs.get('comment_key_with_union_id', str())
                                        ...
                                        if content and isinstance(content, str):
                                            ...
                                            if a and isinstance(a, Tag):
                                                ...
                                                if comment_content_value.find('：') >= 0:
                                                    element_item['comment_content'] = comment_content_value.strip().split(r'：',
                                                                                                                          maxsplit=1)[1]
                                                else:
    ...
    def get_comment_lists(self, replyId, pageNum, timeout=600):
        ...
        url = '{}?method={}&params.item={}&params.articleId={}&params.replyId={}&params.pageNum={}'.format(
            api, method, self.get_item_with_comments_params(), self.get_article_id_with_comments_params(), replyId, pageNum)
