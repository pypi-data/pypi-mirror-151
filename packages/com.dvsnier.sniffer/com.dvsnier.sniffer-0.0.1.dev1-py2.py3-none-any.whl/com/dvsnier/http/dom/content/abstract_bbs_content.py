...
class AbstractBbsContent(IBbsContent, object):
    ...
    def __bbs_content_with_tag(self, *element_item):
        ...
        if element_item:
            ...
            if item_with_bbs_content.name == 'br':
                ...
                element_item_set.append(element_content_item)
                logging.error(
                    'the current parse bbs content node({}) error that contains property values that have not been processed.'.format(
                        item_with_bbs_content))
        return element_item_set
            ...
            if item_with_bbs_content and isinstance(item_with_bbs_content, Tag):
                ...
                try:
                    ...
                    if response:
                        ...
                        element_content_item['resource_base64'] = str(base64.b64encode(resource_bytes), encoding='utf-8')
        ...
        if __common_object_model.get_attribute().get_persistence():  # the persistence
            ...
            if crs:
                ...
                if __ext_dir:
                    ...
                    output_dir_name = __common_object_model.get_brs().mk_output_dir(os.path.join('image', multi_media_name))
                if persistence_url and isinstance(persistence_url, str):
                    ...
                    if match and match.string:
                        ...
                        img_name = 'img_{}_{}.{}'.format(__common_object_model.get_comment_model().get_article_serial_number(),
                                                         int(datetime.timestamp(datetime.now())), img_suffix)
                        file_name = ''
                        if __ext_dir:
                            file_name = generate_file_name_only(output_dir_name, img_name)
                        else:
                            file_name = __common_object_model.get_brs().generate_file_name_only(output_dir_name, img_name)
                        # logging.debug('the current img path that is {}'.format(file_name))
                        try:
                            persistence_response = __common_object_model.get_http().get(persistence_url,
                                                                                        timeout=600)  # url data to persistence
                            if persistence_response:
                                try:
                                ...
                                except (FileNotFoundError, PIL.UnidentifiedImageError, ValueError, TypeError, KeyError) as e:
                                    # private property
                                    __common_object_model = AnalysisFactory.get_com()
                                    __common_object_model.get_eoe().append({
                                        'id': persistence_url,
                                    ...
                                    if __common_object_model.get_debug().is_debug():
                                        logging.debug('the current persistent image data({}) succeeded.'.format(file_name))
                        except (ConnectionError, HTTPError, RequestException) as e:
                            # private property
                            __common_object_model = AnalysisFactory.get_com()
                            __common_object_model.get_eoe().append({
            ...
            if item_with_bbs_content and isinstance(item_with_bbs_content, Tag):
                ...
                if span_dot_img_result_set:
                    for index, item_with_img in enumerate(span_dot_img_result_set):
                        element_item_set.extend(self.__bbs_content_with_tag_dot_img(element_content_item, item_with_img))
                else:
                    element_content_item['type'] = 1
                    element_content_item['resource_content'] = item_with_bbs_content.get_text().strip()
                    element_item_set.append(element_content_item)
    ...
    def __bbs_content_with_illegal(self, *element_item):
        ...
        if element_item:
            ...
            element_content_item['type'] = 0
            element_item_set.append(element_content_item)
            logging.error('the current parse bbs content node({}) error that contains property values that have not been processed.'.format(
                item_with_bbs_content.text))
        return element_item_set
