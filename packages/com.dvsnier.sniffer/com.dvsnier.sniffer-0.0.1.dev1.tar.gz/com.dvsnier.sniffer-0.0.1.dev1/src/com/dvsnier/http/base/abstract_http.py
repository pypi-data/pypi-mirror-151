    ...
    def write_to_file_with_multiple(self, url, file_prefix, file_suffix, output_dir_name='http', index=None):
        ...
        if self.brs:
            ...
            if __ext_dir:
                ...
                if index:
                    file_name = generate_complex_or_fmt_file_name(output_dir_name, '{}_{}.{}'.format(file_prefix, index, file_suffix))
                else:
                    file_name = generate_complex_or_fmt_file_name(output_dir_name, '{}.{}'.format(file_prefix, file_suffix))
            else:
                if index:
                    file_name = self.brs.generate_complex_or_fmt_file_name(output_dir_name,
                                                                           '{}_{}.{}'.format(file_prefix, index, file_suffix))
                else:
                    file_name = self.brs.generate_complex_or_fmt_file_name(output_dir_name, '{}.{}'.format(file_prefix, file_suffix))
            if url:
