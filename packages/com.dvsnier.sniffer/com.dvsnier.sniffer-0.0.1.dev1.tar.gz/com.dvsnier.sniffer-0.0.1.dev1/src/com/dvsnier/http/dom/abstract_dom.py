...
class AbstractDom(IDom, object):
    ...
    def __init__(self):
        ...
        self.__common_object_model.set_attribute(self._attribute,
                                                 lambda it: logging.debug('the {} object configured successfully.'.format('attribute')))
        self.__common_object_model.set_content(self._content,
                                               lambda it: logging.debug('the {} object configured successfully.'.format('content')))

    def get_attribute(self):
        'the get attribute instance'
        if not self._attribute:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('attribute'))
        return self._attribute

    def get_content(self):
        'the get content instance'
        if not self._content:
            raise KeyError('The current value({}) is invalid and illegal, and then please reset the value'.format('content'))
