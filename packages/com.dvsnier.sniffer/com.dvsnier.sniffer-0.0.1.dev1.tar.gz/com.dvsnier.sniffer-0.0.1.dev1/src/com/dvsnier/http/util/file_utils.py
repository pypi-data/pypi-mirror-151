        ...
        if output_dir_name:
            ...
            if not __output_dir_root_name:
                ...
                if __output_uuid_encryptor:
                    __output_dir_root_name = os.path.join(output_dir_name,
                ...
                logging.warning(
                    'the output directory with absolute path prefix currently written is {}\\...'.format(__output_dir_root_name))
...
def generate_complex_or_fmt_file_name(output_dir_name, file_name, fmt='%Y%m%d_%H%M%S'):
    ...
    if '.' in file_name:
        ...
        if rdot_index > 0:
            file_name = str("%s_%s%s" % (file_name[0:rdot_index], datetime.datetime.now().strftime(fmt), file_name[rdot_index:]))
    else:
