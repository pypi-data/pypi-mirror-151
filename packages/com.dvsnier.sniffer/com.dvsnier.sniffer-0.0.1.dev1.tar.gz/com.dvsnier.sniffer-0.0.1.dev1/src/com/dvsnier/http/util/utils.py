...
def task_sort_with_major_two(file_name_previous, file_name_next):
    ...
    '''
    if file_name_previous and isinstance(file_name_previous, str) and file_name_next and isinstance(file_name_next, str):
...
def obtain_file_and_resolver(_on_callback=None):
    ...
    beautiful_soup = __model.beautiful_soup(os.path.join(output_dir_name, file_list[0]), _on_callback=_on_callback_with_beautiful_soup)
