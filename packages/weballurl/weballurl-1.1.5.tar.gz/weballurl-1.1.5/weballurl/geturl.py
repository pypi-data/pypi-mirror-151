...
class geturl:
    ...
    @delayed(0)
    def request(self, url, domain, timeout, count):
        try:
            res = requests.get(url, headers=self.header, timeout=3)
            res.encoding = res.apparent_encoding
            html = res.text
            self.all_url(html, domain, timeout, count)
        except:
            self.err_url.append(url)

    def all_url(self, resp, domain, timeout, count):
    ...
    def startres(self, url, domain, timeout, count):
        self.request(url, domain, timeout, count)
        return self.url_list


if __name__ == '__main__':
    allurl = geturl()
    url_list = allurl.startres('https://blog.csdn.net', 'csdn.net', 300, 300)