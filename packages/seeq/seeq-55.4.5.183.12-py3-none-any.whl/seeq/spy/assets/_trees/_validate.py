...
def validate_and_filter(session: Session, df_to_validate, status, errors, stage, temporal_description='',
    ...
    error_summaries, error_series = validate(session, df_to_validate, stage)
...
def validate(session: Session, df, stage='final'):
    ...
    if stage == 'final':
        ...
        error_summaries_dependencies, error_series_dependencies = validate_dependencies(df, ignore_rows=ignore_rows)
    else:
...
def validate_dependencies(df, ignore_rows=None):
    error_series = pd.Series('', index=df.index)
    error_summaries = set()
    if ignore_rows is None:
        ignore_rows = pd.Series(False, index=df.index)
    ...
    for node in _match.FrozenTree.of(df.loc[~ignore_rows]):  # type: _match.FrozenTree
        dependencies = list()
        ...
        for dependency in dependencies:
            if not isinstance(dependency, str) or _common.is_guid(dependency):
                continue
            try:
                resolved_dependency = node.parent.resolve_child_reference(dependency)
