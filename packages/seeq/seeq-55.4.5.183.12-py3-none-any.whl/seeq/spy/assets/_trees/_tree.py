...
from seeq.spy import _login, _push
...
class Tree:
    ...
    def insert(self, children=None, parent=None, *, name=None, formula=None, formula_parameters=None,
               friendly_name=None, errors=None, quiet=None, status=None):
        """
        ...
        if formula:
            if 'Formula' in children.columns or 'Formula Parameters' in children.columns:
                raise SPyValueError(f"Children DataFrame cannot contain a 'Formula' or 'Formula Parameters' column "
                                    f"when inserting a formula.")
            else:
    ...
    def visualize(self, subtree=None):
        """
