...
def parse_input_datetime(session: Session,
    ...
    if not isinstance(input_datetime, pd.Timestamp):
        ...
        except parser.UnknownTimezoneWarning as e:
            raise SPyValueError(f"Timezone detected as '{str(e).split()[1]}' could not be parsed. You can "
                                f"specify a timezone in a string as e.g. '-05:00' for a fixed offset from UTC or use "
                                f"pd.Timestamp(), datetime.date() to create a timezone-aware datetime")
