...
def test_pull_input_params_property():
    ...
    with tempfile.TemporaryDirectory() as dir_path:
        ...
        assert df.spy.kwargs['errors'] == 'raise'
        ...
        assert df.spy.status.df['Result'].all() == 'Success'
...
def test_pull_signal_performance():
    ...
    push_df = pd.DataFrame({
        'Name': pd.Series(np.arange(1, size, 1)).apply(lambda v: f't{v}'),
        'Type': 'Signal',
    ...
    timer = _common.timer_start()
    pull_df = spy.pull(push_results, start='2021-01-01', end='2021-03-01')
    seconds = _common.timer_elapsed(timer)
