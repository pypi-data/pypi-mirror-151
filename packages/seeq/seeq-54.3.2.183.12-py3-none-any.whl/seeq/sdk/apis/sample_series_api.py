...
class SampleSeriesApi(object):
    ...
    def archive_series(self, **kwargs):
        """
        Archive a series and its samples
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.archive_series(id=id_value, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str id: ID of the series to archive (required)
        ...
        if kwargs.get('callback'):
            return self.archive_series_with_http_info(**kwargs)
        else:
            (data) = self.archive_series_with_http_info(**kwargs)
            return data

    def archive_series_with_http_info(self, **kwargs):
        """
        Archive a series and its samples
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.archive_series_with_http_info(id=id_value, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method archive_series" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `archive_series`")
