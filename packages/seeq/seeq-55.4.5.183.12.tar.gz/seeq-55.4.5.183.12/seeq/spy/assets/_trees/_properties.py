...
def format_references(df) -> pd.DataFrame:
    resolved_formula_parameters = pd.Series(np.nan, index=df.index, dtype='object')
    resolved_measured_item = pd.Series(np.nan, index=df.index, dtype='object')
    resolved_bounding_condition = pd.Series(np.nan, index=df.index, dtype='object')
    resolved_thresholds = pd.Series(np.nan, index=df.index, dtype='object')
    for node in _match.FrozenTree.of(df):  # type: _match.FrozenTree
        if pd.api.types.is_dict_like(df.loc[node.index, 'Formula Parameters']):
            ...
            for name, reference in formula_parameters.items():
                if not isinstance(reference, str) or _common.is_guid(reference):
                    continue
                formula_parameters[name] = node.parent.resolve_child_reference(reference).full_path
            resolved_formula_parameters[node.index] = formula_parameters

        if isinstance((df.loc[node.index, 'Measured Item']), str):
            if not _common.is_guid(df.loc[node.index, 'Measured Item']):
                resolved_measured_item[node.index] = node.parent.resolve_child_reference(
                    df.loc[node.index, 'Measured Item']).full_path
            else:
                resolved_measured_item[node.index] = df.loc[node.index, 'Measured Item']

        if isinstance(df.loc[node.index, 'Bounding Condition'], str):
            if not _common.is_guid(df.loc[node.index, 'Bounding Condition']):
                resolved_bounding_condition[node.index] = node.parent.resolve_child_reference(
                    df.loc[node.index, 'Bounding Condition']).full_path
            else:
                resolved_bounding_condition[node.index] = df.loc[node.index, 'Bounding Condition']
        ...
        if pd.api.types.is_dict_like(df.loc[node.index, 'Thresholds']):
            ...
            for name, reference in thresholds.items():
                if not isinstance(reference, str) or _common.is_guid(reference):
                    continue
                try:
                    path_list = _common.path_string_to_list(node.parent.resolve_child_reference(reference).full_path)
                    ...
                    if 'invalid, missing, or has been removed' not in e.args[0]:
                        raise
            resolved_thresholds[node.index] = thresholds

    df = df.copy()
    df['Formula Parameters'] = resolved_formula_parameters
    df['Measured Item'] = resolved_measured_item
    df['Bounding Condition'] = resolved_bounding_condition
    df['Thresholds'] = resolved_thresholds
    return df
