...
def pull(items, *, include_my_effective_permissions=False, errors='raise', quiet=False, status=None,
    ...
    input_args = _common.validate_argument_types([
        (items, 'items', (pd.DataFrame, list, dict, str)),
    ...
    for index, metadata_row in pull_result_df.iterrows():
        ...
        try:
            ...
            pull_result_df.at[index, 'Pull Result'] = _common.format_exception(e)
...
def put_permissions_on_df_row(session: Session, df, index, item_id, include_my_effective_permissions):
