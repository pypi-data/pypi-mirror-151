...
from seeq.spy import _common, _url, Status
...
def test_push_capsules_property_unit_in_metadata():
    ...
    # search_result = spy.search(push_result)

    # TODO CRAB-30195 - Capsule property metadata (on the Condition itself) inference has been disabled,
    #  so this assertion can no longer be true. Once inference is re-enabled, this assertion will ensure that
    #  metadata is handled by SPy in a similar manner to how it is handled in the REST API
...
@pytest.mark.system
def test_push_capsules_property_unit_in_metadata_case_insensitivity():
...
def test_push_capsule_property_units_push_result_as_metadata():
    ...
    search_result = spy.search(data_second_push)
    ...
    search_result = spy.search(no_data_second_push)
...
def test_push_threshold_metric_metadata():
    ...
    test_dict = {'Type': 'Threshold Metric',
                 ...
                 'Thresholds': [{'LoLo': signals_for_testing[signals_for_testing['Name'] == 'Wet Bulb']['ID'].iloc[0],
                                 '3': 95}]
                 }
    ...
    assert confirm_push_output.thresholds[tp.index(-2)].item.id == test_metadata['Thresholds'].iloc[0]['LoLo']
...
def test_push_all_stored_types_by_id():
    ...
    assert 'Description' not in search_df
