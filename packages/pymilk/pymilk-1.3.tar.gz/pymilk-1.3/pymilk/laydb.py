...
项目介绍
    layDB 是由本人自主研发的一款非关系型数据库, 具有以下特点：
        ● 免安装, 开箱即用。(只需使用pip安装Python库)
        ● 无须启动/停止, 不操作时不占用CPU和内存。
        ● 无须建表和字段。
        ● 可以存储任何"无须保持活跃"的Python对象, 如:int、str、set、def、class、pandas的对象、tensorflow的对象、
          opencv的对象等。(requests等需要保持网络连接的对象无法存储)
        ● 数据文件少, 且每个项目可单独指定数据目录, 方便使用Git等工具进行备份。
        ● 语法简洁清晰, 极易上手。
...
from .toolkit.data_structures import SetAttrError
from .toolkit.data_structures import undefined
from .toolkit.sorted_parts import sorted_parts as SorPa


newpk = pk_factory().pk1
WindowsPath = type(libpath(''))
...
class _Filter():
    ...
    def __ne__(self, obj):  # 这个用get, 而不用try
        return _Factory(lambda line: self._GetData(line) != obj)

    def __eq__(self, obj):  # 这个用get, 而不用try
        return _Factory(lambda line: self._GetData(line) == obj)
    ...
    def custom(self, func): return _Factory(func)
...
class _layline():
    ...
    def update(self, data):
        r = 0
        file = libpath(f"{self.laydb.folder}/{self.pk}.layline")
        if file.exists():
            line = pickle_loads(file.read_bytes())
            r = 1
        else:
            line = {}
            r = 2
        if not data.get('_pk'): data['_pk'] = self.pk
        line.update(data)
        if self.pk != line['_pk']:
            libpath(f"{self.laydb.folder}/{self.pk}.layline").unlink(missing_ok=True)
            object.__setattr__(self, 'pk', line['_pk'])
        self.laydb.__add__(line)
        return r
    ...
    def setdefault(self, default):
        ...
        if file.exists():
            ...
            if not default.get('_pk'): default['_pk'] = self.pk
            if self.pk != default['_pk']:
                libpath(f"{self.laydb.folder}/{self.pk}.layline").unlink(missing_ok=True)
                object.__setattr__(self, 'pk', default['_pk'])
            self.laydb.__add__(default)
            return default
...
class laydb():
    ...
    def __init__(self, folder, factory=None, columns=('*', ), _sort=None):
        ...
        if type(folder) is WindowsPath:
            setv(self, 'folder', folder)
        elif folder[-6:] == '.laydb':
            setv(self, 'folder', libpath(folder))
        else:
            setv(self, 'folder', libpath(f"{folder}.laydb"))
        self.folder.mkdir(parents=True, exist_ok=True)
        setv(self, 'factory', factory or _Factory(uniset))
        setv(self, 'columns', columns)  # str型 或 tuple型
        setv(self, '_sort', deepcopy(_sort or {}))
    ...
    def __add__(self, data):
        if type(data) in (dict, ):
            if '_pk' in data:
                ...
                data['_pk'] = pk = newpk()
            fpath = f"{self.folder}/{pk}.layline"
            write_pickle(fpath, data)
            return pk
            ...
            for x in data:
                if '_pk' in x:
                    ...
                    x['_pk'] = pk = newpk()
                fpath = f"{self.folder}/{pk}.layline"
                write_pickle(fpath, x)
            return [x['_pk'] for x in data]
    
    def __setitem__(self, pk, value):
        if not value.get('_pk'): value['_pk'] = pk
        if pk != value['_pk']:
            libpath(f"{self.folder}/{pk}.layline").unlink(missing_ok=True)
        self.__add__(value)
    ...
    def _RStyleIndex(self, index):
        ...
        if filtor is empset: return [][0]
        if index == 0: return [][0]
        if type(index) is not int: return [][0]
        # 读取数据
        lines = []
        i = 0
