import pandas as pd
try:
    from torch import eye as identity, dot, vstack, ones, mean, std, FloatTensor, device, cuda, square
    use_torch = True
    device = device('cuda' if cuda.is_available() else 'cpu')
    def float_array(x) -> FloatTensor:
        return FloatTensor(x, device)
except ImportError:
    from numpy import identity, dot, vstack, array, ones, mean, std, array, square
    use_torch = False
...
def alpha_sens_curves(jacobian: Jacobian_mlp, tol: float = None, 
    ...
    alphas = np.arange(1, max_alpha + 1)
...
def alpha_curve(raw_sens:list, tol: float = None, 
    ...
    N = raw_sens.shape[0]
    scl_factor = 10**(max(np.floor(np.log10(abs(raw_sens)))))
    alpha = np.arange(1,max_alpha + 1)
    scl_rs = np.abs(raw_sens) / scl_factor
