...
def jacobian_mlp(
    ...
    for lyr in range(1, len(mlpstr)):
        ...
        aux[:, 1:] = O[lyr - 1]
        Z.append(matmul(aux, W[lyr].to(aux.get_device())))
        ...
        pd.DataFrame(
            {
                "mean": meanSens[:, icol],
                "std": stdSens[:, icol],
                "mean_squared": meansquareSens[:, icol],
            },
            index=input_name,
        )
    ...
    raw_sens = [
        pd.DataFrame(
            D_accum[counter][:, :, out], index=range(X.shape[0]), columns=input_name
        )
...
def alpha_sens_curves(
    ...
    if dev == "gpu" or use_torch:
        try:
            from torch import tensor, device, arange, from_numpy, is_tensor
            from numpy import ndarray, asarray
    ...
    for out, rs in enumerate(raw_sens):
        ...
        for inp, input in enumerate(input_name):
            alpha_curves = alpha_curve(
                rs.iloc[:, inp], tol=tol, max_alpha=max_alpha, dev=dev
            )
            if curve_equal_origin:
                alpha_curves -= alpha_curves[0]
            alpha_curves_scl = alpha_curves / alpha_curves[-1]
            ax[axes[0]].plot(
                alphas, alpha_curves, label=input, color=cmap(inp / rs.shape[1])
            )
            ax[axes[0]].axhline(
                y=alpha_curves[-1], color=cmap(inp / rs.shape[1]), linestyle="dashed"
            )
            ax[axes[1]].plot(
                alphas, alpha_curves_scl, label=input, color=cmap(inp / rs.shape[1])
            )
...
def alpha_curve(
    ...
    alpha_curve = float_array(
        [scl_factor * sum(scl_rs ** a / N) ** (1 / a) for a in alpha], dev=dev
    )
