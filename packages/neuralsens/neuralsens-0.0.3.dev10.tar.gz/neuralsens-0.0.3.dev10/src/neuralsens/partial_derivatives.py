import pandas as pd
try:
    from torch import eye as identity, dot, vstack, ones, mean, std, FloatTensor, device, cuda, square, arange, matmul, zeros, log10, abs
    use_torch = True
    def float_array(x, device="cpu") -> FloatTensor:
        return FloatTensor(x, device)
except ImportError:
    from numpy import identity, dot, vstack, array, ones, mean, std, array, square, arange, matmul, zeros, floor, log10, abs
    use_torch = False
    def float_array(x, device="cpu") -> array:
        return array(x) 
...
def jacobian_mlp(wts: list[float], 
    ...
    if not (sens_end_layer > sens_origin_layer) or ((sens_end_layer == sens_origin_layer) and (sens_origin_input and not sens_end_input)):
        raise ValueError('There must be a layer of neurons or weights between end layer and origin layer.')

    if (sens_origin_layer > len(actfunc)):
        raise ValueError('Origin layer should be less than number of layers in the model.')
    ...
    Z = [dot(float_array(X, device), W[0])]
    ...
    if use_torch:
        ...
        D = [float_array([deractfunc[0](Z[0][irow,]) for irow in r_samples], device=device)]
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        aux[:,1:] = O[lyr-1]
        Z.append(dot(aux,W[lyr]))
        
        # Calculate output of each layer
        O.append(actfunc[lyr](Z[lyr]))
        
        # Calculate derivative of each layer
        D.append(float_array([deractfunc[lyr](Z[lyr][irow,]) for irow in r_samples], device=device))
    ...
    if not (sens_origin_layer == sens_end_layer):
        for layer in range(sens_origin_layer + 1, sens_end_layer):
            ...
            #     D_accum.append(float_array([dot(D_accum[counter - 1][irow,], W[layer][1:,:]) for irow in range(X.shape[0])]))
            # else:
            #     D_accum.append(float_array([dot(dot(D_accum[counter - 1][irow,], W[layer][1:,]), D[layer][irow,]) for irow in range(X.shape[0])]))
            if (layer == sens_end_layer) and sens_end_input:
...
def hessian_mlp(wts: list[float], 
    ...
    D2 = [float_array([der2actfunc[0](Z[0][irow,]) for irow in r_samples], device=device)]
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D2.append(float_array([der2actfunc[lyr](Z[lyr][irow,]) for irow in r_samples], device=device))
    ...
    if not (sens_origin_layer == sens_end_layer):
        for layer in range(sens_origin_layer + 1, sens_end_layer):
            ...
            # Second derivatives
            q = matmul(matmul(H[counter-1], W[layer][1:,:]).swapaxes(0,1), D[counter]).swapaxes(0,1)
            h = matmul(d_accum, matmul(d_accum, D2[layer].swapaxes(0,1)).swapaxes(0,2)).swapaxes(0,2).swapaxes(0,1)
    ...
    sens = [pd.DataFrame(float_array([meanSens[:,:,out], stdSens[:,:,out], meansquareSens[:,:,out]], device=device).T.reshape(meanSens.shape[1],-1), 
...
class Hessian_mlp:
    ...
    def hesstosens(self):
        ...
        for out in range(len(self.raw_sens)):
            ...
            raw_sens = float_array([raw_sens[:,x,y] for x,y in zip(index_of_interest[0],index_of_interest[1])])
...
def jerkian_mlp(wts: list[float], 
    ...
    D3 = [float_array([der3actfunc[0](Z[0][irow,]) for irow in r_samples], device=device)]
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D3.append(float_array([der3actfunc[lyr](Z[lyr][irow,]) for irow in r_samples], device=device))
    ...
    for layer in range(sens_origin_layer + 1, sens_end_layer):
        ...
        # Third derivatives
        m = matmul(matmul(J[counter-1], W[layer][1:,:]).swapaxes(0,2), D[counter]).swapaxes(0,2)
        n = matmul(z_,matmul(matmul(np.broadcast_to(np.expand_dims(q, axis=4), list(q.shape) + [q.shape[-1]]).swapaxes(0,2).swapaxes(0,3),D2[layer][:,:,0,:]).swapaxes(1,3),D2[layer][:,0,:,:]).swapaxes(0,3)).swapaxes(0,2)
        j = matmul(z_, matmul(z_, matmul(z_, D3[layer].swapaxes(0,2)).swapaxes(0,3)).swapaxes(1,3)).swapaxes(0,2)
...
def sensitivity_plots(jacobian: Jacobian_mlp):
    ...
    for out, raw_sens in enumerate(jacobian.raw_sens):
        ...
        # Plot mean-std plot
        ax[axes[0]].set_xlim([min(sens['mean']) - 0.2 * max(abs(sens['mean'])), max(sens['mean']) + 0.2 * max(abs(sens['mean']))])
        ax[axes[0]].hlines(y=0, xmin=min(sens['mean']) - 0.2 * max(abs(sens['mean'])), xmax=max(sens['mean']) + 0.2 * max(abs(sens['mean'])), color='blue')
        ...
        for i, txt in enumerate(sens.index.values.tolist()):
            ax[axes[0]].annotate(txt, xy=(sens['mean'][i], sens['std'][i]),  xycoords='data', va="center", ha="center", fontsize='large',
        ...
        sens.plot.bar(y='mean_squared', ax=ax[axes[1]], color=colors, legend=False, rot=0)
...
def alpha_sens_curves(jacobian: Jacobian_mlp, tol: float = None, 
                max_alpha: int = 100, curve_equal_origin: bool = False):
    # Initialize variables
    raw_sens = jacobian.raw_sens
    input_name = jacobian.X.columns
    alphas = arange(1, max_alpha + 1)
    ...
    for out, rs in enumerate(raw_sens):
        ...
        for inp, input in enumerate(input_name):
            alpha_curves = alpha_curve(rs.iloc[:,inp], tol=tol, max_alpha=max_alpha)
            if curve_equal_origin:
                alpha_curves -= alpha_curves[0]
            alpha_curves_scl = alpha_curves / alpha_curves[-1] 
            ax[axes[0]].plot(alphas, alpha_curves, label=input, color=cmap(inp/rs.shape[1]))
            ax[axes[0]].axhline(y=alpha_curves[-1], color=cmap(inp/rs.shape[1]), linestyle = 'dashed')
            ax[axes[1]].plot(alphas, alpha_curves_scl, label=input, color=cmap(inp/rs.shape[1]))
...
def alpha_curve(raw_sens:list, tol: float = None, 
                max_alpha: int = 100, device="cpu") -> list[float]:
    ...
    N = raw_sens.shape[0]
    scl_factor = 10**(max(floor(log10(abs(raw_sens)))))
    alpha = arange(1,max_alpha + 1)
    scl_rs = abs(raw_sens) / scl_factor
    alpha_curve = float_array([scl_factor * sum(scl_rs ** a / N) ** (1/a) for a in alpha], device=device)

    if (math.isinf(alpha_curve[-1]) or alpha_curve[-1] == 0):
        alpha_curve = alpha_curve[:-1]
