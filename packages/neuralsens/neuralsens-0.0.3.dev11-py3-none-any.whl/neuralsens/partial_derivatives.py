import pandas as pd
try:
    from torch import eye as identity, dot, vstack, ones, mean, std, FloatTensor, device, square, arange, matmul, zeros, log10, abs
    use_torch = True
    def float_array(x, dev="cpu") -> FloatTensor:
        dev = device(dev)
        return FloatTensor(x, dev)
except ImportError:
...
def jacobian_mlp(wts: list[float], 
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D.append(float_array([deractfunc[lyr](Z[lyr][irow,]) for irow in r_samples], dev=device))
...
def hessian_mlp(wts: list[float], 
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D2.append(float_array([der2actfunc[lyr](Z[lyr][irow,]) for irow in r_samples], dev=device))
    ...
    sens = [pd.DataFrame(float_array([meanSens[:,:,out], stdSens[:,:,out], meansquareSens[:,:,out]], dev=device).T.reshape(meanSens.shape[1],-1), 
...
def jerkian_mlp(wts: list[float], 
    ...
    for lyr in range(1,len(mlpstr)):
        ...
        D3.append(float_array([der3actfunc[lyr](Z[lyr][irow,]) for irow in r_samples], dev=device))
...
def alpha_curve(raw_sens:list, tol: float = None, 
                max_alpha: int = 100, dev="cpu") -> list[float]:
    ...
    alpha_curve = float_array([scl_factor * sum(scl_rs ** a / N) ** (1/a) for a in alpha], dev=device)
