import pandas as pd
try:
    from torch import eye as identity, vstack, ones, mean, std, tensor, device, square, arange, matmul, zeros, log10, abs, from_numpy
    from numpy import ndarray
    use_torch = True
    def float_array(x, dev="cpu") -> tensor:
        dev = device(dev)
        if isinstance(x, pd.DataFrame):
            return from_numpy(x.to_numpy()).double()
        if isinstance(x, ndarray):
            return from_numpy(x.to_numpy()).double()
        return tensor(x, dev)
except ImportError:
    from numpy import identity, vstack, array, ones, mean, std, array, square, arange, matmul, zeros, floor, log10, abs
    use_torch = False
...
def alpha_sens_curves(jacobian: Jacobian_mlp, tol: float = None, 
                max_alpha: int = 100, curve_equal_origin: bool = False, device: str = "cpu"):
    ...
    for out, rs in enumerate(raw_sens):
        ...
        for inp, input in enumerate(input_name):
            alpha_curves = alpha_curve(rs.iloc[:,inp], tol=tol, max_alpha=max_alpha, device=device)
