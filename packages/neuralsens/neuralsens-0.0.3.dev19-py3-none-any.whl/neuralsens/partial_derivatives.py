    ...
    for out, rs in enumerate(raw_sens):
        ...
        for inp, input in enumerate(input_name):
            alpha_curves = alpha_curve(
                rs.iloc[:, inp], tol=tol, max_alpha=max_alpha, device=device
            )
...
def alpha_curve(
    raw_sens: list, tol: float = None, max_alpha: int = 100, device="cpu"
) -> list[float]:
    ...
    alpha_curve = float_array(
        [scl_factor * sum(scl_rs ** a / N) ** (1 / a) for a in alpha], dev=device
    )
