#
#   Copyright (c) 2014-2019, 2021-2022 eGauge Systems LLC
...
class Part(models.Model):
    ...
    mounting = models.IntegerField(choices=Enum.MOUNTING_CHOICES,
                                   default=Enum.MOUNTING_SMD, blank=False,
    ...
    status = models.IntegerField(choices=Enum.STATUS_CHOICES,
                                 default=Enum.STATUS_PREVIEW, blank=False,
    ...
    last_bom_mod_type = models.IntegerField(choices=Enum.LAST_MOD_CHOICES,
                                            default=Enum.LAST_MOD_TYPE_USER,
                                            blank=False)
    ...
    def best_part(self):
        ...
        for p in self.equivalent_parts():
            ...
            if (p.status == Enum.STATUS_PREFERRED) != \
               (best_part.status == Enum.STATUS_PREFERRED):
                if p.status == Enum.STATUS_PREFERRED:
                    best_part = p
                continue
    ...
    def best_vendor(self):
        ...
        vps = Vendor_Part.objects.filter(part_id=self.id) \
                                 .order_by('price')
        for vp in vps:
            if vp.status in Enum.STATUS_ORDERABLE:
                return vp.vendor
        return None

    def is_orderable(self):
        return self.status in Enum.STATUS_ORDERABLE
    ...
    def strstatus(self):
        return strchoice(Enum.STATUS_CHOICES, self.status)
...
class Vendor_Part(models.Model):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=6,
                                validators=PART_PRICE_VALIDATORS)
    status = models.IntegerField(choices=Enum.STATUS_CHOICES,
                                 default=Enum.STATUS_ACTIVE, blank=False,
                                 verbose_name='Life-time Status',
                                 help_text='The life-time status of this '
                                 'vendor part.')
...
class Delta(models.Model):
    ...
    part = models.ForeignKey(Part, verbose_name='Part #',
                             ...
                             'adjusted.', on_delete=models.PROTECT)
...
class Assembly_Item(models.Model):
    ...
    comp = models.ForeignKey(Part, related_name='assembly_item_comp',
                             ...
                             'this item.', on_delete=models.PROTECT)
...
class Order(Transaction):
    expected_arrival_date = models.DateField(verbose_name='Expected Arrival '
                                             ...
                                             'arrive.')
    status = models.IntegerField(choices=Enum.TXTN_STATUS_CHOICES,
                                 default=Enum.TXTN_STATUS_OPEN, blank=False,
                                 verbose_name='Order Status')
    vendor = models.ForeignKey(Vendor, help_text='The name of the '
                               'vendor (distributor) where the order '
                               'was placed.', on_delete=models.PROTECT)
    ...
    def is_open(self):
        return self.status == Enum.TXTN_STATUS_OPEN

    def days_until_past_due(self):
        if self.status != Enum.TXTN_STATUS_OPEN:
            return None
...
class Shipment(Transaction):
    ...
    def html_tracking_links(self):
        ...
        url_pattern = 'https://pkge.net/parcel/%s'
        result = []
        for trk in trk_list:
            trk = trk.strip()
            url = url_pattern % trk
...
class Part_Inventory:
    ...
    def __str__(self):
        ...
        for name, warehouse in self.warehouse.items():
            s = ''
            for p in warehouse:
                if warehouse[p].qty != 0:
                    s += (' %s*%d' % (format_part_number(p), warehouse[p].qty))
            lines.append('  Warehouse %s:%s' % (name, s))
        return '\n'.join(lines)
