...
def epic_bom(epic_url, auth):
    ...
    try:
        bom = BOM(epic_api, input_file, 'eGauge', flavor=args.flavor,
                  toolname='epic-bom')
        assy, created = bom.save_as_epic_assembly(force_update=
                                                  args.force_overwrite)
        if not args.quiet:
            print('%s part %s (%s %s) with %d components.'
                  % ('Created' if created else 'Updated',
                     format_part_number(assy.id), assy.mfg, assy.mfg_pn,
                     len(bom.comps)))
