...
class NapariSTPT:
    ...
    def Load(self, text):
        ...
        # Pring file to read
        print(str(self.comboBoxPath.currentText()) + '/mos.zarr')
        ...
        try:
            ...
            self.ds1 = xr.open_zarr(str(self.comboBoxPath.currentText())+'/mos.zarr', consolidated=False)
        ...
        if self.old_method:
            # number_of_section = len(self.ds1.attrs['cube_reg']['slice'])
            number_of_section = len(set(self.ds1.attrs['cube_reg']['slice']))
        else:    
            number_of_section = int(json.loads(self.ds1['S001'].attrs['raw_meta'])['sections'])
        ...
        if self.old_method:
            ...
            ds = xr.open_zarr(str(self.comboBoxPath.currentText())+'/mos.zarr', group=gr)
        else:
    ...
    def Load2D(self, text):
        ...
        self.ds1 = xr.open_zarr(str(self.comboBoxPath.currentText())+'/mos.zarr')
        ...
        ds16 = xr.open_zarr(str(self.comboBoxPath.currentText())+'/mos.zarr', group='l.16')
    ...
    def on_combobox_changed(self):
        if os.path.exists(str(self.comboBoxPath.currentText())+'/mos.zarr/.zmetadata'):
            print("metadata is available")
            
            try:
                ds = xr.open_zarr(str(self.comboBoxPath.currentText())+'/mos.zarr', consolidated=True)
                print("")
                print("")
