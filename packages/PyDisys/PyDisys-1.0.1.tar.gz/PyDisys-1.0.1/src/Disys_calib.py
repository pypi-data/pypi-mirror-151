...
class Disys_Calib:
    ...
    def __init__(self,path_to_data : str):
        ...
        try :         
            ...
            log.error("Error : files not found. Input path may be not correct.")
            return 
    ...
    def __init__(self,df_input : pd.DataFrame, mainpath ):
        ...
        try :    
            ...
            log.error("Error : data not found. Check the names!")
            return 
    ...
    def CreateNN(self,width:int,height:int):
        ...
        with open(self.main_path+R"\dati calibrazione\Report_Rete.txt",'w') as f:
            f.write("---   Final network    ---\nmin error  {:.5f}mm,\nmean error {:.5f}mm,\nmax error  {:.5f}mm. "
    ...
    def plotError(self,w,h):
        ...
        p = plt.scatter(self.X[:,0], self.X[:,1],
                ...
                s = 5,
                c=self.err)
        plt.title("Calibration error",fontsize=30)
        plt.xlabel("Columns [px]")
        plt.ylabel("Rows [px]")
        plt.xlim(0, w)
        plt.ylim(h, 0)
    ...
    def plotErrorXZ(self,w,h):
        ...
        plt.xlabel("X [mm]")
        plt.ylabel("Z [mm]")
        plt.xlim(max(self.Y[:,0]*1.05), min(self.Y[:,0]*0.95))
        #plt.ylim(0, h)
        plt.gca().set_aspect('equal', adjustable='box')
        cb = fig.colorbar(p)
        cb.set_label("Error [mm]")
    ...
    def CreateLuts(self,net_mlp,width:int,height:int):
        ...
        Image.fromarray(X_im).save(self.main_path+R"\X_image.tif")
        Image.fromarray(Y_im).save(self.main_path+R"\Y_image.tif")
        Image.fromarray(Z_im).save(self.main_path+R"\Z_image.tif")
        ...
        ha.overpaint_region(himage,region_c,1,'fill')
        ha.write_image(himage,'tiff',0,self.main_path+R'\Mask.tif')

    def MoveFilesToFolders(self):
        onlyfiles = [f for f in listdir(self.main_path) if isfile(join(self.main_path, f))]
        for f in onlyfiles:
            if (not (f.endswith(".tif")) and not (f.endswith(".csv"))):
