...
def showSchema(gClient,gSpace,tabuEdgeList=[],tabuTagList=[]):
    '''显示图数据库schema'''
    gClient.execute_query("USE {}".format(gSpace))
    gClient.set_space(gSpace)
    ...
    edgeDf=wrapNebula2Df(gClient.execute_query("SHOW EDGES"))
    if edgeDf.shape[0]>0:
    ...
    for edgeTypeItem in edgeList:
        if edgeTypeItem not in tabuEdgeList:
            for headTypeI in range(len(tagList)):
                for tailTypeI in range(headTypeI+1,len(tagList)):
                    headType=tagList[headTypeI]
                    tailType=tagList[tailTypeI]
                    if headType in tagAttrDict and tailType in tagAttrDict and \
                        headType not in tabuTagList and tailType not in tabuTagList:
                            ...
                            for tailAttr in tagAttrDict[tailType]:
                                if htPairStr not in htSetList:
                                    htDf=wrapNebula2Df(gClient.execute_query("LOOKUP ON {headType} WHERE {headType}.{headIdAttr}!='不可能的名字'|\
                                                                                GO FROM $-.VertexID OVER {edgeType} YIELD $$.{tailType}.{tailIdAttr}|LIMIT 5".format(
                                                                                    headType=headType,
                                                                                    headIdAttr=headAttr,
...
if __name__=="__main__":
    ...
    start=time.time()
    tagList,htList=showSchema(srcClient,"post_skill_school_ianxu")
    end=time.time()
