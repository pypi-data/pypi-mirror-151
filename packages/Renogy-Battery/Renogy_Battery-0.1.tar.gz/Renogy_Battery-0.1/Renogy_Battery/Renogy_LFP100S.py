...
class RenogySmartBattery(minimalmodbus.Instrument):
    ...
    def update(self):
      ...
      #Gets heater status - returns as 0 - 255. Changes it to a percentage.
      try:
        self.heater = (self.read_register(5013) / 255 )* 100
      except Exception as e:
...
# -------------------------------------------- Getters ----------------------------------------------------------
    ...
    #gets the address of the battery
    def getAddress(self):
       return self.address
    ...
    def getCellVolts(self):
        return [x / 10 for x in self.cellVolts]
    ...
    #The temperature around 32f (0C) gets funky. Still needs work.
    def getBatteryTemp(self):
      temp = sum(self.cellTemp) / len(self.cellTemp)
...
def main():
  ...
  print(renogy.getUnitVolts())
  print(renogy.getAmps())
