...
from django_books.objects import add_credit_card_payment, add_customer, process_response
...
class QuickBooksService(ServiceBase): 
    ...
    def authenticate(ctx, strUserName, strPassword):
        ...
        """
        if strUserName == 'b235fc42-0fd3-4356-b520-a2a0750e3765':    
            return ['none', 'none']
        if user.objects.get(qb_id=strUserName):
            ...
            if user.objects.get(qb_id=strUserName).check_password(strPassword):
                # If there is new work to be processed check for it here. 
                # if TicketQueue.objects.filter(status='APPROVED').count() > 0:
                    # ticket = str(uuid4())
                    # print('Success!')
                    # return [ticket, '']
                # else:
...
def support(request):
    return render(request, 'django_quickbooks/support.html', {})
