...
class Provenance:
	...
	def __init__(self,executable:str,version:str = "",arguments:list = [],prev_provenances:list = [],verbose=False,tmpFilePath=""):
		...
		tmpFilePath: if provided all logs will be immediately written to a temporary file which will be delted once process finished properly
		"""
	...
	def addPreviousProvenance(self,prev_provenance):
		...
		with open(prev_provenance) as file:
			...
			try:
				tmp_prov = yaml.load(file_content)
	...
	def write2TmpLogFile(self):
		""" writes to temporara log file in case program exits before properly calling write() """
