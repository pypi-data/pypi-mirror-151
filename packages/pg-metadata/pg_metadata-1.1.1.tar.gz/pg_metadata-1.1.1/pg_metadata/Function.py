...
class Function(DDL):
    def __init__(self, parent, data):
        ...
        self.IsRecord = data.get("is_record") or False
    ...
    def DDL_Drop(self):
        return "DROP FUNCTION %s(%s);" % (self.GetFullName(), self.ArgsInTypes)
    ...
    def DDL_Create(self):
        r = ""
        r += "-- Function: %s(%s)" % (self.GetFullName(), self.ArgsInTypes)
        r += SEP
        r += SEP
        r += "-- %s" % (self.DDL_Drop())
        r += SEP
        ...
        r += self.DDL_ArgsIn()
        r += "RETURNS %s AS" % (self.DDL_ArgsOut())
        r += SEP
        r += "$BODY$"
        r += SEP
        r += self.Code
    ...
    def GetFileName(self):
        if self.HasDuplicate:
            return "{0}.sql".format(self.NameWithParams)
        else:
