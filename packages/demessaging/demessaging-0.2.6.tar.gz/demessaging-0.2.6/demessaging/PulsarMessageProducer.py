...
class PulsarMessageProducer(PulsarConnection):
            ...
            for sock in sockets:
                if sock.connected:
                    try:
                        ...
                        print("error in socket ping routine: {}".format(e))
                        break
                else:
                    print(
                        "quitting socket ping loop due to unconnected or missing socket"
                    )
                    break
    ...
    def start_ping_loop(self, sockets: List[WebSocket]):
        ...
        thread = threading.Thread(
            ...
            args=(sockets, PulsarMessageProducer.SOCKET_PING_INTERVAL, event),
        )
        thread.setDaemon(True)
        thread.start()

    async def send_request(self, request_msg, topic: str = None) -> Any:
        ...
        out_topic = self.pulsarConfig[PulsarConfigKeys.TOPIC]
