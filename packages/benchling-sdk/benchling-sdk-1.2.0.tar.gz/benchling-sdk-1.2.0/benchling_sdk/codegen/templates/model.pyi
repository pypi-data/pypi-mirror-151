...
class {{ model.reference.class_name }}:
    ...
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        d = src_dict.copy()
{% for property in model.required_properties + model.optional_properties %}
        def get_{{property.python_name}}() -> {{property.get_type_string()}}:
            {% set property_source = 'd.pop("' + property.name + '")' %}
        {% if property.template %}
            {% from "property_templates/" + property.template import construct %}
            {{ construct(property, property_source) | indent(12)}}
        {% else %}
            {{ property.python_name }} = {{ property_source }}
        {% endif %}
            return {{ property.python_name }}

        {{ property.python_name }} = get_{{ property.python_name }}() if "{{property.name}}" in d else cast({{property.get_type_string()}}, UNSET)
