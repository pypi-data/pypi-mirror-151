...
def _element(name: str, content: str, indentation: int, inline: bool, **element_attributes) -> str:
    ...
    if content is not None:
        ...
        element_text = (f'{newline}<{name}{attributes}>{indented_newline}{content}{newline}</{name}>'
                        f'{trailing_newline}')
    else:
...
class FunctionalPage:
    ...
    def inject_file(self, path: str, pattern: re.Pattern, element=None, **element_attributes):
        with open(path, 'r') as file:
            file_text = file.read()
    ...
    @property
    def content(self):
        html_match = Placeholder.html_file_pattern.match(self.text)
        if html_match:
            return html_match.group('body')
