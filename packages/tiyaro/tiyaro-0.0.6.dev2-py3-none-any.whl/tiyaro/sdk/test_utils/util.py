...
from ...common.constants import KEYWORD_FROM_CONFIG, HANDLER_MODEL_MANIFEST_FILE, C_PRETRAINED_FILE_OR_URL, CLASS_CUSTOM, CLASS_IMAGE_CLASSIFICATION
...
def _get_input_image_classification(x):
    if os.path.isfile(x):
        with open(x, 'r') as f:
            return f.read()
    if isinstance(x, str):
...
def get_input_by_class(x):
    clazz = get_model_class()
    if clazz == CLASS_CUSTOM:
        return _get_input_json(x)
    elif clazz == CLASS_IMAGE_CLASSIFICATION:
        return _get_input_image_classification(x)

    raise ValueError(f'ERROR - cannot get input for unknown class: {clazz}')


def validate_and_save_test_input(handler: TiyaroBase, input):
    clazz = get_model_class()
    if clazz == CLASS_CUSTOM:
        if handler.input_schema:
            # we validate if schema is declared
            handler.input_schema().load(input)
            save_input(input)
        ...
        if not isinstance(input, str):
            raise ValueError(f'ERROR - for class: {CLASS_IMAGE_CLASSIFICATION}, input must be of type: {type("str")}')
        save_output(input)
    else:
        print(f'WARN - unknown class: {clazz}')

def validate_and_save_test_output(handler: TiyaroBase, output):
    clazz = get_model_class()
    if clazz == CLASS_CUSTOM:
        if handler.output_schema:
            # we validate if schema is declared
            handler.output_schema().load(output)
            save_output(output)
        ...
        if not isinstance(input, dict):
            raise ValueError(
                f'ERROR - for class: {CLASS_IMAGE_CLASSIFICATION}, input must be of type: {type("str")}')
        save_output(output)
    else:
        print(f'WARN - unknown class: {clazz}')
