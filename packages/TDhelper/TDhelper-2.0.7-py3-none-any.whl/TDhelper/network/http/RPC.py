                ...
                for param in m_params:
                    if param["key"].lower() != "pk":
                        if param["key"] in kwargs:
                            params_data += (
                                param["key"] + "=" + parse.quote(kwargs[param["key"]]) if isinstance(kwargs[param["key"]],str) else kwargs[param["key"]]
                            )
                        else:
                            if param["defaultValue"]:
                                params_data += (
                                    param["key"]
                                    + "="
                                    + parse.quote(param["defaultValue"]) if isinstance(param["defaultValue"],str) else param["defaultValue"]
                                )
                        m_count += 1
                        ...
                        m_uri = m_uri.replace(
                            "{pk}", parse.quote(kwargs[param["key"]]) if isinstance(kwargs[param["key"]],str) else kwargs[param["key"]]
                        )
                if params_data:
