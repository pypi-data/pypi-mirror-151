...
class RPCRegister:
    ...
    def _registerHost(self, pk, hosts):
        ...
        for i in range(0, len(hosts)):
            ...
            if state != 200:
                logging.error(
                    "register service hosts failed. msg:{}" % str(ret, encoding="utf-8")
                )
            else:
                ...
                if ret["state"] != 200:
                    ...
                    logging.info(
                        "register service hosts(%s:%s) success."
                        ...
                        try:
                            ...
                            for i in range(0, len(methods)):
                                ...
                                state, ret = POST(
                                    ...
                                    post_data=bytes(
                                        m_service_uri_post_data, encoding="utf8"
                                    ),
                                    http_headers=self._m_heads,
                                    time_out=15,
                                )
...
class RPC:
    ...
    def RPC_ACCESS(self, entryPoint:List[str], access_token:str, method:str, **kwargs:'dict|Tuple'):
    ...
    def register(self, service: Dict, hosts: List, methods: List)-> Tuple:
        ...
        if state == 200:
            if ret:
                ...
                if ret["state"] == 200:
                    ...
                    for i in range(0, len(hosts)):
                        ...
                        state, ret = POST(
                            self._sc_uri + "hosts/",
                            post_data=bytes(
                                m_service_hosts_post_data, encoding="utf-8"
                            ),
                            http_headers=self._m_heads,
                            time_out=15,
                        )
                    ...
                    for i in range(0, len(methods)):
                        ...
                        state, ret = POST(
                            self._sc_uri + "uri/",
                            post_data=bytes(m_service_uri_post_data, encoding="utf8"),
                            http_headers=self._m_heads,
                            time_out=15,
                        )
                        if state != 200:
                                    ...
                                    if "returns" in methods[i]:
                                        ...
                                        if m_returns:
                                            ...
                                            state, ret = POST(
                                                ...
                                                post_data=bytes(
                                                    reg_returns_params, encoding="utf-8"
                                                ),
                                                http_headers=self._m_heads,
                                                time_out=15,
                                            )
                ...
                raise Exception("register return body is none, please checked.")
        else:
    ...
    def call(self, method, **kwargs):
        ...
        state, ret = self._getApi(
            self._current_service, self._current_service + "." + method
        )
        if state:
            try:
                m_uri = ret["uri"]
                ...
                if "headers" in kwargs:
                    if isinstance(kwargs['headers'],dict):
                        for o in kwargs['headers']:
                            m_headers[o.lower().replace('http_','').replace('_','-')]=kwargs['headers'][o]
                    else:
                        return {"state": -1, "msg": "http request headers must is dict type."}
                m_headers["api-token"] = ret["secret"] if ret["secret"] else ""
                params_data = ""
                m_count = 0
                for param in m_params:
                    if param['key'].lower() != "pk":
                        if param['key'].startswith('http_header_') or param['key'].startswith('HTTP_HEADER_'):
                            if param['key'] not in m_headers:
                                if param['key'] not in kwargs:
                                    return {'state':-1,'msg':"not found parameter '%s'"%(param['key'])}
                                m_headers[param['key'].replace('_',"-").replace('http-header-','').replace('HTTP-HEADER-','')]= kwargs[param['key']]
                        else:
                            if param["key"] in kwargs:
                                params_data += (
                                    param["key"] + "=" + (parse.quote(kwargs[param["key"]]) if isinstance(kwargs[param["key"]],str) else parse.quote(str(kwargs[param["key"]])))
                                )
                            else:
                                if param["defaultValue"]:
                                    params_data += (
                                        param["key"]
                                        + "="
                                        + (parse.quote(param["defaultValue"]) if isinstance(param["defaultValue"],str) else parse.quote(str(param["defaultValue"])))
                                    )
                            m_count += 1
                        ...
                        m_uri = m_uri.replace(
                            "{pk}", parse.quote(kwargs[param["key"]]) if isinstance(kwargs[param["key"]],str) else parse.quote(str(kwargs[param["key"]]))
                        )
                if params_data:
            ...
            try:
                if m_method == u"GET":
                    ...
                    state, ret = GET(uri=m_uri, http_headers=m_headers, time_out=15)
                    if state == 200:
                        try:
                            ret = json.loads(str(ret, encoding="utf-8"))
                        ...
                        try:
                            ...
                            return {"state": state, "msg": 'remote call "%s" error.'%m_uri}
                elif m_method == u"POST":
                    logging.info(
                        "access api:(%s), method(POST),postdata:%s."
                        % (m_uri, params_data)
                    )
                    state, ret = POST(
                        m_uri,
                        bytes(params_data, encoding="utf-8"),
                    ...
                    logging.info(
                        "access api:(%s), method(PUT),postdata:%s."
                        % (m_uri, params_data)
                    )
