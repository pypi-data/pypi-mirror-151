...
class register:
    ...
    def _register_permission(self, config, **kwargs):
        ...
        if state == 200:
            ...
            logging.info("create permission '%s' failed." % config["permission_name"])
...
class perACL:
    def __init__(
        self, rpc_key, params_container_class, platformKey=None, tokenKey="usr-token"
    ):
...
if __name__ == "__main__":
    P = register(
        ["http://192.168.50.2:9003"], "CMS_SERVICE", "d74218cff240f446d341c4a2f3a8c588"
    )
    per_register = None
    with open('C:\Home\dev\Python\pypi\src\TDhelper\web\permission.json', mode='r', encoding='utf-8') as fs:
        per_config = json.load(fs)
        fs.close()
    P.RegisterByCfg(per_config)
