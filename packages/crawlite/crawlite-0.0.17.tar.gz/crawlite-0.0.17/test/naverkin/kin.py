...
def event_listener(module, event, count):
    print(module, event, count)
    return True

def test_naverkin(keywords, page_range):
    nkc = NaverKinCrawler(keywords, page_range,
        settings=settings,crawl_listener=event_listener
    )
    nkc.crawl()
    return nkc
