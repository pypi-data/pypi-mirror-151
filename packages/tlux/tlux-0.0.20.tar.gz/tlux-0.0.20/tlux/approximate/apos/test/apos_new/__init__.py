...
_shared_object_name = "apos_new." + platform.machine() + ".so"
...
_ordered_dependencies = ['apos_0-0-20.f90', 'apos_new_c_wrapper.f90']
# 
# --------------------------------------------------------------------
#               AUTO-COMPILING
#
...
! radializing data matrices with the SVD.'''
    ...
    def orthogonalize(self, a, lengths, rank=None, order=None):
...
class apos:
    ...
    class MODEL_CONFIG(ctypes.Structure):
        ...
        def __init__(self, value=0, **kwargs):
            ...
            self.step_replacement = kwargs.get("step_replacement", getattr(value, "step_replacement", value))
            ...
            self.orthogonalizing_step_frequency = kwargs.get("orthogonalizing_step_frequency", getattr(value, "orthogonalizing_step_frequency", value))
            ...
            self.equalize_y = kwargs.get("equalize_y", getattr(value, "equalize_y", value))
...
!  gradient by aggregating over the repeated occurrences of the embedding.'''
    ...
    def model_gradient(self, config, model, ax, axi, ay, sizes, x, xi, y, yw, sum_squared_gradient, model_grad, ay_gradient, y_gradient, a_states, a_grads, m_states, m_grads, info):
...
! model with respect to its variables given input and output pairs.'''
        ...
        # Call C-accessible Fortran wrapper.
        clib.c_model_gradient(ctypes.byref(config), ctypes.byref(model_dim_1), ctypes.c_void_p(model.ctypes.data), ctypes.byref(ax_dim_1), ctypes.byref(ax_dim_2), ctypes.c_void_p(ax.ctypes.data), ctypes.byref(axi_dim_1), ctypes.byref(axi_dim_2), ctypes.c_void_p(axi.ctypes.data), ctypes.byref(ay_dim_1), ctypes.byref(ay_dim_2), ctypes.c_void_p(ay.ctypes.data), ctypes.byref(sizes_dim_1), ctypes.c_void_p(sizes.ctypes.data), ctypes.byref(x_dim_1), ctypes.byref(x_dim_2), ctypes.c_void_p(x.ctypes.data), ctypes.byref(xi_dim_1), ctypes.byref(xi_dim_2), ctypes.c_void_p(xi.ctypes.data), ctypes.byref(y_dim_1), ctypes.byref(y_dim_2), ctypes.c_void_p(y.ctypes.data), ctypes.byref(yw_dim_1), ctypes.byref(yw_dim_2), ctypes.c_void_p(yw.ctypes.data), ctypes.byref(sum_squared_gradient), ctypes.byref(model_grad_dim_1), ctypes.c_void_p(model_grad.ctypes.data), ctypes.byref(ay_gradient_dim_1), ctypes.byref(ay_gradient_dim_2), ctypes.c_void_p(ay_gradient.ctypes.data), ctypes.byref(y_gradient_dim_1), ctypes.byref(y_gradient_dim_2), ctypes.c_void_p(y_gradient.ctypes.data), ctypes.byref(a_states_dim_1), ctypes.byref(a_states_dim_2), ctypes.byref(a_states_dim_3), ctypes.c_void_p(a_states.ctypes.data), ctypes.byref(a_grads_dim_1), ctypes.byref(a_grads_dim_2), ctypes.byref(a_grads_dim_3), ctypes.c_void_p(a_grads.ctypes.data), ctypes.byref(m_states_dim_1), ctypes.byref(m_states_dim_2), ctypes.byref(m_states_dim_3), ctypes.c_void_p(m_states.ctypes.data), ctypes.byref(m_grads_dim_1), ctypes.byref(m_grads_dim_2), ctypes.byref(m_grads_dim_3), ctypes.c_void_p(m_grads.ctypes.data), ctypes.byref(info))
    
        # Return final results, 'INTENT(OUT)' arguments only.
        return ax, ay, x, sum_squared_gradient.value, model_grad, ay_gradient, y_gradient, a_states, a_grads, m_states, m_grads, info.value

    
    # ----------------------------------------------
    # Wrapper for the Fortran subroutine NORMALIZE_DATA
    
    def normalize_data(self, config, model, ax, axi, ay, sizes, x, xi, y, yw, ax_rescale, axi_shift, axi_rescale, ay_rescale, x_rescale, xi_shift, xi_rescale, y_rescale, a_states, a_emb_vecs, m_emb_vecs, a_out_vecs, info):
...
!  more well spaced and lower the curvature of the error gradient).'''
        ...
        # Return final results, 'INTENT(OUT)' arguments only.
        return config, model, ax, ay, x, y, yw, ax_rescale, axi_shift, axi_rescale, ay_rescale, x_rescale, xi_shift, xi_rescale, y_rescale, a_states, a_emb_vecs, m_emb_vecs, a_out_vecs, info.value

    
    # ----------------------------------------------
    # Wrapper for the Fortran subroutine CONDITION_MODEL
    
    def condition_model(self, config, model, a_input_vecs, a_input_shift, a_state_vecs, a_state_shift, a_output_vecs, ay_shift, m_input_vecs, m_input_shift, m_state_vecs, m_state_shift, m_output_vecs, a_input_vecs_grad_mean, a_input_shift_grad_mean, a_state_vecs_grad_mean, a_state_shift_grad_mean, a_output_vecs_grad_mean, m_input_vecs_grad_mean, m_input_shift_grad_mean, m_state_vecs_grad_mean, m_state_shift_grad_mean, m_output_vecs_grad_mean, a_input_vecs_grad_curv, a_input_shift_grad_curv, a_state_vecs_grad_curv, a_state_shift_grad_curv, a_output_vecs_grad_curv, m_input_vecs_grad_curv, m_input_shift_grad_curv, m_state_vecs_grad_curv, m_state_shift_grad_curv, m_output_vecs_grad_curv, ax, axi, ay, ay_gradient, x, xi, y, y_gradient, num_threads, fit_step, a_states, m_states, a_grads, m_grads, a_lengths, m_lengths, a_state_temp, m_state_temp, a_order, m_order, total_eval_rank, total_grad_rank):
...
!  (ensure that mean squared error is effectively reduced).'''
        ...
        # Setting up "a_input_vecs"
        if ((not issubclass(type(a_input_vecs), numpy.ndarray)) or
            (not numpy.asarray(a_input_vecs).flags.f_contiguous) or
            (not (a_input_vecs.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_input_vecs' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_input_vecs = numpy.asarray(a_input_vecs, dtype=ctypes.c_float, order='F')
        a_input_vecs_dim_1 = ctypes.c_long(a_input_vecs.shape[0])
        a_input_vecs_dim_2 = ctypes.c_long(a_input_vecs.shape[1])
        
        # Setting up "a_input_shift"
        if ((not issubclass(type(a_input_shift), numpy.ndarray)) or
            (not numpy.asarray(a_input_shift).flags.f_contiguous) or
            (not (a_input_shift.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_input_shift' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_input_shift = numpy.asarray(a_input_shift, dtype=ctypes.c_float, order='F')
        a_input_shift_dim_1 = ctypes.c_long(a_input_shift.shape[0])
        
        # Setting up "a_state_vecs"
        if ((not issubclass(type(a_state_vecs), numpy.ndarray)) or
            (not numpy.asarray(a_state_vecs).flags.f_contiguous) or
            (not (a_state_vecs.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_state_vecs' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_state_vecs = numpy.asarray(a_state_vecs, dtype=ctypes.c_float, order='F')
        a_state_vecs_dim_1 = ctypes.c_long(a_state_vecs.shape[0])
        a_state_vecs_dim_2 = ctypes.c_long(a_state_vecs.shape[1])
        a_state_vecs_dim_3 = ctypes.c_long(a_state_vecs.shape[2])
        
        # Setting up "a_state_shift"
        if ((not issubclass(type(a_state_shift), numpy.ndarray)) or
            (not numpy.asarray(a_state_shift).flags.f_contiguous) or
            (not (a_state_shift.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_state_shift' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_state_shift = numpy.asarray(a_state_shift, dtype=ctypes.c_float, order='F')
        a_state_shift_dim_1 = ctypes.c_long(a_state_shift.shape[0])
        a_state_shift_dim_2 = ctypes.c_long(a_state_shift.shape[1])
        
        # Setting up "a_output_vecs"
        if ((not issubclass(type(a_output_vecs), numpy.ndarray)) or
            (not numpy.asarray(a_output_vecs).flags.f_contiguous) or
            (not (a_output_vecs.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_output_vecs' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_output_vecs = numpy.asarray(a_output_vecs, dtype=ctypes.c_float, order='F')
        a_output_vecs_dim_1 = ctypes.c_long(a_output_vecs.shape[0])
        a_output_vecs_dim_2 = ctypes.c_long(a_output_vecs.shape[1])
        ...
        # Setting up "m_input_vecs"
        if ((not issubclass(type(m_input_vecs), numpy.ndarray)) or
            (not numpy.asarray(m_input_vecs).flags.f_contiguous) or
            (not (m_input_vecs.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_input_vecs' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_input_vecs = numpy.asarray(m_input_vecs, dtype=ctypes.c_float, order='F')
        m_input_vecs_dim_1 = ctypes.c_long(m_input_vecs.shape[0])
        m_input_vecs_dim_2 = ctypes.c_long(m_input_vecs.shape[1])
        
        # Setting up "m_input_shift"
        if ((not issubclass(type(m_input_shift), numpy.ndarray)) or
            (not numpy.asarray(m_input_shift).flags.f_contiguous) or
            (not (m_input_shift.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_input_shift' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_input_shift = numpy.asarray(m_input_shift, dtype=ctypes.c_float, order='F')
        m_input_shift_dim_1 = ctypes.c_long(m_input_shift.shape[0])
        
        # Setting up "m_state_vecs"
        if ((not issubclass(type(m_state_vecs), numpy.ndarray)) or
            (not numpy.asarray(m_state_vecs).flags.f_contiguous) or
            (not (m_state_vecs.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_state_vecs' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_state_vecs = numpy.asarray(m_state_vecs, dtype=ctypes.c_float, order='F')
        m_state_vecs_dim_1 = ctypes.c_long(m_state_vecs.shape[0])
        m_state_vecs_dim_2 = ctypes.c_long(m_state_vecs.shape[1])
        m_state_vecs_dim_3 = ctypes.c_long(m_state_vecs.shape[2])
        
        # Setting up "m_state_shift"
        if ((not issubclass(type(m_state_shift), numpy.ndarray)) or
            (not numpy.asarray(m_state_shift).flags.f_contiguous) or
            (not (m_state_shift.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_state_shift' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_state_shift = numpy.asarray(m_state_shift, dtype=ctypes.c_float, order='F')
        m_state_shift_dim_1 = ctypes.c_long(m_state_shift.shape[0])
        m_state_shift_dim_2 = ctypes.c_long(m_state_shift.shape[1])
        
        # Setting up "m_output_vecs"
        if ((not issubclass(type(m_output_vecs), numpy.ndarray)) or
            (not numpy.asarray(m_output_vecs).flags.f_contiguous) or
            (not (m_output_vecs.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_output_vecs' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_output_vecs = numpy.asarray(m_output_vecs, dtype=ctypes.c_float, order='F')
        m_output_vecs_dim_1 = ctypes.c_long(m_output_vecs.shape[0])
        m_output_vecs_dim_2 = ctypes.c_long(m_output_vecs.shape[1])
        
        # Setting up "a_input_vecs_grad_mean"
        if ((not issubclass(type(a_input_vecs_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(a_input_vecs_grad_mean).flags.f_contiguous) or
            (not (a_input_vecs_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_input_vecs_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_input_vecs_grad_mean = numpy.asarray(a_input_vecs_grad_mean, dtype=ctypes.c_float, order='F')
        a_input_vecs_grad_mean_dim_1 = ctypes.c_long(a_input_vecs_grad_mean.shape[0])
        a_input_vecs_grad_mean_dim_2 = ctypes.c_long(a_input_vecs_grad_mean.shape[1])
        
        # Setting up "a_input_shift_grad_mean"
        if ((not issubclass(type(a_input_shift_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(a_input_shift_grad_mean).flags.f_contiguous) or
            (not (a_input_shift_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_input_shift_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_input_shift_grad_mean = numpy.asarray(a_input_shift_grad_mean, dtype=ctypes.c_float, order='F')
        a_input_shift_grad_mean_dim_1 = ctypes.c_long(a_input_shift_grad_mean.shape[0])
        
        # Setting up "a_state_vecs_grad_mean"
        if ((not issubclass(type(a_state_vecs_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(a_state_vecs_grad_mean).flags.f_contiguous) or
            (not (a_state_vecs_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_state_vecs_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_state_vecs_grad_mean = numpy.asarray(a_state_vecs_grad_mean, dtype=ctypes.c_float, order='F')
        a_state_vecs_grad_mean_dim_1 = ctypes.c_long(a_state_vecs_grad_mean.shape[0])
        a_state_vecs_grad_mean_dim_2 = ctypes.c_long(a_state_vecs_grad_mean.shape[1])
        a_state_vecs_grad_mean_dim_3 = ctypes.c_long(a_state_vecs_grad_mean.shape[2])
        
        # Setting up "a_state_shift_grad_mean"
        if ((not issubclass(type(a_state_shift_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(a_state_shift_grad_mean).flags.f_contiguous) or
            (not (a_state_shift_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_state_shift_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_state_shift_grad_mean = numpy.asarray(a_state_shift_grad_mean, dtype=ctypes.c_float, order='F')
        a_state_shift_grad_mean_dim_1 = ctypes.c_long(a_state_shift_grad_mean.shape[0])
        a_state_shift_grad_mean_dim_2 = ctypes.c_long(a_state_shift_grad_mean.shape[1])
        
        # Setting up "a_output_vecs_grad_mean"
        if ((not issubclass(type(a_output_vecs_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(a_output_vecs_grad_mean).flags.f_contiguous) or
            (not (a_output_vecs_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_output_vecs_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_output_vecs_grad_mean = numpy.asarray(a_output_vecs_grad_mean, dtype=ctypes.c_float, order='F')
        a_output_vecs_grad_mean_dim_1 = ctypes.c_long(a_output_vecs_grad_mean.shape[0])
        a_output_vecs_grad_mean_dim_2 = ctypes.c_long(a_output_vecs_grad_mean.shape[1])
        
        # Setting up "m_input_vecs_grad_mean"
        if ((not issubclass(type(m_input_vecs_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(m_input_vecs_grad_mean).flags.f_contiguous) or
            (not (m_input_vecs_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_input_vecs_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_input_vecs_grad_mean = numpy.asarray(m_input_vecs_grad_mean, dtype=ctypes.c_float, order='F')
        m_input_vecs_grad_mean_dim_1 = ctypes.c_long(m_input_vecs_grad_mean.shape[0])
        m_input_vecs_grad_mean_dim_2 = ctypes.c_long(m_input_vecs_grad_mean.shape[1])
        
        # Setting up "m_input_shift_grad_mean"
        if ((not issubclass(type(m_input_shift_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(m_input_shift_grad_mean).flags.f_contiguous) or
            (not (m_input_shift_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_input_shift_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_input_shift_grad_mean = numpy.asarray(m_input_shift_grad_mean, dtype=ctypes.c_float, order='F')
        m_input_shift_grad_mean_dim_1 = ctypes.c_long(m_input_shift_grad_mean.shape[0])
        
        # Setting up "m_state_vecs_grad_mean"
        if ((not issubclass(type(m_state_vecs_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(m_state_vecs_grad_mean).flags.f_contiguous) or
            (not (m_state_vecs_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_state_vecs_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_state_vecs_grad_mean = numpy.asarray(m_state_vecs_grad_mean, dtype=ctypes.c_float, order='F')
        m_state_vecs_grad_mean_dim_1 = ctypes.c_long(m_state_vecs_grad_mean.shape[0])
        m_state_vecs_grad_mean_dim_2 = ctypes.c_long(m_state_vecs_grad_mean.shape[1])
        m_state_vecs_grad_mean_dim_3 = ctypes.c_long(m_state_vecs_grad_mean.shape[2])
        
        # Setting up "m_state_shift_grad_mean"
        if ((not issubclass(type(m_state_shift_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(m_state_shift_grad_mean).flags.f_contiguous) or
            (not (m_state_shift_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_state_shift_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_state_shift_grad_mean = numpy.asarray(m_state_shift_grad_mean, dtype=ctypes.c_float, order='F')
        m_state_shift_grad_mean_dim_1 = ctypes.c_long(m_state_shift_grad_mean.shape[0])
        m_state_shift_grad_mean_dim_2 = ctypes.c_long(m_state_shift_grad_mean.shape[1])
        
        # Setting up "m_output_vecs_grad_mean"
        if ((not issubclass(type(m_output_vecs_grad_mean), numpy.ndarray)) or
            (not numpy.asarray(m_output_vecs_grad_mean).flags.f_contiguous) or
            (not (m_output_vecs_grad_mean.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_output_vecs_grad_mean' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_output_vecs_grad_mean = numpy.asarray(m_output_vecs_grad_mean, dtype=ctypes.c_float, order='F')
        m_output_vecs_grad_mean_dim_1 = ctypes.c_long(m_output_vecs_grad_mean.shape[0])
        m_output_vecs_grad_mean_dim_2 = ctypes.c_long(m_output_vecs_grad_mean.shape[1])
        
        # Setting up "a_input_vecs_grad_curv"
        if ((not issubclass(type(a_input_vecs_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(a_input_vecs_grad_curv).flags.f_contiguous) or
            (not (a_input_vecs_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_input_vecs_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_input_vecs_grad_curv = numpy.asarray(a_input_vecs_grad_curv, dtype=ctypes.c_float, order='F')
        a_input_vecs_grad_curv_dim_1 = ctypes.c_long(a_input_vecs_grad_curv.shape[0])
        a_input_vecs_grad_curv_dim_2 = ctypes.c_long(a_input_vecs_grad_curv.shape[1])
        
        # Setting up "a_input_shift_grad_curv"
        if ((not issubclass(type(a_input_shift_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(a_input_shift_grad_curv).flags.f_contiguous) or
            (not (a_input_shift_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_input_shift_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_input_shift_grad_curv = numpy.asarray(a_input_shift_grad_curv, dtype=ctypes.c_float, order='F')
        a_input_shift_grad_curv_dim_1 = ctypes.c_long(a_input_shift_grad_curv.shape[0])
        
        # Setting up "a_state_vecs_grad_curv"
        if ((not issubclass(type(a_state_vecs_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(a_state_vecs_grad_curv).flags.f_contiguous) or
            (not (a_state_vecs_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_state_vecs_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_state_vecs_grad_curv = numpy.asarray(a_state_vecs_grad_curv, dtype=ctypes.c_float, order='F')
        a_state_vecs_grad_curv_dim_1 = ctypes.c_long(a_state_vecs_grad_curv.shape[0])
        a_state_vecs_grad_curv_dim_2 = ctypes.c_long(a_state_vecs_grad_curv.shape[1])
        a_state_vecs_grad_curv_dim_3 = ctypes.c_long(a_state_vecs_grad_curv.shape[2])
        
        # Setting up "a_state_shift_grad_curv"
        if ((not issubclass(type(a_state_shift_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(a_state_shift_grad_curv).flags.f_contiguous) or
            (not (a_state_shift_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_state_shift_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_state_shift_grad_curv = numpy.asarray(a_state_shift_grad_curv, dtype=ctypes.c_float, order='F')
        a_state_shift_grad_curv_dim_1 = ctypes.c_long(a_state_shift_grad_curv.shape[0])
        a_state_shift_grad_curv_dim_2 = ctypes.c_long(a_state_shift_grad_curv.shape[1])
        
        # Setting up "a_output_vecs_grad_curv"
        if ((not issubclass(type(a_output_vecs_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(a_output_vecs_grad_curv).flags.f_contiguous) or
            (not (a_output_vecs_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'a_output_vecs_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            a_output_vecs_grad_curv = numpy.asarray(a_output_vecs_grad_curv, dtype=ctypes.c_float, order='F')
        a_output_vecs_grad_curv_dim_1 = ctypes.c_long(a_output_vecs_grad_curv.shape[0])
        a_output_vecs_grad_curv_dim_2 = ctypes.c_long(a_output_vecs_grad_curv.shape[1])
        
        # Setting up "m_input_vecs_grad_curv"
        if ((not issubclass(type(m_input_vecs_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(m_input_vecs_grad_curv).flags.f_contiguous) or
            (not (m_input_vecs_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_input_vecs_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_input_vecs_grad_curv = numpy.asarray(m_input_vecs_grad_curv, dtype=ctypes.c_float, order='F')
        m_input_vecs_grad_curv_dim_1 = ctypes.c_long(m_input_vecs_grad_curv.shape[0])
        m_input_vecs_grad_curv_dim_2 = ctypes.c_long(m_input_vecs_grad_curv.shape[1])
        
        # Setting up "m_input_shift_grad_curv"
        if ((not issubclass(type(m_input_shift_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(m_input_shift_grad_curv).flags.f_contiguous) or
            (not (m_input_shift_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_input_shift_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_input_shift_grad_curv = numpy.asarray(m_input_shift_grad_curv, dtype=ctypes.c_float, order='F')
        m_input_shift_grad_curv_dim_1 = ctypes.c_long(m_input_shift_grad_curv.shape[0])
        
        # Setting up "m_state_vecs_grad_curv"
        if ((not issubclass(type(m_state_vecs_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(m_state_vecs_grad_curv).flags.f_contiguous) or
            (not (m_state_vecs_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_state_vecs_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_state_vecs_grad_curv = numpy.asarray(m_state_vecs_grad_curv, dtype=ctypes.c_float, order='F')
        m_state_vecs_grad_curv_dim_1 = ctypes.c_long(m_state_vecs_grad_curv.shape[0])
        m_state_vecs_grad_curv_dim_2 = ctypes.c_long(m_state_vecs_grad_curv.shape[1])
        m_state_vecs_grad_curv_dim_3 = ctypes.c_long(m_state_vecs_grad_curv.shape[2])
        
        # Setting up "m_state_shift_grad_curv"
        if ((not issubclass(type(m_state_shift_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(m_state_shift_grad_curv).flags.f_contiguous) or
            (not (m_state_shift_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_state_shift_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_state_shift_grad_curv = numpy.asarray(m_state_shift_grad_curv, dtype=ctypes.c_float, order='F')
        m_state_shift_grad_curv_dim_1 = ctypes.c_long(m_state_shift_grad_curv.shape[0])
        m_state_shift_grad_curv_dim_2 = ctypes.c_long(m_state_shift_grad_curv.shape[1])
        
        # Setting up "m_output_vecs_grad_curv"
        if ((not issubclass(type(m_output_vecs_grad_curv), numpy.ndarray)) or
            (not numpy.asarray(m_output_vecs_grad_curv).flags.f_contiguous) or
            (not (m_output_vecs_grad_curv.dtype == numpy.dtype(ctypes.c_float)))):
            import warnings
            warnings.warn("The provided argument 'm_output_vecs_grad_curv' was not an f_contiguous NumPy array of type 'ctypes.c_float' (or equivalent). Automatically converting (probably creating a full copy).")
            m_output_vecs_grad_curv = numpy.asarray(m_output_vecs_grad_curv, dtype=ctypes.c_float, order='F')
        m_output_vecs_grad_curv_dim_1 = ctypes.c_long(m_output_vecs_grad_curv.shape[0])
        m_output_vecs_grad_curv_dim_2 = ctypes.c_long(m_output_vecs_grad_curv.shape[1])
        ...
        # Call C-accessible Fortran wrapper.
        clib.c_condition_model(ctypes.byref(config), ctypes.byref(model_dim_1), ctypes.c_void_p(model.ctypes.data), ctypes.byref(a_input_vecs_dim_1), ctypes.byref(a_input_vecs_dim_2), ctypes.c_void_p(a_input_vecs.ctypes.data), ctypes.byref(a_input_shift_dim_1), ctypes.c_void_p(a_input_shift.ctypes.data), ctypes.byref(a_state_vecs_dim_1), ctypes.byref(a_state_vecs_dim_2), ctypes.byref(a_state_vecs_dim_3), ctypes.c_void_p(a_state_vecs.ctypes.data), ctypes.byref(a_state_shift_dim_1), ctypes.byref(a_state_shift_dim_2), ctypes.c_void_p(a_state_shift.ctypes.data), ctypes.byref(a_output_vecs_dim_1), ctypes.byref(a_output_vecs_dim_2), ctypes.c_void_p(a_output_vecs.ctypes.data), ctypes.byref(ay_shift_dim_1), ctypes.c_void_p(ay_shift.ctypes.data), ctypes.byref(m_input_vecs_dim_1), ctypes.byref(m_input_vecs_dim_2), ctypes.c_void_p(m_input_vecs.ctypes.data), ctypes.byref(m_input_shift_dim_1), ctypes.c_void_p(m_input_shift.ctypes.data), ctypes.byref(m_state_vecs_dim_1), ctypes.byref(m_state_vecs_dim_2), ctypes.byref(m_state_vecs_dim_3), ctypes.c_void_p(m_state_vecs.ctypes.data), ctypes.byref(m_state_shift_dim_1), ctypes.byref(m_state_shift_dim_2), ctypes.c_void_p(m_state_shift.ctypes.data), ctypes.byref(m_output_vecs_dim_1), ctypes.byref(m_output_vecs_dim_2), ctypes.c_void_p(m_output_vecs.ctypes.data), ctypes.byref(a_input_vecs_grad_mean_dim_1), ctypes.byref(a_input_vecs_grad_mean_dim_2), ctypes.c_void_p(a_input_vecs_grad_mean.ctypes.data), ctypes.byref(a_input_shift_grad_mean_dim_1), ctypes.c_void_p(a_input_shift_grad_mean.ctypes.data), ctypes.byref(a_state_vecs_grad_mean_dim_1), ctypes.byref(a_state_vecs_grad_mean_dim_2), ctypes.byref(a_state_vecs_grad_mean_dim_3), ctypes.c_void_p(a_state_vecs_grad_mean.ctypes.data), ctypes.byref(a_state_shift_grad_mean_dim_1), ctypes.byref(a_state_shift_grad_mean_dim_2), ctypes.c_void_p(a_state_shift_grad_mean.ctypes.data), ctypes.byref(a_output_vecs_grad_mean_dim_1), ctypes.byref(a_output_vecs_grad_mean_dim_2), ctypes.c_void_p(a_output_vecs_grad_mean.ctypes.data), ctypes.byref(m_input_vecs_grad_mean_dim_1), ctypes.byref(m_input_vecs_grad_mean_dim_2), ctypes.c_void_p(m_input_vecs_grad_mean.ctypes.data), ctypes.byref(m_input_shift_grad_mean_dim_1), ctypes.c_void_p(m_input_shift_grad_mean.ctypes.data), ctypes.byref(m_state_vecs_grad_mean_dim_1), ctypes.byref(m_state_vecs_grad_mean_dim_2), ctypes.byref(m_state_vecs_grad_mean_dim_3), ctypes.c_void_p(m_state_vecs_grad_mean.ctypes.data), ctypes.byref(m_state_shift_grad_mean_dim_1), ctypes.byref(m_state_shift_grad_mean_dim_2), ctypes.c_void_p(m_state_shift_grad_mean.ctypes.data), ctypes.byref(m_output_vecs_grad_mean_dim_1), ctypes.byref(m_output_vecs_grad_mean_dim_2), ctypes.c_void_p(m_output_vecs_grad_mean.ctypes.data), ctypes.byref(a_input_vecs_grad_curv_dim_1), ctypes.byref(a_input_vecs_grad_curv_dim_2), ctypes.c_void_p(a_input_vecs_grad_curv.ctypes.data), ctypes.byref(a_input_shift_grad_curv_dim_1), ctypes.c_void_p(a_input_shift_grad_curv.ctypes.data), ctypes.byref(a_state_vecs_grad_curv_dim_1), ctypes.byref(a_state_vecs_grad_curv_dim_2), ctypes.byref(a_state_vecs_grad_curv_dim_3), ctypes.c_void_p(a_state_vecs_grad_curv.ctypes.data), ctypes.byref(a_state_shift_grad_curv_dim_1), ctypes.byref(a_state_shift_grad_curv_dim_2), ctypes.c_void_p(a_state_shift_grad_curv.ctypes.data), ctypes.byref(a_output_vecs_grad_curv_dim_1), ctypes.byref(a_output_vecs_grad_curv_dim_2), ctypes.c_void_p(a_output_vecs_grad_curv.ctypes.data), ctypes.byref(m_input_vecs_grad_curv_dim_1), ctypes.byref(m_input_vecs_grad_curv_dim_2), ctypes.c_void_p(m_input_vecs_grad_curv.ctypes.data), ctypes.byref(m_input_shift_grad_curv_dim_1), ctypes.c_void_p(m_input_shift_grad_curv.ctypes.data), ctypes.byref(m_state_vecs_grad_curv_dim_1), ctypes.byref(m_state_vecs_grad_curv_dim_2), ctypes.byref(m_state_vecs_grad_curv_dim_3), ctypes.c_void_p(m_state_vecs_grad_curv.ctypes.data), ctypes.byref(m_state_shift_grad_curv_dim_1), ctypes.byref(m_state_shift_grad_curv_dim_2), ctypes.c_void_p(m_state_shift_grad_curv.ctypes.data), ctypes.byref(m_output_vecs_grad_curv_dim_1), ctypes.byref(m_output_vecs_grad_curv_dim_2), ctypes.c_void_p(m_output_vecs_grad_curv.ctypes.data), ctypes.byref(ax_dim_1), ctypes.byref(ax_dim_2), ctypes.c_void_p(ax.ctypes.data), ctypes.byref(axi_dim_1), ctypes.byref(axi_dim_2), ctypes.c_void_p(axi.ctypes.data), ctypes.byref(ay_dim_1), ctypes.byref(ay_dim_2), ctypes.c_void_p(ay.ctypes.data), ctypes.byref(ay_gradient_dim_1), ctypes.byref(ay_gradient_dim_2), ctypes.c_void_p(ay_gradient.ctypes.data), ctypes.byref(x_dim_1), ctypes.byref(x_dim_2), ctypes.c_void_p(x.ctypes.data), ctypes.byref(xi_dim_1), ctypes.byref(xi_dim_2), ctypes.c_void_p(xi.ctypes.data), ctypes.byref(y_dim_1), ctypes.byref(y_dim_2), ctypes.c_void_p(y.ctypes.data), ctypes.byref(y_gradient_dim_1), ctypes.byref(y_gradient_dim_2), ctypes.c_void_p(y_gradient.ctypes.data), ctypes.byref(num_threads), ctypes.byref(fit_step), ctypes.byref(a_states_dim_1), ctypes.byref(a_states_dim_2), ctypes.byref(a_states_dim_3), ctypes.c_void_p(a_states.ctypes.data), ctypes.byref(m_states_dim_1), ctypes.byref(m_states_dim_2), ctypes.byref(m_states_dim_3), ctypes.c_void_p(m_states.ctypes.data), ctypes.byref(a_grads_dim_1), ctypes.byref(a_grads_dim_2), ctypes.byref(a_grads_dim_3), ctypes.c_void_p(a_grads.ctypes.data), ctypes.byref(m_grads_dim_1), ctypes.byref(m_grads_dim_2), ctypes.byref(m_grads_dim_3), ctypes.c_void_p(m_grads.ctypes.data), ctypes.byref(a_lengths_dim_1), ctypes.byref(a_lengths_dim_2), ctypes.c_void_p(a_lengths.ctypes.data), ctypes.byref(m_lengths_dim_1), ctypes.byref(m_lengths_dim_2), ctypes.c_void_p(m_lengths.ctypes.data), ctypes.byref(a_state_temp_dim_1), ctypes.byref(a_state_temp_dim_2), ctypes.c_void_p(a_state_temp.ctypes.data), ctypes.byref(m_state_temp_dim_1), ctypes.byref(m_state_temp_dim_2), ctypes.c_void_p(m_state_temp.ctypes.data), ctypes.byref(a_order_dim_1), ctypes.byref(a_order_dim_2), ctypes.c_void_p(a_order.ctypes.data), ctypes.byref(m_order_dim_1), ctypes.byref(m_order_dim_2), ctypes.c_void_p(m_order.ctypes.data), ctypes.byref(total_eval_rank), ctypes.byref(total_grad_rank))
    
        # Return final results, 'INTENT(OUT)' arguments only.
        return model, a_input_vecs, a_input_shift, a_state_vecs, a_state_shift, a_output_vecs, ay_shift, m_input_vecs, m_input_shift, m_state_vecs, m_state_shift, m_output_vecs, a_input_vecs_grad_mean, a_input_shift_grad_mean, a_state_vecs_grad_mean, a_state_shift_grad_mean, a_output_vecs_grad_mean, m_input_vecs_grad_mean, m_input_shift_grad_mean, m_state_vecs_grad_mean, m_state_shift_grad_mean, m_output_vecs_grad_mean, a_input_vecs_grad_curv, a_input_shift_grad_curv, a_state_vecs_grad_curv, a_state_shift_grad_curv, a_output_vecs_grad_curv, m_input_vecs_grad_curv, m_input_shift_grad_curv, m_state_vecs_grad_curv, m_state_shift_grad_curv, m_output_vecs_grad_curv, ax, x, a_states, m_states, a_grads, m_grads, a_lengths, m_lengths, a_state_temp, m_state_temp, a_order, m_order, total_eval_rank.value, total_grad_rank.value

    
    # ----------------------------------------------
    # Wrapper for the Fortran subroutine MINIMIZE_MSE
    
    def minimize_mse(self, config, model, rwork, iwork, ax, axi, sizes, x, xi, y, yw, steps, record=None):
        ...
        # Return final results, 'INTENT(OUT)' arguments only.
        return config, model, rwork, iwork, ax, x, y, yw, (record if record_present else None), sum_squared_error.value, info.value
