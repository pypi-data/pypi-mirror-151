...
class BatchEndpointOperations(_ScopeDependentOperations):
    ...
    def invoke(
        ...
        **kwargs,
        ...
        :param (Union[str, Input], optional) input: To use a pre-registered dataset asset, pass str in format "<dataset-name>:<dataset-version>". To use a uri file or folder pass in a Input object, for batch endpoints only.
        ...
        if isinstance(input, str):
            name, version = parse_name_version(input)
            if name and version:
                input_data_id = generate_data_arm_id(self._operation_scope, name, version)
            else:
                msg = (
                    "Input data needs to be in the supported format: \n"
                    "If you wanted to use AzureML Registered dataset, please use the format "
                    '"azureml:<data-name>:<data-version>" for --input. \n'
                    "If you wanted to use files/folder on datastore, please use the format "
                    '"azureml://datastores/<datastore-name>/paths/<path-to-data>/<folder" name or file name>'
                    "for --input and specify --input-type (either uri_file or uri_folder).\n"
                    'If you wanted to use files/folder on public URI, please use the format "https://<uri>" '
                    "for --input and specify --input-type (either uri_file or uri_folder)."
                )
        ...
        try:
            if os.path.isabs(entry.path):  # absolute local path, upload, transform to remote url
                if entry.type == AssetTypes.URI_FOLDER and not os.path.isdir(entry.path):
                    raise ValidationException(
                        messge="There is no folder on target path: {}".format(entry.path),
                        target=ErrorTarget.BATCH_ENDPOINT,
            ...
            raise ValidationException(
                message=f"Supported input path value are: path on the datastore, public URI, a registered dataset, or a local folder path.\n"
                f"Met {type(e)}:\n{e}",
                target=ErrorTarget.BATCH_ENDPOINT,
                no_personal_data_message="Supported input path value are: path on the datastore, public URI, a registered dataset, or a local folder path.",
                error=e,
            )
