...
from gateways.common.cs18_api_responses import (AccountStatusResponse, BlueprintFileResponse, BlueprintResponse,
                                                ...
                                                BlueprintRawResponse, BlueprintCandidateResponse)
...
class Colony:
    ...
    def update_blueprint(self, updated_blueprint: UpdatedBlueprint, blueprint_name: str,
                         deployment_type: str = 'sandbox', space_name: str = None):
        space_name = space_name or self.space
        method_url = self._api_root.blueprint.blueprint_crud(space_name, deployment_type, blueprint_name)
        resp = self.session.put(url=method_url, json=updated_blueprint.__dict__)
        GatewayUtils.handle_response(response=resp, return_codes=[200])

    def delete_blueprint(self, blueprint_name: str,
                         deployment_type: str = 'sandbox', space_name: str = None):
        space_name = space_name or self.space
        method_url = self._api_root.blueprint.blueprint_crud(space_name, deployment_type, blueprint_name)
    ...
    def get_blueprint_candidates(
            self, space_name: str = None
    ) -> List[BlueprintCandidateResponse]:
        selected_space = space_name or self.space
        method_url = self._api_root.blueprint.get_blueprint_candidates(selected_space)
        resp = self.session.get(url=method_url)
        GatewayUtils.handle_response(response=resp, return_codes=[200, 400, 404])
        if resp.status_code in [400, 404]:
            return []
        blueprint_candidates_json = json.loads(resp.text)
        all_blueprint_candidates = [
            Converters.create_blueprint_candidate_response(blueprint_candidate)
            for blueprint_candidate in blueprint_candidates_json
        ]
        return all_blueprint_candidates
    ...
    def create_blueprints_from_candidates(self, candidates: List[BlueprintCandidateResponse]):
        method_url = self._api_root.blueprint.create_blueprints_from_candidates(space_name=self.space)
        candidates_request = [
            {"name": candidate.name,
        ...
        request = {"BlueprintCandidates": candidates_request}
        resp = self.session.post(url=method_url, json=request)
        GatewayUtils.handle_response(response=resp, return_codes=[200])
    ...
    def get_editable_blueprint(
            self, deployment_type: str, blueprint_name: str
    ) -> BlueprintRawResponse:
        method_url = self._api_root.blueprint.blueprint_crud(
            deployment_type=deployment_type, space_name=self.space, blueprint_name=blueprint_name
        )
        resp = self.session.get(url=method_url)
        GatewayUtils.handle_response(response=resp, return_codes=[200])
        blueprint = json.loads(resp.text)
        return Converters.create_blueprint_raw_response(blueprint)
