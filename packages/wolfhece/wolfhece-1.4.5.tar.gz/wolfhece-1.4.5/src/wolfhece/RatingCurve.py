...
from wx.core import CustomDataFormat
...
class ratingcurve():
    ...
    def plot(self,factextra=1.,newfig=False):
        
        curpart:coeffratingcurve
        ...
        plt.plot(np.asarray(x).flatten(),np.asarray(y).flatten(),label=self.startdate.strftime(FMTDATE2) +' - '+self.enddate.strftime(FMTDATE2))

        extra=4
        for curpart in self.mycoeffs:
            xp=curpart.hmin
            yp=curpart.myfunc(xp)
            plt.plot([xp,xp],[min(yp-extra,yp*.8),max(yp+extra,yp*1.2)],'k-.', linewidth=1)
...
class gaugingstation():
    ...
    def gettype(self)->int:
        ...
        if self.maintainer=='SPW/DO223':
            rest=int(self.id-int(self.id/100)*100)
    ...
    def plotRT(self):
        fig=plt.figure()
        plt.suptitle('Rating curves - ' + self.name +' - '+ self.river +' - '+str(self.id))
        plt.xlabel('Water depth [m]')
        plt.ylabel('Discharge [m³/s]')
                
        for curcurve in self.mycurves:
            curcurve.plot(1.1,False)
