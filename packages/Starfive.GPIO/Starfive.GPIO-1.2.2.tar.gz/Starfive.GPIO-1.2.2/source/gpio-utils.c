...
int gpiotools_request_config(const char *device_name, unsigned int lineoffset,
				struct gpio_v2_line_config *config)
{
	...
	linfo.offset = lineoffset;
	ret = ioctl(fd1, GPIO_V2_GET_LINEINFO_IOCTL, &linfo);
	if (ret == -1) {
		ret = -errno;
		fprintf(stderr, "Failed to issue %s (%d), %s\n",
			"GPIO_GET_LINE_IOCTL", ret, strerror(errno));
	} else {
		memcpy(&config->flags, &linfo.flags, sizeof(linfo.flags));
		memcpy(&config->num_attrs, &linfo.num_attrs, sizeof(linfo.num_attrs));
		memcpy(&config->attrs, &linfo.attrs, sizeof(linfo.attrs));	
	}
...
/**
 * gpiotools_set_config() - Set the config of gpio(s)
...
int gpiotools_set_config(const int fd, struct gpio_v2_line_config *config)
{
	int ret;

	ret = ioctl(fd, GPIO_V2_LINE_SET_CONFIG_IOCTL, config);
	if (ret == -1) {
		ret = -errno;
		fprintf(stderr, "Failed to issue %s (%d), %s\n",
