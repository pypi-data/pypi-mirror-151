...
class AutoModel(DataPretreatment, FeatureProject, FeatureAnalysis, ModelBuild):
    ...
    def __init__(
        ...
        DataPretreatment.__init__(
            ...
            form=form, is_extreme=is_extreme, lower=lower, upper=upper
        )
        FeatureProject.__init__(
            ...
            is_feature_derive=is_feature_derive, derive_n=derive_n
        )
    ...
    def fit(
        ...
        if self.is_analysis:
            ...
            analysis = FeatureAnalysis(
                ...
                top_importance_k=self.top_importance_k
            )
        ...
        model_build = ModelBuild(
            baseline=self.baseline, field_dict=self.field_dict, max_depth=self.max_depth, weight=self.weight,
            lgb_params=self.lgb_params, xgb_params=self.xgb_params, early_stopping=self.early_stopping,
            verbose=self.verbose, epoch=self.epoch, is_evaluate=self.is_evaluate, evaluate_func=self.evaluate_func,
            is_adjust_parameter=self.is_adjust_parameter
        )
    ...
    def fit_transform(
        ...
        self.test_project_df = test_project_df
        print('==========\n 模型预测')
        pre_df = self.model_build.fit_transform(test_project_df, prob=prob)
        return pre_df
