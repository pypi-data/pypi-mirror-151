...
def output(importance, y_train, y_val, pre_train, pre_val, field_dict):
    if field_dict is not None:
        importance['name'] = importance['feature'].map(field_dict)
...
def rfc_model_importance(x_train, x_val, y_train, y_val, max_depth, weight, field_dict):
    ...
    feature_importance = pd.DataFrame(data={
        'feature': x_train.columns, 'importance': model.feature_importances_.round(4)})
    feature_importance = feature_importance.sort_values('importance', ascending=False)
    output(feature_importance, y_train, y_val, pre_train, pre_val, field_dict)
...
class ModelBuild:
    ...
    def fit(
        ...
        self.field_dict = self.field_dict if filed_dict is None else filed_dict
    ...
    def fit_transform(
        ...
        if self.baseline in model_name['RF']:
            ...
            pre = pre if prob else np.where(pre > 0.5, 1, 0)
    ...
    def rfc_fit(
        ...
        if self.weight is None:
            ...
            weight = self.weight
        model, feature_importance = rfc_model_importance(
            x_train, x_val, y_train, y_val, max_depth, weight, self.field_dict)
        return model, feature_importance
    ...
    def lgb_fit(
        ...
        importance = pd.DataFrame(data={'feature': x_train.columns, 'importance': importance.round(4)})
        importance = importance.sort_values('importance', ascending=False)
        pre_train = model.predict(x_train)
        pre_val = model.predict(x_val)
        pre_train = np.where(pre_train >= 0.5, 1, 0)
        pre_val = np.where(pre_val >= 0.5, 1, 0)
        output(importance, y_train, y_val, pre_train, pre_val, self.field_dict)
    ...
    def xgb_fit(
        ...
        if self.is_evaluate:
            ...
            model = xgb.train(dtrain=train_data, num_boost_round=self.epoch, evals=watchlist,
                              early_stopping_rounds=self.early_stopping, verbose_eval=self.verbose, params=xgb_params)
        importance = pd.DataFrame(list(model.get_score().items()), columns=['feature', 'importance'])
        importance = importance.sort_values('importance', ascending=False)
