...
@retry_rest_api
def download_by_file_link(file_link: str, timeout: int = 20) -> bytes:
    warnings.warn(
        'Use download_file_from_link method instead',
        DeprecationWarning,
        stacklevel=2,
    )
    resp = requests.get(
...
blank_reg = re.compile(r'[\f\t\v\u00a0\u1680\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\ufeff]+')
return_reg = re.compile(r'\r')
line_break_reg = re.compile(r'[\n]{2,}')
word_blank_reg = re.compile(r'([\u4E00-\u9FA5])[ ]+([\u4E00-\u9FA5])')


def normalize_file_content(content: str, limit: int = None) -> str:
    content = blank_reg.sub(' ', content)
    content = return_reg.sub('', content)
