...
from tdf_tools.tools.module.module_tools import ModuleTools
...
class DependencyAnalysis(object):
    def __init__(self):
        ...
        self.__moduleNameList = None
        self.__shellName = None
        
    @property
    def __initJsonData(self):
        if self.__initJsonData is None:
            self.__initJsonData = ModuleTools.getInitJsonData()
        return self.__initJsonData
    
    @property
    def __moduleNameList(self):
        if self.__moduleNameList is None:
            self.__moduleNameList = ModuleTools.getModuleNameList()
        return self.__moduleNameList
    
    @property
    def __shellName(self):
        if self.__shellName is None:
            self.__shellName = self.__initJsonData["shellName"]
        return self.__shellName
    ...
    def generate(self):
        ...
        # 读取壳模块lock文件
        self.__moduleName = self.shellName
        os.chdir(self._mNodeDict[self.shellName].modulePath)
        self.__moduleDependenciesMap = self._analysisLock()
        self._generateDependenciesMap()
