...
def create_project(db, name):
    """Create a project."""
    return db.insert("projects", name=name, pypi=name)
...
def get_project_from_pypi(db, pypi_name):
    ...
    return db.select("projects", what="name", where="pypi = ?", vals=[pypi_name])[0][
        "name"
    ]
...
class Code:
    ...
    def post(self):
        ...
        warez.clone_repo(bare_repo, working_repo)  # TODO shallow?
...
class Project:
    ...
    def get(self, project):
        ...
        return app.view.project(
            project,
            warez.Repo(code_dir / project / "source.git"),
...
class ProjectReleasePackage:
    ...
    def get(self, project, package):
        """Return the package file."""
        return code_dir / project / "releases" / package
...
class PyPIIndex:
    ...
    def post(self):
        ...
        release_name, release_remaining = re.match(
            r"([\w-]+)\-(\d+\.\d+\.\d+.*)", release_file.name
        ).groups()
