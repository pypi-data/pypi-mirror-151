...
class FileUploadModule:
    ...
    def upload_dataset_file(self, file_path: Path) -> str:
        ...
        try:
            ...
            self._impose_size_constraint(file_size, resp.impose_limit)
    ...
    def upload_model_directory(
        ...
        try:
            ...
            self._impose_size_constraint(total_size, resp.impose_limit)
        except grpc.RpcError as e:
            # TODO(blaine): differentiate on different error types.
            raise ValueError(e)
    ...
    def _impose_size_constraint(self, size: int, impose_limit: bool) -> None:
        if (size > self.MAX_SIZE) and impose_limit is True:
            raise ValueError(
                "Input is too large to be uploaded. Maximum size permitted "
                + f"is {self.MAX_SIZE / (2 ** 20)}MB. "
                + f"Input size given is {size / (2 ** 20)}MB"
            )
