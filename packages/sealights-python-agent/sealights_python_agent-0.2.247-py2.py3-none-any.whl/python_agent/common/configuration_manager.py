...
class ConfigurationManager(object):
    ...
    def init_configuration(self, is_config_cmd, token, buildsessionid, labid, tokenfile=TOKEN_FILE, buildsessionidfile=BUILD_SESSION_ID_FILE,
                           proxy=None, scm_args=None, test_selection_enable=True):
        self.try_load_configuration(scm_args, token, buildsessionid, tokenfile, buildsessionidfile, proxy,
                                    is_config_cmd, test_selection_enable, labid)
        self.init_features()
    ...
    def try_load_configuration(self, scm_args, token, buildsessionid, tokenfile, buildsessionidfile, proxy, is_config_cmd, test_selection_enable, labid):
        ...
        if not is_config_cmd:
            is_resolved_build_session_id = self.resolve_build_session_id(buildsessionid, buildsessionidfile, self.config_data.buildSessionIdFile, labid, backend_proxy)
    ...
    def resolve_build_session_id(self, buildsessionid, buildsessionidfile, env_buildsessionidfile, labid, backend_proxy):
        build_session_id = buildsessionid or self.read_from_file(buildsessionidfile) or self.read_from_file(env_buildsessionidfile)
        build_session = None
        if build_session_id:
            build_session = self.resolve_build_session_from_id(build_session_id, backend_proxy)
        if not build_session_id and labid:
            build_session = self.resolve_build_session_from_labid(labid, backend_proxy)
            self.config_data.labId = labid
    ...
    def resolve_build_session_from_labid(self, labid, backend_proxy):
        build_session = backend_proxy.get_build_session_from_labid(labid)
        if build_session:
            Config.write_build_session_to_file(build_session.buildSessionId)
            log.info("build session id was resolved using lab id and saved to file. build session id: %s. labid: %s" % (build_session.buildSessionId, labid))
        else:
            log.error("build session id was not resolved using lab id: %s. Supply buildsessionid or make sure you have a running app configured with this labid" % labid)
        return build_session

    def resolve_build_session_from_id(self, build_session_id, backend_proxy):
        build_session = backend_proxy.get_build_session(build_session_id)
        if not build_session:
