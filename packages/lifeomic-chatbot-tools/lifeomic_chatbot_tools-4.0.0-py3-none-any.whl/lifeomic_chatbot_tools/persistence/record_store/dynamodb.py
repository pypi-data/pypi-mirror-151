...
class IDynamoDBRecord(IRecord, t.Protocol):
    def get_sort_key(self) -> t.Any:
        ...
    ...
    @classmethod
    def parse_obj(cls, obj: t.Any) -> "IDynamoDBRecord":
        ...
...
class DynamoDBRecordStore(t.Generic[TIDynamoDBRecord], IRecordStore[TIDynamoDBRecord]):
    """
    ...
    def save(self, record: TIDynamoDBRecord):
        ...
        item_dict = remap(item_dict, self.use_correct_type)
        self._table.put_item(Item={**item_dict, self._pk: record.get_id()})

    def get(self, id_: str, sk_=None) -> t.Optional[TIDynamoDBRecord]:
        if self._sk is None:
            res = self._table.get_item(Key={self._pk: id_})
        else:
            sk_ = self.use_correct_type(sk_)
            res = self._table.get_item(Key={self._pk: id_, self._sk: sk_})
        if "Item" not in res:
            return None
        return t.cast(TIDynamoDBRecord, self.record_cls.parse_obj(res["Item"]))
    ...
    def get_all(self, *conditions: t.Tuple[str, str, t.Any], **where_equals) -> t.Iterable[TIDynamoDBRecord]:
        ...
        while not done:
            ...
            for item in res.get("Items", []):
                yield t.cast(TIDynamoDBRecord, self.record_cls.parse_obj(item))
