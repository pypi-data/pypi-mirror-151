...
from lifeomic_chatbot_tools.persistence.record_store.base import IRecordStore
from lifeomic_chatbot_tools.types.data import DataModel
from lifeomic_chatbot_tools.types.utils import hash_model


class ServiceVersionMetadata(BaseModel):
    name: str
...
class BaseDatasetRecord(DataModel):
    """
...
TBaseDatasetRecord = t.TypeVar("TBaseDatasetRecord", bound=BaseDatasetRecord)


def make_artifact_record_id(service_version: str, agent_id: str):
    """
...
class BaseArtifactRecord(DataModel):
    """
    ...
    def get_id(self) -> str:
        return make_artifact_record_id(self.service_version, self.agent_id)

    def get_sort_key(self):
        return None


TBaseArtifactRecord = t.TypeVar("TBaseArtifactRecord", bound=BaseArtifactRecord)


class BaseArtifactManager(t.Generic[TBaseArtifactRecord, TBaseDatasetRecord], ABC):
    """
    ...
    def __init__(
        self,
        artifacts: IRecordStore[TBaseArtifactRecord],
        datasets: IRecordStore[TBaseDatasetRecord],
        versions: IRecordStore[ServiceVersionMetadata],
    ...
    @abstractmethod
    def create_artifact_from_dataset(self, dataset: TBaseDatasetRecord) -> TBaseArtifactRecord:
        """
    ...
    def save_artifact(self, artifact: TBaseArtifactRecord, dataset: TBaseDatasetRecord):
        """
    ...
    def load_artifact(self, agent_id: str) -> TBaseArtifactRecord:
        """
        ...
        artifact = self._artifacts.get(make_artifact_record_id(version, agent_id))
    ...
    def sync(self) -> int:
        ...
        self._versions.save(ServiceVersionMetadata(name=version, synced_at=time.time()))
