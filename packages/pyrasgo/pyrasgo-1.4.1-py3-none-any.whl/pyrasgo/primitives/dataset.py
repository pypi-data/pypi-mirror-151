...
from pyrasgo.schemas.dataset import DatasetBulk, DatasetSourceType
...
class Dataset(Connection):
    ...
    def _refresh(self) -> None:
        ...
        """
        if self._api_dataset:
            self._api_dataset = schemas.Dataset(**self._get(f"/datasets/{self.id}", api_version=2).json())
    ...
    def to_accelerator(self) -> Optional[str]:
        if self._api_dataset:
            return exports.dataset_to_accelerator_yaml(
                api_dataset=self._api_dataset, api_operation_set=self._api_operation_set
            )
    ...
    def _create_transform_function(self, transform: schemas.Transform) -> Callable:
        """
    ...
    ) -> Tuple[List[str], List[int], List[schemas.OperationCreate], Dict[str, Union[str, int, List, Dict, 'Dataset']]]:
        """
    ...
    def _update_op_deps__ds_deps__and_parent_ops(
        ds_arg: 'Dataset', op_deps: List[str], parent_ops: List[schemas.OperationCreate], ds_deps: List[int]
    ) -> Tuple[List[str], List[schemas.OperationCreate], List[int]]:
        """
...
def _gen_func_signature(func: Callable, transform: schemas.Transform) -> inspect.Signature:
    """
...
def _gen_func_docstring(transform: schemas.Transform) -> str:
    """
