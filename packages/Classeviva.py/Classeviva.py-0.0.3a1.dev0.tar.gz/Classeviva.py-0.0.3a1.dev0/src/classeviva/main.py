...
class ListaUtenti(set):
    ...
    async def accedi(self) -> None:
        asyncio.gather(*[utente() for utente in self.connessi])

    def iterante(self, ripetizioni: int=0):
        def decoratore(funzione) -> function:
            def involucro(*args, **kwargs) -> None:
                if (ripetizioni):
                    for i in range(ripetizioni):
                        yield funzione(self[i], *args, **kwargs)
                    return
                for elemento in self:
                    yield funzione(elemento, *args, **kwargs)
            return involucro
        return decoratore
