...
_PRINT_CONTAINER_LABEL = 'Container'
_PRINT_VIRTUAL_MACHINE_LABEL = 'Virtual Machine'
...
def _iter_resources(
    ...
    for instance in instance_collection:
        ...
        row = (
            ...
            f'{config.get("limits.cpu", _DEFAULT_CPU_COUNT)} '
            f'(allowance: {config.get("limits.cpu.allowance", "100%")})',
            ...
            '\n'.join(
                (
                    f'{device["nictype"]}: {device["parent"]}'
                    for device in devices.values()
                    if device['type'] == 'nic'
                )
            ),
    ...
    yield (
        ...
        *repeat(_EMPTY_STRING, 2 if show_instance_type else 1),
