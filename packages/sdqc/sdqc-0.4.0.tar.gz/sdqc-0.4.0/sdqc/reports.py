...
class Report(object):
    ...
    def __df_to_buffer(self, df, index=True):
        ...
        """
        self.buffer.write("\n")
        # the issue with thead should have been fixed in pandoc 2.7.3
        if index:
            df.index.names = [None]*len(df.index.names)
            self.buffer.write(df.to_html(justify="left").replace(
                "<thead>", "").replace("</thead>", ""))
        else:
            self.buffer.write(df.to_html(justify="left", index=index).replace(
                "<thead>", "").replace("</thead>", ""))
