...
class Listener(object):
    ...
    def to_tab(self,
        ...
        tab_id = tab_handle.split('-')[-1] if tab_handle else self.current_tab
        tab_data = {"id": tab_id, "type": "page",
                    "webSocketDebuggerUrl": f"ws://{self._browser}/devtools/page/{tab_id}"}
    ...
    def listen(self,
               targets: Union[str, List[str], Tuple[str], bool, None] = None,
               count: int = None,
               timeout: float = None,
               asyn: bool = True) -> None:
    ...
    def _get_response_body(self, requestId: str) -> Union[str, None]:
        ...
        """
        try:
            return self._tab.call_method('Network.getResponseBody', requestId=requestId)['body']
...
def _get_active_tab_id(url: str) -> Union[str, None]:
    """获取浏览器活动标签页id          \n
