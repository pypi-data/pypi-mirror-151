...
from com.interface.mdc_gateway_base_define import ErrorInfo, SortType, Play_Back_Type, QueryType
...
class GatewayInterface():
    ...
    def subscribe(self, subscribestr):
    ...
    def playCallback(self, id_list, marketDataType, exrightsType, startTime, stopTime):
        if self.get_login_flag():
            for id in id_list:
                playbackjson = {}
    ...
    def queryMdContantCallback(self, securityIDSourceAndTypes, securityIdList):
        self.queryCallback(QueryType.QUERY_CONSTANT, securityIDSourceAndTypes, securityIdList)

    def queryLastMdContantCallback(self, securityIDSourceAndTypes, securityIdList):
        self.queryCallback(QueryType.QUERY_CONSTANT_TODAY, securityIDSourceAndTypes, securityIdList)

    def queryETFInfoCallback(self, securityIDSourceAndTypes, securityIdList):
        self.queryCallback(QueryType.QUERY_ETFBASICINFO, securityIDSourceAndTypes, securityIdList)

    def queryLastMdTickCallback(self, securityIDSourceAndTypes, securityIdList):
        self.queryCallback(QueryType.QUERY_TICK, securityIDSourceAndTypes, securityIdList)

    # 查询  params:queryType 为int值;securityIdSource 为市场ESecurityIDSource 枚举值;securityType 为 ESecurityIDSource枚举值集合
    def queryCallback(self, queryType, securityIDSourceAndTypes, securityIdList):
        if self.get_login_flag():
    ...
    def login(self, ip, port, username, pwd, istoken, certlog, backuplist, thread_count=1):
        self.__init_log__()
        ...
        if ret != 0:
            print("login failed!!! reason:%s" % (self.get_error_code_value(ret)))
            exit(-1)
