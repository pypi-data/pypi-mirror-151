...
class Oracle:
    def __init__(self, loop_list_path: str, hic_file: str, models_dir: str, norm: str, width: int = 56,chr_list : list = None):
    ...
    def makePredictions(self,chr):
        ...
        for reso,sublist in self.__resolution_sub_lists.items():
          print ('ORACLE is running at', reso, 'kb', 'on', chr)
          ...
          sublist = sublist[sublist['chr1']==chr]
          ...
          for index, row in sublist.iterrows():   
            x1,x2,y1,y2 = make_region((row[CHROM_KEY],row[X1_KEY],row[X2_KEY],row[Y1_KEY],row[Y2_KEY]),self.__width-1,reso)
    ...
    def run(self):
        ...
        for chrom in self.__chr:
          preds = self.makePredictions(chrom)
          passed.append(preds[0])
          failed.append(preds[1])
