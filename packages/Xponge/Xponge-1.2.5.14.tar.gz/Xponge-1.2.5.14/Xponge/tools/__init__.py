...
def name2name(args):
    ...
    if args.to_format == "mol2":
        ...
        _to = Xponge.assign.Get_Assignment_From_PDB(args.to_file, determine_bond_order = False, only_residue = args.to_residue)
    ...
    if args.from_format == "mol2":
        ...
        _from = Xponge.assign.Get_Assignment_From_PDB(args.from_file, determine_bond_order = False, only_residue = args.from_residue)
        ...
        for i, atom in enumerate(_from.atoms):
            if i in matchA:
                towrite += "ATOM  %5d %4s %3s %1s%4d    %8.3f%8.3f%8.3f%17s%2s\n" % (i + 1, _from.names[i],
                                                                                 _from.name, " ", 1, _from.coordinate[i][0],
