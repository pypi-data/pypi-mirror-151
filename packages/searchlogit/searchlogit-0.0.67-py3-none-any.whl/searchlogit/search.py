...
class Search():
    def __init__(self, dist=None, code_name="abc", avail=None, weights=None,
                 ...
                 allow_random=True, base_alt=None):
    ...
    def fit_mnl(self, dat, test_df, as_vars, is_vars, bcvars, choice, alt,
        ...
        try:
            ...
            ## Calculating MAE
            model = MultinomialLogit()
            model.fit(X_test, y_test, varnames=all_vars, isvars=is_vars, alts=alt_var, ids=self.test_chid, fit_intercept=asc_ind,
        ...
        print("MAE", MAE)
        print("MAPE", MAPE)
        print("model convergence", conv)
        print("dof", len(coef_names))


        if any(l for l in coef_names if l.startswith("lambda.")):
            print("model type is nonlinear MNL")
        else:  # TODO? startswith?
            print("model type is MNL")
    ...
    def fit_mxl(self, dat, test_df, as_vars, is_vars, rand_vars, bcvars, corvars,
        ...
        if any(l for l in coef_names if l.startswith("chol.")):
            if any(l for l in coef_names if l.startswith("lambda.")):
                print("model type is nonlinear correlated MXL")
            else:
                print("model type is linear correlated MXL")
        elif any(l for l in coef_names if l.startswith("lambda.")):
            print("model type is nonlinear MXL") 
        else:
            print("model type is MXL")
            ...
            if conv:
                ...
                if all(v for v in sig <= self.p_val):
                    ...
                    while any([v for v in sig if self.p_val]):
                        ...
                        if self.latent_class and check_latent:
                            # print("TEMP DEBUGGING")  # TODO
                            i = 0
                            # rem_class_params_spec = class_params_spec.copy()
                            # print('sig: ', sig)
                            # print('sig_member: ', sig_member)
                            # print('class_params_spec: ', class_params_spec)
                            # print('member_params_spec: ', member_params_spec)
                            # print('asc_ind', asc_ind)
                            for ii, class_params in enumerate(class_params_spec):
        ...
        while itr < max_itr:
            ...
            if itr == max_itr:  # Plot on final iteration
                if self.multi_objective:  # Plot for MOOF
                    Fronts = self.get_fronts(har_mem)
                    Pareto = self.pareto(Fronts, har_mem)
                    har_mem = [har_mem for ii, har_mem_ii in enumerate(har_mem) if har_mem_ii[0] != 1000000]

                    fig, ax = plt.subplots()
                    all_bic = [har_mem[i][0] for i in range(len(har_mem))]
                    all_val = [har_mem[i][1] for i in range(len(har_mem))]
                    ax.scatter(all_bic, all_val)
                    init_bic = [init_sol[0] for _, init_sol in enumerate(HM)]
                    init_val = [init_sol[1] for _, init_sol in enumerate(HM)]
                    ax.scatter(init_bic, init_val)
                    if hasattr(self, 'benchmark_bic'):
                        ax.scatter(self.benchmark_bic, self.benchmark_val)
                    pareto_bic = [pareto[0] for _, pareto in enumerate(Pareto)]
                    pareto_val = [pareto[1] for _, pareto in enumerate(Pareto)]
                    ax.scatter(pareto_bic, pareto_val)
                    ax.plot(pareto_bic, pareto_val)
                    ax.set_xlabel("BIC - training dataset")
                    ax.set_ylabel("MAE - testing dataset")
                    ax.legend()
                    plt.savefig(self.code_name + "_MOOF.png") #, bbox_extra_artists=(lgd,), bbox_inches='tight')

                else:  # Plot for SOOF
                    fig, ax1 = plt.subplots()
                    ax2 = ax1.twinx()

                    lns1 = ax1.plot(np.arange(len(current_bic_points)), current_bic_points, label="BIC of solution estimated at current iteration")
                    lns2 = ax1.plot(np.arange(len(current_bic_points)), best_bic_points, label="BIC of best solution in memory at current iteration", linestyle="dotted")
                    lns3 = ax2.plot(np.arange(len(current_bic_points)), best_val_points, label="In-sample LL of best solution in memory at current iteration", linestyle="dashed")
                    lns = lns1 + lns2 + lns3
                    ...
                    plt.savefig(self.code_name + "_SOOF.png", bbox_extra_artists=(lgd,), bbox_inches='tight')
