...
from alira.email.smtp import SMTP
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            self.smtp_username = smtp_username or os.environ["ALIRA_SMTP_USERNAME"]
            self.smtp_password = smtp_password or os.environ["ALIRA_SMTP_PASSWORD"]
        except Exception as e:
            message = (
                ...
                "ALIRA_SMTP_HOST, ALIRA_SMTP_PORT, ALIRA_SMTP_USERNAME, ALIRA_SMTP_PASSWORD."
            )
    ...
    def send(self, instance: Instance, message: str, rest_authentication, **kwargs):
        ...
        url = "/".join(
            map(
                ...
                [rest_authentication.service, "email", instance.id],
            )
        )
        ...
        return False, (
            f"Server error: {response.status_code} - {response.json()['message']}"
        )
