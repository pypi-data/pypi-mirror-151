...
class CogniteGeospatialClient:
    ...
    def create_spatial(self, items: Union[SpatialItem, List[SpatialItem]]) -> Union[SpatialItem, List[SpatialItem]]:
        ...
        layer_map = {}
        self._store_vector_data(stored_items, items, layer_map)

        return _single_or_array(stored_items, single_item)

    def _store_vector_data(self, stored_items, items, layer_map):
        for spatial_item, create_item in zip(stored_items, items):
            ...
            if len(attributes) > 0:
                ...
                attribute_map = {}
                for attribute in spatial_item._layer_info.attributes:
                ...
                for name, value in attributes.items():
                    if name not in attribute_map:
                        raise GeospatialError(
                            "Attribute " + name + " is not defined in the layer " + spatial_item._layer_info.name
                        )

                    attribute_values.append((attribute_map[name], value))

                with tempfile.NamedTemporaryFile() as fp:
                    _write_attributes_to_parquet(fp.name, attribute_values)
                    self.parquet_file_save(file=fp.name, id=spatial_item.id)
    ...
    def find_spatial(
        ...
        _limit = 100 if limit is None or limit == -1 or limit == float("inf") or limit > 100 else limit
        ...
        while True:
            if total - len(spatial_list) < _limit:
                _limit = total - len(spatial_list)
            spatial_search_request.cursor = next_cursor
            ...
            if response_size < _limit or len(spatial_list) == total or next_cursor is None:
                spatial_list.next_cursor = next_cursor
                break
        return spatial_list
    ...
    def update_spatial(
        ...
        layer_map = {}
        self._store_vector_data(updated_items, items, layer_map)
    ...
    def persist_spatial_attribute(self, id: int, external_id: str, name: str, value) -> SpatialItem:
        ...
        Returns:
            Update spatial item with attribute
        """
        spatial_item = self.get_spatial_info(id, external_id)
        if spatial_item is None:
            raise GeospatialError("Spatial item not found")
        layer_name = spatial_item.layer
        layer = self.list_layers(layer_name)
        attributes = layer.attributes
        attribute = next((attribute for attribute in attributes if attribute.name == name), None)
        if attribute is None:
            raise GeospatialError("Attribute " + name + " is not defined in the layer " + layer_name)
        if (
            type(value) is dict
            or np.isscalar(value)
            or value.size <= 100
            or attribute.type == "string"
            or attribute.type == "boolean"
        ):
            self.add_spatial_item_data(spatial_item.id, name, value, attribute.type)
        else:
            item_attributes = {"attributes": {attribute.name: value}}
            self._store_vector_data([spatial_item], [item_attributes], {layer_name: layer})
    ...
    def get_layer_items(
        ...
        """
        return self.find_spatial(
            layer=layer, attributes=attributes, limit=None, output_crs=output_crs, geometry_format=geometry_format
        )
        ...
        while True:
            ...
            for item in items:
                if item.external_id == geometry.external_id or item.id == geometry.id:
                    return True
            if len(items) < limit or items.next_cursor is None:
                return False
