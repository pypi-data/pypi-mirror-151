...
from cognite.geospatial.shape import GeometryDefinition, Index2, Surface
...
class SpatialItem(UpdateSpatialItem, Geometry, Plot):
    ...
    def __getitem__(self, name: str):
        ...
        if name in attributes:
            self.attributes[name] = attributes[name]  # store retrievd value
    ...
    def _geometry_definition(self) -> GeometryDefinition:
        ...
        if grid_box is not None:
            geometry.grid_top_left = grid_box[0]
            geometry.grid_bottom_right = grid_box[1]
    ...
    def surface(self) -> Surface:
        ...
        return Surface(data=points, grid=grid, geometry=geometry, name=self.name, id=self.id)

    @property
    def extent(self):
        """Extent of the spatial item
        ...
        if extent is not None:
            return np.array(
                [
                    [self[layer_meta.extent_top_left_x()], self[layer_meta.extent_top_left_y()]],
                    [self[layer_meta.extent_top_right_x()], self[layer_meta.extent_top_right_y()]],
            ...
            if geometry is not None:
                ...
                return np.array([[minx, miny], [minx, maxy], [maxx, maxy], [maxx, miny], [minx, miny]])
        return None
    ...
    def grid_box(self):
        ...
        if grid_box is not None:
            return np.array(
                [
                    [self[layer_meta.grid_box_min_i()], self[layer_meta.grid_box_min_j()]],
                    [self[layer_meta.grid_box_max_i()], self[layer_meta.grid_box_max_j()]],
                ],
                dtype=np.int32,
            )
        return None
    ...
    @property
    def points(self):
        """ Get numpy arrays of x,y,z. Otherwise, get geometry in the form of wkt
