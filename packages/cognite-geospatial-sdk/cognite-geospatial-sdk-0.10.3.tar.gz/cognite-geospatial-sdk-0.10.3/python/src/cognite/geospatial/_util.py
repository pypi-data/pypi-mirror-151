...
def _to_parquet_data_type(attribute_type):
    ...
    raise GeospatialError("Not supported format " + attribute_type)


def _write_attributes_to_parquet(file, attributes: list):
    fields = list()
    columns = list()
    for attribute, value in attributes:
        ...
        data_type = _to_parquet_data_type(attribute.type)
        field_name = attribute.name
        filed_type = pa.field(field_name, data_type)
        fields.append(filed_type)
        columns.append(pa.array(value, type=data_type))

    schema = pa.schema(fields)
    with pq.ParquetWriter(file, schema, compression="zstd") as writer:
