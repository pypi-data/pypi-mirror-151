...
class IbisDto(object):
    ...
    def start_child_element(self, tagname):
        ...
        if tagname in self.__class__.xml_arrays:
            if getattr(self, tagname) is None:
...
class IbisPerson(IbisDto):
    ...
    def is_staff(self):
        ...
        misAffiliation.
        """
        return self.misAffiliation is None or self.misAffiliation != "student"
    ...
    def is_student(self):
        ...
        ``"staff,student"``.
        """
        return self.misAffiliation is not None and self.misAffiliation.find("student") != -1
...
class IbisAttribute(IbisDto):
    ...
    def end_child_element(self, tagname, data):
        ...
        if tagname == "binaryData" and self.binaryData is not None:
    ...
    def encoded_string(self):
        ...
        **Returns**
        ...
        result = "scheme:%s" % base64.b64encode(self.scheme.encode('UTF-8')).decode('UTF-8')
        if self.attrid is not None:
            result = "%s,attrid:%d" % (result, self.attrid)
        if self.value is not None:
            result = "%s,value:%s" % (
              result, base64.b64encode(self.value.encode('UTF-8')).decode('UTF-8'))
        if self.binaryData is not None:
            result = "%s,binaryData:%s" %\
                     (result, base64.b64encode(self.binaryData.encode('UTF-8')).decode('UTF-8'))
        if self.comment is not None:
            result = "%s,comment:%s" %\
                     (result, base64.b64encode(self.comment.encode('UTF-8')).decode('UTF-8'))
        if self.instid is not None:
            result = "%s,instid:%s" %\
                     (result, base64.b64encode(self.instid.encode('UTF-8')).decode('UTF-8'))
        if self.visibility is not None:
            result = "%s,visibility:%s" %\
                     (result, base64.b64encode(self.visibility.encode('UTF-8')).decode('UTF-8'))
        if self.effectiveFrom is not None:
        ...
        if self.owningGroupid is not None:
            result = "%s,owningGroupid:%s" %\
                     (result, base64.b64encode(self.owningGroupid.encode('UTF-8')).decode('UTF-8'))
        return result
...
class IbisAttributeScheme(IbisDto):
    ...
    def __init__(self, attrs={}):
        ...
        if self.multiLined is not None:
