...
"""
  ...
  def GetVfo(self):
    self.Reply('VFO', "Main", 0)		# The type of VFO we have
  def GetPtt(self):
    if QS.is_key_down():
      self.Reply('PTT', 1, 0)
    else:
...
class ConfigConfig(wx.ScrolledWindow):
  def __init__(self, parent, width):
    ...
    c1 = SliderBoxH(self, "Tx level %d%%  ", 100, 0, 100, self.OnTxLevel, True, (tab0, self.y), tab2-tab0)
    self.y += self.dy
    level = conf.digital_tx_level
    c2 = SliderBoxH(self, "Digital Tx level %d%%  ", level, 0, level, self.OnDigitalTxLevel, True, (tab0, self.y), tab2-tab0)
    self.y += self.dy
    if not hasattr(Hardware, "SetTxLevel"):
      c1.slider.Enable(0)
      c2.slider.Enable(0)
    # Band plan
    t = wx.StaticText(self, -1, 'Colors on X-axis for CW, Phone')
    t.SetPosition((tab0, self.y))
    ctrl = QuiskPushbutton(self, self.OnBtnBandPlan, "Band plan...")
    ctrl.SetColorGray()
    ctrl.SetSize(wx.Size(x2, y2))
    ctrl.SetPosition((tab2 - x2, self.y - self.offset))
    self.y += self.dy
    # Configure WSJT-X
    t = wx.StaticText(self, -1, 'Configure WSJT-X')
    t.SetPosition((tab0, self.y))
    ctrl = QuiskPushbutton(self, self.OnConfigureWsjtx, "WSJT-X...")
    ctrl.SetColorGray()
    ctrl.SetSize(wx.Size(x2, y2))
    ctrl.SetPosition((tab2 - x2, self.y - self.offset))
    self.y += self.dy
    # Start WSJT-X
    t = wx.StaticText(self, -1, "Start WSJT-X")
    t.SetPosition((tab0, self.y))
    lst = ("Never", "Main Rx0 on startup", "Main Rx0 now") #, "All Rx on startup", "All Rx now")
    value = application.local_conf.globals.get("start_wsjtx", "Never")
    ctrl = configure.ChoiceCombo(self, value, lst)
    ctrl.handler = application.OnStartWsjtx
    w = x2 * 22 // 10
    ctrl.SetSize(wx.Size(w, y2))
    ctrl.SetPosition((tab2 - w, self.y - self.offset))
    self.y += self.dy
    # End of first column
...
class App(wx.App):
  ...
  def OnInit(self):
    ...
    self.split_offset = 3000	# current frequency difference when using Split
    self.savedState = {}
  ...
  def MakeButtons(self, frame, gbs):
    ...
    # Noise Blanker
    self.NB_menu = wx.Menu()
    for t in ("NB 1", "NB 2", "NB 3"):
      item = self.NB_menu.AppendRadioItem(-1, t)
      self.Bind(wx.EVT_MENU, self.OnBtnNB, item)
      if t == "NB 1":
        item.Check()
    item = self.NB_menu.AppendRadioItem(-1, "SNB")
    if self.wdsp.version:
      self.Bind(wx.EVT_MENU, self.OnBtnNB, item)
    else:
      item.Enable(False)
  ...
  def OnFreedvMenu(self, event, text=None):
    if event:
      idd = event.GetId()
    ...
    else:		# change to new mode failed
