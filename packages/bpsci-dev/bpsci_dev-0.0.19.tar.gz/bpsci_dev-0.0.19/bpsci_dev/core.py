...
class dyn_obj:
    ...
    def apply_streamline(self, staticity, int_x, int_y, int_z, thickness):
        ...
        #introduce a little jitter to allow the "Build" animation to work properly in blender
        int_x[np.where(np.diff(int_x) == 0)[0] + 1] = np.random.rand(len(int_x))*.00001*min(int_x) + int_x[np.where(np.diff(int_x) == 0)[0] + 1]
        int_y[np.where(np.diff(int_y) == 0)[0] + 1] = np.random.rand(len(int_y))*.00001*min(int_y) + int_y[np.where(np.diff(int_y) == 0)[0] + 1]
        int_z[np.where(np.diff(int_z) == 0)[0] + 1] = np.random.rand(len(int_z))*.00001*min(int_z) + int_z[np.where(np.diff(int_z) == 0)[0] + 1]
        ...
        if staticity == 'dynamic':
            for i in range(len(frames)):
                ...
                bpy.data.objects[name].data.bevel_factor_end = 1/(frames[-1]+1) * (cur_frame-1)
