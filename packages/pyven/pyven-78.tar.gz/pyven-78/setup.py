...
setuptools.setup(
        ...
        install_requires = ['aridity>=52', 'diapyr>=18', 'lagoon>=24', 'setuptools>=44.1.1', 'twine>=1.15.0'],
        package_data = {'': ['*.pxd', '*.pyx', '*.pyxbld', '*.arid', '*.aridt']},
        scripts = ['bootstrap', 'foreignsyms'],
        entry_points = {'console_scripts': ['tests=pyven.checks:main_tests', 'drmake=pyven.drmake:main_drmake', 'gclean=pyven.gclean:main_gclean', 'initopt=pyven.initopt:main_initopt', 'launch=pyven.launch:main_launch', 'pipify=pyven.pipify:main_pipify', 'minreqs=pyven.projectinfo:main_minreqs', 'release=pyven.release:main_release', 'tasks=pyven.tasks:main_tasks', 'tryinstall=pyven.tryinstall:main_tryinstall']},
        **ext_modules())
