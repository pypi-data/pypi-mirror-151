...
class Session(object):
        ...
        async def wrapper(self, *args, **kwargs):
            if self.__wykop_hash is None:
                async with self.__client.get(ROOT) as response:
                    ...
                    if match := re.search("hash\\s+:\\s+\\\"(?P<hash>[0-f]{32}-[0-9]{10})\\\"", body):
    ...
    async def attach_url(self, url):
        ...
        async with self.__client.post(f"{ROOT}/ajax2/embed/url/hash/{self.__wykop_hash}/", data=form) as response:
            if (response.status == 200):
                response_json = await response.json(loads=json_loads)
    ...
    async def attach_file(self, file, *, rename = None):
        ...
        with file.open("rb") as file_handle:
            ...
            form.add_field("file", file_handle, filename=rename or file.name, content_type=CONTENT_TYPE)

            async with self.__client.post(f"{ROOT}/ajax2/embed/upload/hash/{self.__wykop_hash}/", data=form) as response:
                if (response.status == 200):
                    response_json = await response.json(loads=json_loads)
    ...
    def post(self, body = None, attachment_id = None):
        ...
        return self.__client.post(f"{ROOT}/ajax2/wpis/dodaj/hash/{self.__wykop_hash}/", data=form)
    ...
    def comment(self, post_id, body = None, attachment_id = None):
        ...
        return self.__client.post(f"{ROOT}/ajax2/wpis/CommentAdd/{post_id}/hash/{self.__wykop_hash}/", data=form)
