...
class GitLab(CLI):
    ...
    def get_pipeline(project):
        ...
        last_commit = project.commits.list(ref_name=branch.strip(), all=False)[0]
    ...
    def trigger_job(job_name, pipeline_id=None):
        polidoro_gitlab = import_dependency('polidoro_gitlab')
        gl = polidoro_gitlab.GitLab('https://gitlab.buser.com.br/',
                                    private_token=os.environ.get('PRIVATE_TOKEN', ''))
        project = GitLab.get_project(gl)
        ...
        for pipe_job in jobs:
            if pipe_job.name == job_name:
                ...
                print(f'Starting Job {pipe_job.name} in Pipeline#{pipeline.id}')
                job.play()
