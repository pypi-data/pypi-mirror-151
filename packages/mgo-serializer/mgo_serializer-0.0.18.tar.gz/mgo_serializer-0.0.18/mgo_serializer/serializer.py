...
def dict_to_lower_case_keys(d):
    to_dict = {}
    for key, value in d.items():
        parse = re.findall(r'[A-Z]?[a-z]+', key)
        key = '_'.join([item.lower() for item in parse])
        to_dict[key] = value
    return to_dict
...
def proto_to_dict(data):
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            data[key] = dict_to_lower_case_keys(value)
...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    ...
    def to_protobuf(self, data):
        if self.pb:
            return mongo_to_protobuf(data, self.pb)
        return self.pb
