...
def read_write_pyx_to_pyi(target_pyx_path: str, target_pyi_path:str):
    ...
    for line_idx in range(len(pyx_lines)):
        ...
        cur_line_is_attribute = cur_line_indentation == 1 and not cur_line_is_function and not cur_line_is_class and not cur_line_is_import

        # Add python decorators like @property and @dataclass
        if (pyx_line.strip()[0] == '@'):
            py_lines.append(pyx_line)

        # FILTERING     Skip if indentation > 2. That way we are one indentation deeper than method content
        if (not cur_line_is_import and not cur_line_is_class and not cur_line_is_function and not cur_line_is_attribute):
            continue
    ...
    # Post-Process
    post_process(py_lines=py_lines, spaces_for_one_tab=spaces_for_one_tab)
...
def post_process(py_lines:[str], spaces_for_one_tab:int):
    """ """
    for i, line in enumerate(py_lines):
        words_array = line.split(" ")
        if (words_array[0] == 'class'):
            next_line = py_lines[i+1]
            if (next_line[:spaces_for_one_tab] != '    '):
                py_lines[i] = line + f"\n{' ' * spaces_for_one_tab}..."
    return py_lines
...
def pyx_line_to_pyi(line: str, spaces_for_one_tab: int):
    ...
    begin_words = ['def', 'cdef', 'cpdef', 'class', 'cimport', 'import', 'from']
