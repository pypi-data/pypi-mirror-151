...
async def approve_request(request,timestamp):
    ...
    async with httpx.AsyncClient(base_url=settings.DRONE_SERVER,headers={'Authorization':f'Bearer {settings.DRONE_TOKEN}'}) as http_cl:
        ...
        approval_result = ApprovalResult(request_id=request.id,build=build_info['number'],created_at=datetime.now(),approved=True)
...
@router.get("/approval-result/{build_nr}",response_model=schemas.ApprovalRequest)
async def get_approval_result(build_nr:int,response:Response):
    res = await ApprovalResult.filter(build=build_nr)
    if res:
        item= res[0]
        request = await fetch_approval_request(item.request_id)
