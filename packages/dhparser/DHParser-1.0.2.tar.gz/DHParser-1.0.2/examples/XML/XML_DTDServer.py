...
"""XML_DTDServer.py - starts a server (if not already running) for the
                    compilation of XML_DTD
"""
...
class XML_DTDLanguageServerProtocol:
    """
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "XML_DTD-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = XML_DTDCPUBoundTasks(self.lsp_data)
        self.blocking = XML_DTDBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from XML_DTDParser import batch_process
        args = argstr.split(' ')
    ...
    #     from XML_DTDParser import compile_src
...
def run_server(host, port, log_path=None):
    """
    Starts a new XML_DTDServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('XML_DTD_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    XML_DTD_lsp = XML_DTDLanguageServerProtocol()
    lsp_table = XML_DTD_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', XML_DTD_lsp.simply_compile)
    XML_DTD_server = Server(rpc_functions=lsp_table,
                        cpu_bound=XML_DTD_lsp.cpu_bound.lsp_table.keys(),
                        blocking=XML_DTD_lsp.blocking.lsp_table.keys(),
                        connection_callback=XML_DTD_lsp.connect,
                        server_name="XML_DTDServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        XML_DTD_server.echo_log = True if port >= 0 and host else False
        msg = XML_DTD_server.start_logging(log_path.strip('" \''))
        if XML_DTD_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        XML_DTD_server.run_stream_server(reader, writer)
        return
    ...
    while ports:
        ...
        try:
            ...
            XML_DTD_server.run_tcp_server(host, port)  # returns only after server has stopped!
            ports = []
        except OSError as e:
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing XML_DTD-files.")
    ...
    if args.status:
        ...
        echo('Usages:\n'
             + '    python XML_DTDServer.py --startserver [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python XML_DTDServer.py --startdaemon [--host host] [--port port] [--logging [ON|LOG_PATH|OFF]]\n'
             + '    python XML_DTDServer.py --stream\n'
             + '    python XML_DTDServer.py --stopserver\n'
             + '    python XML_DTDServer.py --status\n'
             + '    python XML_DTDServer.py --logging [ON|LOG_PATH|OFF]\n'
             + '    python XML_DTDServer.py FILENAME.dsl [--host host] [--port port]  [--logging [ON|LOG_PATH|OFF]]')
        sys.exit(1)
