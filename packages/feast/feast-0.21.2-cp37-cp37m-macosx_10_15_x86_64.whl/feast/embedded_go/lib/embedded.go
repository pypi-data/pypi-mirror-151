...
#cgo CFLAGS: -I/Library/Frameworks/Python.framework/Versions/3.7/include/python3.7m -Wno-error -Wno-implicit-function-declaration -Wno-int-conversion
#cgo LDFLAGS:  -undefined dynamic_lookup -arch x86_64 -g
...
//export embedded_OnlineFeatureService_Stop
func embedded_OnlineFeatureService_Stop(_handle CGoHandle, goRun C.char) {
	vifc, __err := gopyh.VarFromHandleTry((gopyh.CGoHandle)(_handle), "*embedded.OnlineFeatureService")
	if __err != nil {
		return
	}
	...
	if boolPyToGo(goRun) {
		go gopyh.Embed(vifc, reflect.TypeOf(embedded.OnlineFeatureService{})).(*embedded.OnlineFeatureService).Stop()
	} else {
		gopyh.Embed(vifc, reflect.TypeOf(embedded.OnlineFeatureService{})).(*embedded.OnlineFeatureService).Stop()
	}
}
