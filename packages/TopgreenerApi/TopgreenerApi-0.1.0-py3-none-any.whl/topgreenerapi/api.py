...
class TopgreenerCloud:
    ...
    async def sendRequest(self, action, data=None, gid=None, requiresSID=True):
    ...
    async def get_devices(self, locationId) -> list[topgreenerapi.device.Device]:
        ...
        for devi in deviceArr:
            dev = tuya_device.tuya_device_from_dict(devi)
            device = topgreenerapi.device.Device(dev.name, dev.dev_id, locationId, self)
            device.set_schema(devschemas[dev.product_id])
