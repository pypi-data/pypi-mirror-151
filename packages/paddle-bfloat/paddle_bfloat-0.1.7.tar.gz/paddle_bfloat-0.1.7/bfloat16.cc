  ...
  #define HASH_RETURN_TYPE long
				...
				bfloat16 operator()(bfloat16 from, bfloat16 to)
				{
					...
					if (from_as_float == 0)
					{
						...
						else
						{
							...
							bfloat16 out;
							memcpy(reinterpret_cast<uint16_t*>(&out), &out_int, sizeof(bfloat16));
							return out;
						}
					}
	...
	bool Initialize()
	{
		...
		bool ok =
			...
			RegisterUFunc<BinaryUFunc<bfloat16, bfloat16, ufuncs::Power>>(numpy.get(), "power") &&
			RegisterUFunc<BinaryUFunc<bfloat16, bfloat16, ufuncs::Remainder>>(
				numpy.get(), "remainder") &&
			RegisterUFunc<BinaryUFunc<bfloat16, bfloat16, ufuncs::Remainder>>(
				numpy.get(), "mod") &&
			RegisterUFunc<BinaryUFunc<bfloat16, bfloat16, ufuncs::Fmod>>(numpy.get(), "fmod") &&
			RegisterUFunc<ufuncs::DivmodUFunc>(numpy.get(), "divmod") &&
			RegisterUFunc<UnaryUFunc<bfloat16, bfloat16, ufuncs::Abs>>(numpy.get(), "absolute") &&
