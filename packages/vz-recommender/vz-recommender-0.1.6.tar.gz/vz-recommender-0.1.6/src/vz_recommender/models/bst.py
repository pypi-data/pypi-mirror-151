...
class ContextHead(nn.Module):
    ...
    def forward(self, deep_in:List[Tensor], wide_in:List[Tensor]=None, device_in:Tensor=None):
        """
...
class BST(nn.Module):
    def __init__(self, deep_dims, seq_dim, seq_embed_dim, deep_embed_dims, seq_hidden_size, seq_cross_size=200,
                 num_wide=0, num_device=0, context_head_kwargs=None, sequence_transformer_kwargs=None):
        super().__init__()
        ...
        self.dense1 = torch.nn.Linear(in_features=len(deep_dims)*deep_embed_dims+num_wide+seq_embed_dim+seq_embed_dim+(num_device*seq_embed_dim), out_features=2 * seq_embed_dim)
    ...
    def forward(self, deep_in, seq_in, vl_in, wide_in=None, device_in=None):
        """
