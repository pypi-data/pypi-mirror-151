    ...
    def visit(self, node: ast.Module) -> ast.Module:
        ...
        for descendant in ast.walk(node):
            ...
            if descendant.func.id == 'get_ipython':
                msg = '`get_ipython` cannot be transpiled to pure-Python. Check for magics in exported code cells'
...
class PatchTransformer(Configurable):
    ...
    def _is_patch_decorator(self, node) -> bool:
        ...
        :return:
        """
        return isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and (node.func.id == self.patch_decorator_id)
    ...
    def _apply_patches_to_node(self, decorated, classes: tp.Dict[str, tp.Any]):
        ...
        for node in decorated.decorator_list:
            if self._is_patch_decorator(node):
                if other_decorators:
                    raise ValueError('Applying decorators to a patched function is not safe at runtime, and is hence prohibited.')
    ...
    def _transform_module_body(self, nodes: tp.Iterable[T]) -> tp.Iterable[T]:
        ...
        for child in nodes:
            ...
            if hasattr(child, 'decorator_list') and self._apply_patches_to_node(child, classes):
                continue
            yield child
