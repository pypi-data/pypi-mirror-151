...
class Endpoints:
    ...
    def create_endpoints(self):
        ...
        def connection_test():
            """THIS ENDPOINT IS NOT IN USE AS IT DOES NOT GET INITIALIZED IN THE CONNECTION
            The "/actions/<string:name>/test" endpoint is currently being used for connection tests
    ...
    def run_action_trigger_task(self, input_message, test=False):
        ...
        try:
            ...
            if isinstance(wrapped_exception, ClientException):
                status_code = 400
            elif isinstance(wrapped_exception, PluginException):
                if wrapped_exception.preset is PluginException.Preset.BAD_REQUEST:
                    status_code = 400
