...
def moving_stats(values, windows,threshold=3):


    mean = np.mean(rolling_window(values, windows), -1)
    std = np.std(rolling_window(values, windows), -1)


    return mean, std


def detectAnomalies(values, window,scale=3,upper=None, lower = None):

    mean = np.mean(rolling_window(values, window), -1)
    std = np.std(rolling_window(values, window), -1)
    #
    upper = mean + scale*std if upper is  None else upper
    lower = mean - scale*std if lower is  None else lower
    ...
    # df['rolling_mean'] = df['Values'].rolling(window=window).mean()
    # df['rolling_std'] = df['Values'].rolling(window=window).std()
    #
    # import ipdb; ipdb.set_trace()
    # # Calculate difference
    # df['diff'] = df['Values'] - df['rolling_mean']
    #
...
class anom_detect():
    ...
    def evaluate(self,data,anom_detect=True):
        ...
        if anom_detect:
            ...
            anoma_points = pd.DataFrame(df[['Values']].iloc[outliers_index,0].sort_index())
