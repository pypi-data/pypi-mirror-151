...
WHEEL_DESTINATIONS = [
    Path(r"r:\watermondev\waterscience-code\apps\services-data\pypiserver\wheels"),
...
def publish_locally(tag="latest_github", sphinx_build_options="", **kwargs):
    ...
    Args:
        tag (str): the tag which was just built by setuptool_scm e.g. 'v0.8'
            or 'latest_github', which means all you are doing is building docs.
            By default, 'latest_github' is used.
    ...
    .. todo::
        ...
        publish the release to the GitHub website.
    ...
    assert tag == "latest_github" or tag.startswith("v")

    # Step 1 - copy wheel file to relevant destinations
    if tag != "latest_github":
...
def generate_package_docs_index_page(path):
    ...
    for dirname in dirs:
        if dirname.is_dir() and (
            dirname.stem.startswith("v") or dirname.stem == "latest_github"
        ):
