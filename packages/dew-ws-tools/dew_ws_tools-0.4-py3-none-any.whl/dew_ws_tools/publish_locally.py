...
def publish_locally(tag, sphinx_build_options="", **kwargs):
    ...
    1. Copies wheel file to package repositories: the Groundwater Toolbox folder,
       and the pypiserver on envtelem04.
    ...
    subprocess.call(f'sphinx-build docs docs\_build {sphinx_build_options}', shell=True)
    ...
    with open(DOCS_BASE / "index.html", "w") as f:
        f.write(base.format(list_html=list_html, style=STYLE, DOCS_BASE_TITLE=DOCS_BASE_TITLE))
