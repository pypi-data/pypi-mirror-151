...
from elit_morph_analyzer.util import read_word_set, NoIndentEncoder, NoIndent
...
class EnglishMorphAnalyzer:
    ...
    def init(self):
        self.load('elit_morph_analyzer.resources.english')
    ...
    def decode(self, tokens: Sequence[str], postags: Sequence[str], derivation=False, prefix=PREFIX_NONE, baseonly=True):
        def aux(token, pos):
            a = self.analyze(token, pos, derivation, prefix)
            return a[0][0] if baseonly else a
        return [aux(token, pos) for token, pos in zip(tokens, postags)]
    ...
    def _load_inflection_lexicons(self, resource_path: str) -> Dict[str, SimpleNamespace]:
        ...
        return {
            EnglishMorphTag.VB: SimpleNamespace(
                lemma_set=read_word_set(resource_filename(resource_path, 'verb.txt')),
            ...
            EnglishMorphTag.NN: SimpleNamespace(
                lemma_set=read_word_set(resource_filename(resource_path, 'noun.txt')),
            ...
            EnglishMorphTag.JJ: SimpleNamespace(
                lemma_set=read_word_set(resource_filename(resource_path, 'adjective.txt')),
            ...
            EnglishMorphTag.RB: SimpleNamespace(
                lemma_set=read_word_set(resource_filename(resource_path, 'adverb.txt')),
