...
def jinja2_render(from_path, to_path, d):
    import time
    import os
    start = time.time()
    from jinja2 import Environment,FileSystemLoader
    from_path_dir = os.path.dirname(from_path)
    name = os.path.basename(from_path)
    env = Environment(loader=FileSystemLoader([from_path_dir]))
    template = env.get_template(name)
    with open(to_path, 'w', encoding='utf-8') as f:
        f.write(template.render(**d))
    print("="*80)
    print("render success {}=>{}".format(from_path, to_path))
