...
# [START cloud_tasks_create_queue]
    ...
    @classmethod
    def enqueue_http_request(cls, url, payload=None):
        ...
        if not cls.does_queue_exists(project=project, location=location, queue_name=queue_name):
            ...
            print('create_queue_response is : {}'.format(create_queue_response))

        try:
            create_task_response = cls.create_http_task(project=project, queue=queue_name, location=location,
                                                        task_name='{}_{}'.format(
                                                            os.getenv('TASK_NAME_PREFIX'), CommonUtil.generate_random_string()),
                                                        url=url, payload=payload)

            return "success"
        except Exception as e:
            print('Exception in enqueue_http_request ')
            print(e)
