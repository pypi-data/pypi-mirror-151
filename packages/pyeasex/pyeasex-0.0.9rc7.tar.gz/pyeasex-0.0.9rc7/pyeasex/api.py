...
class EaseApi():
  def __init__(self, username=None, password=None, token=None, device_token=None):
    ...
    self.bucket_resource_uuid = None
  ...
  def _get_bucket_resource_uuid(self, project_uuid):
    if self.bucket_resource_uuid == None:
  ...
  def fetch_data(self, project_uuid, maplayer_resource_uuid, keep=True):
    endpoint = f'{self.host}/api/s3/maplayer/data'
    headers = {"Authorization": f"Bearer {self.token}"}
    params = {
      "resource_uuid": maplayer_resource_uuid,
    ...
    if os.path.isfile(maplayer_resource_uuid) != True:
      with requests.get(endpoint, params=params, headers=headers, stream=True) as r:
        r.raise_for_status()
        with open(maplayer_resource_uuid, 'wb') as f:
          for chunk in r.iter_content(chunk_size=8192): 
            f.write(chunk)

    with open(maplayer_resource_uuid, 'r') as f:
      data = json.loads(f.read())
      if keep != True:
        os.remove(maplayer_resource_uuid)
      return data

  def fetch_dataframe(self, project_uuid, measurement_resource_uuid, range=[], tags=[], fields=[], keep=True):
    ...
    with open(measurement_resource_uuid, 'r') as f:
      data = pd.read_json(f.read(), orient ='index')  
      if keep != True:
