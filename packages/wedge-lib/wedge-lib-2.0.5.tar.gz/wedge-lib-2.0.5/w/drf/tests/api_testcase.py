...
class ApiTestCase(DjangoTestCase):
    ...
    def set_client_params(self, params, **options):
        ...
        """
        if (
            "auth_by_token" in options
            and options["auth_by_token"]
            and "user" in options
        ):
            params["HTTP_AUTHORIZATION"] = self._get_token_header(options["user"])
    ...
    def client_post_multipart_content(self, url, data):
        """
        ...
        return self.client_post(url, data=data, content_type=MULTIPART_CONTENT)

    def client_post_upload(self, url, full_path, data):
        """
        ...
        with date_test_helper.today_is("2020-02-15 12:34:56"):
            with open(full_path, "rb") as f:
                data["file"] = f
                return self.client_post_multipart_content(url, data)
