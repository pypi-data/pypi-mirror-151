...
from dagster_cloud.workspace.user_code_launcher.user_code_launcher import ServerHandle
...
class DockerUserCodeLauncher(DagsterCloudUserCodeLauncher[Container], ConfigurableClass):
    ...
    def get_sandbox_connection_info(
        self, location_name: str, excluded_handles: Optional[List[Container]] = None
    ) -> DagsterCloudSandboxConnectionInfo:
        excluded_handles = excluded_handles or []
        excluded_container_ids = [handle.id for handle in excluded_handles]
        containers = list(
            handle
            for handle in self._get_server_handles_for_location(location_name)
            if handle.id not in excluded_container_ids
        )
