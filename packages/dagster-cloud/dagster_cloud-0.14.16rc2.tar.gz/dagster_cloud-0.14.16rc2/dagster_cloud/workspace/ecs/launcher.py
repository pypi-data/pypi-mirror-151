...
class EcsUserCodeLauncher(DagsterCloudUserCodeLauncher[EcsServerHandleType], ConfigurableClass):
    ...
    def get_sandbox_connection_info(
        self, location_name: str, excluded_handles: Optional[List[EcsServerHandleType]] = None
    ) -> DagsterCloudSandboxConnectionInfo:
        excluded_handles = excluded_handles or []
        excluded_arns = [handle.arn for handle in excluded_handles]

        services = list(
            handle
            for handle in self._get_server_handles_for_location(location_name)
            if handle.arn not in excluded_arns
        )
