...
def correctPredictionsSingleDate(folder, dStr, mode):
    ...
    for feature_file in sorted(glob(os.path.join(prevFolder, '*/AndroidWearWatch-AccelerationCalibrated-NA.*.featureOri0to180.csv.gz'))):
    ...
    for feature_file in sorted(glob(os.path.join(thisFolder, '*/AndroidWearWatch-AccelerationCalibrated-NA.*.featureOri0to180.csv.gz'))):
        odf = pd.read_csv(feature_file, header=0, skiprows=0, sep=',', compression="infer", quotechar='"',
                          parse_dates=['HEADER_TIME_STAMP', 'START_TIME', 'STOP_TIME'],
                          date_parser=mhealth_timestamp_parser)
        oriDF = pd.concat([oriDF, odf], ignore_index=True)
    ...
    for feature_file in sorted(glob(os.path.join(nextFolder, '*/AndroidWearWatch-AccelerationCalibrated-NA.*.featureOri0to180.csv.gz'))):
        odf = pd.read_csv(feature_file, header=0, skiprows=0, sep=',', compression="infer", quotechar='"',
                          parse_dates=['HEADER_TIME_STAMP', 'START_TIME', 'STOP_TIME'],
                          date_parser=mhealth_timestamp_parser)
        oriDF = pd.concat([oriDF, odf], ignore_index=True)
...
def main(day_folder=None, debug='No'):
    ...
    if debug == 'Yes':
        path = os.path.join(day_folder,'SWaN_' + dateSt + '_debug_afterOri0to180.csv')
    else:
