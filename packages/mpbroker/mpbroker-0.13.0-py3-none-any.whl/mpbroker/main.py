...
def injest(
    base: str = typer.Option(
        ...
        "--base",
        help="the base path to search for media items to injest",
    ),
...
def injest_file(
    ...
    try:
        ...
        if (
            il.reason == InjestLogReason.updated
            or il.reason == InjestLogReason.metadata_extract_issue
        ):
...
def list(
    ...
    _results = results_by_name(
        name=name, user=user, status=_status, filter_directory=directory
    )
    ...
    if auto_play:
        ...
        table = results_to_table(_results, name=name, user=user)
        console = Console()
    ...
    try:
        ...
        if typer.confirm("  Update Status?"):
            _status = typer.prompt(
                "    New Status",
                default=MediaPlayStatus.watched,
