...
def determine_status_from_flags(
    ...
    _status = []
    if new:
        _status.append(MediaPlayStatus.new)
    if played:
        _status.append(MediaPlayStatus.played)
    if watched:
        _status.append(MediaPlayStatus.watched)
    if len(_status) < 1:
        return None
    elif len(_status) > 1:
        typer.secho(
...
def results_by_name(
    name: str, user: str, status: str = None, filter_directory: str = ""
):
    """
    ...
    try:
        ...
        if results:
            ...
            if filter_directory:
                _results = []
                for item in results:
                    if item["value"][0] and filter_directory in item["value"][0]:
