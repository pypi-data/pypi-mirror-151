...
def sanitize_bucket(buckets):
    """
    Verify bucket and re-compute upper and lower bound et ensure continuous borders.
    """
...
class Quantile:
    ...
    def quantile_from_borders(self, borders: List = None, compute_pareto=False):
        """
        Arg:
            variable_values: Liste de toutes les valeurs de la variable, que l'on va triéer par ordre croissant.
...
def get_primary_buckets(
    ...
    ::debug:: Pour activer un mode debug, qui affiche des traces
    """
    dataset_size = vdx_sort.shape[0]  # Nb de foyers fiscaux

    # On vérifie que le dataset est bien trié
    previous = vdx_sort[[variable_to_split_on]][dataset_size - 1][0]
...
def get_copulas(
    ...
    # On vérifie que le dataset est bien trié
    dataset_size = vdf.shape[0]  # Nb de foyers fiscaux
    previous = vdf[[primary_variable]][dataset_size - 1][0]
    for i in range(1, 1000):
        idx = dataset_size // i
        idx = idx if idx != dataset_size else dataset_size - 1
...
def keep_upper_bound_secret(json_content):
    if json_content == [] or type(json_content) is str:
        return
    if json_content[-1].get("buckets"):
        ...
        for buck in json_content:
            if buck["buckets"] == [] or type(buck["buckets"]) is str:
                continue
            val = buck["buckets"][-1].get("upper_bound")
            if val:
                buck["buckets"][-1]["upper_bound"] = get_fake_upper_bound(val)
    else:
        # Calib
        val = json_content[-1].get("upper_bound")
        if val:
...
def copulas_to_array(copulas, key: str = "bucket_mean"):
    ...
    for cop in copulas:
        ...
        if type(cop["buckets"]) is not list:
