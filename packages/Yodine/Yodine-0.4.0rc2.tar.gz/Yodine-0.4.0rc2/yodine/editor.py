...
class EditorToolkit(Frame):
    ...
    def add_component(self):
        ...
        try:
            ...
            ttk.Label(
                self.component_list, text=nam, justify="right", font="Helvetica 12 bold"
            ).grid(row=len(self.editor.extra_components) - 1, column=0, sticky=(E,))
    ...
    def init_widgets(self):
        ...
        ttk.Label(self, text="Component Name", justify="center").grid(
            row=0, column=0, sticky=(S,)
        )
        ttk.Label(self, text="Component Value", justify="center").grid(
        ...
        error_lbl = ttk.Label(
            self, textvariable=self.error, justify="right", foreground="red"
        )
        error_lbl.grid(row=2, column=0, sticky=(N, S, E))

        self.component_list = Frame(self, background="#DDD", height=350)
        self.component_list.grid(
            column=0, columnspan=2, row=3, rowspan=3, sticky="nswe"
        )
...
def editor_plugin(loader: ModLoader):
        ...
        def render(
            ...
            **kwargs
            ...
            label = pyglet.text.Label(
                name.value, anchor_x="center", anchor_y="center", x=x, y=y, bold=True
            )
            label.draw()
    ...
    class S_EditorCursor(System):
        ...
        def on_mouse_enter(self, event: EventContext, window, xy, shown, **kwargs):
            if self.keyboard and (
                self.keyboard[key.LSHIFT] or self.keyboard[key.RSHIFT]
            ):
                shown.value = False
    ...
    class S_Clickables(System):
        ...
        def inside(self, mousepos, position, clickable):
            ...
            return (
                mousepos.x > pos.x
                and mousepos.x <= pos.x + cli.x
                and mousepos.y > pos.y
                and mousepos.y <= pos.y + cli.y
            )

        def on_mouse_release(
            self, event, window, pos, button, modifiers, clickable, position, down
        ):
            entity = event.source
            down = down.value
    ...
    class S_ObjectPalette(System):
        ...
        def on_mouse_release(
            self, event, window, xy, button, modifiers, template, selected, **kwargs
        ):
            entity = event.source
        ...
        def put(self, entity, window, xy, template, selected):
            if self.selected is not None and selected.value:
                ...
                self.spawned.append(
                    self.manager.current_level.create_templated_entity(
                        self.selected,
                        [
    ...
    class S_TilePalette(System):
        ...
        def on_mouse_release(
            self, event, window, xy, button, modifiers, tiletype, selected, **kwargs
        ):
            entity = event.source
            ...
            if (
                ...
                and not (
                    modifiers
                    & (pyglet.window.key.MOD_SHIFT | pyglet.window.key.MOD_CTRL)
                )
                and not self.start_pos
            ):
        ...
        def on_mouse_drag(
            ...
            **kwargs
            ...
            if button & pgmouse.LEFT and not (
                modifiers & (pyglet.window.key.MOD_SHIFT | pyglet.window.key.MOD_CTRL)
            ):
                self.put(entity, window, xy, tiletype, selected)
        ...
        def put_rectangle(self):
            ...
            self.manager.current_level.rectangle(
                Vector((x1, y1)), x2 - x1 + 1, y2 - y1 + 1, self.selected
            )
        ...
        def put(self, entity, window, xy, tiletype, selected):
            if self.selected is not None and selected.value:
                ...
                if (
                    (tx, ty) not in self.manager.current_level.tiles
                    or self.manager.current_level.tiles[tx, ty] != self.selected
                ):
        ...
        def render(
            ...
            **kwargs
            ...
            if selected.value:
                ...
                if self.start_pos:
                    sp = self.manager.camera_transform(
                        window, *(Vector(self.start_pos) * 35)
                    ).ints()
                    cp = (
                        self.manager.camera_transform(
                            window, *(Vector(self.curr_pos) * 35)
                        )
                    ).ints()
    ...
    class S_FPSCounter(System):
        ...
        def render(
            self, entity: Entity, window: pyglet.window.Window, *args, **kwargs
        ) -> None:
    ...
    class S_SizeRandom(System):
        ...
        def on_spawned(self, event: EventContext, sizerandom, **kwargs):
            ...
            entity["radius"].value = entity["radius"].value + random.uniform(
    ...
    def editor_init(game: Editor):
        ...
        manag.create_templated_entity("edcursor", [("cshown", True)])

        for i, tt in enumerate(manag.tile_types.keys()):
            manag.create_templated_entity(
                "edpaletteitem", [("index", i), ("tiletype", tt)]
            )
...
def editor_main():
    editor = Editor(
        os.environ.get("YODINE_GAME", "yodine_data"),
        sys.argv[1] if len(sys.argv) > 1 else "main.save.json",
    )
