...
def box_regex_flag(typ, val, c):
    uoz__dtdy = c.box(types.int64, val)
    mzk__wmjz = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(mzk__wmjz, (uoz__dtdy,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    uoz__dtdy = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, uoz__dtdy)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    jigir__prvdd = context.get_python_api(builder)
    esyel__qczj = builder.icmp_signed('==', val, jigir__prvdd.borrow_none())
    with builder.if_then(esyel__qczj):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    zxv__gwik = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(zxv__gwik)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        rhw__uidk = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if rhw__uidk > 1:
            typ = types.List(types.Tuple([string_type] * rhw__uidk))
...
def overload_match_group(m, *args):
    ...
    mcmtf__zjp = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            dgs__dwpu = args[0]
            with numba.objmode(out=mcmtf__zjp):
                out = m.group(dgs__dwpu)
            return out
        return _match_group_impl_one
    gndvp__pyz = 'tuple_str_{}'.format(len(args))
    setattr(types, gndvp__pyz, types.Tuple([mcmtf__zjp] * len(args)))
    elap__zayex = ', '.join('group{}'.format(aaf__frm + 1) for aaf__frm in
        range(len(args)))
    ydd__uaoxz = 'def _match_group_impl(m, *args):\n'
    ydd__uaoxz += '  ({}) = args\n'.format(elap__zayex)
    ydd__uaoxz += "  with numba.objmode(out='{}'):\n".format(gndvp__pyz)
    ydd__uaoxz += '    out = m.group({})\n'.format(elap__zayex)
    ydd__uaoxz += '  return out\n'
    nmyqi__mzm = {}
    exec(ydd__uaoxz, globals(), nmyqi__mzm)
    jjw__grl = nmyqi__mzm['_match_group_impl']
    return jjw__grl
...
def overload_match_groups(m, default=None):
    wmba__bgko = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=wmba__bgko):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(jvko__zye is None for jvko__zye in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=mcmtf__zjp):
