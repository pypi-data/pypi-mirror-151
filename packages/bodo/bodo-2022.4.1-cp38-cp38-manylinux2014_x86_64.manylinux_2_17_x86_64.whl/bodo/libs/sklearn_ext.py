...
except ImportError as nene__dlk:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        nby__izijf = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(nby__izijf)


def random_forest_model_fit(m, X, y):
    ydf__nrqci = m.n_estimators
    ulgxu__dgob = MPI.Get_processor_name()
    keyxp__jofb = get_host_ranks()
    bbb__nmkdz = len(keyxp__jofb)
    lvg__bcbwf = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(ydf__nrqci,
        bbb__nmkdz, lvg__bcbwf)
    if lvg__bcbwf == keyxp__jofb[ulgxu__dgob][0]:
        m.n_jobs = len(keyxp__jofb[ulgxu__dgob])
    ...
    tfq__ouu = create_subcomm_mpi4py(first_rank_node)
    if tfq__ouu != MPI.COMM_NULL:
        qdlri__bepex = 10
        lms__mnbck = bodo.libs.distributed_api.get_node_portion(ydf__nrqci,
            bbb__nmkdz, 0)
        tcejc__pyq = lms__mnbck // qdlri__bepex
        if lms__mnbck % qdlri__bepex != 0:
            tcejc__pyq += 1
        cyg__nzmm = []
        for ltq__yvij in range(tcejc__pyq):
            pwo__olni = tfq__ouu.gather(m.estimators_[ltq__yvij *
                qdlri__bepex:ltq__yvij * qdlri__bepex + qdlri__bepex])
            if lvg__bcbwf == 0:
                cyg__nzmm += list(itertools.chain.from_iterable(pwo__olni))
        if lvg__bcbwf == 0:
            m.estimators_ = cyg__nzmm
    xzth__hvpc = MPI.COMM_WORLD
    if lvg__bcbwf == 0:
        for ltq__yvij in range(0, ydf__nrqci, 10):
            xzth__hvpc.bcast(m.estimators_[ltq__yvij:ltq__yvij + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            xzth__hvpc.bcast(m.n_classes_)
            xzth__hvpc.bcast(m.classes_)
        xzth__hvpc.bcast(m.n_outputs_)
    else:
        pze__wbr = []
        for ltq__yvij in range(0, ydf__nrqci, 10):
            pze__wbr += xzth__hvpc.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = xzth__hvpc.bcast(None)
            m.classes_ = xzth__hvpc.bcast(None)
        m.n_outputs_ = xzth__hvpc.bcast(None)
        m.estimators_ = pze__wbr
    assert len(m.estimators_) == ydf__nrqci
    m.n_estimators = ydf__nrqci
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    crzyb__ksacs = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    vaag__bkja = len(labels)
    qyo__bxrxs = np.zeros(vaag__bkja, np.int64)
    qrj__mywxl = np.zeros(vaag__bkja, np.int64)
    fms__ena = np.zeros(vaag__bkja, np.int64)
    ekzm__kfsr = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for ltq__yvij in range(len(y_true)):
        qrj__mywxl[ekzm__kfsr[y_true[ltq__yvij]]] += 1
        if y_pred[ltq__yvij] not in ekzm__kfsr:
            continue
        jryg__ojzex = ekzm__kfsr[y_pred[ltq__yvij]]
        fms__ena[jryg__ojzex] += 1
        if y_true[ltq__yvij] == y_pred[ltq__yvij]:
            qyo__bxrxs[jryg__ojzex] += 1
    qyo__bxrxs = bodo.libs.distributed_api.dist_reduce(qyo__bxrxs, np.int32
        (Reduce_Type.Sum.value))
    qrj__mywxl = bodo.libs.distributed_api.dist_reduce(qrj__mywxl, np.int32
        (Reduce_Type.Sum.value))
    fms__ena = bodo.libs.distributed_api.dist_reduce(fms__ena, np.int32(
        Reduce_Type.Sum.value))
    yghgx__qkunb = fms__ena - qyo__bxrxs
    hnn__xaxla = qrj__mywxl - qyo__bxrxs
    zztf__keoa = qyo__bxrxs
    bgt__lbmlc = y_true.shape[0] - zztf__keoa - yghgx__qkunb - hnn__xaxla
    with numba.objmode(result='float64[:]'):
        MCM = np.array([bgt__lbmlc, yghgx__qkunb, hnn__xaxla, zztf__keoa]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        afhhl__siqcl = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        afhhl__siqcl = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    xzth__hvpc = MPI.COMM_WORLD
    woav__lad = xzth__hvpc.Get_size()
    if sample_weight is not None:
        qgt__ahnov = np.sum(sample_weight)
    else:
        qgt__ahnov = np.float64(y_true.shape[0])
    ong__ehc = np.zeros(woav__lad, dtype=type(qgt__ahnov))
    xzth__hvpc.Allgather(qgt__ahnov, ong__ehc)
    pwgvw__xgu = np.zeros((woav__lad, *afhhl__siqcl.shape), dtype=
        afhhl__siqcl.dtype)
    xzth__hvpc.Allgather(afhhl__siqcl, pwgvw__xgu)
    tupyx__ahs = np.average(pwgvw__xgu, weights=ong__ehc, axis=0)
    if metric == 'mse' and not squared:
        tupyx__ahs = np.sqrt(tupyx__ahs)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return tupyx__ahs
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(tupyx__ahs)
    else:
        return np.average(tupyx__ahs, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    xzth__hvpc = MPI.COMM_WORLD
    score = xzth__hvpc.allreduce(score, op=MPI.SUM)
    if normalize:
        njp__wty = np.sum(sample_weight) if sample_weight is not None else len(
            y_true)
        njp__wty = xzth__hvpc.allreduce(njp__wty, op=MPI.SUM)
        score = score / njp__wty
    return score
...
def check_consistent_length_parallel(*arrays):
    xzth__hvpc = MPI.COMM_WORLD
    ibj__rvsqt = True
    dbcb__iftvw = [len(jkop__xetih) for jkop__xetih in arrays if 
        jkop__xetih is not None]
    if len(np.unique(dbcb__iftvw)) > 1:
        ibj__rvsqt = False
    ibj__rvsqt = xzth__hvpc.allreduce(ibj__rvsqt, op=MPI.LAND)
    return ibj__rvsqt
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    xnch__vkj = y_true.shape[0]
    xzv__qlrl = xzth__hvpc.allreduce(xnch__vkj, op=MPI.SUM)
    if xzv__qlrl < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        byzj__xufh = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        byzj__xufh = 1.0
    tqkk__mkfgk = (byzj__xufh * (y_true - y_pred) ** 2).sum(axis=0, dtype=
        np.float64)
    ejp__wwgk = np.zeros(tqkk__mkfgk.shape, dtype=tqkk__mkfgk.dtype)
    xzth__hvpc.Allreduce(tqkk__mkfgk, ejp__wwgk, op=MPI.SUM)
    fmt__occn = np.nansum(y_true * byzj__xufh, axis=0, dtype=np.float64)
    mitvx__vmbr = np.zeros_like(fmt__occn)
    xzth__hvpc.Allreduce(fmt__occn, mitvx__vmbr, op=MPI.SUM)
    wkx__ajtf = np.nansum(sample_weight, dtype=np.float64)
    pqu__ygvh = xzth__hvpc.allreduce(wkx__ajtf, op=MPI.SUM)
    sevps__vejoe = mitvx__vmbr / pqu__ygvh
    anop__amg = (byzj__xufh * (y_true - sevps__vejoe) ** 2).sum(axis=0,
        dtype=np.float64)
    ayi__txx = np.zeros(anop__amg.shape, dtype=anop__amg.dtype)
    xzth__hvpc.Allreduce(anop__amg, ayi__txx, op=MPI.SUM)
    xmtvc__sjmk = ayi__txx != 0
    jyvjh__kdmw = ejp__wwgk != 0
    jza__hbmm = xmtvc__sjmk & jyvjh__kdmw
    gbwwu__dnc = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    gbwwu__dnc[jza__hbmm] = 1 - ejp__wwgk[jza__hbmm] / ayi__txx[jza__hbmm]
    gbwwu__dnc[jyvjh__kdmw & ~xmtvc__sjmk] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            qowe__nto = ayi__txx
            if not np.any(xmtvc__sjmk):
                if not np.any(jyvjh__kdmw):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        qowe__nto = multioutput
    return np.array([np.average(gbwwu__dnc, weights=qowe__nto)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    xzth__hvpc = MPI.COMM_WORLD
    try:
        kbm__ejmkt = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as pxn__uley:
        kbm__ejmkt = pxn__uley
    ywf__ipmx = isinstance(kbm__ejmkt, ValueError
        ) and 'At least one label specified must be in y_true' in kbm__ejmkt.args[
        0]
    jrl__uef = xzth__hvpc.allreduce(ywf__ipmx, op=MPI.LAND)
    if jrl__uef:
        raise kbm__ejmkt
    elif ywf__ipmx:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        rsm__ocm = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        rsm__ocm = kbm__ejmkt
    xdz__jvcf = np.zeros_like(rsm__ocm)
    xzth__hvpc.Allreduce(rsm__ocm, xdz__jvcf)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            xdz__jvcf = xdz__jvcf / xdz__jvcf.sum(axis=1, keepdims=True)
        elif normalize == 'pred':
            xdz__jvcf = xdz__jvcf / xdz__jvcf.sum(axis=0, keepdims=True)
        elif normalize == 'all':
            xdz__jvcf = xdz__jvcf / xdz__jvcf.sum()
        xdz__jvcf = np.nan_to_num(xdz__jvcf)
    return xdz__jvcf
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    ejifl__ose = 'def _confusion_matrix_impl(\n'
    ejifl__ose += '    y_true, y_pred, labels=None,\n'
    ejifl__ose += '    sample_weight=None, normalize=None,\n'
    ejifl__ose += '    _is_data_distributed=False,\n'
    ejifl__ose += '):\n'
    ejifl__ose += (
        '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n')
    ejifl__ose += (
    ...
    caqz__ofmxm = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        caqz__ofmxm = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        ejifl__ose += '    labels = bodo.allgatherv(labels, False)\n'
        ejifl__ose += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    ejifl__ose += f"    with numba.objmode(cm='{caqz__ofmxm[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        ejifl__ose += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        ejifl__ose += '      cm = confusion_matrix_dist_helper(\n'
    ejifl__ose += '        y_true, y_pred, labels=labels,\n'
    ejifl__ose += '        sample_weight=sample_weight, normalize=normalize,\n'
    ejifl__ose += f'      ).astype({caqz__ofmxm[1]})\n'
    ejifl__ose += '    return cm\n'
    pmw__rytq = {}
    exec(ejifl__ose, globals(), pmw__rytq)
    yfvq__mbo = pmw__rytq['_confusion_matrix_impl']
    return yfvq__mbo
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    xzth__hvpc = MPI.COMM_WORLD
    qudrm__xkqoy = xzth__hvpc.allreduce(len(X), op=MPI.SUM)
    umb__jvyh = len(X) / qudrm__xkqoy
    pkmsm__uxa = xzth__hvpc.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    hfh__osntg = np.inf
    lglqu__niyd = 0
    if m.loss == 'hinge':
        fpwpi__qbh = hinge_loss
    elif m.loss == 'log':
        fpwpi__qbh = log_loss
    elif m.loss == 'squared_error':
        fpwpi__qbh = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    wwar__jfrmm = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        wwar__jfrmm = True
    for mtp__wjk in range(m.max_iter):
        if wwar__jfrmm:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * umb__jvyh
        m.coef_ = xzth__hvpc.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * umb__jvyh
        m.intercept_ = xzth__hvpc.allreduce(m.intercept_, op=MPI.SUM)
        if wwar__jfrmm:
            y_pred = m.predict(X)
            rqse__ivfh = fpwpi__qbh(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            rqse__ivfh = fpwpi__qbh(y, y_pred, labels=y_classes)
        nmsz__nyfn = xzth__hvpc.allreduce(rqse__ivfh, op=MPI.SUM)
        rqse__ivfh = nmsz__nyfn / pkmsm__uxa
        if m.tol > np.NINF and rqse__ivfh > hfh__osntg - m.tol * qudrm__xkqoy:
            lglqu__niyd += 1
        else:
            lglqu__niyd = 0
        if rqse__ivfh < hfh__osntg:
            hfh__osntg = rqse__ivfh
        if lglqu__niyd >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    xzth__hvpc = MPI.COMM_WORLD
    lvg__bcbwf = xzth__hvpc.Get_rank()
    ulgxu__dgob = MPI.Get_processor_name()
    keyxp__jofb = get_host_ranks()
    bgvm__xgv = m.n_jobs if hasattr(m, 'n_jobs') else None
    rgypt__wdr = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(keyxp__jofb[ulgxu__dgob])
    if lvg__bcbwf == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if lvg__bcbwf == 0:
        xzth__hvpc.bcast(m.cluster_centers_)
        xzth__hvpc.bcast(m.inertia_)
        xzth__hvpc.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = xzth__hvpc.bcast(None)
        m.inertia_ = xzth__hvpc.bcast(None)
        m.n_iter_ = xzth__hvpc.bcast(None)
    if _is_data_distributed:
        cxx__shu = xzth__hvpc.allgather(len_X)
        if lvg__bcbwf == 0:
            yzpz__atjef = np.empty(len(cxx__shu) + 1, dtype=int)
            np.cumsum(cxx__shu, out=yzpz__atjef[1:])
            yzpz__atjef[0] = 0
            ocgg__lbd = [m.labels_[yzpz__atjef[ihyn__ydtbo]:yzpz__atjef[
                ihyn__ydtbo + 1]] for ihyn__ydtbo in range(len(cxx__shu))]
            rjod__jxdb = xzth__hvpc.scatter(ocgg__lbd)
        else:
            rjod__jxdb = xzth__hvpc.scatter(None)
        m.labels_ = rjod__jxdb
    elif lvg__bcbwf == 0:
        xzth__hvpc.bcast(m.labels_)
    else:
        m.labels_ = xzth__hvpc.bcast(None)
    m._n_threads = rgypt__wdr
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                xzth__hvpc = MPI.COMM_WORLD
                result = xzth__hvpc.allreduce(result, op=MPI.SUM)
            m._n_threads = rgypt__wdr
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        ejifl__ose = 'def _model_multinomial_nb_fit_impl(\n'
        ejifl__ose += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        ejifl__ose += '):  # pragma: no cover\n'
        ejifl__ose += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            ejifl__ose += '    X = X.to_numpy()\n'
        else:
            ejifl__ose += (
                '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n')
        ejifl__ose += '    my_rank = bodo.get_rank()\n'
        ejifl__ose += '    nranks = bodo.get_size()\n'
        ejifl__ose += '    total_cols = X.shape[1]\n'
        ejifl__ose += '    for i in range(nranks):\n'
        ejifl__ose += """        start = bodo.libs.distributed_api.get_start(total_cols, nranks, i)
"""
        ejifl__ose += (
            '        end = bodo.libs.distributed_api.get_end(total_cols, nranks, i)\n'
            )
        ejifl__ose += '        if i == my_rank:\n'
        ejifl__ose += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        ejifl__ose += '        else:\n'
        ejifl__ose += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        ejifl__ose += '    y_train = bodo.allgatherv(y, False)\n'
        ejifl__ose += '    with numba.objmode(m="multinomial_nb_type"):\n'
        ejifl__ose += '        m = fit_multinomial_nb(\n'
        ejifl__ose += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        ejifl__ose += '        )\n'
        ejifl__ose += '    bodo.barrier()\n'
        ejifl__ose += '    return m\n'
        pmw__rytq = {}
        exec(ejifl__ose, globals(), pmw__rytq)
        bkifk__svs = pmw__rytq['_model_multinomial_nb_fit_impl']
        return bkifk__svs
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    mtp__wjk, n_features = X_train.shape
    m.n_features_in_ = n_features
    bqufy__rzujn = LabelBinarizer()
    yji__hltk = bqufy__rzujn.fit_transform(y_train)
    m.classes_ = bqufy__rzujn.classes_
    if yji__hltk.shape[1] == 1:
        yji__hltk = np.concatenate((1 - yji__hltk, yji__hltk), axis=1)
    if sample_weight is not None:
        yji__hltk = yji__hltk.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        yji__hltk *= sample_weight.T
    class_prior = m.class_prior
    fbk__volp = yji__hltk.shape[1]
    m._init_counters(fbk__volp, n_features)
    m._count(X_train.astype('float64'), yji__hltk)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    jgwh__xncz = m.feature_count_ + alpha
    xflq__vxxn = jgwh__xncz.sum(axis=1)
    xzth__hvpc = MPI.COMM_WORLD
    pkmsm__uxa = xzth__hvpc.Get_size()
    dhglz__thmo = np.zeros(fbk__volp)
    xzth__hvpc.Allreduce(xflq__vxxn, dhglz__thmo, op=MPI.SUM)
    seyt__ihl = np.log(jgwh__xncz) - np.log(dhglz__thmo.reshape(-1, 1))
    fiop__yslnj = seyt__ihl.T.reshape(n_features * fbk__volp)
    vkr__ljch = np.ones(pkmsm__uxa) * (total_cols // pkmsm__uxa)
    haxez__nzsud = total_cols % pkmsm__uxa
    for paphh__fub in range(haxez__nzsud):
        vkr__ljch[paphh__fub] += 1
    vkr__ljch *= fbk__volp
    qgrmf__waz = np.zeros(pkmsm__uxa, dtype=np.int32)
    qgrmf__waz[1:] = np.cumsum(vkr__ljch)[:-1]
    bhv__xjd = np.zeros((total_cols, fbk__volp), dtype=np.float64)
    xzth__hvpc.Allgatherv(fiop__yslnj, [bhv__xjd, vkr__ljch, qgrmf__waz,
        MPI.DOUBLE_PRECISION])
    m.feature_log_prob_ = bhv__xjd.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    xzth__hvpc = MPI.COMM_WORLD
    woav__lad = xzth__hvpc.Get_size()
    onnwq__zxad = m.with_std
    viu__lyxqm = m.with_mean
    m.with_std = False
    if onnwq__zxad:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = onnwq__zxad
    m.with_mean = viu__lyxqm
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    yhzsz__zmu = np.zeros((woav__lad, *m.n_samples_seen_.shape), dtype=m.
        n_samples_seen_.dtype)
    xzth__hvpc.Allgather(m.n_samples_seen_, yhzsz__zmu)
    mfa__wvm = np.sum(yhzsz__zmu, axis=0)
    m.n_samples_seen_ = mfa__wvm
    if m.with_mean or m.with_std:
        puqb__iae = np.zeros((woav__lad, *m.mean_.shape), dtype=m.mean_.dtype)
        xzth__hvpc.Allgather(m.mean_, puqb__iae)
        puqb__iae[np.isnan(puqb__iae)] = 0
        airv__ppjt = np.average(puqb__iae, axis=0, weights=yhzsz__zmu)
        m.mean_ = airv__ppjt
    if m.with_std:
        krk__wsba = sklearn_safe_accumulator_op(np.nansum, (X - airv__ppjt) **
            2, axis=0) / mfa__wvm
        fhfis__oiyjf = np.zeros_like(krk__wsba)
        xzth__hvpc.Allreduce(krk__wsba, fhfis__oiyjf, op=MPI.SUM)
        m.var_ = fhfis__oiyjf
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    jjbp__ccpl = xzth__hvpc.allreduce(jjbp__ccpl, op=MPI.LAND)
    if jjbp__ccpl:
...
def get_data_slice_parallel(data, labels, len_train):
    erurq__stmr = data[:len_train]
    phref__iubx = data[len_train:]
    erurq__stmr = bodo.rebalance(erurq__stmr)
    phref__iubx = bodo.rebalance(phref__iubx)
    oxmq__fyfgo = labels[:len_train]
    rmg__vrrgi = labels[len_train:]
    oxmq__fyfgo = bodo.rebalance(oxmq__fyfgo)
    rmg__vrrgi = bodo.rebalance(rmg__vrrgi)
    return erurq__stmr, phref__iubx, oxmq__fyfgo, rmg__vrrgi
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    nbgeo__bnmsg = {'stratify': stratify}
    pyfe__mxed = {'stratify': None}
    check_unsupported_args('train_test_split', nbgeo__bnmsg, pyfe__mxed, 'ml')
    if is_overload_false(_is_data_distributed):
        bvtfp__mpc = f'data_split_type_{numba.core.ir_utils.next_label()}'
        jax__mok = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for lxkd__jthjy, fnz__dtmvo in ((data, bvtfp__mpc), (labels, jax__mok)
            ):
            if isinstance(lxkd__jthjy, (DataFrameType, SeriesType)):
                cit__npz = lxkd__jthjy.copy(index=NumericIndexType(types.int64)
                    )
                setattr(types, fnz__dtmvo, cit__npz)
            else:
                setattr(types, fnz__dtmvo, lxkd__jthjy)
        ejifl__ose = 'def _train_test_split_impl(\n'
        ejifl__ose += '    data,\n'
        ejifl__ose += '    labels=None,\n'
        ejifl__ose += '    train_size=None,\n'
        ejifl__ose += '    test_size=None,\n'
        ejifl__ose += '    random_state=None,\n'
        ejifl__ose += '    shuffle=True,\n'
        ejifl__ose += '    stratify=None,\n'
        ejifl__ose += '    _is_data_distributed=False,\n'
        ejifl__ose += '):  # pragma: no cover\n'
        ejifl__ose += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(bvtfp__mpc, bvtfp__mpc, jax__mok, jax__mok))
        ejifl__ose += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        ejifl__ose += '            data,\n'
        ejifl__ose += '            labels,\n'
        ejifl__ose += '            train_size=train_size,\n'
        ejifl__ose += '            test_size=test_size,\n'
        ejifl__ose += '            random_state=random_state,\n'
        ejifl__ose += '            shuffle=shuffle,\n'
        ejifl__ose += '            stratify=stratify,\n'
        ejifl__ose += '        )\n'
        ejifl__ose += (
            '    return data_train, data_test, labels_train, labels_test\n')
        pmw__rytq = {}
        exec(ejifl__ose, globals(), pmw__rytq)
        _train_test_split_impl = pmw__rytq['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            yshpp__nftdg = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(yshpp__nftdg * train_size)
            pho__fqup = yshpp__nftdg - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                lvg__bcbwf = bodo.get_rank()
                pkmsm__uxa = bodo.get_size()
                pgzii__banvy = np.empty(pkmsm__uxa, np.int64)
                bodo.libs.distributed_api.allgather(pgzii__banvy, len(data))
                qxiie__zteq = np.cumsum(pgzii__banvy[0:lvg__bcbwf + 1])
                bkjzd__baaqp = np.full(yshpp__nftdg, True)
                bkjzd__baaqp[:pho__fqup] = False
                np.random.seed(42)
                np.random.permutation(bkjzd__baaqp)
                if lvg__bcbwf:
                    exqu__rzbrz = qxiie__zteq[lvg__bcbwf - 1]
                else:
                    exqu__rzbrz = 0
                bap__kztat = qxiie__zteq[lvg__bcbwf]
                csyn__ixdvt = bkjzd__baaqp[exqu__rzbrz:bap__kztat]
                erurq__stmr = data[csyn__ixdvt]
                phref__iubx = data[~csyn__ixdvt]
                oxmq__fyfgo = labels[csyn__ixdvt]
                rmg__vrrgi = labels[~csyn__ixdvt]
                erurq__stmr = bodo.random_shuffle(erurq__stmr, seed=
                    random_state, parallel=True)
                phref__iubx = bodo.random_shuffle(phref__iubx, seed=
                    random_state, parallel=True)
                oxmq__fyfgo = bodo.random_shuffle(oxmq__fyfgo, seed=
                    random_state, parallel=True)
                rmg__vrrgi = bodo.random_shuffle(rmg__vrrgi, seed=
                    random_state, parallel=True)
                oxmq__fyfgo = reset_labels_type(oxmq__fyfgo, label_type)
                rmg__vrrgi = reset_labels_type(rmg__vrrgi, label_type)
            else:
                erurq__stmr, phref__iubx, oxmq__fyfgo, rmg__vrrgi = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    woav__lad = xzth__hvpc.Get_size()
    m = m.fit(X)
    mfa__wvm = xzth__hvpc.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = mfa__wvm
    ohnmw__qzs = np.zeros((woav__lad, *m.data_min_.shape), dtype=m.
        data_min_.dtype)
    xzth__hvpc.Allgather(m.data_min_, ohnmw__qzs)
    xnhsl__ffvg = np.nanmin(ohnmw__qzs, axis=0)
    afuo__idgk = np.zeros((woav__lad, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    xzth__hvpc.Allgather(m.data_max_, afuo__idgk)
    dtf__dtlm = np.nanmax(afuo__idgk, axis=0)
    ppe__akfx = dtf__dtlm - xnhsl__ffvg
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(ppe__akfx)
    m.min_ = m.feature_range[0] - xnhsl__ffvg * m.scale_
    m.data_min_ = xnhsl__ffvg
    m.data_max_ = dtf__dtlm
    m.data_range_ = ppe__akfx
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    qrdp__lbylb = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        qrdp__lbylb = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{qrdp__lbylb}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            bbb__nmkdz = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if bbb__nmkdz > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=bbb__nmkdz)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        noxx__pyduc = True
    return noxx__pyduc, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                fgb__zdli = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                fgb__zdli = bodo.allgatherv(fgb__zdli, False)
                fgb__zdli = bodo.libs.array_kernels.sort(fgb__zdli,
                    ascending=True, inplace=True)
                befg__sqha = {}
                for ltq__yvij in range(len(fgb__zdli)):
                    befg__sqha[fgb__zdli[ltq__yvij]] = ltq__yvij
            else:
                befg__sqha = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = befg__sqha
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
