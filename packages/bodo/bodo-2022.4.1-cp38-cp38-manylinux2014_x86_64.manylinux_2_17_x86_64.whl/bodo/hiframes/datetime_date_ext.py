...
def unbox_datetime_date(typ, val, c):
    aae__pup = c.pyapi.object_getattr_string(val, 'year')
    shy__ykn = c.pyapi.object_getattr_string(val, 'month')
    mmt__dimrj = c.pyapi.object_getattr_string(val, 'day')
    wro__kjpu = c.pyapi.long_as_longlong(aae__pup)
    fynb__rgd = c.pyapi.long_as_longlong(shy__ykn)
    orw__ezo = c.pyapi.long_as_longlong(mmt__dimrj)
    bkc__ogywi = c.builder.add(orw__ezo, c.builder.add(c.builder.shl(
        wro__kjpu, lir.Constant(lir.IntType(64), 32)), c.builder.shl(
        fynb__rgd, lir.Constant(lir.IntType(64), 16))))
    c.pyapi.decref(aae__pup)
    c.pyapi.decref(shy__ykn)
    c.pyapi.decref(mmt__dimrj)
    eikj__klh = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(bkc__ogywi, is_error=eikj__klh)
...
def lower_constant_datetime_date(context, builder, ty, pyval):
    ...
    bkc__ogywi = builder.add(day, builder.add(builder.shl(year, lir.
...
def box_datetime_date(typ, val, c):
    aae__pup = c.pyapi.long_from_longlong(c.builder.lshr(val, lir.Constant(
        lir.IntType(64), 32)))
    shy__ykn = c.pyapi.long_from_longlong(c.builder.and_(c.builder.lshr(val,
        lir.Constant(lir.IntType(64), 16)), lir.Constant(lir.IntType(64), 
        65535)))
    mmt__dimrj = c.pyapi.long_from_longlong(c.builder.and_(val, lir.
        Constant(lir.IntType(64), 65535)))
    jawfw__ndope = c.pyapi.unserialize(c.pyapi.serialize_object(datetime.date))
    dcq__unrmp = c.pyapi.call_function_objargs(jawfw__ndope, (aae__pup,
        shy__ykn, mmt__dimrj))
    c.pyapi.decref(aae__pup)
    c.pyapi.decref(shy__ykn)
    c.pyapi.decref(mmt__dimrj)
    c.pyapi.decref(jawfw__ndope)
    return dcq__unrmp
...
def _ymd2ord(year, month, day):
    xsliq__oqwct = _days_in_month(year, month)
...
def _ord2ymd(n):
    n -= 1
    phx__aikgm, n = divmod(n, _DI400Y)
    year = phx__aikgm * 400 + 1
    bnf__perlz, n = divmod(n, _DI100Y)
    xdhd__uip, n = divmod(n, _DI4Y)
    eegp__aqe, n = divmod(n, 365)
    year += bnf__perlz * 100 + xdhd__uip * 4 + eegp__aqe
    if eegp__aqe == 4 or bnf__perlz == 4:
        return year - 1, 12, 31
    bjynp__wzwu = eegp__aqe == 3 and (xdhd__uip != 24 or bnf__perlz == 3)
    month = n + 50 >> 5
    oohqu__zdk = _DAYS_BEFORE_MONTH[month] + (month > 2 and bjynp__wzwu)
    if oohqu__zdk > n:
        month -= 1
        oohqu__zdk -= _DAYS_IN_MONTH[month] + (month == 2 and bjynp__wzwu)
    n -= oohqu__zdk
...
def get_isocalendar(typingctx, dt_year, dt_month, dt_day):
    ...
    def codegen(context, builder, sig, args):
        ...
        nofa__pscbn = cgutils.alloca_once(builder, lir.IntType(64))
        noh__bakt = cgutils.alloca_once(builder, lir.IntType(64))
        bhk__ztzk = lir.FunctionType(lir.VoidType(), [lir.IntType(64), lir.
            IntType(64), lir.IntType(64), lir.IntType(64).as_pointer(), lir
            .IntType(64).as_pointer(), lir.IntType(64).as_pointer()])
        mcf__fhqeb = cgutils.get_or_insert_function(builder.module,
            bhk__ztzk, name='get_isocalendar')
        builder.call(mcf__fhqeb, [args[0], args[1], args[2], year,
            nofa__pscbn, noh__bakt])
        return cgutils.pack_array(builder, [builder.load(year), builder.
            load(nofa__pscbn), builder.load(noh__bakt)])
    dcq__unrmp = types.Tuple([types.int64, types.int64, types.int64])(types
...
def fromordinal_impl(n):
    y, hql__jqfgn, d = _ord2ymd(n)
    return datetime.date(y, hql__jqfgn, d)
...
def replace_overload(date, year=None, month=None, day=None):
    ...
    def impl(date, year=None, month=None, day=None):
        dsl__zwc = date.year if year is None else year
        sqy__lvmef = date.month if month is None else month
        rlhny__wqbvv = date.day if day is None else day
        return datetime.date(dsl__zwc, sqy__lvmef, rlhny__wqbvv)
    return impl
...
def overload_pd_timestamp_isocalendar(date):
    ...
    def impl(date):
        year, nofa__pscbn, poqr__cjjkg = get_isocalendar(date.year, date.
            month, date.day)
        return year, nofa__pscbn, poqr__cjjkg
    return impl
...
def overload_add_operator_datetime_date(lhs, rhs):
    if lhs == datetime_date_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            nip__ngu = lhs.toordinal() + rhs.days
            if 0 < nip__ngu <= _MAXORDINAL:
                return fromordinal_impl(nip__ngu)
            raise OverflowError('result out of range')
        return impl
    elif lhs == datetime_timedelta_type and rhs == datetime_date_type:

        def impl(lhs, rhs):
            nip__ngu = lhs.days + rhs.toordinal()
            if 0 < nip__ngu <= _MAXORDINAL:
                return fromordinal_impl(nip__ngu)
            raise OverflowError('result out of range')
        return impl
...
def overload_sub_operator_datetime_date(lhs, rhs):
    if lhs == datetime_date_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ekaq__lnu = lhs.toordinal()
            bwujy__dvd = rhs.toordinal()
            return datetime.timedelta(ekaq__lnu - bwujy__dvd)
        return impl
    if lhs == datetime_date_array_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            for jshla__pqywd in numba.parfors.parfor.internal_prange(n):
                A[jshla__pqywd] = xsrb__hryej[jshla__pqywd] - rhs
            return A
        return impl
...
def __hash__(td):
    ...
    def impl(td):
        kvgi__jcac = np.uint8(td.year // 256)
        rhjp__zpil = np.uint8(td.year % 256)
        month = np.uint8(td.month)
        day = np.uint8(td.day)
        ltpzb__jqv = kvgi__jcac, rhjp__zpil, month, day
        return hash(ltpzb__jqv)
    return impl
...
class DatetimeDateArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        aekk__bai = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, aekk__bai)
...
def unbox_datetime_date_array(typ, val, c):
    ...
    bpx__nxh = types.Array(types.intp, 1, 'C')
    fmg__xhv = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        bpx__nxh, [n])
    mekln__hhpeq = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType
        (64), 7)), lir.Constant(lir.IntType(64), 8))
    gjz__dpju = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [mekln__hhpeq])
    bhk__ztzk = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer
        (), lir.IntType(64), lir.IntType(64).as_pointer(), lir.IntType(8).
        as_pointer()])
    cqmi__czahe = cgutils.get_or_insert_function(c.builder.module,
        bhk__ztzk, name='unbox_datetime_date_array')
    c.builder.call(cqmi__czahe, [val, n, fmg__xhv.data, gjz__dpju.data])
    mzwv__kors = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    mzwv__kors.data = fmg__xhv._getvalue()
    mzwv__kors.null_bitmap = gjz__dpju._getvalue()
    eikj__klh = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(mzwv__kors._getvalue(), is_error=eikj__klh)
...
def box_datetime_date_array(typ, val, c):
    xsrb__hryej = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    fmg__xhv = c.context.make_array(types.Array(types.int64, 1, 'C'))(c.
        context, c.builder, xsrb__hryej.data)
    txpl__pfg = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, xsrb__hryej.null_bitmap).data
    n = c.builder.extract_value(fmg__xhv.shape, 0)
    bhk__ztzk = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(64).as_pointer(), lir.IntType(8).as_pointer()])
    mbxyn__dhfq = cgutils.get_or_insert_function(c.builder.module,
        bhk__ztzk, name='box_datetime_date_array')
    tlzf__kgg = c.builder.call(mbxyn__dhfq, [n, fmg__xhv.data, txpl__pfg])
...
def init_datetime_date_array(typingctx, data, nulls=None):
    ...
    def codegen(context, builder, signature, args):
        ogq__hrxi, zkxgu__pwzhw = args
        bfpyh__ptp = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        bfpyh__ptp.data = ogq__hrxi
        bfpyh__ptp.null_bitmap = zkxgu__pwzhw
        context.nrt.incref(builder, signature.args[0], ogq__hrxi)
        context.nrt.incref(builder, signature.args[1], zkxgu__pwzhw)
        return bfpyh__ptp._getvalue()
...
def lower_constant_datetime_date_arr(context, builder, typ, pyval):
    n = len(pyval)
    cubr__vhqa = (1970 << 32) + (1 << 16) + 1
    fmg__xhv = np.full(n, cubr__vhqa, np.int64)
    tybdz__zuuwq = np.empty(n + 7 >> 3, np.uint8)
    for jshla__pqywd, ijspa__ifc in enumerate(pyval):
        ftj__cpki = pd.isna(ijspa__ifc)
        bodo.libs.int_arr_ext.set_bit_to_arr(tybdz__zuuwq, jshla__pqywd,
            int(not ftj__cpki))
        if not ftj__cpki:
            fmg__xhv[jshla__pqywd] = (ijspa__ifc.year << 32) + (ijspa__ifc.
                month << 16) + ijspa__ifc.day
    ryc__viu = context.get_constant_generic(builder, data_type, fmg__xhv)
    bxj__dmduc = context.get_constant_generic(builder, nulls_type, tybdz__zuuwq
        )
    return lir.Constant.literal_struct([ryc__viu, bxj__dmduc])


@numba.njit(no_cpython_wrapper=True)
def alloc_datetime_date_array(n):
    fmg__xhv = np.empty(n, dtype=np.int64)
    nulls = np.full(n + 7 >> 3, 255, np.uint8)
    return init_datetime_date_array(fmg__xhv, nulls)
...
def dt_date_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            polz__vqixv, spikf__dmb = array_getitem_bool_index(A, ind)
            return init_datetime_date_array(polz__vqixv, spikf__dmb)
        return impl_bool
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):

        def impl(A, ind):
            polz__vqixv, spikf__dmb = array_getitem_int_index(A, ind)
            return init_datetime_date_array(polz__vqixv, spikf__dmb)
        return impl
    if isinstance(ind, types.SliceType):

        def impl_slice(A, ind):
            polz__vqixv, spikf__dmb = array_getitem_slice_index(A, ind)
            return init_datetime_date_array(polz__vqixv, spikf__dmb)
        return impl_slice
    raise BodoError(
...
def create_cmp_op_overload(op):
    ...
    def overload_date_cmp(lhs, rhs):
        if lhs == datetime_date_type and rhs == datetime_date_type:
            ...
            def impl(lhs, rhs):
                y, nssx__fio = lhs.year, rhs.year
                hql__jqfgn, gtdxx__ugml = lhs.month, rhs.month
                d, quhx__dxepz = lhs.day, rhs.day
                return op(_cmp((y, hql__jqfgn, d), (nssx__fio, gtdxx__ugml,
                    quhx__dxepz)), 0)
            return impl
...
def create_datetime_date_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        ...
        warnings.warn(ricxz__xzvd, bodo.utils.typing.BodoWarning)
        if op == operator.eq:
    ...
    def overload_date_arr_cmp(lhs, rhs):
        ...
        if lhs == datetime_date_array_type and rhs == datetime_date_array_type:
            ...
            def impl(lhs, rhs):
                numba.parfors.parfor.init_prange()
                n = len(lhs)
                gxuc__ykho = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for jshla__pqywd in numba.parfors.parfor.internal_prange(n):
                    qiv__cwiu = bodo.libs.array_kernels.isna(lhs, jshla__pqywd)
                    xxkhi__jyue = bodo.libs.array_kernels.isna(rhs,
                        jshla__pqywd)
                    if qiv__cwiu or xxkhi__jyue:
                        nlflz__isc = jhwfo__zqh
                    else:
                        nlflz__isc = op(lhs[jshla__pqywd], rhs[jshla__pqywd])
                    gxuc__ykho[jshla__pqywd] = nlflz__isc
                return gxuc__ykho
            return impl
            ...
            def impl(lhs, rhs):
                ...
                gxuc__ykho = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for jshla__pqywd in numba.parfors.parfor.internal_prange(n):
                    ersu__dpw = bodo.libs.array_kernels.isna(lhs, jshla__pqywd)
                    if ersu__dpw:
                        nlflz__isc = jhwfo__zqh
                    else:
                        nlflz__isc = op(lhs[jshla__pqywd], rhs)
                    gxuc__ykho[jshla__pqywd] = nlflz__isc
                return gxuc__ykho
            return impl
            ...
            def impl(lhs, rhs):
                ...
                gxuc__ykho = bodo.libs.bool_arr_ext.alloc_bool_array(n)
                for jshla__pqywd in numba.parfors.parfor.internal_prange(n):
                    ersu__dpw = bodo.libs.array_kernels.isna(rhs, jshla__pqywd)
                    if ersu__dpw:
                        nlflz__isc = jhwfo__zqh
                    else:
                        nlflz__isc = op(lhs, rhs[jshla__pqywd])
                    gxuc__ykho[jshla__pqywd] = nlflz__isc
                return gxuc__ykho
            return impl
