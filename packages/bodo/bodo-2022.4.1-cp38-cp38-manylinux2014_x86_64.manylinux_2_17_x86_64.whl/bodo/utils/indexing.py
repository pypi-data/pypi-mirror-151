...
def get_new_null_mask_bool_index(old_mask, ind, n):
    hynhd__yiqsd = n + 7 >> 3
    jcbn__hspr = np.empty(hynhd__yiqsd, np.uint8)
    jsl__lxyan = 0
    for alc__fclk in range(len(ind)):
        if ind[alc__fclk]:
            nvtv__vmq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                alc__fclk)
            bodo.libs.int_arr_ext.set_bit_to_arr(jcbn__hspr, jsl__lxyan,
                nvtv__vmq)
            jsl__lxyan += 1
    return jcbn__hspr
...
def array_getitem_bool_index(A, ind):
    ...
    rim__jkuv = A._data[ind]
    n = len(rim__jkuv)
    jcbn__hspr = get_new_null_mask_bool_index(old_mask, ind, n)
    return rim__jkuv, jcbn__hspr
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    jsl__lxyan = 0
    for alc__fclk in range(len(ind)):
        nvtv__vmq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind[
            alc__fclk])
        bodo.libs.int_arr_ext.set_bit_to_arr(jcbn__hspr, jsl__lxyan, nvtv__vmq)
        jsl__lxyan += 1
    return jcbn__hspr
...
def array_getitem_int_index(A, ind):
    ykqlz__wcex = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    rim__jkuv = A._data[ykqlz__wcex]
    n = len(rim__jkuv)
    jcbn__hspr = get_new_null_mask_int_index(old_mask, ykqlz__wcex, n)
    return rim__jkuv, jcbn__hspr


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    rcbbm__ria = numba.cpython.unicode._normalize_slice(ind, n)
    wcta__xouks = numba.cpython.unicode._slice_span(rcbbm__ria)
    hynhd__yiqsd = wcta__xouks + 7 >> 3
    jcbn__hspr = np.empty(hynhd__yiqsd, np.uint8)
    jsl__lxyan = 0
    for alc__fclk in range(rcbbm__ria.start, rcbbm__ria.stop, rcbbm__ria.step):
        nvtv__vmq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            alc__fclk)
        bodo.libs.int_arr_ext.set_bit_to_arr(jcbn__hspr, jsl__lxyan, nvtv__vmq)
        jsl__lxyan += 1
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    rim__jkuv = np.ascontiguousarray(A._data[ind])
    jcbn__hspr = get_new_null_mask_slice_index(old_mask, ind, n)
    return rim__jkuv, jcbn__hspr
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for alc__fclk in range(n):
                A._data[idx[alc__fclk]] = val._data[alc__fclk]
                nvtv__vmq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, alc__fclk)
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            for alc__fclk in idx:
                A._data[alc__fclk] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            kxro__jryy = 0
            for alc__fclk in range(n):
                if not bodo.libs.array_kernels.isna(idx, alc__fclk) and idx[
                    alc__fclk]:
                    A._data[alc__fclk] = val._data[kxro__jryy]
                    nvtv__vmq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, kxro__jryy)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        alc__fclk, nvtv__vmq)
                    kxro__jryy += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    rcbbm__ria = numba.cpython.unicode._normalize_slice(idx, n)
    kxro__jryy = 0
    for alc__fclk in range(rcbbm__ria.start, rcbbm__ria.stop, rcbbm__ria.step):
        nvtv__vmq = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            kxro__jryy)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, alc__fclk, nvtv__vmq)
        kxro__jryy += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            rcbbm__ria = numba.cpython.unicode._normalize_slice(idx, len(A))
            for alc__fclk in range(rcbbm__ria.start, rcbbm__ria.stop,
                rcbbm__ria.step):
                A._data[alc__fclk] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    alc__fclk, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for alc__fclk in idx:
                    bodo.libs.array_kernels.setna(A, alc__fclk)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    offv__jttfn = bodo.libs.str_arr_ext.pre_alloc_string_array(
                        n, -1)
                    for alc__fclk in numba.parfors.parfor.internal_prange(n):
                        if idx[alc__fclk] or bodo.libs.array_kernels.isna(A,
                            alc__fclk):
                            offv__jttfn[alc__fclk] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(offv__jttfn,
                                alc__fclk)
                        else:
                            offv__jttfn[alc__fclk] = A[alc__fclk]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for alc__fclk in range(n):
                    if not bodo.libs.array_kernels.isna(idx, alc__fclk
                        ) and idx[alc__fclk]:
                        bodo.libs.array_kernels.setna(A, alc__fclk)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for alc__fclk in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, alc__fclk)
                        continue
                    A[alc__fclk] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        zlol__gkw = context.make_helper(builder, val_t, args[0])
        phq__thhyn = zlol__gkw.data
        context.nrt.incref(builder, val_t.type, phq__thhyn)
        return phq__thhyn
