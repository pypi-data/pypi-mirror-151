...
except ImportError as gqufr__qrjxj:
    pass


def check_sklearn_version():
    if not _is_sklearn_supported_version:
        yiq__xbrj = f""" Bodo supports scikit-learn version >= {_min_sklearn_ver_str} and < {_max_sklearn_ver_str}.
             Installed version is {sklearn.__version__}.
"""
        raise BodoError(yiq__xbrj)


def random_forest_model_fit(m, X, y):
    phumy__xdc = m.n_estimators
    vfur__upjb = MPI.Get_processor_name()
    fkjtz__kdqaa = get_host_ranks()
    iceil__zrpvh = len(fkjtz__kdqaa)
    trusk__qvd = bodo.get_rank()
    m.n_estimators = bodo.libs.distributed_api.get_node_portion(phumy__xdc,
        iceil__zrpvh, trusk__qvd)
    if trusk__qvd == fkjtz__kdqaa[vfur__upjb][0]:
        m.n_jobs = len(fkjtz__kdqaa[vfur__upjb])
    ...
    skbd__nvgw = create_subcomm_mpi4py(first_rank_node)
    if skbd__nvgw != MPI.COMM_NULL:
        uit__zdf = 10
        sezhq__lgc = bodo.libs.distributed_api.get_node_portion(phumy__xdc,
            iceil__zrpvh, 0)
        cnjru__lwbn = sezhq__lgc // uit__zdf
        if sezhq__lgc % uit__zdf != 0:
            cnjru__lwbn += 1
        mzfm__gmyil = []
        for erw__nwx in range(cnjru__lwbn):
            btsxl__yha = skbd__nvgw.gather(m.estimators_[erw__nwx *
                uit__zdf:erw__nwx * uit__zdf + uit__zdf])
            if trusk__qvd == 0:
                mzfm__gmyil += list(itertools.chain.from_iterable(btsxl__yha))
        if trusk__qvd == 0:
            m.estimators_ = mzfm__gmyil
    dord__cuxwx = MPI.COMM_WORLD
    if trusk__qvd == 0:
        for erw__nwx in range(0, phumy__xdc, 10):
            dord__cuxwx.bcast(m.estimators_[erw__nwx:erw__nwx + 10])
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            dord__cuxwx.bcast(m.n_classes_)
            dord__cuxwx.bcast(m.classes_)
        dord__cuxwx.bcast(m.n_outputs_)
    else:
        qejcf__ahqn = []
        for erw__nwx in range(0, phumy__xdc, 10):
            qejcf__ahqn += dord__cuxwx.bcast(None)
        if isinstance(m, sklearn.ensemble.RandomForestClassifier):
            m.n_classes_ = dord__cuxwx.bcast(None)
            m.classes_ = dord__cuxwx.bcast(None)
        m.n_outputs_ = dord__cuxwx.bcast(None)
        m.estimators_ = qejcf__ahqn
    assert len(m.estimators_) == phumy__xdc
    m.n_estimators = phumy__xdc
...
def precision_recall_fscore_support_helper(MCM, average):
    ...
    def multilabel_confusion_matrix(y_true, y_pred, *, sample_weight=None,
        ...
        return MCM
    wna__lphdg = sklearn.metrics._classification.multilabel_confusion_matrix
    result = -1.0
    try:
...
def precision_recall_fscore_parallel(y_true, y_pred, operation, average=
    ...
    aloy__vmhn = len(labels)
    chd__ucdg = np.zeros(aloy__vmhn, np.int64)
    zozl__joj = np.zeros(aloy__vmhn, np.int64)
    vfza__xzsmw = np.zeros(aloy__vmhn, np.int64)
    lnfz__ehfs = (bodo.hiframes.pd_categorical_ext.
        get_label_dict_from_categories(labels))
    for erw__nwx in range(len(y_true)):
        zozl__joj[lnfz__ehfs[y_true[erw__nwx]]] += 1
        if y_pred[erw__nwx] not in lnfz__ehfs:
            continue
        trs__lqq = lnfz__ehfs[y_pred[erw__nwx]]
        vfza__xzsmw[trs__lqq] += 1
        if y_true[erw__nwx] == y_pred[erw__nwx]:
            chd__ucdg[trs__lqq] += 1
    chd__ucdg = bodo.libs.distributed_api.dist_reduce(chd__ucdg, np.int32(
        Reduce_Type.Sum.value))
    zozl__joj = bodo.libs.distributed_api.dist_reduce(zozl__joj, np.int32(
        Reduce_Type.Sum.value))
    vfza__xzsmw = bodo.libs.distributed_api.dist_reduce(vfza__xzsmw, np.
        int32(Reduce_Type.Sum.value))
    gro__rxhdq = vfza__xzsmw - chd__ucdg
    ycoq__fcp = zozl__joj - chd__ucdg
    gaaom__hsk = chd__ucdg
    ldhiv__zurb = y_true.shape[0] - gaaom__hsk - gro__rxhdq - ycoq__fcp
    with numba.objmode(result='float64[:]'):
        MCM = np.array([ldhiv__zurb, gro__rxhdq, ycoq__fcp, gaaom__hsk]
            ).T.reshape(-1, 2, 2)
...
def mse_mae_dist_helper(y_true, y_pred, sample_weight, multioutput, squared,
    metric):
    if metric == 'mse':
        nmmyp__ltsal = sklearn.metrics.mean_squared_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values', squared=True
            )
    elif metric == 'mae':
        nmmyp__ltsal = sklearn.metrics.mean_absolute_error(y_true, y_pred,
            sample_weight=sample_weight, multioutput='raw_values')
    else:
        raise RuntimeError(
            f"Unrecognized metric {metric}. Must be one of 'mae' and 'mse'")
    dord__cuxwx = MPI.COMM_WORLD
    vmfz__muvxr = dord__cuxwx.Get_size()
    if sample_weight is not None:
        ltd__nckw = np.sum(sample_weight)
    else:
        ltd__nckw = np.float64(y_true.shape[0])
    bafzr__yqjh = np.zeros(vmfz__muvxr, dtype=type(ltd__nckw))
    dord__cuxwx.Allgather(ltd__nckw, bafzr__yqjh)
    uea__zcr = np.zeros((vmfz__muvxr, *nmmyp__ltsal.shape), dtype=
        nmmyp__ltsal.dtype)
    dord__cuxwx.Allgather(nmmyp__ltsal, uea__zcr)
    csl__hej = np.average(uea__zcr, weights=bafzr__yqjh, axis=0)
    if metric == 'mse' and not squared:
        csl__hej = np.sqrt(csl__hej)
    if isinstance(multioutput, str) and multioutput == 'raw_values':
        return csl__hej
    elif isinstance(multioutput, str) and multioutput == 'uniform_average':
        return np.average(csl__hej)
    else:
        return np.average(csl__hej, weights=multioutput)
...
def accuracy_score_dist_helper(y_true, y_pred, normalize, sample_weight):
    ...
    dord__cuxwx = MPI.COMM_WORLD
    score = dord__cuxwx.allreduce(score, op=MPI.SUM)
    if normalize:
        ymcm__bawd = np.sum(sample_weight
            ) if sample_weight is not None else len(y_true)
        ymcm__bawd = dord__cuxwx.allreduce(ymcm__bawd, op=MPI.SUM)
        score = score / ymcm__bawd
    return score
...
def check_consistent_length_parallel(*arrays):
    dord__cuxwx = MPI.COMM_WORLD
    hmi__yga = True
    dnxtr__okdf = [len(diz__tia) for diz__tia in arrays if diz__tia is not None
        ]
    if len(np.unique(dnxtr__okdf)) > 1:
        hmi__yga = False
    hmi__yga = dord__cuxwx.allreduce(hmi__yga, op=MPI.LAND)
    return hmi__yga
...
def r2_score_dist_helper(y_true, y_pred, sample_weight, multioutput):
    ...
    ritzr__vqeoy = y_true.shape[0]
    opsg__rzruy = dord__cuxwx.allreduce(ritzr__vqeoy, op=MPI.SUM)
    if opsg__rzruy < 2:
        warnings.warn(
    ...
    if sample_weight is not None:
        ...
        agrs__gnthw = sample_weight[:, np.newaxis]
    else:
        sample_weight = np.float64(y_true.shape[0])
        agrs__gnthw = 1.0
    kky__ogwir = (agrs__gnthw * (y_true - y_pred) ** 2).sum(axis=0, dtype=
        np.float64)
    pcmw__gan = np.zeros(kky__ogwir.shape, dtype=kky__ogwir.dtype)
    dord__cuxwx.Allreduce(kky__ogwir, pcmw__gan, op=MPI.SUM)
    lziyf__vqpk = np.nansum(y_true * agrs__gnthw, axis=0, dtype=np.float64)
    fjnij__hhup = np.zeros_like(lziyf__vqpk)
    dord__cuxwx.Allreduce(lziyf__vqpk, fjnij__hhup, op=MPI.SUM)
    qdhji__ercf = np.nansum(sample_weight, dtype=np.float64)
    lxfr__fis = dord__cuxwx.allreduce(qdhji__ercf, op=MPI.SUM)
    xfltt__nbzz = fjnij__hhup / lxfr__fis
    vauq__quve = (agrs__gnthw * (y_true - xfltt__nbzz) ** 2).sum(axis=0,
        dtype=np.float64)
    beai__iivzl = np.zeros(vauq__quve.shape, dtype=vauq__quve.dtype)
    dord__cuxwx.Allreduce(vauq__quve, beai__iivzl, op=MPI.SUM)
    zhso__yag = beai__iivzl != 0
    thxf__yjau = pcmw__gan != 0
    npk__djj = zhso__yag & thxf__yjau
    icpa__hfqmx = np.ones([y_true.shape[1] if len(y_true.shape) > 1 else 1])
    icpa__hfqmx[npk__djj] = 1 - pcmw__gan[npk__djj] / beai__iivzl[npk__djj]
    icpa__hfqmx[thxf__yjau & ~zhso__yag] = 0.0
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            wmppq__ybb = beai__iivzl
            if not np.any(zhso__yag):
                if not np.any(thxf__yjau):
                    return np.array([1.0])
                else:
                    return np.array([0.0])
    else:
        wmppq__ybb = multioutput
    return np.array([np.average(icpa__hfqmx, weights=wmppq__ybb)])
...
def confusion_matrix_dist_helper(y_true, y_pred, labels=None, sample_weight
    ...
    dord__cuxwx = MPI.COMM_WORLD
    try:
        kzfz__vvydf = sklearn.metrics.confusion_matrix(y_true, y_pred,
            labels=labels, sample_weight=sample_weight, normalize=None)
    except ValueError as hhlt__sism:
        kzfz__vvydf = hhlt__sism
    aponp__qycxj = isinstance(kzfz__vvydf, ValueError
        ) and 'At least one label specified must be in y_true' in kzfz__vvydf.args[
        0]
    vdwa__txjns = dord__cuxwx.allreduce(aponp__qycxj, op=MPI.LAND)
    if vdwa__txjns:
        raise kzfz__vvydf
    elif aponp__qycxj:
        dtype = np.int64
        if sample_weight is not None and sample_weight.dtype.kind not in {'i',
            'u', 'b'}:
            dtype = np.float64
        pnbq__ihek = np.zeros((labels.size, labels.size), dtype=dtype)
    else:
        pnbq__ihek = kzfz__vvydf
    bwpvp__zdzyd = np.zeros_like(pnbq__ihek)
    dord__cuxwx.Allreduce(pnbq__ihek, bwpvp__zdzyd)
    with np.errstate(all='ignore'):
        if normalize == 'true':
            bwpvp__zdzyd = bwpvp__zdzyd / bwpvp__zdzyd.sum(axis=1, keepdims
                =True)
        elif normalize == 'pred':
            bwpvp__zdzyd = bwpvp__zdzyd / bwpvp__zdzyd.sum(axis=0, keepdims
                =True)
        elif normalize == 'all':
            bwpvp__zdzyd = bwpvp__zdzyd / bwpvp__zdzyd.sum()
        bwpvp__zdzyd = np.nan_to_num(bwpvp__zdzyd)
    return bwpvp__zdzyd
...
def overload_confusion_matrix(y_true, y_pred, labels=None, sample_weight=
    ...
    check_sklearn_version()
    lua__qbot = 'def _confusion_matrix_impl(\n'
    lua__qbot += '    y_true, y_pred, labels=None,\n'
    lua__qbot += '    sample_weight=None, normalize=None,\n'
    lua__qbot += '    _is_data_distributed=False,\n'
    lua__qbot += '):\n'
    lua__qbot += '    y_true = bodo.utils.conversion.coerce_to_array(y_true)\n'
    lua__qbot += '    y_pred = bodo.utils.conversion.coerce_to_array(y_pred)\n'
    lua__qbot += (
        '    y_true = bodo.utils.typing.decode_if_dict_array(y_true)\n')
    lua__qbot += (
        '    y_pred = bodo.utils.typing.decode_if_dict_array(y_pred)\n')
    itwtm__rzjkf = 'int64[:,:]', 'np.int64'
    if not is_overload_none(normalize):
        itwtm__rzjkf = 'float64[:,:]', 'np.float64'
    ...
    if not is_overload_none(labels):
        ...
        lua__qbot += '    labels = bodo.allgatherv(labels, False)\n'
        lua__qbot += """    labels = bodo.libs.array_kernels.sort(labels, ascending=True, inplace=False)
"""
    lua__qbot += f"    with numba.objmode(cm='{itwtm__rzjkf[0]}'):\n"
    if is_overload_false(_is_data_distributed):
        lua__qbot += '      cm = sklearn.metrics.confusion_matrix(\n'
    else:
        lua__qbot += '      cm = confusion_matrix_dist_helper(\n'
    lua__qbot += '        y_true, y_pred, labels=labels,\n'
    lua__qbot += '        sample_weight=sample_weight, normalize=normalize,\n'
    lua__qbot += f'      ).astype({itwtm__rzjkf[1]})\n'
    lua__qbot += '    return cm\n'
    lebvb__mluwi = {}
    exec(lua__qbot, globals(), lebvb__mluwi)
    kjfrf__bxe = lebvb__mluwi['_confusion_matrix_impl']
    return kjfrf__bxe
...
def fit_sgd(m, X, y, y_classes=None, _is_data_distributed=False):
    dord__cuxwx = MPI.COMM_WORLD
    btzk__tae = dord__cuxwx.allreduce(len(X), op=MPI.SUM)
    bkc__qbkhf = len(X) / btzk__tae
    kepep__wdi = dord__cuxwx.Get_size()
    m.n_jobs = 1
    m.early_stopping = False
    oonoj__egcwr = np.inf
    obc__xzo = 0
    if m.loss == 'hinge':
        nqum__wyr = hinge_loss
    elif m.loss == 'log':
        nqum__wyr = log_loss
    elif m.loss == 'squared_error':
        nqum__wyr = mean_squared_error
    else:
        raise ValueError('loss {} not supported'.format(m.loss))
    drmev__egzop = False
    if isinstance(m, sklearn.linear_model.SGDRegressor):
        drmev__egzop = True
    for cbnjm__rblba in range(m.max_iter):
        if drmev__egzop:
            m.partial_fit(X, y)
        else:
            m.partial_fit(X, y, classes=y_classes)
        m.coef_ = m.coef_ * bkc__qbkhf
        m.coef_ = dord__cuxwx.allreduce(m.coef_, op=MPI.SUM)
        m.intercept_ = m.intercept_ * bkc__qbkhf
        m.intercept_ = dord__cuxwx.allreduce(m.intercept_, op=MPI.SUM)
        if drmev__egzop:
            y_pred = m.predict(X)
            dumy__hng = nqum__wyr(y, y_pred)
        else:
            y_pred = m.decision_function(X)
            dumy__hng = nqum__wyr(y, y_pred, labels=y_classes)
        pvyh__kra = dord__cuxwx.allreduce(dumy__hng, op=MPI.SUM)
        dumy__hng = pvyh__kra / kepep__wdi
        if m.tol > np.NINF and dumy__hng > oonoj__egcwr - m.tol * btzk__tae:
            obc__xzo += 1
        else:
            obc__xzo = 0
        if dumy__hng < oonoj__egcwr:
            oonoj__egcwr = dumy__hng
        if obc__xzo >= m.n_iter_no_change:
            break
    return m
...
def kmeans_fit_helper(m, len_X, all_X, all_sample_weight, _is_data_distributed
    ...
    dord__cuxwx = MPI.COMM_WORLD
    trusk__qvd = dord__cuxwx.Get_rank()
    vfur__upjb = MPI.Get_processor_name()
    fkjtz__kdqaa = get_host_ranks()
    ytine__bspd = m.n_jobs if hasattr(m, 'n_jobs') else None
    fkxh__upoie = m._n_threads if hasattr(m, '_n_threads') else None
    m._n_threads = len(fkjtz__kdqaa[vfur__upjb])
    if trusk__qvd == 0:
        m.fit(X=all_X, y=None, sample_weight=all_sample_weight)
    if trusk__qvd == 0:
        dord__cuxwx.bcast(m.cluster_centers_)
        dord__cuxwx.bcast(m.inertia_)
        dord__cuxwx.bcast(m.n_iter_)
    else:
        m.cluster_centers_ = dord__cuxwx.bcast(None)
        m.inertia_ = dord__cuxwx.bcast(None)
        m.n_iter_ = dord__cuxwx.bcast(None)
    if _is_data_distributed:
        cppcr__qsli = dord__cuxwx.allgather(len_X)
        if trusk__qvd == 0:
            jwpl__objkw = np.empty(len(cppcr__qsli) + 1, dtype=int)
            np.cumsum(cppcr__qsli, out=jwpl__objkw[1:])
            jwpl__objkw[0] = 0
            efras__oijpf = [m.labels_[jwpl__objkw[rgzo__xajwe]:jwpl__objkw[
                rgzo__xajwe + 1]] for rgzo__xajwe in range(len(cppcr__qsli))]
            ckpr__khnd = dord__cuxwx.scatter(efras__oijpf)
        else:
            ckpr__khnd = dord__cuxwx.scatter(None)
        m.labels_ = ckpr__khnd
    elif trusk__qvd == 0:
        dord__cuxwx.bcast(m.labels_)
    else:
        m.labels_ = dord__cuxwx.bcast(None)
    m._n_threads = fkxh__upoie
    return m
...
def overload_kmeans_clustering_score(m, X, y=None, sample_weight=None,
    ...
    def _cluster_kmeans_score(m, X, y=None, sample_weight=None,
        ...
        with numba.objmode(result='float64'):
            ...
            if _is_data_distributed:
                dord__cuxwx = MPI.COMM_WORLD
                result = dord__cuxwx.allreduce(result, op=MPI.SUM)
            m._n_threads = fkxh__upoie
        return result
...
def overload_multinomial_nb_model_fit(m, X, y, sample_weight=None,
    ...
    if is_overload_false(_is_data_distributed):
        ...
        lua__qbot = 'def _model_multinomial_nb_fit_impl(\n'
        lua__qbot += (
            '    m, X, y, sample_weight=None, _is_data_distributed=False\n')
        lua__qbot += '):  # pragma: no cover\n'
        lua__qbot += '    y = bodo.utils.conversion.coerce_to_ndarray(y)\n'
        if isinstance(X, DataFrameType):
            lua__qbot += '    X = X.to_numpy()\n'
        else:
            lua__qbot += '    X = bodo.utils.conversion.coerce_to_ndarray(X)\n'
        lua__qbot += '    my_rank = bodo.get_rank()\n'
        lua__qbot += '    nranks = bodo.get_size()\n'
        lua__qbot += '    total_cols = X.shape[1]\n'
        lua__qbot += '    for i in range(nranks):\n'
        lua__qbot += (
        ...
        lua__qbot += '        if i == my_rank:\n'
        lua__qbot += (
            '            X_train = bodo.gatherv(X[:, start:end:1], root=i)\n')
        lua__qbot += '        else:\n'
        lua__qbot += '            bodo.gatherv(X[:, start:end:1], root=i)\n'
        lua__qbot += '    y_train = bodo.allgatherv(y, False)\n'
        lua__qbot += '    with numba.objmode(m="multinomial_nb_type"):\n'
        lua__qbot += '        m = fit_multinomial_nb(\n'
        lua__qbot += """            m, X_train, y_train, sample_weight, total_cols, _is_data_distributed
"""
        lua__qbot += '        )\n'
        lua__qbot += '    bodo.barrier()\n'
        lua__qbot += '    return m\n'
        lebvb__mluwi = {}
        exec(lua__qbot, globals(), lebvb__mluwi)
        ufq__mnbqq = lebvb__mluwi['_model_multinomial_nb_fit_impl']
        return ufq__mnbqq
...
def fit_multinomial_nb(m, X_train, y_train, sample_weight=None, total_cols=
    ...
    cbnjm__rblba, n_features = X_train.shape
    m.n_features_in_ = n_features
    yym__ppa = LabelBinarizer()
    fsa__lpi = yym__ppa.fit_transform(y_train)
    m.classes_ = yym__ppa.classes_
    if fsa__lpi.shape[1] == 1:
        fsa__lpi = np.concatenate((1 - fsa__lpi, fsa__lpi), axis=1)
    if sample_weight is not None:
        fsa__lpi = fsa__lpi.astype(np.float64, copy=False)
        sample_weight = _check_sample_weight(sample_weight, X_train)
        sample_weight = np.atleast_2d(sample_weight)
        fsa__lpi *= sample_weight.T
    class_prior = m.class_prior
    jth__fvvu = fsa__lpi.shape[1]
    m._init_counters(jth__fvvu, n_features)
    m._count(X_train.astype('float64'), fsa__lpi)
    alpha = m._check_alpha()
    m._update_class_log_prior(class_prior=class_prior)
    mbvm__hnl = m.feature_count_ + alpha
    hqx__lfrud = mbvm__hnl.sum(axis=1)
    dord__cuxwx = MPI.COMM_WORLD
    kepep__wdi = dord__cuxwx.Get_size()
    jaklr__dkw = np.zeros(jth__fvvu)
    dord__cuxwx.Allreduce(hqx__lfrud, jaklr__dkw, op=MPI.SUM)
    ofk__etyur = np.log(mbvm__hnl) - np.log(jaklr__dkw.reshape(-1, 1))
    hiw__blx = ofk__etyur.T.reshape(n_features * jth__fvvu)
    dldm__ihl = np.ones(kepep__wdi) * (total_cols // kepep__wdi)
    rivuz__ianh = total_cols % kepep__wdi
    for ztum__qaec in range(rivuz__ianh):
        dldm__ihl[ztum__qaec] += 1
    dldm__ihl *= jth__fvvu
    gbxm__aps = np.zeros(kepep__wdi, dtype=np.int32)
    gbxm__aps[1:] = np.cumsum(dldm__ihl)[:-1]
    erpx__copr = np.zeros((total_cols, jth__fvvu), dtype=np.float64)
    dord__cuxwx.Allgatherv(hiw__blx, [erpx__copr, dldm__ihl, gbxm__aps, MPI
        .DOUBLE_PRECISION])
    m.feature_log_prob_ = erpx__copr.T
...
def sklearn_preprocessing_standard_scaler_fit_dist_helper(m, X):
    dord__cuxwx = MPI.COMM_WORLD
    vmfz__muvxr = dord__cuxwx.Get_size()
    dxiqz__ijst = m.with_std
    qkjyr__dnmia = m.with_mean
    m.with_std = False
    if dxiqz__ijst:
        m.with_mean = True
    m = m.fit(X)
    m.with_std = dxiqz__ijst
    m.with_mean = qkjyr__dnmia
    if not isinstance(m.n_samples_seen_, numbers.Integral):
        ...
        m.n_samples_seen_ = np.repeat(m.n_samples_seen_, X.shape[1]).astype(np
            .int64, copy=False)
    pfrlh__idgv = np.zeros((vmfz__muvxr, *m.n_samples_seen_.shape), dtype=m
        .n_samples_seen_.dtype)
    dord__cuxwx.Allgather(m.n_samples_seen_, pfrlh__idgv)
    falv__jxxr = np.sum(pfrlh__idgv, axis=0)
    m.n_samples_seen_ = falv__jxxr
    if m.with_mean or m.with_std:
        trz__imxrg = np.zeros((vmfz__muvxr, *m.mean_.shape), dtype=m.mean_.
            dtype)
        dord__cuxwx.Allgather(m.mean_, trz__imxrg)
        trz__imxrg[np.isnan(trz__imxrg)] = 0
        bsxt__akjf = np.average(trz__imxrg, axis=0, weights=pfrlh__idgv)
        m.mean_ = bsxt__akjf
    if m.with_std:
        iqt__kot = sklearn_safe_accumulator_op(np.nansum, (X - bsxt__akjf) **
            2, axis=0) / falv__jxxr
        jszwx__nvih = np.zeros_like(iqt__kot)
        dord__cuxwx.Allreduce(iqt__kot, jszwx__nvih, op=MPI.SUM)
        m.var_ = jszwx__nvih
        m.scale_ = sklearn_handle_zeros_in_scale(np.sqrt(m.var_))
    hpu__zmp = dord__cuxwx.allreduce(hpu__zmp, op=MPI.LAND)
    if hpu__zmp:
...
def get_data_slice_parallel(data, labels, len_train):
    drtoz__bmr = data[:len_train]
    qrrhd__adz = data[len_train:]
    drtoz__bmr = bodo.rebalance(drtoz__bmr)
    qrrhd__adz = bodo.rebalance(qrrhd__adz)
    jrghr__ffwrh = labels[:len_train]
    irxdj__snlhh = labels[len_train:]
    jrghr__ffwrh = bodo.rebalance(jrghr__ffwrh)
    irxdj__snlhh = bodo.rebalance(irxdj__snlhh)
    return drtoz__bmr, qrrhd__adz, jrghr__ffwrh, irxdj__snlhh
...
def overload_train_test_split(data, labels=None, train_size=None, test_size
    ...
    _is_data_distributed=False):
    check_sklearn_version()
    qvhje__pbo = {'stratify': stratify}
    nbpzz__psxm = {'stratify': None}
    check_unsupported_args('train_test_split', qvhje__pbo, nbpzz__psxm, 'ml')
    if is_overload_false(_is_data_distributed):
        rhqrt__ugro = f'data_split_type_{numba.core.ir_utils.next_label()}'
        tlm__qtams = f'labels_split_type_{numba.core.ir_utils.next_label()}'
        for mkgip__mqiy, cnxm__duix in ((data, rhqrt__ugro), (labels,
            tlm__qtams)):
            if isinstance(mkgip__mqiy, (DataFrameType, SeriesType)):
                wzx__nmoh = mkgip__mqiy.copy(index=NumericIndexType(types.
                    int64))
                setattr(types, cnxm__duix, wzx__nmoh)
            else:
                setattr(types, cnxm__duix, mkgip__mqiy)
        lua__qbot = 'def _train_test_split_impl(\n'
        lua__qbot += '    data,\n'
        lua__qbot += '    labels=None,\n'
        lua__qbot += '    train_size=None,\n'
        lua__qbot += '    test_size=None,\n'
        lua__qbot += '    random_state=None,\n'
        lua__qbot += '    shuffle=True,\n'
        lua__qbot += '    stratify=None,\n'
        lua__qbot += '    _is_data_distributed=False,\n'
        lua__qbot += '):  # pragma: no cover\n'
        lua__qbot += (
            """    with numba.objmode(data_train='{}', data_test='{}', labels_train='{}', labels_test='{}'):
"""
            .format(rhqrt__ugro, rhqrt__ugro, tlm__qtams, tlm__qtams))
        lua__qbot += """        data_train, data_test, labels_train, labels_test = sklearn.model_selection.train_test_split(
"""
        lua__qbot += '            data,\n'
        lua__qbot += '            labels,\n'
        lua__qbot += '            train_size=train_size,\n'
        lua__qbot += '            test_size=test_size,\n'
        lua__qbot += '            random_state=random_state,\n'
        lua__qbot += '            shuffle=shuffle,\n'
        lua__qbot += '            stratify=stratify,\n'
        lua__qbot += '        )\n'
        lua__qbot += (
            '    return data_train, data_test, labels_train, labels_test\n')
        lebvb__mluwi = {}
        exec(lua__qbot, globals(), lebvb__mluwi)
        _train_test_split_impl = lebvb__mluwi['_train_test_split_impl']
        ...
        def _train_test_split_impl(data, labels=None, train_size=None,
            ...
            mauc__xard = bodo.libs.distributed_api.dist_reduce(len(data),
                np.int32(Reduce_Type.Sum.value))
            len_train = int(mauc__xard * train_size)
            fthmr__nml = mauc__xard - len_train
            if shuffle:
                labels = set_labels_type(labels, label_type)
                trusk__qvd = bodo.get_rank()
                kepep__wdi = bodo.get_size()
                xlb__mrhfi = np.empty(kepep__wdi, np.int64)
                bodo.libs.distributed_api.allgather(xlb__mrhfi, len(data))
                gdcuz__gwp = np.cumsum(xlb__mrhfi[0:trusk__qvd + 1])
                kkj__ypoib = np.full(mauc__xard, True)
                kkj__ypoib[:fthmr__nml] = False
                np.random.seed(42)
                np.random.permutation(kkj__ypoib)
                if trusk__qvd:
                    zts__gehae = gdcuz__gwp[trusk__qvd - 1]
                else:
                    zts__gehae = 0
                vod__ebne = gdcuz__gwp[trusk__qvd]
                cyyiw__kts = kkj__ypoib[zts__gehae:vod__ebne]
                drtoz__bmr = data[cyyiw__kts]
                qrrhd__adz = data[~cyyiw__kts]
                jrghr__ffwrh = labels[cyyiw__kts]
                irxdj__snlhh = labels[~cyyiw__kts]
                drtoz__bmr = bodo.random_shuffle(drtoz__bmr, seed=
                    random_state, parallel=True)
                qrrhd__adz = bodo.random_shuffle(qrrhd__adz, seed=
                    random_state, parallel=True)
                jrghr__ffwrh = bodo.random_shuffle(jrghr__ffwrh, seed=
                    random_state, parallel=True)
                irxdj__snlhh = bodo.random_shuffle(irxdj__snlhh, seed=
                    random_state, parallel=True)
                jrghr__ffwrh = reset_labels_type(jrghr__ffwrh, label_type)
                irxdj__snlhh = reset_labels_type(irxdj__snlhh, label_type)
            else:
                drtoz__bmr, qrrhd__adz, jrghr__ffwrh, irxdj__snlhh = (
...
def sklearn_preprocessing_minmax_scaler_fit_dist_helper(m, X):
    ...
    vmfz__muvxr = dord__cuxwx.Get_size()
    m = m.fit(X)
    falv__jxxr = dord__cuxwx.allreduce(m.n_samples_seen_, op=MPI.SUM)
    m.n_samples_seen_ = falv__jxxr
    wgj__ivham = np.zeros((vmfz__muvxr, *m.data_min_.shape), dtype=m.
        data_min_.dtype)
    dord__cuxwx.Allgather(m.data_min_, wgj__ivham)
    cerlv__bar = np.nanmin(wgj__ivham, axis=0)
    pcl__razkr = np.zeros((vmfz__muvxr, *m.data_max_.shape), dtype=m.
        data_max_.dtype)
    dord__cuxwx.Allgather(m.data_max_, pcl__razkr)
    resrw__btv = np.nanmax(pcl__razkr, axis=0)
    hnjmj__omvp = resrw__btv - cerlv__bar
    m.scale_ = (m.feature_range[1] - m.feature_range[0]
        ) / sklearn_handle_zeros_in_scale(hnjmj__omvp)
    m.min_ = m.feature_range[0] - cerlv__bar * m.scale_
    m.data_min_ = cerlv__bar
    m.data_max_ = resrw__btv
    m.data_range_ = hnjmj__omvp
    return m
...
def overload_rf_classifier_model_fit(m, X, y, sample_weight=None,
    _is_data_distributed=False):
    kla__vstvw = 'RandomForestClassifier'
    if isinstance(m, BodoRandomForestRegressorType):
        kla__vstvw = 'RandomForestRegressor'
    if not is_overload_none(sample_weight):
        raise BodoError(
            f"sklearn.ensemble.{kla__vstvw}.fit() : 'sample_weight' is not supported for distributed data."
            )
    ...
    def _model_fit_impl(m, X, y, sample_weight=None, _is_data_distributed=False
        ...
        with numba.objmode(first_rank_node='int32[:]'):
            first_rank_node = get_nodes_first_ranks()
        if _is_data_distributed:
            iceil__zrpvh = len(first_rank_node)
            X = bodo.gatherv(X)
            y = bodo.gatherv(y)
            if iceil__zrpvh > 1:
                X = bodo.libs.distributed_api.bcast_comm(X, comm_ranks=
                    first_rank_node, nranks=iceil__zrpvh)
...
def _cv_fit_transform_helper(m, X):
    ...
    if m.vocabulary is None:
        ...
        pijh__zjct = True
    return pijh__zjct, local_vocabulary
    ...
    if is_overload_true(_is_data_distributed):
        ...
        def _count_vectorizer_fit_transform_impl(m, X, y=None,
            ...
            if changeVoc:
                ...
                teat__lpn = bodo.libs.array_kernels.unique(local_vocabulary,
                    parallel=True)
                teat__lpn = bodo.allgatherv(teat__lpn, False)
                teat__lpn = bodo.libs.array_kernels.sort(teat__lpn,
                    ascending=True, inplace=True)
                doh__kvja = {}
                for erw__nwx in range(len(teat__lpn)):
                    doh__kvja[teat__lpn[erw__nwx]] = erw__nwx
            else:
                doh__kvja = local_vocabulary
            with numba.objmode(transformed_X='csr_matrix_int64_int64'):
                if changeVoc:
                    m.vocabulary = doh__kvja
                transformed_X = m.fit_transform(X, y)
                transformed_X.indices = transformed_X.indices.astype(np.int64)
                transformed_X.indptr = transformed_X.indptr.astype(np.int64)
            return transformed_X
