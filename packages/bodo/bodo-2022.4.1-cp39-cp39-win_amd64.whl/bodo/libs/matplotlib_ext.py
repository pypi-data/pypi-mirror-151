...
def install_mpl_class(types_name, python_type):
    kzzgl__motm = ''.join(map(str.title, types_name.split('_')))
    ngj__kzu = f'class {kzzgl__motm}(types.Opaque):\n'
    ngj__kzu += '    def __init__(self):\n'
    ngj__kzu += f"       types.Opaque.__init__(self, name='{kzzgl__motm}')\n"
    ngj__kzu += '    def __reduce__(self):\n'
    ngj__kzu += (
        f"        return (types.Opaque, ('{kzzgl__motm}',), self.__dict__)\n")
    bzmh__dfdd = {}
    exec(ngj__kzu, {'types': types, 'bodo': bodo}, bzmh__dfdd)
    ceq__zol = bzmh__dfdd[kzzgl__motm]
    hdpt__eeogl = sys.modules[__name__]
    setattr(hdpt__eeogl, kzzgl__motm, ceq__zol)
    class_instance = ceq__zol()
    setattr(types, types_name, class_instance)
    register_model(ceq__zol)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(ceq__zol)(unbox_mpl_obj)
    box(ceq__zol)(box_mpl_obj)
...
def _install_mpl_types():
    tnhs__cetlz = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for dnd__lfe, ofrn__knu in tnhs__cetlz:
        install_mpl_class(dnd__lfe, ofrn__knu)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    aoivr__dmvww = ', '.join(f'e{qzwkt__fgbqq}' for qzwkt__fgbqq in range(
        len(args)))
    if aoivr__dmvww:
        aoivr__dmvww += ', '
    qnanu__qygia = ', '.join(f"{emea__fkkh} = ''" for emea__fkkh in kws.keys())
    rai__qqf = 'matplotlib_obj, ' if obj_typ is not None else ''
    arq__rkwf = f'def mpl_stub({rai__qqf} {aoivr__dmvww} {qnanu__qygia}):\n'
    arq__rkwf += '    pass\n'
    ezh__xur = {}
    exec(arq__rkwf, {}, ezh__xur)
    yken__utag = ezh__xur['mpl_stub']
    jnvpr__wrml = numba.core.utils.pysignature(yken__utag)
    kyzew__bjuj = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(
        kws.values())
    return signature(return_typ, *unliteral_all(kyzew__bjuj)).replace(pysig
        =jnvpr__wrml)


def generate_axes_typing(mod_name, nrows, ncols):
    ghsay__cqra = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(ghsay__cqra.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(ghsay__cqra.format(mod_name, 'ncols'))
    bprx__dtzv = get_overload_const_int(nrows)
    czns__jdc = get_overload_const_int(ncols)
    if bprx__dtzv < 1:
        raise BodoError(ghsay__cqra.format(mod_name, 'nrows'))
    if czns__jdc < 1:
        raise BodoError(ghsay__cqra.format(mod_name, 'ncols'))
    if bprx__dtzv == 1 and czns__jdc == 1:
        qjllt__vuwx = types.mpl_axes_type
    else:
        if czns__jdc == 1:
            ckewm__und = types.mpl_axes_type
        else:
            ckewm__und = types.Tuple([types.mpl_axes_type] * czns__jdc)
        qjllt__vuwx = types.Tuple([ckewm__und] * bprx__dtzv)
    return qjllt__vuwx


def generate_pie_return_type(args, kws):
    lamg__ebprz = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if lamg__ebprz == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    xqr__plr = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(xqr__plr):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(xqr__plr):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ifbz__oxhel = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, ifbz__oxhel]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    ifbz__oxhel = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols
        )
    dnd__lfe = str(ifbz__oxhel)
    if not hasattr(types, dnd__lfe):
        dnd__lfe = f'objmode_type{ir_utils.next_label()}'
        setattr(types, dnd__lfe, ifbz__oxhel)
    arq__rkwf = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{dnd__lfe}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    ezh__xur = {}
    exec(arq__rkwf, {'numba': numba, 'np': np}, ezh__xur)
    impl = ezh__xur['impl']
    return impl
