...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    iszpe__xwyqy = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    iwt__qpi = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    vlzku__hfxla = cgutils.get_or_insert_function(builder.module, iwt__qpi,
        name='decimal_to_str')
    builder.call(vlzku__hfxla, [val, iszpe__xwyqy._get_ptr_by_name(
        'meminfo'), iszpe__xwyqy._get_ptr_by_name('length'), scale])
    iszpe__xwyqy.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    iszpe__xwyqy.is_ascii = context.get_constant(types.int32, 1)
    iszpe__xwyqy.hash = context.get_constant(numba.cpython.unicode.
        _Py_hash_t, -1)
    iszpe__xwyqy.data = context.nrt.meminfo_data(builder, iszpe__xwyqy.meminfo)
    iszpe__xwyqy.parent = cgutils.get_null_value(iszpe__xwyqy.parent.type)
    return iszpe__xwyqy._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, zslb__pmw, zslb__pmw = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    iwt__qpi = lir.FunctionType(lir.IntType(128), [lir.IntType(8).as_pointer()]
        )
    vlzku__hfxla = cgutils.get_or_insert_function(builder.module, iwt__qpi,
        name='str_to_decimal')
    matap__ehhub = builder.call(vlzku__hfxla, [val])
    return matap__ehhub
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        srzb__oryn = cgutils.alloca_once(builder, lir.ArrayType(lir.IntType
            (64), 2))
        builder.store(args[0], builder.bitcast(srzb__oryn, lir.IntType(128)
            .as_pointer()))
        return builder.load(srzb__oryn)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    xii__hhc = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, zslb__pmw = args
        iwt__qpi = lir.FunctionType(lir.IntType(1), [lir.IntType(128), lir.
            IntType(64), lir.IntType(128), lir.IntType(64)])
        vlzku__hfxla = cgutils.get_or_insert_function(builder.module,
            iwt__qpi, name=xii__hhc)
        return builder.call(vlzku__hfxla, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            xii__hhc = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, xii__hhc)
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    pghf__eok = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    xnn__zwn = [context.get_constant_generic(builder, types.int64, v) for v in
        pghf__eok]
    qhwiy__yvgzk = cgutils.pack_array(builder, xnn__zwn)
    srzb__oryn = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(qhwiy__yvgzk, builder.bitcast(srzb__oryn, lir.ArrayType(
...
def unbox_decimal(typ, val, c):
    iwt__qpi = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer(
        ), lir.IntType(128).as_pointer()])
    vlzku__hfxla = cgutils.get_or_insert_function(c.builder.module,
        iwt__qpi, name='unbox_decimal')
    srzb__oryn = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(vlzku__hfxla, [val, srzb__oryn])
    xkdfl__bmx = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    cwaj__zqz = c.builder.load(srzb__oryn)
    return NativeValue(cwaj__zqz, is_error=xkdfl__bmx)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    gbq__liz = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    ttjuk__oemmt = c.pyapi.from_native_value(bodo.string_type, gbq__liz, c.
        env_manager)
    zidjm__roiye = c.context.insert_const_string(c.builder.module, 'decimal')
    hxux__rtu = c.pyapi.import_module_noblock(zidjm__roiye)
    srzb__oryn = c.pyapi.call_method(hxux__rtu, 'Decimal', (ttjuk__oemmt,))
    c.pyapi.decref(ttjuk__oemmt)
    c.pyapi.decref(hxux__rtu)
    return srzb__oryn
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ylaqm__jvgka = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, ylaqm__jvgka)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        pirt__xpr, fiwht__voak, zslb__pmw, zslb__pmw = args
        kmka__xryn = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        kmka__xryn.data = pirt__xpr
        kmka__xryn.null_bitmap = fiwht__voak
        context.nrt.incref(builder, signature.args[0], pirt__xpr)
        context.nrt.incref(builder, signature.args[1], fiwht__voak)
        return kmka__xryn._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    wzd__kwuy = DecimalArrayType(precision, scale)
    qfwix__brk = wzd__kwuy(data, null_bitmap, precision_tp, scale_tp)
    return qfwix__brk, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    ujnl__qaw = context.get_constant(types.int64, n)
    utr__mmqg = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [ujnl__qaw])
    kbtpp__nri = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for wxb__ethy, gaet__lum in enumerate(pyval):
        unh__rcy = pd.isna(gaet__lum)
        bodo.libs.int_arr_ext.set_bit_to_arr(kbtpp__nri, wxb__ethy, int(not
            unh__rcy))
        if not unh__rcy:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [utr__mmqg._getvalue(), context.
                get_constant(types.int64, wxb__ethy), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), gaet__lum)])
    adp__tsje = context.get_constant_generic(builder, nulls_type, kbtpp__nri)
    kmka__xryn = context.make_helper(builder, typ)
    kmka__xryn.data = utr__mmqg._getvalue()
    kmka__xryn.null_bitmap = adp__tsje
    return kmka__xryn._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    asuh__mdxoz = np.empty(n, dtype=int128_type)
    irko__ojga = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(asuh__mdxoz, irko__ojga, precision, scale)
...
def box_decimal_arr(typ, val, c):
    xluf__tgixw = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    asuh__mdxoz = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, xluf__tgixw.data)
    siqw__blbpo = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, xluf__tgixw.null_bitmap).data
    n = c.builder.extract_value(asuh__mdxoz.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    iwt__qpi = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    zxbt__gvsmm = cgutils.get_or_insert_function(c.builder.module, iwt__qpi,
        name='box_decimal_array')
    jiz__cxaq = c.builder.call(zxbt__gvsmm, [n, asuh__mdxoz.data,
        siqw__blbpo, scale])
...
def unbox_decimal_arr(typ, val, c):
    kmka__xryn = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    huax__qhq = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(huax__qhq)
    c.pyapi.decref(huax__qhq)
    wxjyf__eoiu = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    utr__mmqg = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(int128_type, 1, 'C'), [n])
    bnqo__bjd = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(types.uint8, 1, 'C'), [wxjyf__eoiu])
    iwt__qpi = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer(
        ), lir.IntType(64), lir.IntType(128).as_pointer(), lir.IntType(8).
        as_pointer()])
    vlzku__hfxla = cgutils.get_or_insert_function(c.builder.module,
        iwt__qpi, name='unbox_decimal_array')
    c.builder.call(vlzku__hfxla, [val, n, utr__mmqg.data, bnqo__bjd.data])
    kmka__xryn.null_bitmap = bnqo__bjd._getvalue()
    kmka__xryn.data = utr__mmqg._getvalue()
    xkdfl__bmx = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(kmka__xryn._getvalue(), is_error=xkdfl__bmx)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            pxk__soksm, ryt__vkrk = array_getitem_bool_index(A, ind)
            return init_decimal_array(pxk__soksm, ryt__vkrk, precision, scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            pxk__soksm, ryt__vkrk = array_getitem_int_index(A, ind)
            return init_decimal_array(pxk__soksm, ryt__vkrk, precision, scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            pxk__soksm, ryt__vkrk = array_getitem_slice_index(A, ind)
            return init_decimal_array(pxk__soksm, ryt__vkrk, precision, scale)
        return impl_slice
    raise BodoError(
