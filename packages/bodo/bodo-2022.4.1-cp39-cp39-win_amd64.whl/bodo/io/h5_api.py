...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        swqq__edj = []
        bmf__vcyki = h5g_get_num_objs(obj_id)
        for kmmbd__qpm in range(bmf__vcyki):
            mgzh__eye = h5g_get_objname_by_idx(obj_id, kmmbd__qpm)
            swqq__edj.append(mgzh__eye)
        return swqq__edj
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    ahlk__oaph = parse_dtype(dtype)
    tbmov__codb = np.int32(numba_to_c_type(ahlk__oaph))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, tbmov__codb)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            ofyjv__xkco = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    kbmq__vpdh = tuple_to_ptr(starts)
    eilf__ybl = tuple_to_ptr(counts)
    ntmeo__gdkf = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, kbmq__vpdh, eilf__ybl,
        is_parallel, out_arr.ctypes, ntmeo__gdkf)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    eilf__ybl = tuple_to_ptr(counts)
    ntmeo__gdkf = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, kbmq__vpdh, eilf__ybl,
        is_parallel, out_arr.ctypes, ntmeo__gdkf)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        sazks__xikqn = types.Array(dtype, ndim, 'C')
        mig__hwuh = args[3]
        sazks__xikqn = self.context.resolve_function_type(operator.getitem,
            [sazks__xikqn, mig__hwuh], {}).return_type.copy(layout='C')
        return signature(sazks__xikqn, *args)
...
def get_filter_read_indices(bool_arr):
    obx__lmevh = bool_arr.nonzero()[0]
    zvdb__pqoif = bodo.libs.distributed_api.get_rank()
    fwyw__ltcu = bodo.libs.distributed_api.get_size()
    axwk__dup = np.empty(fwyw__ltcu, np.int64)
    sjsnk__zwgor = len(bool_arr)
    bodo.libs.distributed_api.allgather(axwk__dup, sjsnk__zwgor)
    wzi__cryy = axwk__dup.cumsum()[zvdb__pqoif] - sjsnk__zwgor
    obx__lmevh += wzi__cryy
    houg__ziby = bodo.libs.distributed_api.dist_reduce(len(obx__lmevh), np.
        int32(sum_op))
    oco__pjc = bodo.libs.distributed_api.gatherv(obx__lmevh)
    if zvdb__pqoif == 0:
        sdgi__nkac = oco__pjc
    else:
        sdgi__nkac = np.empty(houg__ziby, obx__lmevh.dtype)
    bodo.libs.distributed_api.bcast(sdgi__nkac)
    ghzxn__tnvhb = bodo.libs.distributed_api.get_start(houg__ziby,
        fwyw__ltcu, zvdb__pqoif)
    omyv__wtvb = bodo.libs.distributed_api.get_end(houg__ziby, fwyw__ltcu,
        zvdb__pqoif)
    return sdgi__nkac[ghzxn__tnvhb:omyv__wtvb]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        gwj__vzd = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], gwj__vzd)
        return builder.bitcast(gwj__vzd, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    eilf__ybl = tuple_to_ptr(counts)
    ntmeo__gdkf = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, kbmq__vpdh,
        eilf__ybl, is_parallel, out_arr.ctypes, ntmeo__gdkf, read_indices.
