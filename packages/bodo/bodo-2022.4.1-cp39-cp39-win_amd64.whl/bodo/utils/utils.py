...
def get_constant(func_ir, var, default=NOT_CONSTANT):
    hrsho__eapd = guard(get_definition, func_ir, var)
    if hrsho__eapd is None:
        return default
    if isinstance(hrsho__eapd, ir.Const):
        return hrsho__eapd.value
    if isinstance(hrsho__eapd, ir.Var):
        return get_constant(func_ir, hrsho__eapd, default)
    return default
...
def find_build_tuple(func_ir, var):
    ...
    jogk__ksfi = get_definition(func_ir, var)
    require(isinstance(jogk__ksfi, ir.Expr))
    require(jogk__ksfi.op == 'build_tuple')
    return jogk__ksfi.items
...
def cprint_lower(context, builder, sig, args):
    for jnlrx__ndz, val in enumerate(args):
        typ = sig.args[jnlrx__ndz]
        if isinstance(typ, types.ArrayCTypes):
            cgutils.printf(builder, '%p ', val)
            continue
        feb__zzxvx = typ_to_format[typ]
        cgutils.printf(builder, '%{} '.format(feb__zzxvx), val)
...
def is_whole_slice(typemap, func_ir, var, accept_stride=False):
    ...
    tomy__alh = get_definition(func_ir, var)
    require(isinstance(tomy__alh, ir.Expr) and tomy__alh.op == 'call')
    assert len(tomy__alh.args) == 2 or accept_stride and len(tomy__alh.args
        ) == 3
    assert find_callname(func_ir, tomy__alh) == ('slice', 'builtins')
    eeel__lxgj = get_definition(func_ir, tomy__alh.args[0])
    png__scl = get_definition(func_ir, tomy__alh.args[1])
    require(isinstance(eeel__lxgj, ir.Const) and eeel__lxgj.value == None)
    require(isinstance(png__scl, ir.Const) and png__scl.value == None)
    return True


def is_slice_equiv_arr(arr_var, index_var, func_ir, equiv_set,
    accept_stride=False):
    umqr__yrkgg = get_definition(func_ir, index_var)
    require(find_callname(func_ir, umqr__yrkgg) == ('slice', 'builtins'))
    require(len(umqr__yrkgg.args) in (2, 3))
    require(find_const(func_ir, umqr__yrkgg.args[0]) in (0, None))
    require(equiv_set.is_equiv(umqr__yrkgg.args[1], arr_var.name + '#0'))
    require(accept_stride or len(umqr__yrkgg.args) == 2 or find_const(
        func_ir, umqr__yrkgg.args[2]) == 1)
    return True
...
def get_slice_step(typemap, func_ir, var):
    ...
    tomy__alh = get_definition(func_ir, var)
    require(isinstance(tomy__alh, ir.Expr) and tomy__alh.op == 'call')
    assert len(tomy__alh.args) == 3
    return tomy__alh.args[2]
...
def build_set_seen_na(A):
    ...
    def impl(A):
        s = dict()
        kzi__eiyo = False
        for jnlrx__ndz in range(len(A)):
            if bodo.libs.array_kernels.isna(A, jnlrx__ndz):
                kzi__eiyo = True
                continue
            s[A[jnlrx__ndz]] = 0
        return s, kzi__eiyo
...
def empty_like_type_overload(n, arr):
    ...
    if isinstance(arr, IntegerArrayType):
        ...
        def empty_like_type_int_arr(n, arr):
            return bodo.libs.int_arr_ext.alloc_int_array(n, nthh__ahn)
    ...
    def empty_like_type_str_arr(n, arr):
        afukv__uhz = 20
        if len(arr) != 0:
            afukv__uhz = num_total_chars(arr) // len(arr)
        return pre_alloc_string_array(n, n * afukv__uhz)
    return empty_like_type_str_arr


def _empty_nd_impl(context, builder, arrtype, shapes):
    tbqj__imj = make_array(arrtype)
    qbgyd__vtejc = tbqj__imj(context, builder)
    elog__wszqy = context.get_data_type(arrtype.dtype)
    bmegv__kqxv = context.get_constant(types.intp, get_itemsize(context,
        arrtype))
    jzn__tazd = context.get_constant(types.intp, 1)
    aag__bnr = lir.Constant(lir.IntType(1), 0)
    for s in shapes:
        sqgww__jvon = builder.smul_with_overflow(jzn__tazd, s)
        jzn__tazd = builder.extract_value(sqgww__jvon, 0)
        aag__bnr = builder.or_(aag__bnr, builder.extract_value(sqgww__jvon, 1))
    if arrtype.ndim == 0:
        ztver__uhxay = ()
    elif arrtype.layout == 'C':
        ztver__uhxay = [bmegv__kqxv]
        for kzgf__mca in reversed(shapes[1:]):
            ztver__uhxay.append(builder.mul(ztver__uhxay[-1], kzgf__mca))
        ztver__uhxay = tuple(reversed(ztver__uhxay))
    elif arrtype.layout == 'F':
        ztver__uhxay = [bmegv__kqxv]
        for kzgf__mca in shapes[:-1]:
            ztver__uhxay.append(builder.mul(ztver__uhxay[-1], kzgf__mca))
        ztver__uhxay = tuple(ztver__uhxay)
    else:
        raise NotImplementedError(
            "Don't know how to allocate array with layout '{0}'.".format(
            arrtype.layout))
    gow__zkmdh = builder.smul_with_overflow(jzn__tazd, bmegv__kqxv)
    nfa__ubikq = builder.extract_value(gow__zkmdh, 0)
    aag__bnr = builder.or_(aag__bnr, builder.extract_value(gow__zkmdh, 1))
    with builder.if_then(aag__bnr, likely=False):
        cgutils.printf(builder,
            'array is too big; `arr.size * arr.dtype.itemsize` is larger than the maximum possible size.'
            )
    dtype = arrtype.dtype
    jstu__qzra = context.get_preferred_array_alignment(dtype)
    gtp__yrt = context.get_constant(types.uint32, jstu__qzra)
    iwm__qubtb = context.nrt.meminfo_alloc_aligned(builder, size=nfa__ubikq,
        align=gtp__yrt)
    data = context.nrt.meminfo_data(builder, iwm__qubtb)
    fxws__xowgc = context.get_value_type(types.intp)
    derw__jcwww = cgutils.pack_array(builder, shapes, ty=fxws__xowgc)
    zmoc__hzi = cgutils.pack_array(builder, ztver__uhxay, ty=fxws__xowgc)
    populate_array(qbgyd__vtejc, data=builder.bitcast(data, elog__wszqy.
        as_pointer()), shape=derw__jcwww, strides=zmoc__hzi, itemsize=
        bmegv__kqxv, meminfo=iwm__qubtb)
    return qbgyd__vtejc
...
def alloc_arr_tup(n, arr_tup, init_vals=()):
    qqc__gwd = []
    for frg__gezz in arr_tup:
        qqc__gwd.append(np.empty(n, frg__gezz.dtype))
    return tuple(qqc__gwd)


@overload(alloc_arr_tup, no_unliteral=True)
def alloc_arr_tup_overload(n, data, init_vals=()):
    gela__ghz = data.count
    uzk__vywk = ','.join(['empty_like_type(n, data[{}])'.format(jnlrx__ndz) for
        jnlrx__ndz in range(gela__ghz)])
    if init_vals != ():
        uzk__vywk = ','.join(['np.full(n, init_vals[{}], data[{}].dtype)'.
            format(jnlrx__ndz, jnlrx__ndz) for jnlrx__ndz in range(gela__ghz)])
    wus__wuwkx = 'def f(n, data, init_vals=()):\n'
    wus__wuwkx += '  return ({}{})\n'.format(uzk__vywk, ',' if gela__ghz ==
        1 else '')
    jop__qseoa = {}
    exec(wus__wuwkx, {'empty_like_type': empty_like_type, 'np': np}, jop__qseoa
        )
    nxe__hat = jop__qseoa['f']
    return nxe__hat
...
def overload_full_type(n, val, t):
    ...
    if typ == string_array_type:
        ...
        def impl_str(n, val, t):
            mbjk__oud = n * len(val)
            A = pre_alloc_string_array(n, mbjk__oud)
            for jnlrx__ndz in range(n):
                A[jnlrx__ndz] = val
            return A
        return impl_str
...
def is_null_pointer(typingctx, ptr_typ=None):
    ...
    def codegen(context, builder, signature, args):
        sqxd__isiff, = args
        mekx__lzuia = context.get_constant_null(ptr_typ)
        return builder.icmp_unsigned('==', sqxd__isiff, mekx__lzuia)
...
def is_null_value(typingctx, val_typ=None):
    ...
    def codegen(context, builder, signature, args):
        val, = args
        zzqmp__suyy = cgutils.alloca_once_value(builder, val)
        jna__qgerg = cgutils.alloca_once_value(builder, context.
            get_constant_null(val_typ))
        return is_ll_eq(builder, zzqmp__suyy, jna__qgerg)
...
def tuple_list_to_array(A, data, elem_type):
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(elem_type,
        'tuple_list_to_array()')
    wus__wuwkx = 'def impl(A, data, elem_type):\n'
    wus__wuwkx += '  for i, d in enumerate(data):\n'
    if elem_type == bodo.hiframes.pd_timestamp_ext.pd_timestamp_type:
        ...
        wus__wuwkx += '    A[i] = d\n'
    jop__qseoa = {}
    exec(wus__wuwkx, {'bodo': bodo}, jop__qseoa)
    impl = jop__qseoa['impl']
    return impl


def object_length(c, obj):
    abgmo__bfef = c.context.get_argument_type(types.pyobject)
    ymhrd__rcxh = lir.FunctionType(lir.IntType(64), [abgmo__bfef])
    zgbj__ven = cgutils.get_or_insert_function(c.builder.module,
        ymhrd__rcxh, name='PyObject_Length')
    return c.builder.call(zgbj__ven, (obj,))
...
def incref(typingctx, data=None):
    ...
    def codegen(context, builder, signature, args):
        xdadv__umqn, = args
        context.nrt.incref(builder, signature.args[0], xdadv__umqn)
    return types.void(data), codegen


def gen_getitem(out_var, in_var, ind, calltypes, nodes):
    czejt__ifcuc = out_var.loc
    idh__duund = ir.Expr.static_getitem(in_var, ind, None, czejt__ifcuc)
    calltypes[idh__duund] = None
    nodes.append(ir.Assign(idh__duund, out_var, czejt__ifcuc))
...
def get_getsetitem_index_var(node, typemap, nodes):
    ...
    if index_var is None:
        ...
        try:
            agdf__iqomt = types.literal(node.index)
        except:
            agdf__iqomt = numba.typeof(node.index)
        index_var = ir.Var(node.value.scope, ir_utils.mk_unique_var(
            'dummy_index'), node.loc)
        typemap[index_var.name] = agdf__iqomt
...
def sanitize_varname(varname):
    ...
    varname = str(varname)
    wiub__mzppn = re.sub('\\W+', '_', varname)
    if not wiub__mzppn or not wiub__mzppn[0].isalpha():
        wiub__mzppn = '_' + wiub__mzppn
    if not wiub__mzppn.isidentifier() or keyword.iskeyword(wiub__mzppn):
        wiub__mzppn = mk_unique_var('new_name').replace('.', '_')
    return wiub__mzppn
...
def list_reverse(A):
    if isinstance(A, types.List):
        ...
        def impl_reversed(A):
            ptwl__kxqo = len(A)
            for jnlrx__ndz in range(ptwl__kxqo):
                yield A[ptwl__kxqo - 1 - jnlrx__ndz]
...
@numba.njit
def nanvar_ddof1(a):
    rbghx__zod = count_nonnan(a)
    if rbghx__zod <= 1:
        return np.nan
    return np.nanvar(a) * (rbghx__zod / (rbghx__zod - 1))
...
def has_supported_h5py():
    ...
    except ImportError as kdkv__kseu:
        pltpz__wiupw = False
    else:
        pltpz__wiupw = h5py.version.hdf5_version_tuple[1] in (10, 12)
    return pltpz__wiupw
...
def check_and_propagate_cpp_exception(typingctx):
    ...
    def codegen(context, builder, sig, args):
        pxdmk__iqbzd = context.get_python_api(builder)
        htew__bwmf = pxdmk__iqbzd.err_occurred()
        tnyno__btxqy = cgutils.is_not_null(builder, htew__bwmf)
        with builder.if_then(tnyno__btxqy):
...
def lower_constant_list(context, builder, typ, pyval):
    ...
    for a in pyval:
        ...
        bzdd__vezs.append(context.get_constant_generic(builder, typ.dtype, a))
    xxdt__haukn = context.get_constant_generic(builder, types.int64, len(pyval)
        )
    rexx__wkpn = context.get_constant_generic(builder, types.bool_, False)
    unmvz__llq = context.get_constant_null(types.pyobject)
    ojdqe__yvtg = lir.Constant.literal_struct([xxdt__haukn, xxdt__haukn,
        rexx__wkpn] + bzdd__vezs)
    ojdqe__yvtg = cgutils.global_constant(builder, '.const.payload',
        ojdqe__yvtg).bitcast(cgutils.voidptr_t)
    hexlm__cxp = context.get_constant(types.int64, -1)
    yrjb__iia = context.get_constant_null(types.voidptr)
    iwm__qubtb = lir.Constant.literal_struct([hexlm__cxp, yrjb__iia,
        yrjb__iia, ojdqe__yvtg, hexlm__cxp])
    iwm__qubtb = cgutils.global_constant(builder, '.const.meminfo', iwm__qubtb
        ).bitcast(cgutils.voidptr_t)
    return lir.Constant.literal_struct([iwm__qubtb, unmvz__llq])
...
def lower_constant_set(context, builder, typ, pyval):
    ...
    eyrgn__nmyze = types.List(typ.dtype)
    iqqz__cjhjo = context.get_constant_generic(builder, eyrgn__nmyze, list(
        pyval))
    lpmqy__awgo = context.compile_internal(builder, lambda l: set(l), types
        .Set(typ.dtype)(eyrgn__nmyze), [iqqz__cjhjo])
    return lpmqy__awgo


def lower_const_dict_fast_path(context, builder, typ, pyval):
    from bodo.utils.typing import can_replace
    grh__diwev = pd.Series(pyval.keys()).values
    xenah__qshx = pd.Series(pyval.values()).values
    xby__plszm = bodo.typeof(grh__diwev)
    twtd__octdi = bodo.typeof(xenah__qshx)
    require(xby__plszm.dtype == typ.key_type or can_replace(typ.key_type,
        xby__plszm.dtype))
    require(twtd__octdi.dtype == typ.value_type or can_replace(typ.
        value_type, twtd__octdi.dtype))
    vveno__nawa = context.get_constant_generic(builder, xby__plszm, grh__diwev)
    ajzeo__qgdzs = context.get_constant_generic(builder, twtd__octdi,
        xenah__qshx)
    ...
    def create_dict(keys, vals):
        ...
        for k, v in zip(keys, vals):
            fqu__pkmo[k] = v
        return fqu__pkmo
    yro__baccz = context.compile_internal(builder, create_dict, typ(
        xby__plszm, twtd__octdi), [vveno__nawa, ajzeo__qgdzs])
    return yro__baccz
...
def lower_constant_dict(context, builder, typ, pyval):
    ...
    ajueu__vfda = typ.key_type
    kgcds__vhwgx = typ.value_type

    def make_dict():
        return numba.typed.Dict.empty(ajueu__vfda, kgcds__vhwgx)
    yro__baccz = context.compile_internal(builder, make_dict, typ(), [])

    def set_dict_val(d, k, v):
        d[k] = v
    for k, v in pyval.items():
        esv__oepb = context.get_constant_generic(builder, ajueu__vfda, k)
        ncqu__vhcyh = context.get_constant_generic(builder, kgcds__vhwgx, v)
        context.compile_internal(builder, set_dict_val, types.none(typ,
            ajueu__vfda, kgcds__vhwgx), [yro__baccz, esv__oepb, ncqu__vhcyh])
    return yro__baccz
