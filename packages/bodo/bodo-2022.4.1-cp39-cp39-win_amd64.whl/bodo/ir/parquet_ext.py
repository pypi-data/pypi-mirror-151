...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    srd__sgi = pq_node.out_vars[0].name
    vckf__uzu = pq_node.out_vars[1].name
    if srd__sgi not in lives and vckf__uzu not in lives:
        return None
    elif srd__sgi not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif vckf__uzu not in lives:
