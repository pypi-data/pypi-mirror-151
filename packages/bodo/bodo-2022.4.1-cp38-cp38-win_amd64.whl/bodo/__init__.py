...
def _global_except_hook(exctype, value, traceback):
    ...
    wijwp__arw = MPI.COMM_WORLD
    nca__aiscs = wijwp__arw.Get_rank()
    qonux__lbey = 3.0
    bphcn__uwjec = True
    bzgb__bucx = wijwp__arw.Ibarrier()
    ljeq__isj = time.time()
    while time.time() - ljeq__isj < qonux__lbey:
        time.sleep(0.1)
        if bzgb__bucx.Test():
            bphcn__uwjec = False
            break
    try:
        ...
        if bphcn__uwjec:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {nca__aiscs}. \n')
