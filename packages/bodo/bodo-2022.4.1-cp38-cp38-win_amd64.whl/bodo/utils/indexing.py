...
def get_new_null_mask_bool_index(old_mask, ind, n):
    hyqi__yks = n + 7 >> 3
    kovzu__auw = np.empty(hyqi__yks, np.uint8)
    eww__lwo = 0
    for qrih__kabz in range(len(ind)):
        if ind[qrih__kabz]:
            gqv__dkfa = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                qrih__kabz)
            bodo.libs.int_arr_ext.set_bit_to_arr(kovzu__auw, eww__lwo,
                gqv__dkfa)
            eww__lwo += 1
    return kovzu__auw
...
def array_getitem_bool_index(A, ind):
    ...
    suyc__tac = A._data[ind]
    n = len(suyc__tac)
    kovzu__auw = get_new_null_mask_bool_index(old_mask, ind, n)
    return suyc__tac, kovzu__auw
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    eww__lwo = 0
    for qrih__kabz in range(len(ind)):
        gqv__dkfa = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask, ind[
            qrih__kabz])
        bodo.libs.int_arr_ext.set_bit_to_arr(kovzu__auw, eww__lwo, gqv__dkfa)
        eww__lwo += 1
    return kovzu__auw
...
def array_getitem_int_index(A, ind):
    dqdjy__kjob = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    suyc__tac = A._data[dqdjy__kjob]
    n = len(suyc__tac)
    kovzu__auw = get_new_null_mask_int_index(old_mask, dqdjy__kjob, n)
    return suyc__tac, kovzu__auw


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    ohn__tec = numba.cpython.unicode._normalize_slice(ind, n)
    boc__ckl = numba.cpython.unicode._slice_span(ohn__tec)
    hyqi__yks = boc__ckl + 7 >> 3
    kovzu__auw = np.empty(hyqi__yks, np.uint8)
    eww__lwo = 0
    for qrih__kabz in range(ohn__tec.start, ohn__tec.stop, ohn__tec.step):
        gqv__dkfa = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            qrih__kabz)
        bodo.libs.int_arr_ext.set_bit_to_arr(kovzu__auw, eww__lwo, gqv__dkfa)
        eww__lwo += 1
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    suyc__tac = np.ascontiguousarray(A._data[ind])
    kovzu__auw = get_new_null_mask_slice_index(old_mask, ind, n)
    return suyc__tac, kovzu__auw
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for qrih__kabz in range(n):
                A._data[idx[qrih__kabz]] = val._data[qrih__kabz]
                gqv__dkfa = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, qrih__kabz)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    qrih__kabz], gqv__dkfa)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for qrih__kabz in idx:
                A._data[qrih__kabz] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            ses__ljxx = 0
            for qrih__kabz in range(n):
                if not bodo.libs.array_kernels.isna(idx, qrih__kabz) and idx[
                    qrih__kabz]:
                    A._data[qrih__kabz] = val._data[ses__ljxx]
                    gqv__dkfa = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, ses__ljxx)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        qrih__kabz, gqv__dkfa)
                    ses__ljxx += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    ohn__tec = numba.cpython.unicode._normalize_slice(idx, n)
    ses__ljxx = 0
    for qrih__kabz in range(ohn__tec.start, ohn__tec.stop, ohn__tec.step):
        gqv__dkfa = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            ses__ljxx)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, qrih__kabz, gqv__dkfa)
        ses__ljxx += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            ohn__tec = numba.cpython.unicode._normalize_slice(idx, len(A))
            for qrih__kabz in range(ohn__tec.start, ohn__tec.stop, ohn__tec
                .step):
                A._data[qrih__kabz] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    qrih__kabz, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for qrih__kabz in idx:
                    bodo.libs.array_kernels.setna(A, qrih__kabz)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    gljkm__ovjb = bodo.libs.str_arr_ext.pre_alloc_string_array(
                        n, -1)
                    for qrih__kabz in numba.parfors.parfor.internal_prange(n):
                        if idx[qrih__kabz] or bodo.libs.array_kernels.isna(A,
                            qrih__kabz):
                            gljkm__ovjb[qrih__kabz] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(gljkm__ovjb,
                                qrih__kabz)
                        else:
                            gljkm__ovjb[qrih__kabz] = A[qrih__kabz]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for qrih__kabz in range(n):
                    if not bodo.libs.array_kernels.isna(idx, qrih__kabz
                        ) and idx[qrih__kabz]:
                        bodo.libs.array_kernels.setna(A, qrih__kabz)
            ...
            def setitem_none_slice(A, idx, val):
                n = len(A)
                ohn__tec = numba.cpython.unicode._normalize_slice(idx, n)
                for qrih__kabz in range(ohn__tec.start, ohn__tec.stop,
                    ohn__tec.step):
                    bodo.libs.array_kernels.setna(A, qrih__kabz)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for qrih__kabz in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, qrih__kabz)
                        continue
                    A[qrih__kabz] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        rapau__rre = context.make_helper(builder, val_t, args[0])
        ebl__xne = rapau__rre.data
        context.nrt.incref(builder, val_t.type, ebl__xne)
        return ebl__xne
