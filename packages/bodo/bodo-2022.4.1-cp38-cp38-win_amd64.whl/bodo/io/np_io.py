...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        dcssg__shmp = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, dcssg__shmp)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            pgdr__yidkn = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(pgdr__yidkn.dtype)
            file_write(unicode_to_utf8(fname), pgdr__yidkn.ctypes, 
                dtype_size * pgdr__yidkn.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            pgdr__yidkn = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(pgdr__yidkn.dtype)
            njgp__feleg = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(pgdr__yidkn.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), pgdr__yidkn.ctypes,
                start, count, njgp__feleg)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            xblz__fml = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                xblz__fml = min(xblz__fml, count * dtype_size)
            if xblz__fml < 0:
                return -1
            return xblz__fml
        return impl
