...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        kokdj__ipuz = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(kokdj__ipuz)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vtw__narwx = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, vtw__narwx)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        yny__rxw, = args
        bes__duvha = signature.return_type
        yes__eoewp = cgutils.create_struct_proxy(bes__duvha)(context, builder)
        yes__eoewp.obj = yny__rxw
        context.nrt.incref(builder, signature.args[0], yny__rxw)
        return yes__eoewp._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                qtc__xrnts = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        kokdj__ipuz = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(kokdj__ipuz)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vtw__narwx = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, vtw__narwx)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        yny__rxw, = args
        lld__cfqne = signature.return_type
        cug__dzwq = cgutils.create_struct_proxy(lld__cfqne)(context, builder)
        cug__dzwq.obj = yny__rxw
        context.nrt.incref(builder, signature.args[0], yny__rxw)
        return cug__dzwq._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                arvfd__dkua = bodo.utils.conversion.coerce_to_ndarray(idx)
                vvi__uka = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    arvfd__dkua]
                azqnc__accmb = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    arvfd__dkua]
                kokdj__ipuz = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(vvi__uka,
                    azqnc__accmb, kokdj__ipuz)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                vvi__uka = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                azqnc__accmb = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                kokdj__ipuz = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(vvi__uka,
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        arvfd__dkua = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        qtc__xrnts = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(qtc__xrnts))
                        arvfd__dkua = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            arvfd__dkua] = s
            ...
            def impl_arr(I, idx, val):
                arvfd__dkua = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[arvfd__dkua
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        kokdj__ipuz = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(kokdj__ipuz)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vtw__narwx = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, vtw__narwx)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        yny__rxw, = args
        wmpeb__pca = signature.return_type
        hrd__cxuh = cgutils.create_struct_proxy(wmpeb__pca)(context, builder)
        hrd__cxuh.obj = yny__rxw
        context.nrt.incref(builder, signature.args[0], yny__rxw)
        return hrd__cxuh._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            vvi__uka = bodo.hiframes.pd_series_ext.get_series_data(S)
            azqnc__accmb = bodo.hiframes.pd_series_ext.get_series_index(S)
            fiy__yvcav = (idx - azqnc__accmb._start) // azqnc__accmb._step
            return vvi__uka[fiy__yvcav]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    arvfd__dkua = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(vvi__uka[
                        arvfd__dkua])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    zejy__szq = bodo.hiframes.pd_series_ext.get_series_index(S)
                    wla__qcrqi = zejy__szq.get_loc(idx)
                    if wla__qcrqi is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(wla__qcrqi)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                arvfd__dkua = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[arvfd__dkua
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        lqwst__rztyw = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(nhz__zehj,
            int) for nhz__zehj in lqwst__rztyw):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            qwr__yxg = get_literal_value(idx)
            if qwr__yxg not in lqwst__rztyw:
                raise_bodo_error(
                    f"Series label-based getitem: '{qwr__yxg}' not in {lqwst__rztyw}"
                    )
            arr_ind = lqwst__rztyw.index(qwr__yxg)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    yes__eoewp = cgutils.create_struct_proxy(fromty)(context, builder, val)
    gmrc__wbygj = context.cast(builder, yes__eoewp.obj, fromty.stype, toty.
        stype)
    etw__wxuma = cgutils.create_struct_proxy(toty)(context, builder)
    etw__wxuma.obj = gmrc__wbygj
    return etw__wxuma._getvalue()
