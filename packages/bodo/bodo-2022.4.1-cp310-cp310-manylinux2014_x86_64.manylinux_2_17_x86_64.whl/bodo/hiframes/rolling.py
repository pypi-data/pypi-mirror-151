...
class RollingCovType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        arr = args[0]
        tbgbi__nbifw = arr.copy(dtype=types.float64)
        return signature(tbgbi__nbifw, *unliteral_all(args))
...
def overload_rolling_fixed(arr, index_arr, win, minp, center, fname, raw=
    ...
    pyyh__xelfq = get_overload_const_str(fname)
    if pyyh__xelfq not in ('sum', 'mean', 'var', 'std', 'count', 'median',
        'min', 'max'):
        raise BodoError('invalid rolling (fixed window) function {}'.format
            (pyyh__xelfq))
    if pyyh__xelfq in ('median', 'min', 'max'):
        rtt__wij = 'def kernel_func(A):\n'
        rtt__wij += '  if np.isnan(A).sum() != 0: return np.nan\n'
        rtt__wij += '  return np.{}(A)\n'.format(pyyh__xelfq)
        jtiay__svprd = {}
        exec(rtt__wij, {'np': np}, jtiay__svprd)
        kernel_func = register_jitable(jtiay__svprd['kernel_func'])
        return (lambda arr, index_arr, win, minp, center, fname, raw=True,
            parallel=False: roll_fixed_apply(arr, index_arr, win, minp,
...
def overload_rolling_variable(arr, on_arr, index_arr, win, minp, center,
    ...
    if pyyh__xelfq in ('median', 'min', 'max'):
        rtt__wij = 'def kernel_func(A):\n'
        rtt__wij += '  arr  = dropna(A)\n'
        rtt__wij += '  if len(arr) == 0: return np.nan\n'
        rtt__wij += '  return np.{}(arr)\n'.format(pyyh__xelfq)
        jtiay__svprd = {}
        exec(rtt__wij, {'np': np, 'dropna': _dropna}, jtiay__svprd)
        kernel_func = register_jitable(jtiay__svprd['kernel_func'])
        return (lambda arr, on_arr, index_arr, win, minp, center, fname,
            raw=True, parallel=False: roll_variable_apply(arr, on_arr,
...
def roll_fixed_linear_generic(in_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        xnoe__zxuch = _border_icomm(in_arr, rank, n_pes, halo_size, True,
            center)
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            gbql__mtfs) = xnoe__zxuch
    ...
    if parallel:
        ...
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(gbql__mtfs, True)
            for ryuuw__iyy in range(0, halo_size):
                data = add_obs(r_recv_buff[ryuuw__iyy], *data)
                vglnv__npfvt = in_arr[N + ryuuw__iyy - win]
                data = remove_obs(vglnv__npfvt, *data)
                output[N + ryuuw__iyy - offset] = calc_out(minp, *data)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            data = init_data()
            for ryuuw__iyy in range(0, halo_size):
                data = add_obs(l_recv_buff[ryuuw__iyy], *data)
            for ryuuw__iyy in range(0, win - 1):
                data = add_obs(in_arr[ryuuw__iyy], *data)
                if ryuuw__iyy > offset:
                    vglnv__npfvt = l_recv_buff[ryuuw__iyy - offset - 1]
                    data = remove_obs(vglnv__npfvt, *data)
                if ryuuw__iyy >= offset:
                    output[ryuuw__iyy - offset] = calc_out(minp, *data)
    return output
...
def roll_fixed_linear_generic_seq(in_arr, win, minp, center, init_data,
    ...
    taajz__uzavw = max(minp, 1) - 1
    taajz__uzavw = min(taajz__uzavw, N)
    for ryuuw__iyy in range(0, taajz__uzavw):
        data = add_obs(in_arr[ryuuw__iyy], *data)
        if ryuuw__iyy >= offset:
            output[ryuuw__iyy - offset] = calc_out(minp, *data)
    for ryuuw__iyy in range(taajz__uzavw, N):
        val = in_arr[ryuuw__iyy]
        data = add_obs(val, *data)
        if ryuuw__iyy > win - 1:
            vglnv__npfvt = in_arr[ryuuw__iyy - win]
            data = remove_obs(vglnv__npfvt, *data)
        output[ryuuw__iyy - offset] = calc_out(minp, *data)
    kzfx__klwwe = data
    for ryuuw__iyy in range(N, N + offset):
        if ryuuw__iyy > win - 1:
            vglnv__npfvt = in_arr[ryuuw__iyy - win]
            data = remove_obs(vglnv__npfvt, *data)
        output[ryuuw__iyy - offset] = calc_out(minp, *data)
    return output, kzfx__klwwe
...
def roll_fixed_apply_impl(in_arr, index_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            gbql__mtfs) = xnoe__zxuch
        if raw == False:
            dnuip__gid = _border_icomm(index_arr, rank, n_pes, halo_size, 
                True, center)
            (l_recv_buff_idx, r_recv_buff_idx, wkdkt__brn, yoh__hpm,
                udjp__ldppj, outsa__byyyw) = dnuip__gid
    output = roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
        kernel_func, raw)
    if parallel:
        _border_send_wait(r_send_req, l_send_req, rank, n_pes, True, center)
        if raw == False:
            _border_send_wait(yoh__hpm, wkdkt__brn, rank, n_pes, True, center)
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(gbql__mtfs, True)
            if raw == False:
                bodo.libs.distributed_api.wait(outsa__byyyw, True)
            recv_right_compute(output, in_arr, index_arr, N, win, minp,
                offset, r_recv_buff, r_recv_buff_idx, kernel_func, raw)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            if raw == False:
                bodo.libs.distributed_api.wait(udjp__ldppj, True)
...
def overload_recv_right_compute(output, in_arr, index_arr, N, win, minp,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, N, win, minp, offset,
            r_recv_buff, r_recv_buff_idx, kernel_func, raw):
            kzfx__klwwe = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
            pid__usso = 0
            for ryuuw__iyy in range(max(N - offset, 0), N):
                data = kzfx__klwwe[pid__usso:pid__usso + win]
                if win - np.isnan(data).sum() < minp:
                    output[ryuuw__iyy] = np.nan
                else:
                    output[ryuuw__iyy] = kernel_func(data)
                pid__usso += 1
        return impl
    ...
    def impl_series(output, in_arr, index_arr, N, win, minp, offset,
        r_recv_buff, r_recv_buff_idx, kernel_func, raw):
        kzfx__klwwe = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
        jgv__arrr = np.concatenate((index_arr[N - win + 1:], r_recv_buff_idx))
        pid__usso = 0
        for ryuuw__iyy in range(max(N - offset, 0), N):
            data = kzfx__klwwe[pid__usso:pid__usso + win]
            if win - np.isnan(data).sum() < minp:
                ...
                output[ryuuw__iyy] = kernel_func(pd.Series(data, jgv__arrr[
                    pid__usso:pid__usso + win]))
            pid__usso += 1
    return impl_series
...
def overload_recv_left_compute(output, in_arr, index_arr, win, minp, offset,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, win, minp, offset, l_recv_buff,
            ...
            kzfx__klwwe = np.concatenate((l_recv_buff, in_arr[:win - 1]))
            for ryuuw__iyy in range(0, win - offset - 1):
                data = kzfx__klwwe[ryuuw__iyy:ryuuw__iyy + win]
                if win - np.isnan(data).sum() < minp:
                    output[ryuuw__iyy] = np.nan
                else:
                    output[ryuuw__iyy] = kernel_func(data)
    ...
    def impl_series(output, in_arr, index_arr, win, minp, offset,
        l_recv_buff, l_recv_buff_idx, kernel_func, raw):
        kzfx__klwwe = np.concatenate((l_recv_buff, in_arr[:win - 1]))
        jgv__arrr = np.concatenate((l_recv_buff_idx, index_arr[:win - 1]))
        for ryuuw__iyy in range(0, win - offset - 1):
            data = kzfx__klwwe[ryuuw__iyy:ryuuw__iyy + win]
            if win - np.isnan(data).sum() < minp:
                output[ryuuw__iyy] = np.nan
...
def overload_roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
    ...
    def roll_fixed_apply_seq_impl(in_arr, index_arr, win, minp, center,
        ...
        output = np.empty(N, dtype=np.float64)
        offset = (win - 1) // 2 if center else 0
        for ryuuw__iyy in range(0, N):
            start = max(ryuuw__iyy - win + 1 + offset, 0)
            end = min(ryuuw__iyy + 1 + offset, N)
            data = in_arr[start:end]
            if end - start - np.isnan(data).sum() < minp:
                output[ryuuw__iyy] = np.nan
            else:
                output[ryuuw__iyy] = apply_func(kernel_func, data,
...
def roll_var_linear_generic(in_arr, on_arr_dt, win, minp, center, parallel,
    ...
    if parallel:
        ...
        xnoe__zxuch = _border_icomm_var(in_arr, on_arr, rank, n_pes, win)
        (l_recv_buff, l_recv_t_buff, r_send_req, hnni__fhf, l_recv_req,
            sqkc__ohki) = xnoe__zxuch
    ...
    if parallel:
        ...
        _border_send_wait(hnni__fhf, hnni__fhf, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(sqkc__ohki, True)
            num_zero_starts = 0
            for ryuuw__iyy in range(0, N):
                if start[ryuuw__iyy] != 0:
                    break
                num_zero_starts += 1
            ...
            recv_starts = _get_var_recv_starts(on_arr, l_recv_t_buff,
                num_zero_starts, win)
            data = init_data()
            for fjbwh__scjhz in range(recv_starts[0], len(l_recv_t_buff)):
                data = add_obs(l_recv_buff[fjbwh__scjhz], *data)
            if right_closed:
                data = add_obs(in_arr[0], *data)
            output[0] = calc_out(minp, *data)
            for ryuuw__iyy in range(1, num_zero_starts):
                s = recv_starts[ryuuw__iyy]
                nhue__bgdlp = end[ryuuw__iyy]
                for fjbwh__scjhz in range(recv_starts[ryuuw__iyy - 1], s):
                    data = remove_obs(l_recv_buff[fjbwh__scjhz], *data)
                for fjbwh__scjhz in range(end[ryuuw__iyy - 1], nhue__bgdlp):
                    data = add_obs(in_arr[fjbwh__scjhz], *data)
                output[ryuuw__iyy] = calc_out(minp, *data)
    return output
...
def _get_var_recv_starts(on_arr, l_recv_t_buff, num_zero_starts, win):
    ...
    iwzb__cpkyc = cast_dt64_arr_to_int(on_arr)
    left_closed = False
    dku__dyb = iwzb__cpkyc[0] - win
    if left_closed:
        dku__dyb -= 1
    recv_starts[0] = halo_size
    for fjbwh__scjhz in range(0, halo_size):
        if l_recv_t_buff[fjbwh__scjhz] > dku__dyb:
            recv_starts[0] = fjbwh__scjhz
            break
    for ryuuw__iyy in range(1, num_zero_starts):
        dku__dyb = iwzb__cpkyc[ryuuw__iyy] - win
        if left_closed:
            dku__dyb -= 1
        recv_starts[ryuuw__iyy] = halo_size
        for fjbwh__scjhz in range(recv_starts[ryuuw__iyy - 1], halo_size):
            if l_recv_t_buff[fjbwh__scjhz] > dku__dyb:
                recv_starts[ryuuw__iyy] = fjbwh__scjhz
                break
    return recv_starts
...
def roll_var_linear_generic_seq(in_arr, on_arr, win, minp, start, end,
    ...
    data = init_data()
    for fjbwh__scjhz in range(start[0], end[0]):
        data = add_obs(in_arr[fjbwh__scjhz], *data)
    output[0] = calc_out(minp, *data)
    for ryuuw__iyy in range(1, N):
        s = start[ryuuw__iyy]
        nhue__bgdlp = end[ryuuw__iyy]
        for fjbwh__scjhz in range(start[ryuuw__iyy - 1], s):
            data = remove_obs(in_arr[fjbwh__scjhz], *data)
        for fjbwh__scjhz in range(end[ryuuw__iyy - 1], nhue__bgdlp):
            data = add_obs(in_arr[fjbwh__scjhz], *data)
        output[ryuuw__iyy] = calc_out(minp, *data)
    return output
...
def roll_variable_apply_impl(in_arr, on_arr_dt, index_arr, win, minp,
    ...
    if parallel:
        ...
        (l_recv_buff, l_recv_t_buff, r_send_req, hnni__fhf, l_recv_req,
            sqkc__ohki) = xnoe__zxuch
        if raw == False:
            dnuip__gid = _border_icomm_var(index_arr, on_arr, rank, n_pes, win)
            (l_recv_buff_idx, fvwv__msut, yoh__hpm, odix__cmcf, udjp__ldppj,
                nwpf__pvicw) = dnuip__gid
    start, end = _build_indexer(on_arr, N, win, left_closed, right_closed)
    ...
    if parallel:
        ...
        _border_send_wait(hnni__fhf, hnni__fhf, rank, n_pes, True, False)
        if raw == False:
            _border_send_wait(yoh__hpm, yoh__hpm, rank, n_pes, True, False)
            _border_send_wait(odix__cmcf, odix__cmcf, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(sqkc__ohki, True)
            if raw == False:
                bodo.libs.distributed_api.wait(udjp__ldppj, True)
                bodo.libs.distributed_api.wait(nwpf__pvicw, True)
            num_zero_starts = 0
            for ryuuw__iyy in range(0, N):
                if start[ryuuw__iyy] != 0:
                    break
...
def overload_recv_left_var_compute(output, in_arr, index_arr,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, num_zero_starts, recv_starts,
            ...
            for ryuuw__iyy in range(0, num_zero_starts):
                ntxc__ngrr = recv_starts[ryuuw__iyy]
                urgz__ysnm = np.concatenate((l_recv_buff[ntxc__ngrr:],
                    in_arr[:ryuuw__iyy + 1]))
                if len(urgz__ysnm) - np.isnan(urgz__ysnm).sum() >= minp:
                    output[ryuuw__iyy] = kernel_func(urgz__ysnm)
                else:
                    output[ryuuw__iyy] = np.nan
        return impl
    ...
    def impl_series(output, in_arr, index_arr, num_zero_starts, recv_starts,
        ...
        for ryuuw__iyy in range(0, num_zero_starts):
            ntxc__ngrr = recv_starts[ryuuw__iyy]
            urgz__ysnm = np.concatenate((l_recv_buff[ntxc__ngrr:], in_arr[:
                ryuuw__iyy + 1]))
            kkjhx__bjdt = np.concatenate((l_recv_buff_idx[ntxc__ngrr:],
                index_arr[:ryuuw__iyy + 1]))
            if len(urgz__ysnm) - np.isnan(urgz__ysnm).sum() >= minp:
                output[ryuuw__iyy] = kernel_func(pd.Series(urgz__ysnm,
                    kkjhx__bjdt))
            else:
                output[ryuuw__iyy] = np.nan
    return impl_series
...
def roll_variable_apply_seq_impl(in_arr, on_arr, index_arr, win, minp,
    ...
    for ryuuw__iyy in range(0, N):
        s = start[ryuuw__iyy]
        nhue__bgdlp = end[ryuuw__iyy]
        data = in_arr[s:nhue__bgdlp]
        if nhue__bgdlp - s - np.isnan(data).sum() >= minp:
            output[ryuuw__iyy] = kernel_func(data)
        else:
            output[ryuuw__iyy] = np.nan
    return output
...
def roll_variable_apply_seq_impl_series(in_arr, on_arr, index_arr, win,
    ...
    for ryuuw__iyy in range(0, N):
        ...
        data = in_arr[s:nhue__bgdlp]
        if nhue__bgdlp - s - np.isnan(data).sum() >= minp:
            output[ryuuw__iyy] = kernel_func(pd.Series(data, index_arr[s:
                nhue__bgdlp]))
        else:
            output[ryuuw__iyy] = np.nan
    return output
...
def _build_indexer(on_arr, N, win, left_closed, right_closed):
    ...
    for ryuuw__iyy in range(1, N):
        mnd__yvdiv = iwzb__cpkyc[ryuuw__iyy]
        dku__dyb = iwzb__cpkyc[ryuuw__iyy] - win
        if left_closed:
            dku__dyb -= 1
        start[ryuuw__iyy] = ryuuw__iyy
        for fjbwh__scjhz in range(start[ryuuw__iyy - 1], ryuuw__iyy):
            if iwzb__cpkyc[fjbwh__scjhz] > dku__dyb:
                start[ryuuw__iyy] = fjbwh__scjhz
                break
        if iwzb__cpkyc[end[ryuuw__iyy - 1]] <= mnd__yvdiv:
            end[ryuuw__iyy] = ryuuw__iyy + 1
        else:
            end[ryuuw__iyy] = end[ryuuw__iyy - 1]
        if not right_closed:
            end[ryuuw__iyy] -= 1
    return start, end
...
def calc_mean(minp, nobs, sum_x, neg_ct):
    if nobs >= minp:
        xnsf__kxfha = sum_x / nobs
        if neg_ct == 0 and xnsf__kxfha < 0.0:
            xnsf__kxfha = 0
        elif neg_ct == nobs and xnsf__kxfha > 0.0:
            xnsf__kxfha = 0
    else:
        xnsf__kxfha = np.nan
    return xnsf__kxfha
...
def add_var(val, nobs, mean_x, ssqdm_x):
    if not np.isnan(val):
        nobs += 1
        uczpd__pukb = val - mean_x
        mean_x += uczpd__pukb / nobs
        ssqdm_x += (nobs - 1) * uczpd__pukb ** 2 / nobs
...
def calc_var(minp, nobs, mean_x, ssqdm_x):
    dzkmd__kdjbo = 1.0
    xnsf__kxfha = np.nan
    if nobs >= minp and nobs > dzkmd__kdjbo:
        if nobs == 1:
            xnsf__kxfha = 0.0
        else:
            xnsf__kxfha = ssqdm_x / (nobs - dzkmd__kdjbo)
            if xnsf__kxfha < 0.0:
                xnsf__kxfha = 0.0
    return xnsf__kxfha
...
def calc_std(minp, nobs, mean_x, ssqdm_x):
    gtio__ogwfb = calc_var(minp, nobs, mean_x, ssqdm_x)
    return np.sqrt(gtio__ogwfb)
...
def shift_impl(in_arr, shift, parallel):
    ...
    if parallel:
        ...
        xnoe__zxuch = _border_icomm(in_arr, rank, n_pes, halo_size,
    ...
    if parallel:
        if send_right:
            ...
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(gbql__mtfs, True)
                for ryuuw__iyy in range(0, halo_size):
                    if bodo.libs.array_kernels.isna(r_recv_buff, ryuuw__iyy):
                        bodo.libs.array_kernels.setna(output, N - halo_size +
                            ryuuw__iyy)
                        continue
                    output[N - halo_size + ryuuw__iyy] = r_recv_buff[ryuuw__iyy
                        ]
    return output
...
def shift_seq(in_arr, shift, output, is_parallel_str=False):
    N = len(in_arr)
    bpcw__ntm = 1 if shift > 0 else -1
    shift = bpcw__ntm * min(abs(shift), N)
    if shift > 0 and (not is_parallel_str or bodo.get_rank() == 0):
        bodo.libs.array_kernels.setna_slice(output, slice(None, shift))
    start = max(shift, 0)
    end = min(N, N + shift)
    for ryuuw__iyy in range(start, end):
        if bodo.libs.array_kernels.isna(in_arr, ryuuw__iyy - shift):
            bodo.libs.array_kernels.setna(output, ryuuw__iyy)
            continue
        output[ryuuw__iyy] = in_arr[ryuuw__iyy - shift]
    if shift < 0:
...
def shift_left_recv(r_send_req, l_send_req, rank, n_pes, halo_size,
    ...
    if rank != 0:
        bodo.libs.distributed_api.wait(l_recv_req, True)
        for ryuuw__iyy in range(0, halo_size):
            if bodo.libs.array_kernels.isna(l_recv_buff, ryuuw__iyy):
                bodo.libs.array_kernels.setna(output, ryuuw__iyy)
                continue
            output[ryuuw__iyy] = l_recv_buff[ryuuw__iyy]
    ...
    if parallel:
        if send_right:
            ...
            if rank != 0:
                bodo.libs.distributed_api.wait(l_recv_req, True)
                for ryuuw__iyy in range(0, halo_size):
                    yctjf__gudat = l_recv_buff[ryuuw__iyy]
                    output[ryuuw__iyy] = (in_arr[ryuuw__iyy] - yctjf__gudat
                        ) / yctjf__gudat
        else:
            _border_send_wait(r_send_req, l_send_req, rank, n_pes, False, True)
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(gbql__mtfs, True)
                for ryuuw__iyy in range(0, halo_size):
                    yctjf__gudat = r_recv_buff[ryuuw__iyy]
                    output[N - halo_size + ryuuw__iyy] = (in_arr[N -
                        halo_size + ryuuw__iyy] - yctjf__gudat) / yctjf__gudat
    return output
...
def get_first_non_na(arr):
    ...
    if arr.dtype == types.float32:
        jmw__nxkwf = np.float32('nan')

    def impl(arr):
        for ryuuw__iyy in range(len(arr)):
            if not bodo.libs.array_kernels.isna(arr, ryuuw__iyy):
                return arr[ryuuw__iyy]
        return jmw__nxkwf
    return impl
...
def get_last_non_na(arr):
    ...
    def impl(arr):
        ucz__cpo = len(arr)
        for ryuuw__iyy in range(len(arr)):
            pid__usso = ucz__cpo - ryuuw__iyy - 1
            if not bodo.libs.array_kernels.isna(arr, pid__usso):
                return arr[pid__usso]
        return jmw__nxkwf
    return impl
...
def pct_change_seq(in_arr, shift):
    ...
    if shift > 0:
        ...
        bodo.libs.array_kernels.setna_slice(output, slice(shift, None))
    if shift > 0:
        szm__bus = get_first_non_na(in_arr[:shift])
        hpqle__kcikd = get_last_non_na(in_arr[:shift])
    else:
        szm__bus = get_last_non_na(in_arr[:-shift])
        hpqle__kcikd = get_first_non_na(in_arr[:-shift])
    one = get_one_from_arr_dtype(output)
    start = max(shift, 0)
    end = min(N, N + shift)
    for ryuuw__iyy in range(start, end):
        yctjf__gudat = in_arr[ryuuw__iyy - shift]
        if np.isnan(yctjf__gudat):
            yctjf__gudat = szm__bus
        else:
            szm__bus = yctjf__gudat
        val = in_arr[ryuuw__iyy]
        if np.isnan(val):
            val = hpqle__kcikd
        else:
            hpqle__kcikd = val
        output[ryuuw__iyy] = val / yctjf__gudat - one
    return output
...
def _border_icomm(in_arr, rank, n_pes, halo_size, send_right=True,
    send_left=False):
    swac__tgyl = np.int32(comm_border_tag)
...
def _border_icomm_var(in_arr, on_arr, rank, n_pes, win_size):
    ...
    for fjbwh__scjhz in range(-2, -N, -1):
        rnbk__fkm = on_arr[fjbwh__scjhz]
        if end - rnbk__fkm >= win_size:
            halo_size = -fjbwh__scjhz
            break
    if rank != n_pes - 1:
        bodo.libs.distributed_api.send(halo_size, np.int32(rank + 1),
            swac__tgyl)
        r_send_req = bodo.libs.distributed_api.isend(in_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), swac__tgyl, True)
        hnni__fhf = bodo.libs.distributed_api.isend(on_arr[-halo_size:], np
            .int32(halo_size), np.int32(rank + 1), swac__tgyl, True)
    if rank != 0:
        ...
        sqkc__ohki = bodo.libs.distributed_api.irecv(l_recv_t_buff, np.
            int32(halo_size), np.int32(rank - 1), swac__tgyl, True)
    return (l_recv_buff, l_recv_t_buff, r_send_req, hnni__fhf, l_recv_req,
        sqkc__ohki)
...
def _is_small_for_parallel(N, halo_size):
    nqbi__pfsqb = bodo.libs.distributed_api.dist_reduce(int(N <= 2 *
        halo_size + 1), np.int32(Reduce_Type.Sum.value))
    return nqbi__pfsqb != 0
...
def _handle_small_data(in_arr, win, minp, center, rank, n_pes, init_data,
    add_obs, remove_obs, calc_out):
    N = len(in_arr)
    whlp__pgzgj = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.
        int32(Reduce_Type.Sum.value))
    qquh__zykip = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        lxza__xaskg, dwy__vfrp = roll_fixed_linear_generic_seq(qquh__zykip,
            win, minp, center, init_data, add_obs, remove_obs, calc_out)
    else:
        lxza__xaskg = np.empty(whlp__pgzgj, np.float64)
    bodo.libs.distributed_api.bcast(lxza__xaskg)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
    return lxza__xaskg[start:end]
...
def _handle_small_data_apply(in_arr, index_arr, win, minp, center, rank,
    ...
    whlp__pgzgj = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.
        int32(Reduce_Type.Sum.value))
    qquh__zykip = bodo.libs.distributed_api.gatherv(in_arr)
    rujr__gjgd = bodo.libs.distributed_api.gatherv(index_arr)
    if rank == 0:
        lxza__xaskg = roll_fixed_apply_seq(qquh__zykip, rujr__gjgd, win,
...
def _handle_small_data_shift(in_arr, shift, rank, n_pes):
    ...
    qquh__zykip = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        lxza__xaskg = alloc_shift(len(qquh__zykip), qquh__zykip, (-1,))
        shift_seq(qquh__zykip, shift, lxza__xaskg)
        kekwv__flsn = bcast_n_chars_if_str_binary_arr(lxza__xaskg)
    else:
        kekwv__flsn = bcast_n_chars_if_str_binary_arr(in_arr)
        lxza__xaskg = alloc_shift(whlp__pgzgj, in_arr, (kekwv__flsn,))
    bodo.libs.distributed_api.bcast(lxza__xaskg)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _handle_small_data_pct_change(in_arr, shift, rank, n_pes):
    N = len(in_arr)
    whlp__pgzgj = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    qquh__zykip = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        lxza__xaskg = pct_change_seq(qquh__zykip, shift)
    else:
        lxza__xaskg = alloc_pct_change(whlp__pgzgj, in_arr)
    bodo.libs.distributed_api.bcast(lxza__xaskg)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _is_small_for_parallel_variable(on_arr, win_size):
    if len(on_arr) < 2:
        ...
        yiyo__lzj = end - start
        uibzb__vyug = int(yiyo__lzj <= win_size)
    nqbi__pfsqb = bodo.libs.distributed_api.dist_reduce(uibzb__vyug, np.
        int32(Reduce_Type.Sum.value))
    return nqbi__pfsqb != 0
...
def _handle_small_data_variable(in_arr, on_arr, win, minp, rank, n_pes,
    ...
    whlp__pgzgj = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    qquh__zykip = bodo.libs.distributed_api.gatherv(in_arr)
    fyqa__ehsgt = bodo.libs.distributed_api.gatherv(on_arr)
    if rank == 0:
        start, end = _build_indexer(fyqa__ehsgt, whlp__pgzgj, win, False, True)
        lxza__xaskg = roll_var_linear_generic_seq(qquh__zykip, fyqa__ehsgt,
...
def _handle_small_data_variable_apply(in_arr, on_arr, index_arr, win, minp,
    ...
    if rank == 0:
        start, end = _build_indexer(fyqa__ehsgt, whlp__pgzgj, win, False, True)
        lxza__xaskg = roll_variable_apply_seq(qquh__zykip, fyqa__ehsgt,
            rujr__gjgd, win, minp, start, end, kernel_func, raw)
    else:
        lxza__xaskg = np.empty(whlp__pgzgj, np.float64)
    bodo.libs.distributed_api.bcast(lxza__xaskg)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
...
def _dropna(arr):
    qxqz__htoiu = len(arr)
    ibxvw__osyka = qxqz__htoiu - np.isnan(arr).sum()
    A = np.empty(ibxvw__osyka, arr.dtype)
    ejxhc__zlbl = 0
    for ryuuw__iyy in range(qxqz__htoiu):
        val = arr[ryuuw__iyy]
        if not np.isnan(val):
            A[ejxhc__zlbl] = val
            ejxhc__zlbl += 1
