...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        onpu__sai = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, onpu__sai)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            grbc__sha = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(grbc__sha.dtype)
            file_write(unicode_to_utf8(fname), grbc__sha.ctypes, dtype_size *
                grbc__sha.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            grbc__sha = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(grbc__sha.dtype)
            ggua__vtxq = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                grbc__sha.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), grbc__sha.ctypes,
                start, count, ggua__vtxq)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            hysu__tzb = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                hysu__tzb = min(hysu__tzb, count * dtype_size)
            if hysu__tzb < 0:
                return -1
            return hysu__tzb
        return impl
