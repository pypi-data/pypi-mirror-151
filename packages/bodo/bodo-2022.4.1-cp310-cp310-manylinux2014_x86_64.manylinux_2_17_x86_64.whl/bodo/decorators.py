...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        jotp__pzsps = self.overloads[sig]
        obgo__vqle = jotp__pzsps.metadata.get('distributed_diagnostics', None)
        if obgo__vqle is None:
            nmz__dhef = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(nmz__dhef)
        obgo__vqle.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        cfzks__ube = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(cfzks__ube)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        lwj__cnu = options.pop('distributed')
        pipeline_class = (pipeline_class if lwj__cnu else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        rmm__tuh = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if rmm__tuh else
            pipeline_class)
    xqil__zanxy = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(xqil__zanxy, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(xqil__zanxy)
        else:
            return master_mode_wrapper(xqil__zanxy)
    else:
        return xqil__zanxy
