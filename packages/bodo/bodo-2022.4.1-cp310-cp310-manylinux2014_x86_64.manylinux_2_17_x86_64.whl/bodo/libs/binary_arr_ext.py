...
def init_binary_arr(typingctx, data_typ=None):
    ...
    def codegen(context, builder, sig, args):
        kgtfr__cilp, = args
        wfb__xuujt = context.make_helper(builder, binary_array_type)
        wfb__xuujt.data = kgtfr__cilp
        context.nrt.incref(builder, data_typ, kgtfr__cilp)
        return wfb__xuujt._getvalue()
...
def init_bytes_type(typingctx, data_typ, length_type):
    ...
    def codegen(context, builder, sig, args):
        rgbo__lsyk = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        ywjs__vumxk = args[1]
        gsbm__ftomd = cgutils.create_struct_proxy(bytes_type)(context, builder)
        gsbm__ftomd.meminfo = context.nrt.meminfo_alloc(builder, ywjs__vumxk)
        gsbm__ftomd.nitems = ywjs__vumxk
        gsbm__ftomd.itemsize = lir.Constant(gsbm__ftomd.itemsize.type, 1)
        gsbm__ftomd.data = context.nrt.meminfo_data(builder, gsbm__ftomd.
            meminfo)
        gsbm__ftomd.parent = cgutils.get_null_value(gsbm__ftomd.parent.type)
        gsbm__ftomd.shape = cgutils.pack_array(builder, [ywjs__vumxk],
            context.get_value_type(types.intp))
        gsbm__ftomd.strides = rgbo__lsyk.strides
        cgutils.memcpy(builder, gsbm__ftomd.data, rgbo__lsyk.data, ywjs__vumxk)
        return gsbm__ftomd._getvalue()
...
def binary_arr_hex(arr):
    glb__aonek = numba.cpython.unicode.PY_UNICODE_1BYTE_KIND

    def impl(arr):
        ywjs__vumxk = len(arr) * 2
        output = numba.cpython.unicode._empty_string(glb__aonek, ywjs__vumxk, 1
            )
...
def bytes_to_hex(typingctx, output, arr):
    ...
    def codegen(context, builder, sig, args):
        rbjlq__ijsh = cgutils.create_struct_proxy(sig.args[0])(context,
            builder, value=args[0])
        dlu__qoito = cgutils.create_struct_proxy(sig.args[1])(context,
            builder, value=args[1])
        qln__vdmhc = lir.FunctionType(lir.IntType(8).as_pointer(), [lir.
            IntType(8).as_pointer(), lir.IntType(8).as_pointer(), lir.
            IntType(64)])
        wwgl__bgz = cgutils.get_or_insert_function(builder.module,
            qln__vdmhc, name='bytes_to_hex')
        builder.call(wwgl__bgz, (rbjlq__ijsh.data, dlu__qoito.data,
            dlu__qoito.nitems))
...
def binary_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(arr, ind):
            rlzt__cxi = arr._data[ind]
            return init_bytes_type(rlzt__cxi, len(rlzt__cxi))
        return impl
...
def overload_bytes_fromhex(hex_str):
    ...
    if hex_str == bodo.string_type:
        ...
        def impl(hex_str):
            if not hex_str._is_ascii or hex_str._kind != glb__aonek:
                raise TypeError(
                    'bytes.fromhex is only supported on ascii strings')
            kgtfr__cilp = np.empty(len(hex_str) // 2, np.uint8)
            ywjs__vumxk = _bytes_fromhex(kgtfr__cilp.ctypes, hex_str._data,
                len(hex_str))
            hkleg__qgly = init_bytes_type(kgtfr__cilp, ywjs__vumxk)
            return hkleg__qgly
        return impl
...
def create_binary_cmp_op_overload(op):
    ...
    def overload_binary_cmp(lhs, rhs):
        weet__pttak = lhs == binary_array_type
        yohgz__sbhec = rhs == binary_array_type
        eqtp__fnb = 'lhs' if weet__pttak else 'rhs'
        qhk__otl = 'def impl(lhs, rhs):\n'
        qhk__otl += '  numba.parfors.parfor.init_prange()\n'
        qhk__otl += f'  n = len({eqtp__fnb})\n'
        qhk__otl += '  out_arr = bodo.libs.bool_arr_ext.alloc_bool_array(n)\n'
        qhk__otl += '  for i in numba.parfors.parfor.internal_prange(n):\n'
        zsvjd__ewc = []
        if weet__pttak:
            zsvjd__ewc.append('bodo.libs.array_kernels.isna(lhs, i)')
        if yohgz__sbhec:
            zsvjd__ewc.append('bodo.libs.array_kernels.isna(rhs, i)')
        qhk__otl += f"    if {' or '.join(zsvjd__ewc)}:\n"
        qhk__otl += '      bodo.libs.array_kernels.setna(out_arr, i)\n'
        qhk__otl += '      continue\n'
        fxyfd__ahh = 'lhs[i]' if weet__pttak else 'lhs'
        zoqot__mjnib = 'rhs[i]' if yohgz__sbhec else 'rhs'
        qhk__otl += f'    out_arr[i] = op({fxyfd__ahh}, {zoqot__mjnib})\n'
        qhk__otl += '  return out_arr\n'
        hnd__cdf = {}
        exec(qhk__otl, {'bodo': bodo, 'numba': numba, 'op': op}, hnd__cdf)
        return hnd__cdf['impl']
