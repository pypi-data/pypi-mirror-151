...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    aeuqx__ssxgc = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    bozx__dklah = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    dry__svssq = cgutils.get_or_insert_function(builder.module, bozx__dklah,
        name='decimal_to_str')
    builder.call(dry__svssq, [val, aeuqx__ssxgc._get_ptr_by_name('meminfo'),
        aeuqx__ssxgc._get_ptr_by_name('length'), scale])
    aeuqx__ssxgc.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    aeuqx__ssxgc.is_ascii = context.get_constant(types.int32, 1)
    aeuqx__ssxgc.hash = context.get_constant(numba.cpython.unicode.
        _Py_hash_t, -1)
    aeuqx__ssxgc.data = context.nrt.meminfo_data(builder, aeuqx__ssxgc.meminfo)
    aeuqx__ssxgc.parent = cgutils.get_null_value(aeuqx__ssxgc.parent.type)
    return aeuqx__ssxgc._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, ciycy__ufn, ciycy__ufn = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    bozx__dklah = lir.FunctionType(lir.IntType(128), [lir.IntType(8).
        as_pointer()])
    dry__svssq = cgutils.get_or_insert_function(builder.module, bozx__dklah,
        name='str_to_decimal')
    lty__bdwuv = builder.call(dry__svssq, [val])
    return lty__bdwuv
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        gxmg__qkvvu = cgutils.alloca_once(builder, lir.ArrayType(lir.
            IntType(64), 2))
        builder.store(args[0], builder.bitcast(gxmg__qkvvu, lir.IntType(128
            ).as_pointer()))
        return builder.load(gxmg__qkvvu)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    gyvd__oxtng = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, ciycy__ufn = args
        bozx__dklah = lir.FunctionType(lir.IntType(1), [lir.IntType(128),
            lir.IntType(64), lir.IntType(128), lir.IntType(64)])
        dry__svssq = cgutils.get_or_insert_function(builder.module,
            bozx__dklah, name=gyvd__oxtng)
        return builder.call(dry__svssq, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            gyvd__oxtng = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, gyvd__oxtng
                    )
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    wqq__nhpv = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    isv__tzxs = [context.get_constant_generic(builder, types.int64, v) for
        v in wqq__nhpv]
    cwsb__asjm = cgutils.pack_array(builder, isv__tzxs)
    gxmg__qkvvu = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(cwsb__asjm, builder.bitcast(gxmg__qkvvu, lir.ArrayType(
...
def unbox_decimal(typ, val, c):
    bozx__dklah = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(128).as_pointer()])
    dry__svssq = cgutils.get_or_insert_function(c.builder.module,
        bozx__dklah, name='unbox_decimal')
    gxmg__qkvvu = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(dry__svssq, [val, gxmg__qkvvu])
    kxo__rqvy = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    txb__blz = c.builder.load(gxmg__qkvvu)
    return NativeValue(txb__blz, is_error=kxo__rqvy)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    kiewi__tqkc = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    hoqii__fvgsu = c.pyapi.from_native_value(bodo.string_type, kiewi__tqkc,
        c.env_manager)
    wrjh__wowah = c.context.insert_const_string(c.builder.module, 'decimal')
    cxkq__gcs = c.pyapi.import_module_noblock(wrjh__wowah)
    gxmg__qkvvu = c.pyapi.call_method(cxkq__gcs, 'Decimal', (hoqii__fvgsu,))
    c.pyapi.decref(hoqii__fvgsu)
    c.pyapi.decref(cxkq__gcs)
    return gxmg__qkvvu
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qped__pkm = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, qped__pkm)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        fuffa__cfhx, rtcu__rxvx, ciycy__ufn, ciycy__ufn = args
        mwftr__czue = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        mwftr__czue.data = fuffa__cfhx
        mwftr__czue.null_bitmap = rtcu__rxvx
        context.nrt.incref(builder, signature.args[0], fuffa__cfhx)
        context.nrt.incref(builder, signature.args[1], rtcu__rxvx)
        return mwftr__czue._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    feos__ejjs = DecimalArrayType(precision, scale)
    ytl__xqifb = feos__ejjs(data, null_bitmap, precision_tp, scale_tp)
    return ytl__xqifb, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    ehmps__xbaar = context.get_constant(types.int64, n)
    qwvh__sisre = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [ehmps__xbaar])
    ofp__dbuf = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for dto__jnzq, dmrqz__vocx in enumerate(pyval):
        rgf__zatgw = pd.isna(dmrqz__vocx)
        bodo.libs.int_arr_ext.set_bit_to_arr(ofp__dbuf, dto__jnzq, int(not
            rgf__zatgw))
        if not rgf__zatgw:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [qwvh__sisre._getvalue(), context.
                get_constant(types.int64, dto__jnzq), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), dmrqz__vocx)])
    cdi__lazxp = context.get_constant_generic(builder, nulls_type, ofp__dbuf)
    mwftr__czue = context.make_helper(builder, typ)
    mwftr__czue.data = qwvh__sisre._getvalue()
    mwftr__czue.null_bitmap = cdi__lazxp
    return mwftr__czue._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    vvwa__oniw = np.empty(n, dtype=int128_type)
    egk__fnsn = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(vvwa__oniw, egk__fnsn, precision, scale)
...
def box_decimal_arr(typ, val, c):
    krx__kds = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    vvwa__oniw = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, krx__kds.data)
    qtpr__mxnw = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, krx__kds.null_bitmap).data
    n = c.builder.extract_value(vvwa__oniw.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    bozx__dklah = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    jzk__kfbi = cgutils.get_or_insert_function(c.builder.module,
        bozx__dklah, name='box_decimal_array')
    utrot__uhjpe = c.builder.call(jzk__kfbi, [n, vvwa__oniw.data,
        qtpr__mxnw, scale])
...
def unbox_decimal_arr(typ, val, c):
    mwftr__czue = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    eybis__rmtmd = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(eybis__rmtmd)
    c.pyapi.decref(eybis__rmtmd)
    qftq__lme = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(64
        ), 7)), lir.Constant(lir.IntType(64), 8))
    qwvh__sisre = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(int128_type, 1, 'C'), [n])
    dzr__fkjdz = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(types.uint8, 1, 'C'), [qftq__lme])
    bozx__dklah = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(128).as_pointer(), lir.
        IntType(8).as_pointer()])
    dry__svssq = cgutils.get_or_insert_function(c.builder.module,
        bozx__dklah, name='unbox_decimal_array')
    c.builder.call(dry__svssq, [val, n, qwvh__sisre.data, dzr__fkjdz.data])
    mwftr__czue.null_bitmap = dzr__fkjdz._getvalue()
    mwftr__czue.data = qwvh__sisre._getvalue()
    kxo__rqvy = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(mwftr__czue._getvalue(), is_error=kxo__rqvy)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            xdid__bruq, xeui__zbb = array_getitem_bool_index(A, ind)
            return init_decimal_array(xdid__bruq, xeui__zbb, precision, scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            xdid__bruq, xeui__zbb = array_getitem_int_index(A, ind)
            return init_decimal_array(xdid__bruq, xeui__zbb, precision, scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            xdid__bruq, xeui__zbb = array_getitem_slice_index(A, ind)
            return init_decimal_array(xdid__bruq, xeui__zbb, precision, scale)
        return impl_slice
    raise BodoError(
