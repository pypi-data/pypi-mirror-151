...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        wwyg__bzkjs = []
        yewb__dryv = h5g_get_num_objs(obj_id)
        for qzp__xpeyw in range(yewb__dryv):
            lczc__oeq = h5g_get_objname_by_idx(obj_id, qzp__xpeyw)
            wwyg__bzkjs.append(lczc__oeq)
        return wwyg__bzkjs
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    uir__etnyz = parse_dtype(dtype)
    vfl__mkwx = np.int32(numba_to_c_type(uir__etnyz))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            uclgt__lvcfe = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    rld__jwt = tuple_to_ptr(starts)
    ptq__tth = tuple_to_ptr(counts)
    vyzne__eysz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, rld__jwt, ptq__tth,
        is_parallel, out_arr.ctypes, vyzne__eysz)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    ptq__tth = tuple_to_ptr(counts)
    vyzne__eysz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, rld__jwt, ptq__tth,
        is_parallel, out_arr.ctypes, vyzne__eysz)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        scwt__kwbbu = types.Array(dtype, ndim, 'C')
        gie__barvf = args[3]
        scwt__kwbbu = self.context.resolve_function_type(operator.getitem,
            [scwt__kwbbu, gie__barvf], {}).return_type.copy(layout='C')
        return signature(scwt__kwbbu, *args)
...
def get_filter_read_indices(bool_arr):
    xdjxe__gicuf = bool_arr.nonzero()[0]
    gmxd__igyf = bodo.libs.distributed_api.get_rank()
    nsbdi__zjz = bodo.libs.distributed_api.get_size()
    arfh__wxex = np.empty(nsbdi__zjz, np.int64)
    iolqt__chyj = len(bool_arr)
    bodo.libs.distributed_api.allgather(arfh__wxex, iolqt__chyj)
    ouqwm__wufxb = arfh__wxex.cumsum()[gmxd__igyf] - iolqt__chyj
    xdjxe__gicuf += ouqwm__wufxb
    apqw__gxfj = bodo.libs.distributed_api.dist_reduce(len(xdjxe__gicuf),
        np.int32(sum_op))
    lvsvo__vspb = bodo.libs.distributed_api.gatherv(xdjxe__gicuf)
    if gmxd__igyf == 0:
        hrsqz__bbezq = lvsvo__vspb
    else:
        hrsqz__bbezq = np.empty(apqw__gxfj, xdjxe__gicuf.dtype)
    bodo.libs.distributed_api.bcast(hrsqz__bbezq)
    wuflq__qqui = bodo.libs.distributed_api.get_start(apqw__gxfj,
        nsbdi__zjz, gmxd__igyf)
    zssa__zdp = bodo.libs.distributed_api.get_end(apqw__gxfj, nsbdi__zjz,
        gmxd__igyf)
    return hrsqz__bbezq[wuflq__qqui:zssa__zdp]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        xrg__vxxt = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], xrg__vxxt)
        return builder.bitcast(xrg__vxxt, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    ptq__tth = tuple_to_ptr(counts)
    vyzne__eysz = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, rld__jwt,
        ptq__tth, is_parallel, out_arr.ctypes, vyzne__eysz, read_indices.
