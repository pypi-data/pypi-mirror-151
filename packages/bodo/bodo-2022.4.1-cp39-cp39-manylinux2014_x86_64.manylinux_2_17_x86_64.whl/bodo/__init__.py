...
def _global_except_hook(exctype, value, traceback):
    ...
    nggrq__ipbgb = MPI.COMM_WORLD
    nwhlw__rbxn = nggrq__ipbgb.Get_rank()
    qmz__ccu = 3.0
    lfhn__lvq = True
    ovo__fuze = nggrq__ipbgb.Ibarrier()
    bohn__oyjyy = time.time()
    while time.time() - bohn__oyjyy < qmz__ccu:
        time.sleep(0.1)
        if ovo__fuze.Test():
            lfhn__lvq = False
            break
    try:
        ...
        if lfhn__lvq:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {nwhlw__rbxn}. \n')
