...
class ParquetReader(ir.Stmt):
    ...
    def __repr__(self):
        return (
            ...
            self.index_column_type, self.type_usecol_offset, self.
...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    xfhno__djlxs = pq_node.out_vars[0].name
    ejkz__rbca = pq_node.out_vars[1].name
    if xfhno__djlxs not in lives and ejkz__rbca not in lives:
        return None
    elif xfhno__djlxs not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif ejkz__rbca not in lives:
