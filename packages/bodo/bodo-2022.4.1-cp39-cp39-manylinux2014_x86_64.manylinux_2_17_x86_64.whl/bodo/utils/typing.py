...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        enf__ybi = len(A.types)
        cly__med = 'def f(A):\n'
        rmj__jljqv = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(enf__ybi))
        cly__med += '  return ({}{})\n'.format(rmj__jljqv, ',' if enf__ybi ==
            1 else '')
        wxnj__rufe = {}
        exec(cly__med, {'decode_if_dict_array': decode_if_dict_array},
            wxnj__rufe)
        impl = wxnj__rufe['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            whur__tybt = bodo.hiframes.pd_series_ext.get_series_data(A)
            deutq__vtad = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            nlpuu__ktwv = decode_if_dict_array(whur__tybt)
            return bodo.hiframes.pd_series_ext.init_series(nlpuu__ktwv,
                deutq__vtad, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        bvuy__klod = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(bvuy__klod, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        yziw__elja = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(yziw__elja(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        igzbe__ijurw = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=igzbe__ijurw)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        yyv__dcpp = self.get_full_filename(fname)
        return self._get_schema(yyv__dcpp)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        hrvst__hxxf = dmm.lookup(bodo.typeof(fe_type.fname))
        ywx__jpe = [(a, oqand__ago) for a, oqand__ago in zip(hrvst__hxxf.
            _fields, hrvst__hxxf._members)]
        super().__init__(dmm, fe_type, ywx__jpe)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    swg__yfzds = t.initial_value
    if isinstance(swg__yfzds, dict):
        swg__yfzds = swg__yfzds.values()
    return not any(isinstance(excs__umijc, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for excs__umijc in swg__yfzds)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (excs__umijc) for excs__umijc in val.types
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (excs__umijc) for excs__umijc in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        vdtm__nnrky = []
        for excs__umijc in val.literal_value:
            xrw__iyl = get_overload_const(excs__umijc)
            if xrw__iyl == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                vdtm__nnrky.append(xrw__iyl)
        return vdtm__nnrky
    ...
    if isinstance(val, types.BaseTuple):
        vdtm__nnrky = []
        for excs__umijc in val.types:
            xrw__iyl = get_overload_const(excs__umijc)
            if xrw__iyl == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                vdtm__nnrky.append(xrw__iyl)
        return tuple(vdtm__nnrky)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    xrw__iyl = get_overload_const(val)
    eyps__tqmf = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(xrw__iyl, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {eyps__tqmf} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    xrw__iyl = get_overload_const(val)
    if xrw__iyl not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{xrw__iyl}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    ymkx__bdkn = False
    for a in args_dict:
        lumsd__yfl = get_overload_const(args_dict[a])
        rdo__vgl = arg_defaults_dict[a]
        if (lumsd__yfl is NOT_CONSTANT or lumsd__yfl is not None and 
            rdo__vgl is None or lumsd__yfl is None and rdo__vgl is not None or
            lumsd__yfl != rdo__vgl or lumsd__yfl is not _no_input and 
            rdo__vgl is _no_input or lumsd__yfl is _no_input and rdo__vgl
             is not _no_input):
            ohm__uffdy = (
                f'{fn_str}: {a} parameter only supports default value {rdo__vgl}'
                )
            ymkx__bdkn = True
            break
    if ymkx__bdkn and package_name == 'pandas':
        if module_name == 'IO':
            ohm__uffdy += """
...
"""
    elif ymkx__bdkn and package_name == 'ml':
        ohm__uffdy += """
...
"""
    elif ymkx__bdkn and package_name == 'numpy':
        ohm__uffdy += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(qdkq__gfw): get_literal_value(excs__umijc
            ) for qdkq__gfw, excs__umijc in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    jzsrc__firlq = [get_overload_const(excs__umijc) for excs__umijc in val.
        types[1:]]
    return {jzsrc__firlq[2 * i]: jzsrc__firlq[2 * i + 1] for i in range(len
        (jzsrc__firlq) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(excs__umijc) for excs__umijc in val.
            literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        aobz__plium = get_overload_const_str(dtype)
        if aobz__plium.startswith('Int') or aobz__plium.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(aobz__plium), None)
        if aobz__plium == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if aobz__plium == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(aobz__plium))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    wizqd__ahx = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        rda__huu = []
        for i, uld__htd in enumerate(t.names_typ):
            if uld__htd == types.none:
                rda__huu.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(uld__htd):
                raise BodoError(wizqd__ahx)
            rda__huu.append(get_overload_const_str(uld__htd))
        return tuple(rda__huu)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        hjn__zdtqy = typ.literal_value
        dpvdn__vxx = []
        for hjjib__ymu in ('start', 'stop', 'step'):
            bhb__gnnl = getattr(typ.literal_value, hjjib__ymu)
            uaoe__hqzsq = c.pyapi.make_none(
                ) if bhb__gnnl is None else c.pyapi.from_native_value(types
                .literal(bhb__gnnl), bhb__gnnl, c.env_manager)
            dpvdn__vxx.append(uaoe__hqzsq)
        bnfw__ambt = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        fylo__rqzru = c.pyapi.call_function_objargs(bnfw__ambt, dpvdn__vxx)
        for a in dpvdn__vxx:
            c.pyapi.decref(a)
        c.pyapi.decref(bnfw__ambt)
        return fylo__rqzru
...
def box_list_literal(typ, val, c):
    vkpnw__alea = typ.literal_value
    iqsn__cqtzs = [c.pyapi.from_native_value(types.literal(excs__umijc),
        excs__umijc, c.env_manager) for excs__umijc in vkpnw__alea]
    eav__cxjxk = c.pyapi.list_pack(iqsn__cqtzs)
    for a in iqsn__cqtzs:
        c.pyapi.decref(a)
    return eav__cxjxk
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    bfean__efbcq = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(excs__umijc) for excs__umijc in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(qdkq__gfw): get_literal_value(
                excs__umijc) for qdkq__gfw, excs__umijc in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(excs__umijc) for excs__umijc in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(excs__umijc) for excs__umijc in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        whur__tybt = types.Array(dtype, 1, 'C')
        if luocq__mpfg:
            return to_nullable_type(whur__tybt)
        return whur__tybt
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    opq__cap = dtype_to_array_type(f_return_type)
    opq__cap = to_nullable_type(opq__cap) if return_nullable else opq__cap
    return opq__cap
...
def types_equality_exists(t1, t2):
    mto__brf = numba.core.registry.cpu_target.typing_context
    try:
        mto__brf.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    xnji__jzot = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in xnji__jzot:
        return True
    mto__brf = numba.core.registry.cpu_target.typing_context
    try:
        mto__brf.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        fqw__obdmn = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(fqw__obdmn, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    htptj__rgbgr, bwqa__qpf = get_common_scalar_dtype(scalar_types)
    return bwqa__qpf
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        doymw__zwtou = np.find_common_type([numba.np.numpy_support.as_dtype
            (t) for t in scalar_types], [])
        if doymw__zwtou != object:
            return numba.np.numpy_support.from_dtype(doymw__zwtou), True
    except numba.core.errors.NumbaNotImplementedError as qtf__msn:
        pass
    ...
    xmb__gpk = itertools.groupby(scalar_types)
    if next(xmb__gpk, True) and not next(xmb__gpk, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            skp__sua.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            skp__sua.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                skp__sua.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                skp__sua.append(bodo.libs.int_arr_ext.IntegerArrayType(typ.
                    dtype))
        skp__sua.append(typ)
    return skp__sua
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        fmt__bxpbj = getfullargspec(func)
        assert fmt__bxpbj.varargs is None, 'varargs not supported'
        assert fmt__bxpbj.varkw is None, 'varkw not supported'
        defaults = [] if fmt__bxpbj.defaults is None else fmt__bxpbj.defaults
        nnt__bzc = len(fmt__bxpbj.args) - len(defaults)
        args = fmt__bxpbj.args[1:] if attr_name else fmt__bxpbj.args[:]
        clps__syefp = []
        for i, jpsiz__bvjlh in enumerate(fmt__bxpbj.args):
            if i < nnt__bzc:
                clps__syefp.append(jpsiz__bvjlh)
            elif str(defaults[i - nnt__bzc]) != '<deprecated parameter>':
                clps__syefp.append(jpsiz__bvjlh + '=' + str(defaults[i -
                    nnt__bzc]))
            else:
                args.remove(jpsiz__bvjlh)
        if fmt__bxpbj.kwonlyargs is not None:
            for jpsiz__bvjlh in fmt__bxpbj.kwonlyargs:
                args.append(f'{jpsiz__bvjlh}={jpsiz__bvjlh}')
                clps__syefp.append(
                    f'{jpsiz__bvjlh}={str(fmt__bxpbj.kwonlydefaults[jpsiz__bvjlh])}'
                    )
        sig = ', '.join(clps__syefp)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    mqra__maqin = f'self.{attr_name}' if attr_name else f'{func_name}'
    cly__med = f'def overload_impl({sig}):\n'
    cly__med += f'    def impl({sig}):\n'
    if single_rank:
        cly__med += f'        if bodo.get_rank() == 0:\n'
        ncsyv__mowf = '    '
    else:
        ncsyv__mowf = ''
    cly__med += (
        f"        {ncsyv__mowf}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        cly__med += f'            {ncsyv__mowf}res = {mqra__maqin}({args})\n'
    else:
        cly__med += f'            {ncsyv__mowf}res = {mqra__maqin}\n'
    cly__med += f'        return res\n'
    cly__med += f'    return impl\n'
    wxnj__rufe = {}
    kznt__pjk = globals()
    if not attr_name:
        kznt__pjk[func_name] = func
    exec(cly__med, kznt__pjk, wxnj__rufe)
    tqn__zkldk = wxnj__rufe['overload_impl']
    return tqn__zkldk


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        tqn__zkldk = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(tqn__zkldk)
    except Exception as qtf__msn:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        tqn__zkldk = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(tqn__zkldk)
    except Exception as qtf__msn:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        tqn__zkldk = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(tqn__zkldk)
    except Exception as qtf__msn:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, quxwx__hxp = args
        if isinstance(quxwx__hxp, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[quxwx__hxp]),
                *args)
...
def construct_pysig(arg_names, defaults):
    cly__med = f'def stub('
    for jpsiz__bvjlh in arg_names:
        cly__med += jpsiz__bvjlh
        if jpsiz__bvjlh in defaults:
            if isinstance(defaults[jpsiz__bvjlh], str):
                cly__med += f"='{defaults[jpsiz__bvjlh]}'"
            else:
                cly__med += f'={defaults[jpsiz__bvjlh]}'
        cly__med += ', '
    cly__med += '):\n'
    cly__med += '    pass\n'
    wxnj__rufe = {}
    exec(cly__med, {}, wxnj__rufe)
    gqb__lxpoo = wxnj__rufe['stub']
    return numba.core.utils.pysignature(gqb__lxpoo)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    lizr__vsiy = len(arg_names)
    nmy__pkbde = len(args) + len(kws)
    if nmy__pkbde > lizr__vsiy:
        vam__dbwwn = 'argument' if lizr__vsiy == 1 else 'arguments'
        zmz__jpu = 'was' if nmy__pkbde == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {lizr__vsiy} {vam__dbwwn}, but {nmy__pkbde} {zmz__jpu} provided.'
            )
    acg__soyg = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        naj__mjbf = bodo.utils.transform.fold_argument_types(acg__soyg,
            args, kws)
    except Exception as ehtj__kvs:
        raise_bodo_error(f'{func_name}(): {ehtj__kvs}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                nyji__mop[arg_name] = naj__mjbf[i]
                sgx__fdkay[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, nyji__mop, sgx__fdkay)
    return acg__soyg, naj__mjbf
...
class BodoArrayIterator(types.SimpleIteratorType):
    ...
    def __init__(self, arr_type):
        ...
        uxqlu__dkj = arr_type.dtype
        super(BodoArrayIterator, self).__init__(name, uxqlu__dkj)
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ywx__jpe = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, ywx__jpe)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [mrflw__pmb] = sig.args
    [pvvq__rnbp] = args
    aphv__ceeto = context.make_helper(builder, mrflw__pmb, value=pvvq__rnbp)
    pcvu__pby = signature(types.intp, mrflw__pmb.arr_type)
    eaws__rhab = context.compile_internal(builder, lambda a: len(a),
        pcvu__pby, [aphv__ceeto.array])
    deutq__vtad = builder.load(aphv__ceeto.index)
    owbcr__tnv = builder.icmp_signed('<', deutq__vtad, eaws__rhab)
    result.set_valid(owbcr__tnv)
    with builder.if_then(owbcr__tnv):
        ycycm__edwe = signature(mrflw__pmb.arr_type.dtype, mrflw__pmb.
            arr_type, types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            ycycm__edwe, [aphv__ceeto.array, deutq__vtad])
        result.yield_(value)
        ytlkj__emau = cgutils.increment_index(builder, deutq__vtad)
        builder.store(ytlkj__emau, aphv__ceeto.index)


def index_typ_from_dtype_name(elem_dtype, name):
    kpq__dnj = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        vck__gzotk = None
    else:
        vck__gzotk = types.StringLiteral(name)
    if kpq__dnj == bodo.hiframes.pd_index_ext.NumericIndexType:
        sia__xfva = kpq__dnj(elem_dtype, vck__gzotk)
    elif kpq__dnj == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        sia__xfva = kpq__dnj(bodo.CategoricalArrayType(elem_dtype), vck__gzotk)
    else:
        sia__xfva = kpq__dnj(vck__gzotk)
    return sia__xfva
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(excs__umijc, t) for excs__umijc, t in
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        gcfbu__ubs = pd.Index(val_typ.columns)
        dvexq__ktv = pd.Index(typ.columns)
        eleon__znj = gcfbu__ubs.argsort()
        qus__sqxih = dvexq__ktv.argsort()
        gml__kepn = val_typ.copy(data=tuple(np.array(val_typ.data)[
            eleon__znj]), columns=tuple(gcfbu__ubs[eleon__znj]))
        evut__kqjaa = typ.copy(data=tuple(np.array(typ.data)[qus__sqxih]),
            columns=tuple(dvexq__ktv[qus__sqxih]))
        if gml__kepn == evut__kqjaa:
            val_typ = typ
