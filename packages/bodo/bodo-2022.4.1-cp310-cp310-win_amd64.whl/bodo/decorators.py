...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        myo__hll = self.overloads[sig]
        pzop__dtl = myo__hll.metadata.get('distributed_diagnostics', None)
        if pzop__dtl is None:
            rxycl__btz = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(rxycl__btz)
        pzop__dtl.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        gjg__gksan = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(gjg__gksan)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        pcbv__xml = options.pop('distributed')
        pipeline_class = (pipeline_class if pcbv__xml else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        jqhe__kioa = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if jqhe__kioa else
            pipeline_class)
    enm__hlio = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(enm__hlio, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(enm__hlio)
        else:
            return master_mode_wrapper(enm__hlio)
    else:
        return enm__hlio
