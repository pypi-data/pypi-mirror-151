...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        vbr__ciwoh = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(vbr__ciwoh)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        scoj__enhbc = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, scoj__enhbc)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fzd__ovgdl, = args
        inl__gqhl = signature.return_type
        apnh__lny = cgutils.create_struct_proxy(inl__gqhl)(context, builder)
        apnh__lny.obj = fzd__ovgdl
        context.nrt.incref(builder, signature.args[0], fzd__ovgdl)
        return apnh__lny._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                masz__irmg = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        vbr__ciwoh = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(vbr__ciwoh)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        scoj__enhbc = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, scoj__enhbc)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fzd__ovgdl, = args
        dno__rnpj = signature.return_type
        hesrw__czhiy = cgutils.create_struct_proxy(dno__rnpj)(context, builder)
        hesrw__czhiy.obj = fzd__ovgdl
        context.nrt.incref(builder, signature.args[0], fzd__ovgdl)
        return hesrw__czhiy._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                oym__xdq = bodo.utils.conversion.coerce_to_ndarray(idx)
                qcf__nvu = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    oym__xdq]
                ypcq__fqbdc = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    oym__xdq]
                vbr__ciwoh = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(qcf__nvu,
                    ypcq__fqbdc, vbr__ciwoh)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                qcf__nvu = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                ypcq__fqbdc = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                vbr__ciwoh = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(qcf__nvu,
    ...
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        masz__irmg = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(masz__irmg))
                        oym__xdq = bodo.utils.conversion.coerce_to_ndarray(idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            oym__xdq] = s
                    return impl_dt
            ...
            def impl_arr(I, idx, val):
                oym__xdq = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[oym__xdq
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        vbr__ciwoh = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(vbr__ciwoh)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        scoj__enhbc = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, scoj__enhbc)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fzd__ovgdl, = args
        sksv__bjx = signature.return_type
        vuhjh__iphqs = cgutils.create_struct_proxy(sksv__bjx)(context, builder)
        vuhjh__iphqs.obj = fzd__ovgdl
        context.nrt.incref(builder, signature.args[0], fzd__ovgdl)
        return vuhjh__iphqs._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if is_list_like_index_type(idx) and idx.dtype == types.bool_:
        ...
        def impl(I, idx):
            S = I._obj
            oym__xdq = bodo.utils.conversion.coerce_to_ndarray(idx)
            qcf__nvu = bodo.hiframes.pd_series_ext.get_series_data(S)[oym__xdq]
            ypcq__fqbdc = bodo.hiframes.pd_series_ext.get_series_index(S)[
                oym__xdq]
            vbr__ciwoh = bodo.hiframes.pd_series_ext.get_series_name(S)
            return bodo.hiframes.pd_series_ext.init_series(qcf__nvu,
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            qcf__nvu = bodo.hiframes.pd_series_ext.get_series_data(S)
            ypcq__fqbdc = bodo.hiframes.pd_series_ext.get_series_index(S)
            eyo__nmd = (idx - ypcq__fqbdc._start) // ypcq__fqbdc._step
            return qcf__nvu[eyo__nmd]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    oym__xdq = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(qcf__nvu[oym__xdq]
                        )
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    dms__ctmsx = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    npfmf__ymg = dms__ctmsx.get_loc(idx)
                    if npfmf__ymg is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(npfmf__ymg)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                oym__xdq = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[oym__xdq
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        gonfe__ndai = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(
            hbqer__oepar, int) for hbqer__oepar in gonfe__ndai):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            fsth__ckf = get_literal_value(idx)
            if fsth__ckf not in gonfe__ndai:
                raise_bodo_error(
                    f"Series label-based getitem: '{fsth__ckf}' not in {gonfe__ndai}"
                    )
            arr_ind = gonfe__ndai.index(fsth__ckf)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    apnh__lny = cgutils.create_struct_proxy(fromty)(context, builder, val)
    yrfbf__nwshw = context.cast(builder, apnh__lny.obj, fromty.stype, toty.
        stype)
    srt__xrc = cgutils.create_struct_proxy(toty)(context, builder)
    srt__xrc.obj = yrfbf__nwshw
    return srt__xrc._getvalue()
