...
class RollingCovType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        arr = args[0]
        dhhp__bwuga = arr.copy(dtype=types.float64)
        return signature(dhhp__bwuga, *unliteral_all(args))
...
def overload_rolling_fixed(arr, index_arr, win, minp, center, fname, raw=
    ...
    uhd__wxo = get_overload_const_str(fname)
    if uhd__wxo not in ('sum', 'mean', 'var', 'std', 'count', 'median',
        'min', 'max'):
        raise BodoError('invalid rolling (fixed window) function {}'.format
            (uhd__wxo))
    if uhd__wxo in ('median', 'min', 'max'):
        iqvuw__rumqo = 'def kernel_func(A):\n'
        iqvuw__rumqo += '  if np.isnan(A).sum() != 0: return np.nan\n'
        iqvuw__rumqo += '  return np.{}(A)\n'.format(uhd__wxo)
        ymja__mqcpv = {}
        exec(iqvuw__rumqo, {'np': np}, ymja__mqcpv)
        kernel_func = register_jitable(ymja__mqcpv['kernel_func'])
        return (lambda arr, index_arr, win, minp, center, fname, raw=True,
            parallel=False: roll_fixed_apply(arr, index_arr, win, minp,
...
def overload_rolling_variable(arr, on_arr, index_arr, win, minp, center,
    ...
    if uhd__wxo in ('median', 'min', 'max'):
        iqvuw__rumqo = 'def kernel_func(A):\n'
        iqvuw__rumqo += '  arr  = dropna(A)\n'
        iqvuw__rumqo += '  if len(arr) == 0: return np.nan\n'
        iqvuw__rumqo += '  return np.{}(arr)\n'.format(uhd__wxo)
        ymja__mqcpv = {}
        exec(iqvuw__rumqo, {'np': np, 'dropna': _dropna}, ymja__mqcpv)
        kernel_func = register_jitable(ymja__mqcpv['kernel_func'])
        return (lambda arr, on_arr, index_arr, win, minp, center, fname,
            raw=True, parallel=False: roll_variable_apply(arr, on_arr,
...
def roll_fixed_linear_generic(in_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        opeb__mhv = _border_icomm(in_arr, rank, n_pes, halo_size, True, center)
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            clgui__qgnc) = opeb__mhv
    ...
    if parallel:
        ...
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(clgui__qgnc, True)
            for wrch__zhxa in range(0, halo_size):
                data = add_obs(r_recv_buff[wrch__zhxa], *data)
                gvr__zjhi = in_arr[N + wrch__zhxa - win]
                data = remove_obs(gvr__zjhi, *data)
                output[N + wrch__zhxa - offset] = calc_out(minp, *data)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            data = init_data()
            for wrch__zhxa in range(0, halo_size):
                data = add_obs(l_recv_buff[wrch__zhxa], *data)
            for wrch__zhxa in range(0, win - 1):
                data = add_obs(in_arr[wrch__zhxa], *data)
                if wrch__zhxa > offset:
                    gvr__zjhi = l_recv_buff[wrch__zhxa - offset - 1]
                    data = remove_obs(gvr__zjhi, *data)
                if wrch__zhxa >= offset:
                    output[wrch__zhxa - offset] = calc_out(minp, *data)
    return output
...
def roll_fixed_linear_generic_seq(in_arr, win, minp, center, init_data,
    ...
    oqqp__jnihl = max(minp, 1) - 1
    oqqp__jnihl = min(oqqp__jnihl, N)
    for wrch__zhxa in range(0, oqqp__jnihl):
        data = add_obs(in_arr[wrch__zhxa], *data)
        if wrch__zhxa >= offset:
            output[wrch__zhxa - offset] = calc_out(minp, *data)
    for wrch__zhxa in range(oqqp__jnihl, N):
        val = in_arr[wrch__zhxa]
        data = add_obs(val, *data)
        if wrch__zhxa > win - 1:
            gvr__zjhi = in_arr[wrch__zhxa - win]
            data = remove_obs(gvr__zjhi, *data)
        output[wrch__zhxa - offset] = calc_out(minp, *data)
    yaoq__icv = data
    for wrch__zhxa in range(N, N + offset):
        if wrch__zhxa > win - 1:
            gvr__zjhi = in_arr[wrch__zhxa - win]
            data = remove_obs(gvr__zjhi, *data)
        output[wrch__zhxa - offset] = calc_out(minp, *data)
    return output, yaoq__icv
...
def roll_fixed_apply_impl(in_arr, index_arr, win, minp, center, parallel,
    ...
    if parallel:
        ...
        (l_recv_buff, r_recv_buff, l_send_req, r_send_req, l_recv_req,
            clgui__qgnc) = opeb__mhv
        if raw == False:
            uzdq__lvjnc = _border_icomm(index_arr, rank, n_pes, halo_size, 
                True, center)
            (l_recv_buff_idx, r_recv_buff_idx, noui__xht, bfyal__lsz,
                zrucd__ret, ltwb__hpce) = uzdq__lvjnc
    output = roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
        kernel_func, raw)
    if parallel:
        _border_send_wait(r_send_req, l_send_req, rank, n_pes, True, center)
        if raw == False:
            _border_send_wait(bfyal__lsz, noui__xht, rank, n_pes, True, center)
        if center and rank != n_pes - 1:
            bodo.libs.distributed_api.wait(clgui__qgnc, True)
            if raw == False:
                bodo.libs.distributed_api.wait(ltwb__hpce, True)
            recv_right_compute(output, in_arr, index_arr, N, win, minp,
                offset, r_recv_buff, r_recv_buff_idx, kernel_func, raw)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            if raw == False:
                bodo.libs.distributed_api.wait(zrucd__ret, True)
...
def overload_recv_right_compute(output, in_arr, index_arr, N, win, minp,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, N, win, minp, offset,
            r_recv_buff, r_recv_buff_idx, kernel_func, raw):
            yaoq__icv = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
            pfljh__kdiu = 0
            for wrch__zhxa in range(max(N - offset, 0), N):
                data = yaoq__icv[pfljh__kdiu:pfljh__kdiu + win]
                if win - np.isnan(data).sum() < minp:
                    output[wrch__zhxa] = np.nan
                else:
                    output[wrch__zhxa] = kernel_func(data)
                pfljh__kdiu += 1
        return impl
    ...
    def impl_series(output, in_arr, index_arr, N, win, minp, offset,
        r_recv_buff, r_recv_buff_idx, kernel_func, raw):
        yaoq__icv = np.concatenate((in_arr[N - win + 1:], r_recv_buff))
        uianf__nwnd = np.concatenate((index_arr[N - win + 1:], r_recv_buff_idx)
            )
        pfljh__kdiu = 0
        for wrch__zhxa in range(max(N - offset, 0), N):
            data = yaoq__icv[pfljh__kdiu:pfljh__kdiu + win]
            if win - np.isnan(data).sum() < minp:
                ...
                output[wrch__zhxa] = kernel_func(pd.Series(data,
                    uianf__nwnd[pfljh__kdiu:pfljh__kdiu + win]))
            pfljh__kdiu += 1
    return impl_series
...
def overload_recv_left_compute(output, in_arr, index_arr, win, minp, offset,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, win, minp, offset, l_recv_buff,
            ...
            yaoq__icv = np.concatenate((l_recv_buff, in_arr[:win - 1]))
            for wrch__zhxa in range(0, win - offset - 1):
                data = yaoq__icv[wrch__zhxa:wrch__zhxa + win]
                if win - np.isnan(data).sum() < minp:
                    output[wrch__zhxa] = np.nan
                else:
                    output[wrch__zhxa] = kernel_func(data)
    ...
    def impl_series(output, in_arr, index_arr, win, minp, offset,
        l_recv_buff, l_recv_buff_idx, kernel_func, raw):
        yaoq__icv = np.concatenate((l_recv_buff, in_arr[:win - 1]))
        uianf__nwnd = np.concatenate((l_recv_buff_idx, index_arr[:win - 1]))
        for wrch__zhxa in range(0, win - offset - 1):
            data = yaoq__icv[wrch__zhxa:wrch__zhxa + win]
            if win - np.isnan(data).sum() < minp:
                output[wrch__zhxa] = np.nan
                ...
                output[wrch__zhxa] = kernel_func(pd.Series(data,
                    uianf__nwnd[wrch__zhxa:wrch__zhxa + win]))
    return impl_series
...
def overload_roll_fixed_apply_seq(in_arr, index_arr, win, minp, center,
    ...
    def roll_fixed_apply_seq_impl(in_arr, index_arr, win, minp, center,
        ...
        output = np.empty(N, dtype=np.float64)
        offset = (win - 1) // 2 if center else 0
        for wrch__zhxa in range(0, N):
            start = max(wrch__zhxa - win + 1 + offset, 0)
            end = min(wrch__zhxa + 1 + offset, N)
            data = in_arr[start:end]
            if end - start - np.isnan(data).sum() < minp:
                output[wrch__zhxa] = np.nan
            else:
                output[wrch__zhxa] = apply_func(kernel_func, data,
...
def roll_var_linear_generic(in_arr, on_arr_dt, win, minp, center, parallel,
    ...
    if parallel:
        ...
        opeb__mhv = _border_icomm_var(in_arr, on_arr, rank, n_pes, win)
        (l_recv_buff, l_recv_t_buff, r_send_req, dzxlo__ntdey, l_recv_req,
            ubraq__fjmh) = opeb__mhv
    ...
    if parallel:
        ...
        _border_send_wait(dzxlo__ntdey, dzxlo__ntdey, rank, n_pes, True, False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(ubraq__fjmh, True)
            num_zero_starts = 0
            for wrch__zhxa in range(0, N):
                if start[wrch__zhxa] != 0:
                    break
                num_zero_starts += 1
            ...
            recv_starts = _get_var_recv_starts(on_arr, l_recv_t_buff,
                num_zero_starts, win)
            data = init_data()
            for mlm__zsj in range(recv_starts[0], len(l_recv_t_buff)):
                data = add_obs(l_recv_buff[mlm__zsj], *data)
            if right_closed:
                data = add_obs(in_arr[0], *data)
            output[0] = calc_out(minp, *data)
            for wrch__zhxa in range(1, num_zero_starts):
                s = recv_starts[wrch__zhxa]
                uhvmz__xrp = end[wrch__zhxa]
                for mlm__zsj in range(recv_starts[wrch__zhxa - 1], s):
                    data = remove_obs(l_recv_buff[mlm__zsj], *data)
                for mlm__zsj in range(end[wrch__zhxa - 1], uhvmz__xrp):
                    data = add_obs(in_arr[mlm__zsj], *data)
                output[wrch__zhxa] = calc_out(minp, *data)
    return output
...
def _get_var_recv_starts(on_arr, l_recv_t_buff, num_zero_starts, win):
    ...
    fep__fua = cast_dt64_arr_to_int(on_arr)
    left_closed = False
    yye__lkypz = fep__fua[0] - win
    if left_closed:
        yye__lkypz -= 1
    recv_starts[0] = halo_size
    for mlm__zsj in range(0, halo_size):
        if l_recv_t_buff[mlm__zsj] > yye__lkypz:
            recv_starts[0] = mlm__zsj
            break
    for wrch__zhxa in range(1, num_zero_starts):
        yye__lkypz = fep__fua[wrch__zhxa] - win
        if left_closed:
            yye__lkypz -= 1
        recv_starts[wrch__zhxa] = halo_size
        for mlm__zsj in range(recv_starts[wrch__zhxa - 1], halo_size):
            if l_recv_t_buff[mlm__zsj] > yye__lkypz:
                recv_starts[wrch__zhxa] = mlm__zsj
                break
    return recv_starts
...
def roll_var_linear_generic_seq(in_arr, on_arr, win, minp, start, end,
    ...
    data = init_data()
    for mlm__zsj in range(start[0], end[0]):
        data = add_obs(in_arr[mlm__zsj], *data)
    output[0] = calc_out(minp, *data)
    for wrch__zhxa in range(1, N):
        s = start[wrch__zhxa]
        uhvmz__xrp = end[wrch__zhxa]
        for mlm__zsj in range(start[wrch__zhxa - 1], s):
            data = remove_obs(in_arr[mlm__zsj], *data)
        for mlm__zsj in range(end[wrch__zhxa - 1], uhvmz__xrp):
            data = add_obs(in_arr[mlm__zsj], *data)
        output[wrch__zhxa] = calc_out(minp, *data)
    return output
...
def roll_variable_apply_impl(in_arr, on_arr_dt, index_arr, win, minp,
    ...
    if parallel:
        ...
        (l_recv_buff, l_recv_t_buff, r_send_req, dzxlo__ntdey, l_recv_req,
            ubraq__fjmh) = opeb__mhv
        if raw == False:
            uzdq__lvjnc = _border_icomm_var(index_arr, on_arr, rank, n_pes, win
                )
            (l_recv_buff_idx, xxhk__pyss, bfyal__lsz, bpaz__pbooi,
                zrucd__ret, gfeh__lkk) = uzdq__lvjnc
    start, end = _build_indexer(on_arr, N, win, left_closed, right_closed)
    ...
    if parallel:
        ...
        _border_send_wait(dzxlo__ntdey, dzxlo__ntdey, rank, n_pes, True, False)
        if raw == False:
            _border_send_wait(bfyal__lsz, bfyal__lsz, rank, n_pes, True, False)
            _border_send_wait(bpaz__pbooi, bpaz__pbooi, rank, n_pes, True, 
                False)
        if rank != 0:
            bodo.libs.distributed_api.wait(l_recv_req, True)
            bodo.libs.distributed_api.wait(ubraq__fjmh, True)
            if raw == False:
                bodo.libs.distributed_api.wait(zrucd__ret, True)
                bodo.libs.distributed_api.wait(gfeh__lkk, True)
            num_zero_starts = 0
            for wrch__zhxa in range(0, N):
                if start[wrch__zhxa] != 0:
                    break
...
def overload_recv_left_var_compute(output, in_arr, index_arr,
    ...
    if is_overload_true(raw):
        ...
        def impl(output, in_arr, index_arr, num_zero_starts, recv_starts,
            ...
            for wrch__zhxa in range(0, num_zero_starts):
                ttfs__xcsk = recv_starts[wrch__zhxa]
                uiz__qsuu = np.concatenate((l_recv_buff[ttfs__xcsk:],
                    in_arr[:wrch__zhxa + 1]))
                if len(uiz__qsuu) - np.isnan(uiz__qsuu).sum() >= minp:
                    output[wrch__zhxa] = kernel_func(uiz__qsuu)
                else:
                    output[wrch__zhxa] = np.nan
        return impl
    ...
    def impl_series(output, in_arr, index_arr, num_zero_starts, recv_starts,
        ...
        for wrch__zhxa in range(0, num_zero_starts):
            ttfs__xcsk = recv_starts[wrch__zhxa]
            uiz__qsuu = np.concatenate((l_recv_buff[ttfs__xcsk:], in_arr[:
                wrch__zhxa + 1]))
            lpde__usg = np.concatenate((l_recv_buff_idx[ttfs__xcsk:],
                index_arr[:wrch__zhxa + 1]))
            if len(uiz__qsuu) - np.isnan(uiz__qsuu).sum() >= minp:
                output[wrch__zhxa] = kernel_func(pd.Series(uiz__qsuu,
                    lpde__usg))
            else:
                output[wrch__zhxa] = np.nan
    return impl_series
...
def roll_variable_apply_seq_impl(in_arr, on_arr, index_arr, win, minp,
    ...
    for wrch__zhxa in range(0, N):
        s = start[wrch__zhxa]
        uhvmz__xrp = end[wrch__zhxa]
        data = in_arr[s:uhvmz__xrp]
        if uhvmz__xrp - s - np.isnan(data).sum() >= minp:
            output[wrch__zhxa] = kernel_func(data)
        else:
            output[wrch__zhxa] = np.nan
    return output
...
def roll_variable_apply_seq_impl_series(in_arr, on_arr, index_arr, win,
    ...
    for wrch__zhxa in range(0, N):
        ...
        data = in_arr[s:uhvmz__xrp]
        if uhvmz__xrp - s - np.isnan(data).sum() >= minp:
            output[wrch__zhxa] = kernel_func(pd.Series(data, index_arr[s:
                uhvmz__xrp]))
        else:
            output[wrch__zhxa] = np.nan
    return output
...
def _build_indexer(on_arr, N, win, left_closed, right_closed):
    ...
    for wrch__zhxa in range(1, N):
        mgx__bnj = fep__fua[wrch__zhxa]
        yye__lkypz = fep__fua[wrch__zhxa] - win
        if left_closed:
            yye__lkypz -= 1
        start[wrch__zhxa] = wrch__zhxa
        for mlm__zsj in range(start[wrch__zhxa - 1], wrch__zhxa):
            if fep__fua[mlm__zsj] > yye__lkypz:
                start[wrch__zhxa] = mlm__zsj
                break
        if fep__fua[end[wrch__zhxa - 1]] <= mgx__bnj:
            end[wrch__zhxa] = wrch__zhxa + 1
        else:
            end[wrch__zhxa] = end[wrch__zhxa - 1]
        if not right_closed:
            end[wrch__zhxa] -= 1
    return start, end
...
def calc_mean(minp, nobs, sum_x, neg_ct):
    if nobs >= minp:
        bxvyy__mqqw = sum_x / nobs
        if neg_ct == 0 and bxvyy__mqqw < 0.0:
            bxvyy__mqqw = 0
        elif neg_ct == nobs and bxvyy__mqqw > 0.0:
            bxvyy__mqqw = 0
    else:
        bxvyy__mqqw = np.nan
    return bxvyy__mqqw
...
def add_var(val, nobs, mean_x, ssqdm_x):
    if not np.isnan(val):
        nobs += 1
        pzwd__synam = val - mean_x
        mean_x += pzwd__synam / nobs
        ssqdm_x += (nobs - 1) * pzwd__synam ** 2 / nobs
...
def calc_var(minp, nobs, mean_x, ssqdm_x):
    cnpj__nlky = 1.0
    bxvyy__mqqw = np.nan
    if nobs >= minp and nobs > cnpj__nlky:
        if nobs == 1:
            bxvyy__mqqw = 0.0
        else:
            bxvyy__mqqw = ssqdm_x / (nobs - cnpj__nlky)
            if bxvyy__mqqw < 0.0:
                bxvyy__mqqw = 0.0
    return bxvyy__mqqw
...
def calc_std(minp, nobs, mean_x, ssqdm_x):
    bfm__afyt = calc_var(minp, nobs, mean_x, ssqdm_x)
    return np.sqrt(bfm__afyt)
...
def shift_impl(in_arr, shift, parallel):
    ...
    if parallel:
        ...
        opeb__mhv = _border_icomm(in_arr, rank, n_pes, halo_size,
    ...
    if parallel:
        if send_right:
            ...
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(clgui__qgnc, True)
                for wrch__zhxa in range(0, halo_size):
                    if bodo.libs.array_kernels.isna(r_recv_buff, wrch__zhxa):
                        bodo.libs.array_kernels.setna(output, N - halo_size +
                            wrch__zhxa)
                        continue
                    output[N - halo_size + wrch__zhxa] = r_recv_buff[wrch__zhxa
                        ]
    return output
...
def shift_seq(in_arr, shift, output, is_parallel_str=False):
    N = len(in_arr)
    yyso__yhkc = 1 if shift > 0 else -1
    shift = yyso__yhkc * min(abs(shift), N)
    if shift > 0 and (not is_parallel_str or bodo.get_rank() == 0):
        bodo.libs.array_kernels.setna_slice(output, slice(None, shift))
    start = max(shift, 0)
    end = min(N, N + shift)
    for wrch__zhxa in range(start, end):
        if bodo.libs.array_kernels.isna(in_arr, wrch__zhxa - shift):
            bodo.libs.array_kernels.setna(output, wrch__zhxa)
            continue
        output[wrch__zhxa] = in_arr[wrch__zhxa - shift]
    if shift < 0:
...
def shift_left_recv(r_send_req, l_send_req, rank, n_pes, halo_size,
    ...
    if rank != 0:
        bodo.libs.distributed_api.wait(l_recv_req, True)
        for wrch__zhxa in range(0, halo_size):
            if bodo.libs.array_kernels.isna(l_recv_buff, wrch__zhxa):
                bodo.libs.array_kernels.setna(output, wrch__zhxa)
                continue
            output[wrch__zhxa] = l_recv_buff[wrch__zhxa]
    ...
    if parallel:
        if send_right:
            ...
            if rank != 0:
                bodo.libs.distributed_api.wait(l_recv_req, True)
                for wrch__zhxa in range(0, halo_size):
                    gbey__xuzdf = l_recv_buff[wrch__zhxa]
                    output[wrch__zhxa] = (in_arr[wrch__zhxa] - gbey__xuzdf
                        ) / gbey__xuzdf
        else:
            _border_send_wait(r_send_req, l_send_req, rank, n_pes, False, True)
            if rank != n_pes - 1:
                bodo.libs.distributed_api.wait(clgui__qgnc, True)
                for wrch__zhxa in range(0, halo_size):
                    gbey__xuzdf = r_recv_buff[wrch__zhxa]
                    output[N - halo_size + wrch__zhxa] = (in_arr[N -
                        halo_size + wrch__zhxa] - gbey__xuzdf) / gbey__xuzdf
    return output
...
def get_first_non_na(arr):
    ...
    if arr.dtype == types.float32:
        lzdn__yczkw = np.float32('nan')

    def impl(arr):
        for wrch__zhxa in range(len(arr)):
            if not bodo.libs.array_kernels.isna(arr, wrch__zhxa):
                return arr[wrch__zhxa]
        return lzdn__yczkw
    return impl
...
def get_last_non_na(arr):
    ...
    def impl(arr):
        juo__gzg = len(arr)
        for wrch__zhxa in range(len(arr)):
            pfljh__kdiu = juo__gzg - wrch__zhxa - 1
            if not bodo.libs.array_kernels.isna(arr, pfljh__kdiu):
                return arr[pfljh__kdiu]
        return lzdn__yczkw
    return impl
...
def pct_change_seq(in_arr, shift):
    ...
    if shift > 0:
        ...
        bodo.libs.array_kernels.setna_slice(output, slice(shift, None))
    if shift > 0:
        xzli__tmvu = get_first_non_na(in_arr[:shift])
        jcehw__vcs = get_last_non_na(in_arr[:shift])
    else:
        xzli__tmvu = get_last_non_na(in_arr[:-shift])
        jcehw__vcs = get_first_non_na(in_arr[:-shift])
    one = get_one_from_arr_dtype(output)
    start = max(shift, 0)
    end = min(N, N + shift)
    for wrch__zhxa in range(start, end):
        gbey__xuzdf = in_arr[wrch__zhxa - shift]
        if np.isnan(gbey__xuzdf):
            gbey__xuzdf = xzli__tmvu
        else:
            xzli__tmvu = gbey__xuzdf
        val = in_arr[wrch__zhxa]
        if np.isnan(val):
            val = jcehw__vcs
        else:
            jcehw__vcs = val
        output[wrch__zhxa] = val / gbey__xuzdf - one
    return output
...
def _border_icomm(in_arr, rank, n_pes, halo_size, send_right=True,
    send_left=False):
    omv__wyt = np.int32(comm_border_tag)
...
def _border_icomm_var(in_arr, on_arr, rank, n_pes, win_size):
    ...
    for mlm__zsj in range(-2, -N, -1):
        mjask__lbpd = on_arr[mlm__zsj]
        if end - mjask__lbpd >= win_size:
            halo_size = -mlm__zsj
            break
    if rank != n_pes - 1:
        bodo.libs.distributed_api.send(halo_size, np.int32(rank + 1), omv__wyt)
        r_send_req = bodo.libs.distributed_api.isend(in_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), omv__wyt, True)
        dzxlo__ntdey = bodo.libs.distributed_api.isend(on_arr[-halo_size:],
            np.int32(halo_size), np.int32(rank + 1), omv__wyt, True)
    if rank != 0:
        ...
        ubraq__fjmh = bodo.libs.distributed_api.irecv(l_recv_t_buff, np.
            int32(halo_size), np.int32(rank - 1), omv__wyt, True)
    return (l_recv_buff, l_recv_t_buff, r_send_req, dzxlo__ntdey,
        l_recv_req, ubraq__fjmh)
...
def _is_small_for_parallel(N, halo_size):
    wclay__kvlk = bodo.libs.distributed_api.dist_reduce(int(N <= 2 *
        halo_size + 1), np.int32(Reduce_Type.Sum.value))
    return wclay__kvlk != 0
...
def _handle_small_data(in_arr, win, minp, center, rank, n_pes, init_data,
    add_obs, remove_obs, calc_out):
    N = len(in_arr)
    olipa__yibw = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.
        int32(Reduce_Type.Sum.value))
    eic__pktxr = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        ksrj__uerkb, ify__dpm = roll_fixed_linear_generic_seq(eic__pktxr,
            win, minp, center, init_data, add_obs, remove_obs, calc_out)
    else:
        ksrj__uerkb = np.empty(olipa__yibw, np.float64)
    bodo.libs.distributed_api.bcast(ksrj__uerkb)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
    return ksrj__uerkb[start:end]
...
def _handle_small_data_apply(in_arr, index_arr, win, minp, center, rank,
    ...
    olipa__yibw = bodo.libs.distributed_api.dist_reduce(len(in_arr), np.
        int32(Reduce_Type.Sum.value))
    eic__pktxr = bodo.libs.distributed_api.gatherv(in_arr)
    oydo__qjplo = bodo.libs.distributed_api.gatherv(index_arr)
    if rank == 0:
        ksrj__uerkb = roll_fixed_apply_seq(eic__pktxr, oydo__qjplo, win,
...
def _handle_small_data_shift(in_arr, shift, rank, n_pes):
    ...
    eic__pktxr = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        ksrj__uerkb = alloc_shift(len(eic__pktxr), eic__pktxr, (-1,))
        shift_seq(eic__pktxr, shift, ksrj__uerkb)
        jgk__rnmx = bcast_n_chars_if_str_binary_arr(ksrj__uerkb)
    else:
        jgk__rnmx = bcast_n_chars_if_str_binary_arr(in_arr)
        ksrj__uerkb = alloc_shift(olipa__yibw, in_arr, (jgk__rnmx,))
    bodo.libs.distributed_api.bcast(ksrj__uerkb)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _handle_small_data_pct_change(in_arr, shift, rank, n_pes):
    N = len(in_arr)
    olipa__yibw = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    eic__pktxr = bodo.libs.distributed_api.gatherv(in_arr)
    if rank == 0:
        ksrj__uerkb = pct_change_seq(eic__pktxr, shift)
    else:
        ksrj__uerkb = alloc_pct_change(olipa__yibw, in_arr)
    bodo.libs.distributed_api.bcast(ksrj__uerkb)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
        Sum.value))
    end = start + N
...
def _is_small_for_parallel_variable(on_arr, win_size):
    if len(on_arr) < 2:
        ...
        wynk__whpjz = end - start
        habap__sri = int(wynk__whpjz <= win_size)
    wclay__kvlk = bodo.libs.distributed_api.dist_reduce(habap__sri, np.
        int32(Reduce_Type.Sum.value))
    return wclay__kvlk != 0
...
def _handle_small_data_variable(in_arr, on_arr, win, minp, rank, n_pes,
    ...
    olipa__yibw = bodo.libs.distributed_api.dist_reduce(N, np.int32(
        Reduce_Type.Sum.value))
    eic__pktxr = bodo.libs.distributed_api.gatherv(in_arr)
    rndw__igmj = bodo.libs.distributed_api.gatherv(on_arr)
    if rank == 0:
        start, end = _build_indexer(rndw__igmj, olipa__yibw, win, False, True)
        ksrj__uerkb = roll_var_linear_generic_seq(eic__pktxr, rndw__igmj,
...
def _handle_small_data_variable_apply(in_arr, on_arr, index_arr, win, minp,
    ...
    if rank == 0:
        start, end = _build_indexer(rndw__igmj, olipa__yibw, win, False, True)
        ksrj__uerkb = roll_variable_apply_seq(eic__pktxr, rndw__igmj,
            oydo__qjplo, win, minp, start, end, kernel_func, raw)
    else:
        ksrj__uerkb = np.empty(olipa__yibw, np.float64)
    bodo.libs.distributed_api.bcast(ksrj__uerkb)
    start = bodo.libs.distributed_api.dist_exscan(N, np.int32(Reduce_Type.
...
def _dropna(arr):
    humm__van = len(arr)
    tlg__cgtpn = humm__van - np.isnan(arr).sum()
    A = np.empty(tlg__cgtpn, arr.dtype)
    ees__yfjf = 0
    for wrch__zhxa in range(humm__van):
        val = arr[wrch__zhxa]
        if not np.isnan(val):
            A[ees__yfjf] = val
            ees__yfjf += 1
