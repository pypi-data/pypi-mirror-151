...
def box_regex_flag(typ, val, c):
    knfx__zbcln = c.box(types.int64, val)
    edqui__mcr = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(edqui__mcr, (knfx__zbcln,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    knfx__zbcln = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, knfx__zbcln)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    nasko__zzq = context.get_python_api(builder)
    rdd__mqywx = builder.icmp_signed('==', val, nasko__zzq.borrow_none())
    with builder.if_then(rdd__mqywx):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    hgu__zotb = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(hgu__zotb)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        aje__yrpw = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if aje__yrpw > 1:
            typ = types.List(types.Tuple([string_type] * aje__yrpw))
...
def overload_match_group(m, *args):
    ...
    owr__czjs = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            shl__ivmob = args[0]
            with numba.objmode(out=owr__czjs):
                out = m.group(shl__ivmob)
            return out
        return _match_group_impl_one
    zrz__auj = 'tuple_str_{}'.format(len(args))
    setattr(types, zrz__auj, types.Tuple([owr__czjs] * len(args)))
    bsqwa__zetu = ', '.join('group{}'.format(eeu__fwdxm + 1) for eeu__fwdxm in
        range(len(args)))
    hlk__iqfd = 'def _match_group_impl(m, *args):\n'
    hlk__iqfd += '  ({}) = args\n'.format(bsqwa__zetu)
    hlk__iqfd += "  with numba.objmode(out='{}'):\n".format(zrz__auj)
    hlk__iqfd += '    out = m.group({})\n'.format(bsqwa__zetu)
    hlk__iqfd += '  return out\n'
    rhpl__hdg = {}
    exec(hlk__iqfd, globals(), rhpl__hdg)
    vfwfu__ggra = rhpl__hdg['_match_group_impl']
    return vfwfu__ggra
...
def overload_match_groups(m, default=None):
    wna__zrsmq = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=wna__zrsmq):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(uodl__ebx is None for uodl__ebx in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=owr__czjs):
