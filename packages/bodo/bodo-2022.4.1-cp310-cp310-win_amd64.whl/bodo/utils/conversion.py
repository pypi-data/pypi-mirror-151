...
def overload_coerce_to_ndarray(data, error_on_nonarray=True,
    ...
    if isinstance(data, (types.List, types.UniTuple)):
        juohx__ggk = data.dtype
        if isinstance(juohx__ggk, types.Optional):
            juohx__ggk = juohx__ggk.type
            if bodo.utils.typing.is_scalar_type(juohx__ggk):
                use_nullable_array = True
        if isinstance(juohx__ggk, (types.Boolean, types.Integer,
            Decimal128Type)) or juohx__ggk in [bodo.hiframes.
            ...
            etxe__afck = dtype_to_array_type(juohx__ggk)
            if not is_overload_none(use_nullable_array):
                etxe__afck = to_nullable_type(etxe__afck)

            def impl(data, error_on_nonarray=True, use_nullable_array=None,
                scalar_to_arr_len=None):
                vvxg__ydyf = len(data)
                A = bodo.utils.utils.alloc_type(vvxg__ydyf, etxe__afck, (-1,))
                bodo.utils.utils.tuple_list_to_array(A, data, juohx__ggk)
                return A
            return impl
        return (lambda data, error_on_nonarray=True, use_nullable_array=
    ...
    if not is_overload_none(scalar_to_arr_len):
        if isinstance(data, Decimal128Type):
            vsz__bnt = data.precision
            dmra__buqiy = data.scale

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                vvxg__ydyf = scalar_to_arr_len
                A = bodo.libs.decimal_arr_ext.alloc_decimal_array(vvxg__ydyf,
                    vsz__bnt, dmra__buqiy)
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    A[qskf__ulf] = data
                return A
            return impl_ts
        if data == bodo.hiframes.datetime_datetime_ext.datetime_datetime_type:
            sbr__dhtja = np.dtype('datetime64[ns]')

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                vvxg__ydyf = scalar_to_arr_len
                A = np.empty(vvxg__ydyf, sbr__dhtja)
                hbwu__arx = (bodo.hiframes.pd_timestamp_ext.
                    datetime_datetime_to_dt64(data))
                rxfrb__wypoa = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
                    hbwu__arx)
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    A[qskf__ulf] = rxfrb__wypoa
                return A
            return impl_ts
        if (data == bodo.hiframes.datetime_timedelta_ext.
            datetime_timedelta_type):
            ijn__glw = np.dtype('timedelta64[ns]')

            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                vvxg__ydyf = scalar_to_arr_len
                A = np.empty(vvxg__ydyf, ijn__glw)
                hvv__cwafp = (bodo.hiframes.pd_timestamp_ext.
                    datetime_timedelta_to_timedelta64(data))
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    A[qskf__ulf] = hvv__cwafp
                return A
            return impl_ts
        ...
        if data == bodo.hiframes.pd_timestamp_ext.pd_timestamp_type:
            ...
            def impl_ts(data, error_on_nonarray=True, use_nullable_array=
                None, scalar_to_arr_len=None):
                vvxg__ydyf = scalar_to_arr_len
                A = np.empty(scalar_to_arr_len, sbr__dhtja)
                hbwu__arx = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(data
                    .value)
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    A[qskf__ulf] = hbwu__arx
                return A
            return impl_ts
        ...
        if not is_overload_none(use_nullable_array) and isinstance(dtype,
            ...
            def impl_null_integer(data, error_on_nonarray=True,
                ...
                vvxg__ydyf = scalar_to_arr_len
                epve__ybdz = bodo.libs.int_arr_ext.alloc_int_array(vvxg__ydyf,
                    dtype)
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    epve__ybdz[qskf__ulf] = data
                return epve__ybdz
        ...
        if not is_overload_none(use_nullable_array) and dtype == types.bool_:
            ...
            def impl_null_bool(data, error_on_nonarray=True,
                ...
                numba.parfors.parfor.init_prange()
                vvxg__ydyf = scalar_to_arr_len
                epve__ybdz = bodo.libs.bool_arr_ext.alloc_bool_array(vvxg__ydyf
                    )
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    epve__ybdz[qskf__ulf] = data
        ...
        def impl_num(data, error_on_nonarray=True, use_nullable_array=None,
            ...
            numba.parfors.parfor.init_prange()
            vvxg__ydyf = scalar_to_arr_len
            epve__ybdz = np.empty(vvxg__ydyf, dtype)
            for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
                epve__ybdz[qskf__ulf] = data
            return epve__ybdz
        return impl_num
    if isinstance(data, types.BaseTuple) and all(isinstance(kkkwd__nygtu, (
        types.Float, types.Integer)) for kkkwd__nygtu in data.types):
        return (lambda data, error_on_nonarray=True, use_nullable_array=
...
def overload_coerce_to_array(data, error_on_nonarray=True,
    ...
    if isinstance(data, types.UniTuple) and isinstance(data.dtype, (types.
        ...
        ) and all(isinstance(kkkwd__nygtu, types.StringLiteral) for
        kkkwd__nygtu in data.types):
        return (lambda data, error_on_nonarray=True, use_nullable_array=
            None, scalar_to_arr_len=None: bodo.libs.str_arr_ext.
            str_arr_from_sequence(data))
    ...
    if isinstance(data, (types.List, types.UniTuple)) and isinstance(data.
        ...
        ybnnd__fjp = tuple(dtype_to_array_type(kkkwd__nygtu) for
            kkkwd__nygtu in data.dtype.types)

        def impl_tuple_list(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            vvxg__ydyf = len(data)
            arr = bodo.libs.tuple_arr_ext.pre_alloc_tuple_array(vvxg__ydyf,
                (-1,), ybnnd__fjp)
            for qskf__ulf in range(vvxg__ydyf):
                arr[qskf__ulf] = data[qskf__ulf]
            return arr
    ...
    if isinstance(data, types.List) and (bodo.utils.utils.is_array_typ(data
        ...
        jkt__skpx = dtype_to_array_type(data.dtype.dtype)

        def impl_array_item_arr(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            vvxg__ydyf = len(data)
            ihey__nvj = init_nested_counts(jkt__skpx)
            for qskf__ulf in range(vvxg__ydyf):
                ahh__byoh = bodo.utils.conversion.coerce_to_array(data[
                    qskf__ulf], use_nullable_array=True)
                ihey__nvj = add_nested_counts(ihey__nvj, ahh__byoh)
            epve__ybdz = (bodo.libs.array_item_arr_ext.
                pre_alloc_array_item_array(vvxg__ydyf, ihey__nvj, jkt__skpx))
            rkri__tqzgj = bodo.libs.array_item_arr_ext.get_null_bitmap(
                epve__ybdz)
            for upfc__bvo in range(vvxg__ydyf):
                ahh__byoh = bodo.utils.conversion.coerce_to_array(data[
                    upfc__bvo], use_nullable_array=True)
                epve__ybdz[upfc__bvo] = ahh__byoh
                bodo.libs.int_arr_ext.set_bit_to_arr(rkri__tqzgj, upfc__bvo, 1)
            return epve__ybdz
    ...
    if not is_overload_none(scalar_to_arr_len) and isinstance(data, (types.
        ...
        def impl_str(data, error_on_nonarray=True, use_nullable_array=None,
            scalar_to_arr_len=None):
            vvxg__ydyf = scalar_to_arr_len
            A = bodo.libs.str_arr_ext.pre_alloc_string_array(vvxg__ydyf, -1)
            for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
                A[qskf__ulf] = data
            return A
        return impl_str
    if isinstance(data, types.List) and isinstance(data.dtype, bodo.
        ...
        def impl_list_timestamp(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            vvxg__ydyf = len(data)
            A = np.empty(vvxg__ydyf, np.dtype('datetime64[ns]'))
            for qskf__ulf in range(vvxg__ydyf):
                A[qskf__ulf] = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
                    data[qskf__ulf].value)
            return A
    ...
    if isinstance(data, types.List) and data.dtype == bodo.pd_timedelta_type:
        ...
        def impl_list_timedelta(data, error_on_nonarray=True,
            use_nullable_array=None, scalar_to_arr_len=None):
            vvxg__ydyf = len(data)
            A = np.empty(vvxg__ydyf, np.dtype('timedelta64[ns]'))
            for qskf__ulf in range(vvxg__ydyf):
                A[qskf__ulf
    ...
    if not is_overload_none(scalar_to_arr_len) and data in [bodo.
        ...
        eapv__gbav = ('datetime64[ns]' if data == bodo.pd_timestamp_type else
            'timedelta64[ns]')

        def impl_timestamp(data, error_on_nonarray=True, use_nullable_array
            =None, scalar_to_arr_len=None):
            vvxg__ydyf = scalar_to_arr_len
            A = np.empty(vvxg__ydyf, eapv__gbav)
...
def overload_fix_arr_dtype(data, new_dtype, copy=None, nan_to_str=True,
    ...
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(data,
        'fix_arr_dtype()')
    tykrz__rguel = is_overload_true(copy)
    yek__pjjl = is_overload_constant_str(new_dtype) and get_overload_const_str(
        new_dtype) == 'object'
    if is_overload_none(new_dtype) or yek__pjjl:
        if tykrz__rguel:
    ...
    if isinstance(data, NullableTupleType):
        ...
        dkp__xuh = {types.unicode_type: '', boolean_dtype: False, types.
        ...
        ajmyw__enmtr = {types.unicode_type: str, types.bool_: bool,
            ...
            pd.to_timedelta}
        qgs__wrf = dkp__xuh.keys()
        ajvs__jyxmx = list(data._tuple_typ.types)
        if nb_dtype not in qgs__wrf:
            raise BodoError(f'type conversion to {nb_dtype} types unsupported.'
                )
        for xsxvk__ppwi in ajvs__jyxmx:
            if xsxvk__ppwi == bodo.datetime64ns:
                if nb_dtype not in (types.unicode_type, types.int64, types.
                    uint64, bodo.datetime64ns):
                    raise BodoError(
                        f'invalid type conversion from {xsxvk__ppwi} to {nb_dtype}.'
                        )
            elif xsxvk__ppwi == bodo.timedelta64ns:
                if nb_dtype not in (types.unicode_type, types.int64, types.
        ...
        qzads__uedy += '  data_tup = data._data\n'
        qzads__uedy += '  null_tup = data._null_values\n'
        for qskf__ulf in range(len(ajvs__jyxmx)):
            qzads__uedy += f'  val_{qskf__ulf} = convert_func(default_value)\n'
            qzads__uedy += f'  if not null_tup[{qskf__ulf}]:\n'
            qzads__uedy += (
                f'    val_{qskf__ulf} = convert_func(data_tup[{qskf__ulf}])\n')
        imhc__ctjno = ', '.join(f'val_{qskf__ulf}' for qskf__ulf in range(
            len(ajvs__jyxmx)))
        qzads__uedy += f'  vals_tup = ({imhc__ctjno},)\n'
        qzads__uedy += """  res_tup = bodo.libs.nullable_tuple_ext.build_nullable_tuple(vals_tup, null_tup)
"""
        qzads__uedy += '  return res_tup\n'
        vnf__xkhz = {}
        syh__rqjz = ajmyw__enmtr[nb_dtype]
        mqmof__nwghd = dkp__xuh[nb_dtype]
        exec(qzads__uedy, {'bodo': bodo, 'np': np, 'pd': pd,
            'default_value': mqmof__nwghd, 'convert_func': syh__rqjz},
            vnf__xkhz)
        impl = vnf__xkhz['impl']
        return impl
    if _is_str_dtype(new_dtype):
        if isinstance(data.dtype, types.Integer):
            ...
            def impl_int_str(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for pfgth__cbu in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    if bodo.libs.array_kernels.isna(data, pfgth__cbu):
                        if nan_to_str:
                            bodo.libs.str_arr_ext.str_arr_setitem_NA_str(A,
                                pfgth__cbu)
                        else:
                            bodo.libs.array_kernels.setna(A, pfgth__cbu)
                    else:
                        bodo.libs.str_arr_ext.str_arr_setitem_int_to_str(A,
                            pfgth__cbu, data[pfgth__cbu])
                return A
            return impl_int_str
            ...
            def impl_binary(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for pfgth__cbu in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, pfgth__cbu):
                        ...
                        A[pfgth__cbu] = ''.join([chr(czgjp__wbpd) for
                            czgjp__wbpd in data[pfgth__cbu]])
                return A
            return impl_binary
                ...
                for pfgth__cbu in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, pfgth__cbu):
                        if nan_to_str:
                            ...
                            bodo.libs.array_kernels.setna(A, pfgth__cbu)
                        continue
                    A[pfgth__cbu] = str(box_if_dt64(data[pfgth__cbu]))
                return A
                ...
                for pfgth__cbu in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, pfgth__cbu):
                        if nan_to_str:
                            ...
                            bodo.libs.array_kernels.setna(A, pfgth__cbu)
                        continue
                    A[pfgth__cbu] = str(data[pfgth__cbu])
                return A
    ...
    if isinstance(new_dtype, bodo.hiframes.pd_categorical_ext.
        ...
        def impl_cat_dtype(data, new_dtype, copy=None, nan_to_str=True,
            ...
            vvxg__ydyf = len(data)
            numba.parfors.parfor.init_prange()
            tiq__kwe = (bodo.hiframes.pd_categorical_ext.
                get_label_dict_from_categories(new_dtype.categories.values))
            A = bodo.hiframes.pd_categorical_ext.alloc_categorical_array(
                vvxg__ydyf, new_dtype)
            sgvan__wys = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(A))
            for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
                if bodo.libs.array_kernels.isna(data, qskf__ulf):
                    bodo.libs.array_kernels.setna(A, qskf__ulf)
                    continue
                val = data[qskf__ulf]
                if val not in tiq__kwe:
                    bodo.libs.array_kernels.setna(A, qskf__ulf)
                    continue
                sgvan__wys[qskf__ulf] = tiq__kwe[val]
            return A
    ...
    if is_overload_constant_str(new_dtype) and get_overload_const_str(new_dtype
        ...
        def impl_category(data, new_dtype, copy=None, nan_to_str=True,
            from_series=False):
            gldz__izxp = bodo.libs.array_kernels.unique(data, dropna=True)
            gldz__izxp = pd.Series(gldz__izxp).sort_values().values
            gldz__izxp = bodo.allgatherv(gldz__izxp, False)
            adhzg__qesca = bodo.hiframes.pd_categorical_ext.init_cat_dtype(bodo
                .utils.conversion.index_from_array(gldz__izxp, None), False,
                None, None)
            vvxg__ydyf = len(data)
            numba.parfors.parfor.init_prange()
            tiq__kwe = (bodo.hiframes.pd_categorical_ext.
                get_label_dict_from_categories_no_duplicates(gldz__izxp))
            A = bodo.hiframes.pd_categorical_ext.alloc_categorical_array(
                vvxg__ydyf, adhzg__qesca)
            sgvan__wys = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(A))
            for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
                if bodo.libs.array_kernels.isna(data, qskf__ulf):
    ...
    nb_dtype = bodo.utils.typing.parse_dtype(new_dtype)
    if isinstance(data, bodo.libs.int_arr_ext.IntegerArrayType):
        xilv__uyq = isinstance(nb_dtype, bodo.libs.int_arr_ext.IntDtype
            ) and data.dtype == nb_dtype.dtype
    else:
        xilv__uyq = data.dtype == nb_dtype
    if tykrz__rguel and xilv__uyq:
        return (lambda data, new_dtype, copy=None, nan_to_str=True,
            from_series=False: data.copy())
    if xilv__uyq:
        return (lambda data, new_dtype, copy=None, nan_to_str=True,
    ...
    if isinstance(nb_dtype, bodo.libs.int_arr_ext.IntDtype):
        ...
        if isinstance(data.dtype, types.Float):
            ...
            def impl_float(data, new_dtype, copy=None, nan_to_str=True,
                ...
                vvxg__ydyf = len(data)
                numba.parfors.parfor.init_prange()
                bqlfk__hqcy = bodo.libs.int_arr_ext.alloc_int_array(vvxg__ydyf,
                    eapv__gbav)
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    if bodo.libs.array_kernels.isna(data, qskf__ulf):
                        bodo.libs.array_kernels.setna(bqlfk__hqcy, qskf__ulf)
                    else:
                        bqlfk__hqcy[qskf__ulf] = int(data[qskf__ulf])
                return bqlfk__hqcy
            return impl_float
        else:
            ...
            def impl(data, new_dtype, copy=None, nan_to_str=True,
                ...
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    ...
                    if bodo.libs.array_kernels.isna(data, qskf__ulf):
                        ...
                        bqlfk__hqcy[qskf__ulf] = np.int64(data[qskf__ulf])
                return bqlfk__hqcy
            return impl
    ...
    if nb_dtype == bodo.libs.bool_arr_ext.boolean_dtype:
        ...
        def impl_bool(data, new_dtype, copy=None, nan_to_str=True,
            ...
            vvxg__ydyf = len(data)
            numba.parfors.parfor.init_prange()
            bqlfk__hqcy = bodo.libs.bool_arr_ext.alloc_bool_array(vvxg__ydyf)
            for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
                if bodo.libs.array_kernels.isna(data, qskf__ulf):
                    bodo.libs.array_kernels.setna(bqlfk__hqcy, qskf__ulf)
                else:
                    bqlfk__hqcy[qskf__ulf] = bool(data[qskf__ulf])
            return bqlfk__hqcy
        return impl_bool
    if nb_dtype == bodo.datetime_date_type:
        if data.dtype == bodo.datetime64ns:
            ...
            def impl_date(data, new_dtype, copy=None, nan_to_str=True,
                from_series=False):
                vvxg__ydyf = len(data)
                epve__ybdz = (bodo.hiframes.datetime_date_ext.
                    alloc_datetime_date_array(vvxg__ydyf))
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    if bodo.libs.array_kernels.isna(data, qskf__ulf):
                        bodo.libs.array_kernels.setna(epve__ybdz, qskf__ulf)
                    else:
                        epve__ybdz[qskf__ulf
    ...
    if nb_dtype == bodo.datetime64ns:
        ...
        if isinstance(data.dtype, types.Number) or data.dtype in [bodo.
            ...
            def impl_numeric(data, new_dtype, copy=None, nan_to_str=True,
                ...
                vvxg__ydyf = len(data)
                numba.parfors.parfor.init_prange()
                epve__ybdz = np.empty(vvxg__ydyf, dtype=np.dtype(
                    'datetime64[ns]'))
                for qskf__ulf in numba.parfors.parfor.internal_prange(
                    vvxg__ydyf):
                    if bodo.libs.array_kernels.isna(data, qskf__ulf):
                ...
                def impl_numeric(data, new_dtype, copy=None, nan_to_str=
                    ...
                    for qskf__ulf in numba.parfors.parfor.internal_prange(
                        ...
                        if bodo.libs.array_kernels.isna(data, qskf__ulf):
                            ...
                            epve__ybdz[qskf__ulf] = (bodo.hiframes.
                                pd_timestamp_ext.integer_to_timedelta64(np.
                                int64(data[qskf__ulf])))
                    return epve__ybdz
                return impl_numeric
            else:
                return (lambda data, new_dtype, copy=None, nan_to_str=True,
    ...
    if nb_dtype == types.int64 and data.dtype in [bodo.datetime64ns, bodo.
        ...
        def impl_datelike_to_integer(data, new_dtype, copy=None, nan_to_str
            ...
            vvxg__ydyf = len(data)
            numba.parfors.parfor.init_prange()
            A = np.empty(vvxg__ydyf, types.int64)
            for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
                if bodo.libs.array_kernels.isna(data, qskf__ulf):
                    bodo.libs.array_kernels.setna(A, qskf__ulf)
                else:
                    A[qskf__ulf] = np.int64(data[qskf__ulf])
            return A
...
def flatten_array(A):
    ...
    vvxg__ydyf = len(A)
    for qskf__ulf in range(vvxg__ydyf):
        uybzg__bqxx = A[qskf__ulf]
        for vnp__dzxq in uybzg__bqxx:
            lzoru__rxe.append(vnp__dzxq)
    return bodo.utils.conversion.coerce_to_array(lzoru__rxe)
...
def overload_parse_datetimes_from_strings(data):
    ...
    def parse_impl(data):
        numba.parfors.parfor.init_prange()
        vvxg__ydyf = len(data)
        lmoib__gtgi = np.empty(vvxg__ydyf, bodo.utils.conversion.NS_DTYPE)
        for qskf__ulf in numba.parfors.parfor.internal_prange(vvxg__ydyf):
            lmoib__gtgi[qskf__ulf
...
def overload_convert_to_index(data, name=None):
    ...
    def impl(data, name=None):
        vbmj__wricc = bodo.utils.conversion.coerce_to_array(data)
        return bodo.utils.conversion.index_from_array(vbmj__wricc, name)
    return impl
    ...
    if val == types.Optional(bodo.hiframes.pd_timestamp_ext.pd_timestamp_type):
        ...
        def impl_optional(val):
            if val is None:
                ...
                avat__dheaa = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(
    ...
    if val == types.Optional(bodo.hiframes.datetime_timedelta_ext.
        ...
        def impl_optional_td(val):
            if val is None:
                ...
                avat__dheaa = (bodo.hiframes.pd_timestamp_ext.
...
def overload_to_tuple(val):
    if not isinstance(val, types.BaseTuple) and is_overload_constant_list(val):
        mata__cdrzp = len(val.types if isinstance(val, types.LiteralList) else
            get_overload_const_list(val))
        qzads__uedy = 'def f(val):\n'
        hrxcv__nme = ','.join(f'val[{qskf__ulf}]' for qskf__ulf in range(
            mata__cdrzp))
        qzads__uedy += f'  return ({hrxcv__nme},)\n'
        vnf__xkhz = {}
        exec(qzads__uedy, {}, vnf__xkhz)
        impl = vnf__xkhz['f']
        return impl
...
def overload_get_array_if_series_or_index(data):
    ...
    if isinstance(data, bodo.hiframes.pd_index_ext.HeterogeneousIndexType):
        if not is_heterogeneous_tuple_type(data.data):
            ...
            def impl(data):
                xxcac__jsd = bodo.hiframes.pd_index_ext.get_index_data(data)
                return bodo.utils.conversion.coerce_to_array(xxcac__jsd)
            return impl
...
def overload_extract_index_array(A):
    ...
    if isinstance(A, SeriesType):
        ...
        def impl(A):
            ...
            roj__kikdk = bodo.utils.conversion.coerce_to_array(index)
            return roj__kikdk
        return impl
...
def struct_if_heter_dict(values, names):
    return {xtrj__cyz: hbwu__arx for xtrj__cyz, hbwu__arx in zip(names, values)
        }
...
def overload_struct_if_heter_dict(values, names):
    if not types.is_homogeneous(*values.types):
        return lambda values, names: bodo.libs.struct_arr_ext.init_struct(
            values, names)
    wfi__rbj = len(values.types)
    qzads__uedy = 'def f(values, names):\n'
    hrxcv__nme = ','.join("'{}': values[{}]".format(get_overload_const_str(
        names.types[qskf__ulf]), qskf__ulf) for qskf__ulf in range(wfi__rbj))
    qzads__uedy += '  return {{{}}}\n'.format(hrxcv__nme)
    vnf__xkhz = {}
    exec(qzads__uedy, {}, vnf__xkhz)
    impl = vnf__xkhz['f']
