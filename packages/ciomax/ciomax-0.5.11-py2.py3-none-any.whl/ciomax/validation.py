...
class ValidateInstanceType(Validator):
    ...
    def run(self, _):
        ...
        section = self._submitter.main_tab.section("GeneralSection")

        if not section.instance_type_component.combobox_machine.currentText().strip():
            self.add_error(
...
class ValidateSoftware(Validator):
    ...
    def run(self, _):
        ...
        section = self._submitter.main_tab.section("SoftwareSection")
        if not section.component.combobox.currentText().strip():
            self.add_error(
