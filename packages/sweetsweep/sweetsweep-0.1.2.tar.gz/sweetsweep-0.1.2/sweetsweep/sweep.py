...
# The moment between each part can be called a checkpoint.
# I have to look at the Pytorch sweep tool, that uses checkpoints, but I don't know if that's what I want,
# but it's worth looking at.
def parameter_sweep_multiparts(param_dict, experiment_funcs, sweep_dir, start_index=0, result_csv_filename=""):

    # Fill the current_dict and count number of experiments
    current_dict={}
    num_exp = 1
    for part in param_dict.keys():
        for k in param_dict[part]:
            current_dict[k] = param_dict[part][k][0]
            num_exp *= len(param_dict[part][k])

    print("\nThere are a total of",num_exp,"experiments.\n")

    # Start experiments
    for part in range(len(experiment_funcs)):
        results = experiment_funcs[part](exp_id, current_dict, exp_dir)
        # TODO: results can be intermediate results or the final results to store

    # TODO: Write down on paper the order of the calls, and the content of each argument, for a simple example,
    #  e.g. with 2 parts, and 2 parameters that vary with 2 values in each part.
