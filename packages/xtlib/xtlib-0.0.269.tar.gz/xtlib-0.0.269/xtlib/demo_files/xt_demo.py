...
from xtlib.demo_files import commands_advanced
from xtlib.demo_files import commands_basic
...
def build_cmds(auto_mode, quick_test, nomonitor, nogui, philly=0, basic_mode=None, 
    store=None, readonly_ws="xt-demo"):

    global cmds, cmd_count
    cmds = []
    cmd_count = 0
    ...
    prev_exper = "exper18"
    curr_exper = "exper26"
    big_job = "xtd_job10608" if store.endswith("v1") else "job10608"
    ...
    list(map(
        lambda cmd_dict: add_cmd(cmd_dict["title"], cmd_dict["cmd"]),
        command_dicts
    ))
...
def add_cmd(title=None, xt_cmd=None, silent=False):
    ...
    cmd = {"title": title, "cmd": xt_cmd, "silent": silent}
    cmds.append(cmd)
...
def parse_args(arg_list=None):
    ...
    parser.add_argument('--basic_mode', default=0, type=int, help='should demo be run in basic mode')
    parser.add_argument('--quick_test', default=0, type=int, help='specifies we are running as part of the quick-test')
    parser.add_argument('--store', default="sandbox_v3", type=str, help='the name of the store being used in config file')
    parser.add_argument('--readonly_ws', default="xt-demo", type=str, help='the name of the readonly workspace to use for previous runs')
...
def main(arg_list=None):
    ...
    if not auto_mode:
        ...
        if response == "q" or ord(response) in [3, 27]:
            run_tests = False
