...
from xtlib.qfe import inner_dispatch, see_also, keyword_arg
...
class ImplUtilities(ImplBase):
    ...
    @option("console", values=["none", "normal", "timing", "diagnostics", "detail"], default="$internal.console", help="sets the level of console output")
    @option("config", type=str, default="xt_config.yaml", help="selects the XT config file to be used")
    @option("store", type=str, default="$store", help="selects the store to be used from the stores list")
    ...
    def root(self, name, value):
        ...
        if name == "help":
        ...
        elif name == "store" and value:
            self.config.data["store"] = value
            ...
            # already handled by xt_cmds.main()
            pass
    ...
    @example(task="xt list processes", text="show the running processes on this box")
    @command(help="how the name and command line for running processes on the local machine")
    def list_processes(self):
        ...
        for p in processes:
            try:
                name = p.name().lower()
                console.print("{}\t{}\t{}".format(p.pid, p.name(), p.cmdline()))
            except:
                pass

    #---- GROK command ----
    @option(name="store", default="$store", help="the store (aka team) to navigate to on the XT Grok website")
    ...
    @argument(name="property", type=str, help="specifies the (dotted) name of the xt config property to be changed")
    @argument(name="value", type=str, required=False, help="specifies the new value for the property to be changed")
    @example(task="change the default workspace to 'ws5' in the user's local config file", 
        text="xt change config general.workspace ws5")
    @see_also("XT Config File", "xt_config_file")
    @command(help="changes the value of a property in the user's local XT config file")
    def change_config(self, property, value):
        fn = "xt_config.yaml"
        ...
        for line_index, line in enumerate(lines):
            # find first non-whitespace char in line
            ...
            if stripped:
                ...
                if not stripped.startswith("#") and ":" in stripped:
                    ...
                    prop_value = right.strip()   # .split()[0] if right else ""

                    # apply to props_stack
                    new_entry = (indent, prop_name)

                    if props_stack:
                        # remove any obsolete entries
                        top_indent, top_name = props_stack[-1]
                        while indent <= top_indent:
                            props_stack.pop()
                            current_property = current_property.rsplit(".")[0] if props_stack else ""
                            if not props_stack:
                                break
                            top_indent, top_name = props_stack[-1]

                    props_stack.append(new_entry)
                    if current_property:
                        current_property += "." + prop_name 
                    else:
                        current_property = prop_name

                    #console.print("  " + current_property)

                    if current_property == property:
                        found = True
                        if value:
                            line = line.replace(prop_value, value)
        ...
        if found:
            if value:
                ...
                console.print("{} set to: {}".format(property, value))
            else:
                console.print("{}: {}".format(property, prop_value))
        else:
            console.print("property not found: {}".format(property))
    ...
    @keyword_arg(name="which", keywords=["factory", "global", "local"], help="specifies which config file should be opened")
    ...
    def config_cmd(self, response, which, create):
        '''
        ...
        if which == "factory":
            fn = os.path.join(file_utils.get_xtlib_dir(), "helpers", constants.FACTORY_CONFIG)
            if not os.path.exists(fn):
                errors.env_error("the XT default config file is missing: {}".format(fn))
        elif which == "global":
            fn = os.path.expanduser(os.getenv("XT_GLOBAL_CONFIG", "~/.xt/xt_config.yaml"))
        else:
            fn = constants.CONFIG_FN

        edit = True

        if create:
            if which == "factory":
                errors.general_error("cannot use --create option for the factory config file")

            if os.path.exists(fn):
                console.print("the {} XT config file already exists: {}".format(which, fn))
            ...
            console.print("the {} config file doesn't exist: {}".format(which, fn))
        ...
        if edit:
            fn = os.path.abspath(fn)       # fix up slashes 
    ...
    def create_demo(self, destination, response, overwrite):
        ...
        if create:
            ...
            console.print("to run the demo, move to the demo directory and run 'python xt_demo.py'")
    ...
    def get_registry_creds(self, compute, env):
        ...
        if not env:
            compute_def = self.config.get_target_def(compute)
            env = utils.safe_value(compute_def, "docker")
        ...
        if env and env != "none":
            ...
            if not registry_creds:
                errors.config_error("'{}' must be specified in [external-services] section of XT config file".format(registry_name))
