...
from ._aws_config import AwsConfig
from ._azure_config import AzureConfig
...
_PYTHON_ONLY_CONFIG = ["domain", "certificate_authority"]
...
def get_logging_destination_io(config: SessionConfig) -> Optional[TextIOBase]:
    if config.logging:
        if isinstance(config.logging.destination, TextIOBase):
...
def serialize_config_to_json(
    ...
    https_config.pop("domain", None)
    https_config.pop("certificate_authority", None)
...
def create_authentication_config(
    ...
    if isinstance(authentication, OidcConfig):
