...
from atoti_query import Auth as QueryAuth, QuerySession
from py4j.java_gateway import DEFAULT_PORT as _PY4J_DEFAULT_PORT

from ._content_storage_migration import is_h2_v1_error, migrate_user_content_storage
...
from .config import UserContentStorageConfig
...
class LocalSession(Generic[_LocalCubes], BaseSession[_LocalCubes]):
    ...
    __server_versions: Optional[ServerVersions] = None
    ...
    def __init__(
        ...
        *,
        ...
        self._create_subprocess_and_java_api(distributed=distributed)
    ...
    def _create_subprocess_and_java_api(
        ...
        *,
        ...
        try:
            ...
            process = ServerSubprocess(config=self._config, session_id=self._id)
            self._java_api = JavaApi(
    ...
    def _start_application(self) -> None:
        ...
        except AtotiJavaException as error:
            if (
                isinstance(self._config.user_content_storage, UserContentStorageConfig)
                and is_h2_v1_error(
                    error.java_traceback,
                    driver=self._config.user_content_storage.driver,
                )
                and self._server_subprocess
            ):
                migrate_user_content_storage(
                    self._config.user_content_storage.url,
                    session_directory=self._server_subprocess._session_directory,
                )
                self._start_application()
            else:
                raise error

    def _set_client_side_encryption(
        self, client_side_encryption: ClientSideEncryptionConfig
    ) -> None:
        deprecated(
    ...
    def wait(self) -> None:
        ...
        if self._server_subprocess is None:
            raise NotImplementedError("Server subprocess is not defined")
    ...
    def _server_versions(self) -> ServerVersions:
        if self.__server_versions is None:
            self.__server_versions = QuerySession(
                self._local_url,
    ...
    def _create_query_session(self) -> QuerySession:
        ...
        return QuerySession(
            self._local_url,
            auth=auth,
            # Pass the versions to avoid refetching them.
    ...
    def query_mdx(
        ...
        *,
        ...
        def get_level_java_types(
            levels_coordinates: Iterable[LevelCoordinates], *, cube_name: str
        ) -> Mapping[LevelCoordinates, JavaType]:
            return self.cubes[cube_name]._get_level_java_types(levels_coordinates)

        return self._create_query_session().query_mdx(
            mdx,
            get_level_java_types=get_level_java_types,
    ...
    def explain_mdx_query(
        self, mdx: str, *, timeout: Union[int, timedelta] = timedelta(seconds=30)
    ) -> QueryAnalysis:
        return self._java_api.analyse_mdx(mdx, timeout=timeout)
    ...
    def endpoint(
        ...
        def endpoint_decorator(func: Callable[..., Any]) -> Callable[..., Any]:
            self._java_api.create_endpoint(
                http_method=method,
                route=route,
                handler=EndpointHandler(callback=func, session=self),
            )
            return func
