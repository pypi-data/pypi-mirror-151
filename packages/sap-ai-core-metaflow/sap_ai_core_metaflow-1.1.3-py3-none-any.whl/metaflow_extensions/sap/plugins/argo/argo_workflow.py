...
class ArgoWorkflow:
    def __init__(
        ...
        self.system_tags = {
            "metaflow/%s"
            % sys_tag[: sys_tag.index(":")]: sanitize_label_value(
    ...
    def _commands(self, node, retry_count, user_code_retries):
        ...
        init_cmds = self.environment.get_package_commands(self.code_package_url)
    ...
    def _step_commands(self, node, retry_count, user_code_retries):
        ...
        if node.type == "join" and self.graph[node.split_parents[-1]].type == "foreach":
            module = "metaflow_extensions.sap.plugins.argo.argo_convert_aggregation"
...
def _prepare_environment(env_decorator, k8s_decorator):
    ...
    if METADATA_SERVICE_HEADERS:
        env["METADATA_SERVICE_HEADERS"] = METADATA_SERVICE_HEADERS
