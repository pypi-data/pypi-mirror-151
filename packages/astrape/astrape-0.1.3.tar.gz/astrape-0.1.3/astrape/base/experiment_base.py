...
from pytorch_lightning.utilities.seed import seed_everything as pl_set_seed
...
class BaseExperiment:
    ...
    ***************************************************************************************************************************
    ...
    def update_exp_metadata(self):
        self.exp_meta.update({'date of birth of this experiemt' : str(self.birthday)})
        self.exp_meta.update({'# of checkpoints saved' : int(self.n_ckpts)})
        self.exp_meta.update({'size of the experiment in MB' : float(os.path.getsize(self.exp_path)/10**6)})
        self.exp_meta.update({'size of stack in MB': float(sys.getsizeof(self.stack)/10**6)})
        self.exp_meta.update({'random state' : int(self.random_state)})
        if self.data_module:
            self.exp_meta.update({'size of data in MB' : float(sys.getsizeof(self.X)/10**6)})
        json_name = self.exp_path +"/exp_metadata.json" 
        with open(json_name, 'w') as fout:
            json.dump(self.exp_meta, fout)
    # *********************************************************************        Class Methods        ********************************************************************* 
    #######################################################################  set_model (Begins) #######################################################################
    ...
    def set_trainer(self, **trainer_config):
        ...
        if 'min_epochs' not in trainer_config.keys():
            trainer_setting.update({'min_epochs' : 5})
        if 'max_epochs' not in trainer_config.keys():
            trainer_setting.update({'max_epochs' : 30})
        if 'sample_weight' in trainer_setting.keys():
            trainer_setting.pop('sample_weight')
        if self.logger:
            del self.logger
        self._create_folder(self.log_path+"/tensorboardlogs/"+self.model_metadata)
        self.logger = pl_loggers.TensorBoardLogger(save_dir=self.log_path+"/tensorboardlogs", name=self.model_metadata)

        return pl.Trainer(logger=self.logger, **trainer_config)
    ...
    ** Just in case for hasty act that we might do during the experiment, the model you trained at the last is saved to self.stack even if stack_models is False. ** 
    ...
    def fit_sklearn(
        ...
        folder_dir = self.log_path + "/tensorboardlogs" 
        self._create_folder(folder_dir)
        version = len(os.listdir(folder_dir))
        self._create_folder(folder_dir + "/" + self.model_metadata + "/" + "version_" + str(version))
        log_dir = folder_dir + "/" + self.model_metadata + "/" + "version_" + str(version)
    ...
    def save_ckpt(
        ...
        self.update_log_path()
        checkpoint_path = self.log_path + "/checkpoints"
        ...
        json_dir = self.log_path + "/jsonlogs"
        self._create_folder(json_dir)
        tensorboard_logdir = self.logger.experiment.get_logdir()

        self.n_ckpts += 1 
        self.log_metrics_to_json(model, tensorboard_logdir, val_metrics)
        self.update_exp_metadata()
    ...
    def save_to_stack(self) -> None:
        ...
        if not has_val_loss and not has_val_acc:
            raise AssertionError("Did not log anything.")

        val_loss = math.inf if not has_val_loss else val_loss
        val_acc = 0 if not has_val_acc else val_acc

        if isinstance(self.model, BaseEstimator):
            self.stack.append({"model" : self.model, "val_metrics": {'val/acc' : val_acc, 'val/loss' : val_loss}})
            self.update_exp_metadata()   
        elif isinstance(self.model, pl.LightningModule):
            self.stack.append({"model" : self.model, "trainer" : self.trainer, "val_metrics": {'val/acc' : val_acc, 'val/loss' : val_loss}})
            self.update_exp_metadata()   
        else:
            raise NotImplementedError("Expytorch-Lightning v 0.0.0 only supports variants of sklearn and pl.LightningModule.")
    ...
    def best_ckpt_thus_far(self, val_metric : str = "val/acc", fetch_only_dir : bool = False):
        try:
            ...
            return None
        mode = "max" if val_metric == "val/acc" else "min"
        best_val = None
        ckpt_extension_type = None
        model_type = None
        for file in os.listdir(jsonlogdir):
            with open(jsonlogdir + "/" + file, "r") as json_file:
        ...
        if not fetch_only_dir:
            ckpt_path = self.log_path + '/checkpoints/'
    ...
    def best_ckpt_in_stack(
        ...
        if val_metric not in ["val/loss", "val/acc"]:
            raise ValueError("Wrong type of val_metric. It should be one of [{}]".format(*["val/loss", "val/acc"]))
        if len(self.stack) == 0:
    ...
    def log_metrics_to_json(
        ...
        error_count = 0
        self.update_log_path()
        json_log_dir = self.log_path + "/jsonlogs"
        model_metadata = self.set_model_metadata(model, self.get_hparams(model))
        json_logdir = json_log_dir + "/" + model_metadata + ".json"
