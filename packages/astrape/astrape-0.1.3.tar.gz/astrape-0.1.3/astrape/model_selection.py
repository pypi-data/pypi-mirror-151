...
class CrossValidation(BaseExperiment):
    ...
    def __init__(
        ...
        self.seed_num = int(100 * self.rng.random(1))
        pl_set_seed(self.seed_num)
    ...
    def get_cv_indices(self):
        skf = StratifiedKFold(n_splits=self.cv, random_state=self.seed_num, shuffle=True)
        cv_indices = []
    ...
    def get_data_module_ms(
        ...
        if BaseEstimator in inspect.getmro(self.model_type):
            ...
            if (X_test or y_test) and not (X_test and y_test):
                ...
                X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=test_size, random_state=self.seed_num, stratify=y)
                data_module = (X_train, X_val, y_train, y_val, X_test, y_test)
            elif X_val and y_val:
                X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=self.seed_num, stratify=y)
                X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=test_size, random_state=self.seed_num, stratify=y)
                data_module = (X_train, X_val, y_train, y_val, X_test, y_test)
            elif X_val and y_val:
                X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=self.seed_num, stratify=y)
