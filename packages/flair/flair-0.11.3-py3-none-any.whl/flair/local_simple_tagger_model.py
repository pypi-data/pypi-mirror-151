...
import flair.nn
from flair.data import Dictionary, Label, Sentence, Token, space_tokenizer
...
class SequenceTagger(flair.nn.Model):
    def __init__(
        ...
        # word memory
        self.word_memory_global = {}
        self.word_memory_count = {}
        self.word_memory_last = {}
        ...
        self.rnn_input_dim: int = self.embeddings.embedding_length + len(self.tag_dictionary) + 1

        if self.relearn_embeddings:
            self.embedding2nn = torch.nn.Linear(self.embeddings.embedding_length, self.embeddings.embedding_length)
        ...
        if self.use_rnn:
            ...
            self.linear = torch.nn.Linear(self.rnn_input_dim, len(tag_dictionary))
    ...
    def _get_state_dict(self):
        model_state = {
            ...
            "word_memory_last": self.word_memory_last,
            "word_memory_global": self.word_memory_global,
            "word_memory_count": self.word_memory_count,
        }
        return model_state
    ...
    def _init_model_with_state_dict(state):
        ...
        train_initial_hidden_state = (
            False if not "train_initial_hidden_state" in state.keys() else state["train_initial_hidden_state"]
        )
        ...
        model.word_memory_last = state["word_memory_last"]
        model.word_memory_global = state["word_memory_global"]
        model.word_memory_count = state["word_memory_count"]
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            dataloader = DataLoader(dataset=dataset, batch_size=mini_batch_size, collate_fn=lambda x: x)
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        memory_tensor = torch.zeros(
            [
                len(sentences),
                longest_token_sequence_in_batch,
                len(self.tag_dictionary) + 1,
            ],
            dtype=torch.float,
            device=flair.device,
        )
        ...
        for s_id, sentence in enumerate(sentences):
            for t_id, token in enumerate(sentence):
                if token.text in self.word_memory_last:
                    # fill values with word embeddings
                    # memory_tensor[s_id][t_id] = self.word_memory_last[token.text]

                    word_count = self.word_memory_count[token.text]

                    word_class_softmax = self.word_memory_global[token.text] / word_count
                    word_frequency_quotient = 1.0 if word_count >= 10.0 else word_count / 10.0

                    # vector = torch.cat([word_class_softmax, torch.tensor(word_frequency_quotient)])

                    memory_tensor[s_id][t_id][:-1] = word_class_softmax
                    memory_tensor[s_id][t_id][-1] = word_frequency_quotient
                    # print(memory_tensor[s_id][t_id])
        ...
        # print(memory_tensor.size())
        sentence_tensor = torch.cat([sentence_tensor, memory_tensor], dim=2)
        # print(sentence_tensor.size())
        ...
        if self.use_rnn:
            ...
            if self.train_initial_hidden_state:
                ...
                rnn_output, hidden = self.rnn(sentence_tensor, initial_hidden_state)
            else:
                rnn_output, hidden = self.rnn(sentence_tensor)

            sentence_tensor = rnn_output
        ...
        for s_id, sentence in enumerate(sentences):
            for t_id, token in enumerate(sentence):
                word_label_dist = softmax_batch[s_id, t_id].detach().clone()
                self.word_memory_last[token.text] = word_label_dist

                word_prior = (
                    self.word_memory_global[token.text] + word_label_dist
                    if token.text in self.word_memory_global
                    else torch.zeros(len(self.tag_dictionary))
                )
                word_count = 1 if token.text not in self.word_memory_count else self.word_memory_count[token.text] + 1

                self.word_memory_global[token.text] = word_prior
                self.word_memory_count[token.text] = word_count

                # print(token)
                # print(softmax_batch[s_id, t_id])
                # for tag in range(len(self.tag_dictionary)):
                #     if softmax_batch[s_id, t_id][tag] > 0.1:
                #         print(f' - {self.tag_dictionary.get_item_for_index(tag)}: {softmax_batch[s_id, t_id][tag]}')
    ...
    def _fetch_model(model_name) -> str:
        ...
        model_map["frame"] = "/".join([aws_resource_path_v04, "release-frame-1", "en-frame-ontonotes-v0.4.pt"])
        ...
        model_map["da-pos"] = "/".join([aws_resource_path_v04, "POS-danish", "da-pos-v0.1.pt"])

        model_map["da-ner"] = "/".join([aws_resource_path_v04, "NER-danish", "da-ner-v0.1.pt"])

        model_map["de-pos"] = "/".join([aws_resource_path_v04, "release-de-pos-0", "de-pos-ud-hdt-v0.4.pt"])
        ...
        model_map["de-ner"] = "/".join([aws_resource_path_v04, "release-de-ner-0", "de-ner-conll03-v0.4.pt"])

        model_map["de-ner-germeval"] = "/".join([aws_resource_path_v04, "NER-germeval", "de-ner-germeval-0.4.1.pt"])

        model_map["fr-ner"] = "/".join([aws_resource_path_v04, "release-fr-ner-0", "fr-ner-wikiner-0.4.pt"])
    ...
    def train(self, mode=True):
        ...
        if mode:
            ...
            self.word_memory_global = {}
            self.word_memory_count = {}
            print(f"Train mode - resetting memory.")
        else:
