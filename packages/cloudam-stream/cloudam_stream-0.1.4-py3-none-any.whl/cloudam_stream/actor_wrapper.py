...
from core.actors import Args, Actor, ComputeActor, SinkActor, SourceActor
from core.utils import base64_utils, dict_utils
from core.constants import actor_constant, config
from core.streams import RedisConsumer, RedisProducer
from core.utils.actor_utils import FlowStateUtils
from core.payload import Message
import pickle

upstream_actors_states = {}
...
def loop_input_port(actor: Actor, input_ports: list, state_utils: FlowStateUtils) -> None:
    ...
    group_name = current_actor_name
    while True:
        print("死循环")
        consumer = RedisConsumer(redis_client, group_name, stream_names_imput_map.keys())
        ...
        if not msgs:
            ...
            for upstream_actor in upstream_actors:
                upstream_actor_state = get_actor_final_state(upstream_actor, state_utils)
                # 如果上游算子有至少一个没有结束，继续
            ...
            if not stoppable:
                ...
                return
        actor_running(current_actor_state)
        # msgs消息格式
        ...
        for msg in msgs:
            ...
            for msg_item in msg_items:
                ...
                # 解析消息体json格式
                msg_object = Message(pickle.loads(msg_body))
...
def process_source_actor(actor: Actor) -> None:
    for item in actor.__iter__():
        actor_running(current_actor_state)
        actor.emit(item)
...
def process_sink_actor(actor: Actor, input_ports: list, state_utils) -> None:
    actor.begin()
...
def process_actor_common(actor: Actor, input_ports: list, state_utils: FlowStateUtils) -> None:
    loop_input_port(actor, input_ports, state_utils)
...
def actor_ready() -> None:
    # 调用flow_namager接口修改actor的状态为Ready
    state_utils.update_actor_state(current_actor_name, parallel_index, actor_constant.ACTOR_STATE_READY)
...
def actor_stoppable() -> None:
    # 调用flow_namager接口修改actor的状态为Ready
    state_utils.update_actor_state(current_actor_name, parallel_index, actor_constant.ACTOR_STATE_STOPPABLE)
    current_actor_state = actor_constant.ACTOR_STATE_STOPPABLE
...
#
#
# def actor_succeed() -> None:
#     # 调用flow_namager接口修改actor的状态为Ready
#     state_utils.update_actor_state(current_actor_name, parallel_index, actor_constant.ACTOR_STATE_SUCCESS)
#     current_actor_state = actor_constant.ACTOR_STATE_SUCCESS
...
def actor_running(current_actor_state=None) -> None:
    if current_actor_state == actor_constant.ACTOR_STATE_READY:
        # 调用flow_namager接口修改actor的状态为running
        state_utils.update_actor_state(current_actor_name, parallel_index, actor_constant.ACTOR_STATE_RUNNING)
        current_actor_state = actor_constant.ACTOR_STATE_RUNNING
...
def get_actor_final_state(actor_name: str, state_utils: FlowStateUtils) -> str:
    # 查询actor的状态
    state = state_utils.get_actor_final_state(actor_name)
    print(actor_name+'的状态：'+state)
    return state
...
#     scheduler.add_job(get_upstream_actors_state, 'interval', seconds=10, id='test_job1')
#     scheduler.start()
...
def get_actor_input_ports(actor: Actor, actor_desc: json) -> list:
...
def get_actor_output_ports(actor: Actor, actor_desc: json) -> list:
    output_ports_json = dict_utils.get_property_value(actor_desc, 'output_ports', [])
    output_ports = []
    producer = RedisProducer(redis_client=redis_client, flow_depth=flow_depth, actor_level=actor_level)
    for output_port_json in output_ports_json:
        ...
        output_port = port_class(producer=producer, name=port_name, actor_name=current_actor_name)
        output_ports.append(output_port)
        # 把用户定义的port空对象用真实的对象覆盖
        setattr(actor, port_name, output_port)
    return output_ports
...
def get_actor_params(actor: Actor, actor_desc: json) -> Args:
    ...
    for param in params_json:
        ...
        param = param_class(param_name, param_value)
...
if __name__ == '__main__':
    ...
    actor_desc = args.actor_desc
    flow_manager_server = args.flow_manager_server
    redis_host = args.redis_host
    flow_depth = args.flow_depth
    actor_level = args.actor_level
    ...
    actor.input_ports = input_ports
    actor.output_ports = output_ports
    actor.args = params
    # 设置actor状态为已开始
    actor_ready()
    current_actor_state = actor_constant.ACTOR_STATE_READY
