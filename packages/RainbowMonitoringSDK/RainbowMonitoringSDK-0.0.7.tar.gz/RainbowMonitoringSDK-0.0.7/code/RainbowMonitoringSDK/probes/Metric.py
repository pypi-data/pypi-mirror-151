...
class Metric:
    ...
    def to_dict(self):
        d = dict()
        d['name'] = self.name if self.name is not None else ""
        d['units'] = self.units if self.units is not None else ""
        d['desc'] = self.desc if self.desc is not None else ""
        d['timestamp'] = self.timestamp if self.timestamp is not None else int(time.time())
        d['val'] = self.val if self.val is not None else ""
        d['higherIsBetter'] = self.higherIsBetter if self.higherIsBetter is not None else ""
        d['minVal'] = self.minVal if self.minVal is not None else ""
        d['maxVal'] = self.maxVal if self.maxVal is not None else ""
        d['group'] = self.group if self.group is not None else ""
        return d
...
class SimpleMetric(Metric):
    ...
    def __init__(self, name, units, desc, minVal = -1*__infinity_value__, maxVal = __infinity_value__, higherIsBetter=True):
        Metric.__init__(self, name, units, desc, minVal, maxVal, higherIsBetter)
...
class CounterMetric(SimpleMetric):
    def __init__(self, name, units, desc, minVal = 0, maxVal = __infinity_value__, higherIsBetter=True, step=1, reset=True):
        SimpleMetric.__init__(self, name, units, desc, minVal, maxVal, higherIsBetter)
        self.step = step
        self.counter = minVal
        self.reset = reset
...
class DiffMetric(SimpleMetric):
    ...
    def __init__(self, name, units='%', desc='a diff metric', minVal = -1*__infinity_value__, maxVal = __infinity_value__, higherIsBetter=True):
        SimpleMetric.__init__(self, name, units, desc, minVal, maxVal, higherIsBetter)
        self.prev = 0
        self.cur = 0
        self.diff = 0
