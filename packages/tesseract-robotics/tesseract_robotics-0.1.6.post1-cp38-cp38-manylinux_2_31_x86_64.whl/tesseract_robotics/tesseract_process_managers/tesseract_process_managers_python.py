...
class ProcessPlanningProblem(object, metaclass=_SwigNonDynamicMeta):
    ...
    def __init__(self):
        _tesseract_process_managers_python.ProcessPlanningProblem_swiginit(self, _tesseract_process_managers_python.new_ProcessPlanningProblem())
...
class ProcessPlanningRequest(object, metaclass=_SwigNonDynamicMeta):
    ...
    plan_profile_remapping = property(_tesseract_process_managers_python.ProcessPlanningRequest_plan_profile_remapping_get, _tesseract_process_managers_python.ProcessPlanningRequest_plan_profile_remapping_set, doc=r"""
    This allows the remapping of the Plan Profile identified in the command language to a specific profile for a
    given motion planner. (Optional)
    """)
    composite_profile_remapping = property(_tesseract_process_managers_python.ProcessPlanningRequest_composite_profile_remapping_get, _tesseract_process_managers_python.ProcessPlanningRequest_composite_profile_remapping_set, doc=r"""
    This allows the remapping of the Composite Profile identified in the command language to a specific profile
...
TRAJOPT_IFOPT_PLANNER_NAME = cvar.TRAJOPT_IFOPT_PLANNER_NAME
...
class ProcessPlanningServer(object, metaclass=_SwigNonDynamicMeta):
    ...
    def run(self, request: "ProcessPlanningRequest") -> "std::shared_ptr< tesseract_planning::ProcessPlanningFuture >":
        return _tesseract_process_managers_python.ProcessPlanningServer_run(self, request)

    def waitForAll(self) -> "void":
        r""" Wait for all process currently being executed to finish before returning"""
        return _tesseract_process_managers_python.ProcessPlanningServer_waitForAll(self)

    def enableTaskflowProfiling(self) -> "void":
        r""" This add a Taskflow profiling observer to the executor"""
        return _tesseract_process_managers_python.ProcessPlanningServer_enableTaskflowProfiling(self)

    def disableTaskflowProfiling(self) -> "void":
        r""" This remove the Taskflow profiling observer from the executor if one exists"""
        return _tesseract_process_managers_python.ProcessPlanningServer_disableTaskflowProfiling(self)
