...
def fit(
    ...
    **kwargs,
    ...
    :type seed: int
    :param n_inducing_points: Optional[int] = None,
    :type n_inducing_points: Optional[int]
...
def transfer_to_reference(
    ...
    **kwargs,
    ...
    :type inference_method: Union[Literal["exact", "variational"],
     List[Literal["exact", "variational"]],Dict[str, Literal["exact", "variational"]]]
    :param n_inducing_points: number of inducing points to use in variational inference.
     Defaults to None, which will render 10% of observations in observed data if n_obs < 1e5
     else 10000.
    ...
    for k, _adata in enumerate(adatas):
        ...
        for f, feature in enumerate(features):
            ...
            if inference_method[names[k]] == "exact":
                    ...
                    if n_obs > 1e5:
                        ...
                        if n_obs < 5e3:
                            print(
                                ...
                                "inference on a small data set ({} observations)".format(
                                    n_obs
                                ),
                            )
                ...
                print(batch_size, n_obs)

                if batch_size is None:
                    batch_size = pow2[np.argmax(n_obs < pow2) - 1]
                else:
                    assert (
                        batch_size < n_obs
