...
from Archiver.Extension import Extension
from Archiver.FileName import FileName
from Archiver.PathName import PathName
from Archiver.Type import Type
from Archiver.FolderName import FolderName
import Archiver.Archive as Archive
...
class Bdd:
    ...
    def createDbTables(self) -> bool:
        ...
        Retourne:
            ...
            Bool : False si la création a échoué
        """
        try:
            self.connect = sqlite3.connect(self.pathFile)
            self.cursor = self.connect.cursor()
    ...
    def pathNameExist(self, pathName : PathName) -> bool:
        ...
        """
        self.cursor.execute("select pathName from pathNames where pathName = ?;", (pathName.path,))
    ...
    def folderNameExist(self, folder : FolderName) -> bool:
        ...
        """
        self.cursor.execute("select name from folderNames where name = ?;", (folder.name,))
        if self.cursor.fetchone() is None:
            return False
        return True
    ...
    def fileNameExist(self, fileName : FileName) -> bool:
        ...
        """
        self.cursor.execute("select name from fileNames where name = ?;", (fileName.name,))
        if self.cursor.fetchone() is None:
            return False
        return True
    ...
    def extensionExist(self, extension : Extension) -> bool:
        ...
        """
        self.cursor.execute("select name from extensions where name = ?;", (extension.name,))
        if self.cursor.fetchone() is None:
            return False
        return True
    ...
    def typeExist(self, type : Type) -> bool:
        ...
        """
        self.cursor.execute("select name from Types where name = ?;", (type.name,))
        if self.cursor.fetchone() is None:
            return False
        return True
    ...
    def folderMetadataExist(self, folder : FolderName, m : Metadata) -> bool:
        ...
        """
        self.cursor.execute("select folderName_id, metadata_id from folderMetadata where folderName_id = ? and metadata_id = ?;", (folder.id,m.id))
        if self.cursor.fetchone() is None:
            return False
        return True
    #endregion
    ...
    def addType(self, type : Type) -> bool:
        ...
        try:
            if not self.typeExist(type):
                self.cursor.execute("insert into types (name) values (?);", (type.name,))
                self.connect.commit()
    ...
    def addExtension(self, extension : Extension) -> bool:
        ...
        try:
            if not self.extensionExist(extension):
                self.cursor.execute("insert into extensions (name) values (?);", (extension.name,))
                self.connect.commit()
    ...
    def addFileName(self, fileName : FileName) -> bool:
        ...
        try:
            if not self.fileNameExist(fileName):
                self.cursor.execute("insert into fileNames (name) values (?);", (fileName.name,))
                self.connect.commit()
    ...
    def addFolderNameIdMetadataId(self, folderId : int, mId : int) -> bool:
        ...
        """
        try:
            self.cursor.execute("insert into folderMetadata (folderName_id, metadata_id) values (?,?);", (folderId,mId))
            self.connect.commit()
            return True
        except sqlite3.Error as e:
            print(e)
    ...
    def addFolderName(self, folder : FolderName) -> bool:
        ...
        try:
            if not self.folderNameExist(folder):
                self.cursor.execute("insert into folderNames (name) values (?);", ([folder.name,]))
                self.connect.commit()
    ...
    def addFolderMetadata(self, f : FolderName, m : Metadata) -> bool:
        ...
        try:
            if not self.folderMetadataExist(f,m):
                self.cursor.execute("insert into folderMetadata (folderName_id, metadata_id) values (?,?);", (f.id,m.id))
                self.connect.commit()
            return True
        except sqlite3.Error as e:
            print(e)
    ...
    def addPathName(self, pathName : PathName) -> bool:
        ...
        try:
            if not self.pathNameExist(pathName):
                self.cursor.execute("insert into pathNames (pathName) values (?);", (pathName.path,))
                self.connect.commit()
    ...
    def addResource(self,resource : Resource) -> bool:
    ...
    def addMetadatas(self,r : Resource) -> bool:
        ...
        """
        try:
            for m in r.allMetadatas:
                if not self.metadataExist(m):
                    self.addMetadata(m)
                    self.connect.commit()
            return True
        except sqlite3.Error as e:
    ...
    def addMetadata(self, metadata : Metadata) -> bool:
        ...
        Retourne:
            bool: True si la metadata a été ajoutée
            bool: False si la metadata n'a pas été ajoutée
        """
        try:
            if not self.metadataExist(metadata):
                self.cursor.execute("insert into metadatas (fileName_id, extension_id, creationDate, modificationDate, sizeCompressed, sizeUncompressed, pathName_id, createdAt, sha1) values (?,?,?,?,?,?,?,?,?);",
                 (metadata.fileName.id, metadata.extension.id, metadata.creationDate, metadata.modificationDate, metadata.sizeCompressed, metadata.sizeUncompressed, metadata.pathName.id, metadata.createAt, metadata.sha1))
                self.connect.commit()
    ...
    def getFolderNameByName(self, folder : FolderName) -> FolderName:
        ...
        """
        try:
            self.cursor.execute("select * from folderNames where name = ?", (folder.name,))
    ...
    def getFileNameByName(self, fileName : FileName) -> FileName:
        ...
        """
        try:
            self.cursor.execute("select id from fileNames where name = ?;", (fileName.name,))
    ...
    def getPathNameByPath(self, pathName : PathName) -> PathName:
        ...
        """
        try:
            self.cursor.execute("select id from pathNames where pathName = ?;", (pathName.path,))
    ...
    def getTypeByName(self, type : Type) -> Type:
        ...
        """
        try:
            self.cursor.execute("select id, name from types where name = ?;", (type.name,))
    ...
    def getExtensionByName(self, extension : Extension) -> Extension:
        ...
        """
        try:
            self.cursor.execute("select id, name from extensions where name = ?;", (extension.name,))
            arrValue = self.cursor.fetchone()
            id = arrValue[0]
            name  = arrValue[1]
    ...
    def getAllMetadata(self) -> list:
        """Permet de récupérer toutes les métadonnées
            
        Retourne:
            list: la liste de toutes les métadonnées
        """
        result = pd.read_sql_query("SELECT * from Metadatas;", self.connect)
        metadatas = []
        for index, row in result.iterrows():
            m : Metadata = Metadata(self.getFileNameById(FileName(row["fileName_id"],"")),self.getExtensionById(Extension(row["extension_id"], "")),row["creationDate"],row["modificationDate"],row["sizeCompressed"],row["sizeUncompressed"],self.getPathNameById(PathName(row["pathName_id"],"")),row["sha1"],row["id"],row["createdAt"])
            metadatas.append(m)
        return metadatas
    ...
    def getMetadataById(self, id : int) -> Metadata:
        ...
        """
        try:
            self.cursor.execute("select * from Metadatas where id = ?", (id,))
            result = self.cursor.fetchone()
            if result != None:
                return Metadata(self.getFileNameById(FileName(result[1],"")),self.getExtensionById(Extension(result[2],"")),result[3],result[4],result[5],result[6],self.getPathNameById(PathName(result[7],"")),result[9],result[0],result[8])
            else:
                return False
        except sqlite3.Error as e:
            print(e)
    ...
    def getAllFolderNamePathName(self) -> list:
        """Permet de récupérer tous les noms de dossier et de chemin
        
        Retourne:
            list: la liste de tous les noms de dossier et de chemin
        """
        try:
            folderNamesPathNames = []
            self.cursor.execute("select * from folderNamePathName")
            result = self.cursor.fetchall()
            for r in result:
                arrMF = [self.getFolderNameById(FolderName(r[0], None)), self.getPathNameById(PathName(r[1], None))]
    ...
    def getAllFolderMetadata(self) -> list:
        ...
        try:
            folderMetadatas = []
            self.cursor.execute("select * from folderMetadata")
            result = self.cursor.fetchall()
            for r in result:
                arrMF = [self.getMetadataById(r[0]), self.getFolderNameById(FolderName(r[1], None))]
    ...
    def getAllResources(self) -> list:
        ...
        """
        resources = []
        result = pd.read_sql_query("select * from Resources;", self.connect)
        for index, row in result.iterrows():
            r : Resource = Resource(row["sha1"], True)
            r.type = self.getTypeById(Type(row["type_id"], ""))
            resources.append(r)
                    
        return resources

    def getAllResourcesWithMetadata(self, archive : Archive) -> list:
        """Permet de récupérer tous les ressources avec leurs métadonnées
        
        Paramètres:
            archive (Archive): l'archive à laquelle sont liées les métadonnées
            
        Retourne:
            list: la liste de tous les ressources avec leurs métadonnées"""
        resources = self.getAllResources()
        for r in resources:
            r.allMetadatas = self.getMetadatasBySha1(r.sha1)
    ...
    def getResourceBySha1WithMetadataById(self, sha1, id) -> Resource:
        ...
        sql = ("SELECT r.sha1,t.id as type_id, t.name as 'type', m.id,m.fileName_id, fn2.name as fileName,m.extension_id, e.name as extension, m.pathName_id,pn.pathName,m.creationDate, m.modificationDate, m.sizeCompressed, m.sizeUncompressed,m.createdAt"
               " FROM Resources r"
               " JOIN types t ON t.id = r.type_id"
               " JOIN metadatas m on m.sha1 = r.sha1"
               " JOIN pathNames pn on pn.id = m.pathName_id"
               " JOIN folderMetadata fm on fm.metadata_id = m.id"
               " JOIN extensions e on e.id = m.extension_id"
               " JOIN fileNames fn2 on fn2.id = m.fileName_id"
               " WHERE r.sha1 = ? AND m.id = ?")
        ...
        for index, row in result.iterrows():
            ...
            r : Resource = Resource(row["sha1"], True, m, Type(row["type_id"], row["type"]))
    ...
    def search(self, options : dict) -> list:
        ...
        """
        sql = ("SELECT r.sha1, m.id,m.fileName_id, fn2.name as fileName,m.extension_id, e.name as extension, m.pathName_id,pn.pathName,m.creationDate, m.modificationDate, m.sizeCompressed, m.sizeUncompressed,m.createdAt,t.id as type_id, t.name as 'type', fn.name as folder"
       " FROM resources r"
       " JOIN types t ON t.id = r.type_id"
       " JOIN metadatas m on m.sha1 = r.sha1"
       " JOIN pathNames pn on pn.id = m.pathName_id"
       " JOIN folderMetadata fm on fm.metadata_id = m.id"
       " JOIN folderNames fn on fn.id = fm.folderName_id"
       " JOIN extensions e on e.id = m.extension_id"
       " JOIN fileNames fn2 on fn2.id = m.fileName_id")
        resources = []
        params = []
        for option in options:
            match option:
                case "fileName":
                    sql += " WHERE fn2.name LIKE ? AND"
                    params.append('%'+options[option]+'%')
                case "extension":
                    sql += " WHERE m.extension LIKE ? AND"
                    params.append('%'+options[option]+'%')
                case "creationDateStart":
                    sql += " WHERE creationDate BETWEEN ? AND ? AND"
                    params.append(Metadata.convertDateInTimeStamp(options[option]))
                    if "creationDateEnd" in options:
                        params.append(Metadata.convertDateInTimeStamp(options["creationDateEnd"]))
                    else:
                        today = date.today()
                        today += timedelta(days=1)
                        params.append(Metadata.convertDateInTimeStamp(today.strftime("%Y/%m/%d")))
                case "modificationDateStart":
                    sql += " WHERE modificationDate BETWEEN ? AND ? AND"
                    params.append(Metadata.convertDateInTimeStamp(options[option]))
                    if "modificationDateEnd" in options:
                        params.append(Metadata.convertDateInTimeStamp(options["modificationDateEnd"]))
                    else:
                        today = date.today()
                        today += timedelta(days=1)
                        params.append(Metadata.convertDateInTimeStamp(today.strftime("%Y/%m/%d")))
                case "sizeStart":
                    sql += " WHERE size BETWEEN ? AND ? AND"
                    if "sizeUnit" not in options:
                        raise Exception("sizeUnit is required")
                    ...
                    if "sizeEnd" in options:
                        ...
                        params.append(Metadata.convertSizeToOctet(99999,"TB"))
                    
                case "sha1":
                    sql += " WHERE m.sha1 = :sha1 AND"
                    params.append(options[option])
