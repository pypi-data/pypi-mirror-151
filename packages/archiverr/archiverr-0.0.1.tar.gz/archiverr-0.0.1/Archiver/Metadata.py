...
class Metadata():
    ...
    def __init__(self, fileName : FileName, extension : Extension, creationDate : int, modificationDate : int, sizeCompressed : int, sizeUncompressed : int, pathName : PathName,sha1 : str, id : int = None, createdAt : int = None) -> None:
        ...
        if createdAt is None:
            self.createAt : datetime = datetime.now().timestamp()
        else:
            self.createAt : datetime =  createdAt 
        self.sha1 : str = sha1
        pass
    ...
    def getCreationDate(p) -> int:
        ...
        Retourne:
            - la date de création du fichier en format 
        '''
        file = stat(p)
        try:
...
'''
TO DO:
    - Ajouter des métadonnées spécifiques pour chaque type de fichier
'''
class MetadataJPG(Metadata):
    pass

class MetadataMOV(Metadata):
    pass

class MetadataMP3(Metadata):
    pass