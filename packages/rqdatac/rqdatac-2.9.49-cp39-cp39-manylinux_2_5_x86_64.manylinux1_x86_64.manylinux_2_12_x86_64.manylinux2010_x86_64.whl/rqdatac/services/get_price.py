...
def get_price(
    ...
    :param time_slice: 可选参数。获取分钟线或tick数据时，仅返回指定时间段的数据。类型为(str, str) 或 (datetime.time, datetime.time)
            如：("09:50", "10:11") 或 (datetime.time(9, 50), datetime.time(10, 11))
    ...
    if time_slice:
        ...
        if not isinstance(time_slice, tuple) or len(time_slice) != 2:
            raise ValueError("time_slice: invalid, expect tuple value like ('09:55', '10:11'), got {}".format(time_slice))
        start, end = _parse_time(time_slice[0]), _parse_time(time_slice[1])
    ...
    if not sliceable or not time_slice:
...
def _parse_time(value):
    if isinstance(value, datetime.time):
        ...
        # 如果解析失败，会抛出 ValueError
        return datetime.time.fromisoformat(value)
    else:
        raise TypeError("Invalid time_slice {}, expect type of str or datetime.time, got {}".format(value, type(value)))
...
def get_tick_price_multi_df(order_book_ids, start_date, end_date, fields, market):
    ...
    if "Future" in types or "Option" in types or "Spot" in types:
        ...
        ) or ('CS' in types) or ('LOF' in types) or ('ETF' in types) or ('Convertible' in types):
    ...
    history_earliest_date = 0 if history_df is None else date_to_int8(get_current_trading_date(
        history_df.index.get_level_values(1).min()))
    today = today_int()
    next_trading_date = date_to_int8(get_next_trading_date(today, market=market))
    if ((history_latest_date >= end_date or start_date > next_trading_date) and
        history_earliest_date == history_latest_date) or end_date < today:
        return history_df
    ...
    if end_date >= next_trading_date and (start_date > today or history_latest_date >= today):
        ...
        live_date = today
    if history_latest_date >= live_date and history_earliest_date == history_latest_date:
        return history_df
    live_dfs = []
    for ins in ins_list:
        try:
            if history_df is not None and \
                    date_to_int8(get_current_trading_date(history_df.loc[ins.order_book_id].index.max())) == history_latest_date:
                continue
        except KeyError:
            pass
        try:
