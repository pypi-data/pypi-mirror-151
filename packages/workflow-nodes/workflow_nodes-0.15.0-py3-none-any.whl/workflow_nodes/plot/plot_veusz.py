...
@option(
    ...
    char="i",
    required=True,
    description="Name of the inputfile including import path to the file, header has to"
...
@option(
    ...
    char="p",
    required=True,
    description="Resulting plot, the format is controlled by file extension: *.bmp,"
    " *.eps, *.jpg, *.pdf, *.png *.svg, *.tiff, *.xpm",
)
@option(
    "columns",
    char="u",
...
@option(
    "output",
    char="o",
    description="Text file with Veusz commands, that can be used for reopening with the"
    " Veusz GUI.",
)
...
@option("keylabel", char="l", description="Key label for the curve")
@option(
    "font",
    char="s",
    default="cmr10",
...
def plot_veusz(
    pathfile, plotoutput, columns, output, parameter_x, parameter_y, keylabel, font
):
    ...
    new_linearray = []
    with open(pathfile, encoding="utf-8") as textfile:
        ...
        if headerrow.startswith("descriptor"):
            ...
            sys.exit(
                "Header row of the inputfile has to start with descriptor; use the"
                " 'FileConverter' node."
            )
        textfile.close()
    ...
    if columns is not None and (parameter_x is None and parameter_y is None):
        ...
        if parameter_x in new_linearray:
            ...
            click.echo(
                f"Specified x parameter: {parameter_x} has not been found in the"
                " inputfile columns:"
            )
            click.echo(new_linearray)
            sys.exit(1)
    ...
    with open(f"{output}.vsz", mode="w", encoding="utf-8") as f:
    ...
    with open(f"{output}.vsz", encoding="utf-8") as f:
        subprocess.run(["veusz", "--listen", "--quiet"], stdin=f)
