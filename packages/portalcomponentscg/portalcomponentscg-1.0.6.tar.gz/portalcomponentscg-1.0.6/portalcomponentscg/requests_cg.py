...
from portalcomponentscg.const import USERNAME_LOGIN, PASSWORD_LOGIN, URL_LOGIN, URL_API_POST_FOLHA_DE_PAGAMENTOS
...
def get_cg(url):
    ...
    key = requests.post(URL_LOGIN, data=params)
...
def patch_cg(url, json_i):
    ...
    try:
        ...
        x = requests.patch(url=url, data=json.dumps(json_i),
                           headers=headers, verify=True)
    except:
        pass


def patch_file_cg(url, path):
    ...
    try:
        r = requests.patch(url, headers=headers, files=file_ob)
...
def delete_cg_generic(url, id, dict_inform):
    ...
    try:
        x = requests.patch(url=f'{URL_API_POST_FOLHA_DE_PAGAMENTOS}{return_inform_format["id"]}/',
