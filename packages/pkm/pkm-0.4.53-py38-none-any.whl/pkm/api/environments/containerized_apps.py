...
class ContainerizedApplications:
    ...
    def _install(
        ...
        with temp_dir() as tdir, CopyTransaction() as ct:
            ...
            actual_app = Project(deepcopy(app.config))
            actual_app.config.project = replace(actual_app.config.project, dependencies=app_config.dependencies)
            actual_app.config.pkm_application = None
            WheelDistribution(app.descriptor, build_wheel(actual_app, tdir / "whl", editable=editable)) \
