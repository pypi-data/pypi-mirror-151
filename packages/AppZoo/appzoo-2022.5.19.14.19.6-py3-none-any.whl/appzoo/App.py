...
class App(object):
    ...
    def __init__(self, verbose=True):
        ...
        self.verbose = verbose  # 是否返回请求体 # todo删除
    ...
    def add_route_plus(self, path='/xxx', func=lambda x='demo': x, method="GET", **kwargs):
    ...
    def _handler_plus(self, func, result_key='data', **kwargs):
        """
        ...
        async def handler(request: Request, file: UploadFile = File(...)):  # todo: 兼容其他请求，比如 request.form()
            body_input = await request.json()
            io = BytesIO(await file.read())  # 字节型
            query_input = request.query_params._dict
            input = {**query_input, **body_input}
            ...
            logger.info(f"body_input: {body_input}")
            logger.info(f"input: {input}")

            duplicate_key = query_input.keys() & body_input.keys()

            if duplicate_key:
                logger.warning(f"Duplicate key:{duplicate_key}")
    ...
    def _try_func(self, kwargs, func, result_key='data', **kwargs_):
        __debug = 1 if kwargs.pop('__debug', 0) else 0
