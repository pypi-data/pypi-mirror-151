...
from .accessory import pdag2dag, to_bnlearn, g_test, z_test, find_causes, pairwise, dag2cpdag, \
    independence_test, orient_v_structure, orient_propagation
...
def hc(data, method='standard', score_function='default', debug=False):
    '''
    ...
    if method == 'lw':
        ...
        while diff > 0:
            ...
            cache_dag.append({var: {'par': [v for v in dag[var]['par']], 'nei': [v for v in dag[var]['nei']]} for var in dag})
        dag = to_bnlearn(dag)
        ...
        cache = {var: {tuple([]): local_score(data, [varnames.index(var)], score_function)} for var in varnames}
...
def ges(data, method='complete', score_function='default', prune=True, thres=0.1, debug=False):
    '''
