...
def bic_counter(data, arities, cols, weight=None):
    ...
    for i in range(stride):
        if N_ijk[i] != 0:
            bic += N_ijk[i] * np.log10(N_ijk[i] / N_ij[i])
    bic -= 0.5 * np.log10(data.shape[0]) * (arities[cols[0]] - 1) * strides[0]
    return bic


def nal(data, cols, weight=None):
    arities = np.amax(data, axis=0) + 1
    return nal_counter(data, arities, cols)
...
@njit(fastmath=True)
def nal_counter(data, arities, cols, alpha=0.3, samplesize=None):
    if samplesize == None:
    ...
    nal -= 1 / data.shape[1] * np.power(samplesize, -alpha) * (arities[cols[0]] - 1) * strides[0]
    return nal
...
def local_score(data, cols, score_function='default', weights=None, alpha=0.3):
    '''
    ...
    if len(data) == 0:
        ...
        if score_function in ['nal', 'nal_g']:
            ls = globals()[score_function](data, np.asarray(cols), alpha)
        else:
            try:
