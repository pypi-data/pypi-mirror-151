...
def command(command_group: Group,
    ...
    def decorator(handler: Callable):
        ...
        command = command_group.command(command_name)(handle_invocation)
        ...
        for param_name, param in handler_signature.parameters.items():
            ...
            if param_name in argument_spec_map:
                ...
                _logger.debug(f'C/{command}: SPEC: {spec}')
                ...
                _logger.debug(f'C/{command}: {"OPT" if spec.as_option else "ARG"} ({input_names}, {additional_specs})')

                (click.option if spec.as_option else click.argument)(*input_names, **additional_specs)(command)
            else:
                click.argument(param_name, **additional_specs)(command)
        ...
        _logger.debug(f'C/{command}: Setup complete')
