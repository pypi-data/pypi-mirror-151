...
class OAuth2Authenticator(Authenticator, ABC):
    def __init__(self,
        ...
        self._auth_info = OAuth2Authentication(**auth_info)
    ...
    def authenticate(self) -> SessionInfo:
        ...
        adapter = self._adapter_factory.get_from(self._auth_info)

        if not adapter:
            raise OAuth2MisconfigurationError('Cannot determine the type of authentication '
                                              f'({self._auth_info.json(sort_keys=True, exclude_none=True)})')
        ...
        self._session_manager.save(self.session_id, self._session_info)

        return self._session_info

    def refresh(self) -> SessionInfo:
        session_info = self._session_info or self._session_manager.restore(self.session_id)
    ...
    def _restore_session(self) -> Optional[SessionInfo]:
        session = self._session_info or self._session_manager.restore(self.session_id)

        if not session:
            raise AuthenticationRequired()
        elif session.is_valid():
            current_config_hash = self._endpoint.get_content_hash()
    ...
    def _convert_token_response_to_session(self,
        ...
        return SessionInfo(
            model_version=4,
            config_hash=self._endpoint.get_content_hash(),
