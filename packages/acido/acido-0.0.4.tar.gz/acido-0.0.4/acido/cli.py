...
from acido.utils.functions import chunks, exec_command, jpath, expanduser
...
class Acido(object):
    ...
    def __init__(self, rg: str = None, login: bool = True):
        ...
        try:
            ...
            print(bad('FATAL ERROR: Could not find the user assigned identity "acido".'))
            sys.exit()
    ...
    def save_output(self, command: list[str] = None):
    ...
    def exec(self, command, max_retries=60, input_cmd: str = None):
                ...
                if cg in self.selected_instances:
                    for cont in containers:
                        ...
                        result = pool.apply_async(exec_command, 
                                                (self.rg, cg, cont, 'pip install acido', max_retries), 
                                                callback=instances_outputs)
        ...
        for c, o in global_responses.items():
            if o:
    ...
    def scan(self, command: list[str] = None, instance_num: int = 3, fleet_name: str = 'acido', interactive=False):
        response = {}
        if instance_num > 10:
            instance_num_groups = list(chunks(range(1, instance_num + 1), 10))
            for cg_n, ins_num in enumerate(instance_num_groups):
            ...
            response[fleet_name] = self.instance_manager.deploy(command=command, name=fleet_name, instance_number=instance_num, env_vars={'INSTANCE_NAME': f'{fleet_name}-{instance_num:02d}'})
    ...
    def setup(self):
        rg = input(info('Please provide a Resource Group Name to deploy the ACIs: '))
        self.rg = rg
        image_registry_server = input(info('Image Registry Server: '))
        image_registry_username = input(info('Image Registry Username: '))
        image_registry_password = input(info('Image Registry Password: '))
        self.selected_instances = []
...
if __name__ == "__main__":
    ...
    if args.interactive:
        code.interact(banner='acido 0.0.1', local=locals())