...
class Acido(object):
    ...
    def exec_command(self, cg, cont, command, max_retries):
        env = os.environ.copy()
        env["PATH"] = "/usr/sbin:/sbin:" + env["PATH"]
        subprocess.Popen(["tmux", "new-session", "-d", "-s", cont], env=env)
        time.sleep(1)
        subprocess.Popen(["tmux", "send-keys", "-t", cont,
                        f"az container exec -g {self.rg} -n {cg} --container-name {cont} --exec-command /bin/bash", "Enter",
                        ], env=env)
        time.sleep(15)
        ...
        if not failed:
            ...
            try:
                ...
                output = output.decode().split('Executed command: ')[1].strip()
                print(output)
            except Exception as e:
                output = None
        ...
        for cg, containers in self.instances_named.items():
            if cg in self.selected_instances:
                for cont in containers:
                    executed = True
                    result = pool.apply_async(self.exec_command, 
