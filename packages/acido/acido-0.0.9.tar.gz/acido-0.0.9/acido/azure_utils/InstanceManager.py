...
class InstanceManager(ManagedAuthentication):
    ...
    def provision(
        self, name, memory: float = 1, cpu: float = 1, ports=None,
            command=['/bin/sh' ,'-c', f'python3 -m pip install acido=={__version__} && sleep infinity'], image = 'merabytes.azurecr.io/kali:latest',
            env_vars: dict = {}
    ):
        env = {}
    ...
    def set_variable(self, key: str, value: str):
        self.env_vars[key] = EnvironmentVariable(
            name=key,
            value=value
        )

    def set_variables(self, d: dict):
        for k, v in d.items():
    ...
    def deploy(
        ...
        for i_num in range(1, instance_number + 1):
            deploy_instances.append(self.provision(f'{name}-{i_num:02d}', memory=float(max_ram), cpu=float(max_cpu), env_vars=env_vars))
