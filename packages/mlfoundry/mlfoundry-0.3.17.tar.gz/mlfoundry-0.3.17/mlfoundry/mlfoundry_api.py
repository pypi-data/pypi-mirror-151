...
def init_rest_tracking(tracking_uri: str, api_key: Optional[str]):
    ...
    session = Session(auth_service=auth_service)
...
class MlFoundry:
    ...
    def _get_or_create_project(self, project_name: str, owner: Optional[str]) -> int:
    ...
    def create_run(
        ...
        **kwargs,
        ...
        logger.info(f"Run is created with id {mlf_run_id} and name {run.info.name}")
        return mlf_run
