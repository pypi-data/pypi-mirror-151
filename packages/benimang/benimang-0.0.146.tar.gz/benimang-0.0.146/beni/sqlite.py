...
class SqliteDbPool():
    ...
    def _release_dbread(self, db: _SqliteDbRead):
...
class _SqliteDbWrite(_SqliteDbRead):
    ...
    async def insert_many(self, table: str, data_list: Sequence[dict[str, Any]]):
        ...
        fieldname_list = ','.join([f'`{x}`' for x in keylist])
        placement_list = ','.join(['?' for _ in range(len(keylist))])
        fieldvalue_list = [[data.get(key) for key in keylist] for data in data_list]
