...
def visualize_tsne_phenotypes(dataset, IndexAndClass,clusters):
    ...
    def tsne_with_pheno_thrshold(dataset,features_idx,features,pheno_assignmnents):
        ...
        for ph in range(pheno_assignmnents.shape[1]):
            thr = np.percentile(pheno_assignmnents[:,ph],90)
...
def load_patch_image_for_prototypes(dataset,imList):
    '''
    Load patch image
    # Create label map. 
    labels = []
...
def topk_confident_patches(dataset,n_cell_types,
    ...
    # Create label map. 
    labels,labels_list = load_patch_image_for_prototypes(dataset,imList)
...
def save_heatmap_raw_and_normalized(filename, heatmap, TME_names,Colormap,Marker_Names):    
    y_ticklabels = TME_names
    c_map = Colormap[:heatmap.shape[0]]
    # Figure Heatmap Raw Values
    plt.close('all')
    plt.figure()
    sns.set(font_scale=0.9)
    cluster=True if (heatmap.sum(1)!=0).sum()>1 else False
    h_E_Fig = sns.clustermap(heatmap,col_cluster=cluster, row_cluster=cluster, row_colors=c_map,xticklabels=Marker_Names,yticklabels=y_ticklabels,vmin=-1,vmax=1, linewidths=0.5, cmap="bwr")            
    h_E_Fig.savefig(filename+'_Raw.png',dpi=600) 

    # Save Excel.
    df = pd.DataFrame(heatmap, columns = Marker_Names)
    df.index = y_ticklabels
    df.to_excel(filename+'_Raw.xlsx')  

    # Figure Heatmap Min is 0 and max is 1 Values    
    h_E_COL_MinMax = heatmap - heatmap.min(0,keepdims=True)
    h_E_COL_MinMax = h_E_COL_MinMax/h_E_COL_MinMax.max(0,keepdims=True)
    h_E_COL_MinMax[np.isnan(h_E_COL_MinMax)] = 0 
    plt.close('all')
    plt.figure()
    sns.set(font_scale=0.9)
    h_E_Fig = sns.clustermap(h_E_COL_MinMax,col_cluster=cluster, 
    ...
    h_E_COL_Norm = stats.zscore(heatmap,axis=0)  
    h_E_COL_Norm[np.isnan(h_E_COL_Norm)] = 0              
    plt.close('all')
    plt.figure()
    sns.set(font_scale=0.9)    
    h_E_Fig = sns.clustermap(h_E_COL_Norm,col_cluster=cluster, vmin=-1, vmax=1, row_cluster=cluster, row_colors=c_map,xticklabels=Marker_Names,yticklabels=y_ticklabels, linewidths=0.5, cmap="bwr")            
    h_E_Fig.savefig(filename+'_Norm.png',dpi=600) 

    # Figure Heatmap z-scored values
    h_E_COL_Norm = stats.zscore(heatmap,axis=1)  
    h_E_COL_Norm[np.isnan(h_E_COL_Norm)] = 0              
    plt.close('all')
    plt.figure()
    sns.set(font_scale=0.9)        
...
def Area_to_Neighborhood_to_Phenotype(dataset,clusters,IndexAndClass, ClusteringThreshold):
    ...
    plt.close('all')
    plt.figure()
    Neigh_to_Pheno[Neigh_to_Pheno==0] = np.random.rand(Neigh_to_Pheno[Neigh_to_Pheno==0].shape[0])*1e-5
