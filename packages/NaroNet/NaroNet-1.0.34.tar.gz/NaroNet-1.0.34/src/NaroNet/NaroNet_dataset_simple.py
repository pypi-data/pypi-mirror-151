...
class NARODataset_simple(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 pre_transform=None,
                 pre_filter=None):
        super(NARODataset_simple, self).__init__()
...
class NaroNet_dataset_simple(torch.utils.data.Dataset):
    '''
    ...
    def __init__(self, root, patch_size, transform=None, pre_transform=None, recalculate=False, superPatchEmbedding=None, experiment_label=None):
        ...
        self.raw_dir_Ph = osp.join(osp.join(self.root,'Patch_Contrastive_Learning'),'Image_Patch_Representation_Ph')
        self.raw_dir_Nb = osp.join(osp.join(self.root,'Patch_Contrastive_Learning'),'Image_Patch_Representation_Nb')
    ...
    def getStatistics(self, size):
        ...
        return min(last_data.num_total_nodes,750000), last_data.edge_index_total, last_data.num_features, last_data.mean_STD, last_data.IndexAndClass, last_data.Percentile, last_data.num_classes, last_data.name_labels
    ...
    def gatherOneDataAugment(self,num_total_nodes,num_features,dataNOW,countIter,device,name_labels,y_ALL,x_ALL,edge_index_ALL,num_nodes,i, model_type, k_hops, dataAugmentationPerc):
        ...
        # print('Num. total nodes:'+str(num_total_nodes)+' nodes in graph:'+str(dataNOW.x.shape[0]))
        # Initialize tensors
        ...
        x[:min(dataNOW.x.shape[0],x.shape[0]),:dataNOW.x.shape[1]]+=dataNOW.x[:min(dataNOW.x.shape[0],x.shape[0]),:dataNOW.x.shape[1]]        
        ...
        if normalizeAdjMat:
            ...
            norm = self.normalizeAdjacency(dataNOW.edge_index,min(dataNOW.x.shape[0],x.shape[1]))
    ...
    def process(self):
        ...
        for root, dirs, files in os.walk(self.raw_dir_Ph):
            files.sort()
            for file_index in tqdm(range(len(files)),ascii=True,desc='NaroNet: generating enriched graphs'):
                fullpath_Ph = (osp.join(self.raw_dir_Ph,files[file_index]))                                
                fullpath_Nb = (osp.join(self.raw_dir_Nb,files[file_index]))                                
                    
                # Handle Superpatches npy files
                if '.npy' in fullpath_Ph:
                    file_Ph = np.load(fullpath_Ph,allow_pickle=True)
                    file_Nb = np.load(fullpath_Nb,allow_pickle=True)
                elif '.txt' in fullpath_Ph:
                    file = np.loadtxt(fullpath_Ph,delimiter=' ')[:,2:]                    
                else:
                    continue
                    
                # Obtain edge List
                maxValues = file_Ph[:,[0,1]].max(-2)+1
                minValues = file_Ph[:,[0,1]].min(-2)-1
                ...
                for indexX in range(int(minValues[0]),int(maxValues[0])+ImageSize,ImageSize):
                    for indexY in range(int(minValues[1]),int(maxValues[1])+ImageSize,ImageSize):
                        ...
                        Truex = np.logical_and((indexX+ImageSize)>file_Nb[:,[0]], file_Nb[:,[0]]>indexX)
                        Truey = np.logical_and((indexY+ImageSize)>file_Nb[:,[1]], file_Nb[:,[1]]>indexY)
                        ...
                        edge_index = radius_graph(x=torch.tensor(file_Nb[np.squeeze(Truexy),:][:,[0,1]]), r=self.context_size, loop=True, max_num_neighbors=10)  
                        ...
                        x_Nb = torch.from_numpy(file_Nb[np.squeeze(Truexy),:][:,file_Nb.shape[1]-self.SuperPatchEmbedding:])                                                
                        x_Ph = torch.from_numpy(file_Ph[np.squeeze(Truexy),:][:,file_Ph.shape[1]-self.SuperPatchEmbedding:])                                                
                        x = torch.cat((x_Ph,x_Nb),1)
                        y = patient_label
    ...
    def clusterPresence(self, clusters, IndexAndClass, num_classes,ClusterThreshold, isTraining):
        ...
        if not isTraining:
            save_celltype_abundance_per_patient(self,IndexAndClass, clusters, heatmapClusterPresence, labels[str(clusters[0])], 
                                                ['Phenotypes','Neighborhoods','Areas'],['P','N','A'])
            h_all = {}
            ...
            save_celltype_abundance_per_patient(self,IndexAndClass, [sum(clusters)], h_all, labels[str(clusters[0])], ['ALL'],['X'])
    ...
    def ObtainPhenoExamples(self,IndexAndClass,clusters,Thresholds):
        ...
        CropConfPheno, CropConfTissueComm, TissueArea_Conf = select_patches_from_cohort(self,IndexAndClass,clusters)                  
        
        Channels, Marker_Names = utilz.load_channels(self.root+'Raw_Data/')

        # # Extract topk patches of phenotypes
        # heatmapMarkerExpression, heatmap_MarkerColocalization, heatmap_CellSize_circularity = extract_topk_patches_from_cohort(self, CropConfPheno, [Marker_Names[c] for c in Channels],'Phenotypes',Thresholds)
        # save_heatMapMarker_and_barplot(self, heatmapMarkerExpression, heatmap_MarkerColocalization, heatmap_CellSize_circularity,CropConfPheno,[Marker_Names[c] for c in Channels],'Phenotypes',Thresholds)

        # # Extract topk patches of neighborhoods
        # heatmapMarkerExpression, heatmap_MarkerColocalization, heatmap_CellSize_circularity = extract_topk_patches_from_cohort(self, CropConfTissueComm, [Marker_Names[c] for c in Channels],'Neighborhoods',Thresholds)
        # save_heatMapMarker_and_barplot(self, heatmapMarkerExpression, heatmap_MarkerColocalization, heatmap_CellSize_circularity,CropConfTissueComm,[Marker_Names[c] for c in Channels],'Neighborhoods',Thresholds)

        # Extract topk patients of Tissue areas
        TissueArea_in_image(self,TissueArea_Conf,IndexAndClass,15)
    ...
    def visualize_results(self,model, clusters, IndexAndClass, num_classes,mean_STD, args):        
        ...
        # if ('GBM' in self.raw_dir) or ('KIRC' in self.raw_dir):
        ...
        #     self.ObtainPhenoExamples(IndexAndClass,clusters,[25,30,50,75,90])                
        
        # Cell-types abundances
        heatmapClusterPresence, labels = self.clusterPresence(clusters, IndexAndClass, num_classes,75, isTraining=False)                        
        ...
        All_TMEs_in_Image(self, clusters, [], [i for i in IndexAndClass if '6B' in i[0]], num_classes, args['isAttentionLayer'],0)        
