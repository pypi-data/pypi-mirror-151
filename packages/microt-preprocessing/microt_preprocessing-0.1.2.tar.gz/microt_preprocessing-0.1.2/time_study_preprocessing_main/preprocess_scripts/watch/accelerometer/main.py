...
from .parse import *
from .motion_decompose_minute import *
from .motion_decompose_hour import *
...
def pre_process(microT_root_path, intermediate_file_save_path, p_id, date):
    ...
    if path.exists(area_folder_path):
        ...
        if path.exists(date_folder_path):
            ...
            day_file_name_motion_decompose_minute = file_save_date_path + sep + device + "_" + file_shortname + "_decompose_minute_" + date + ".csv"
            day_file_name_motion_decompose_hour = file_save_date_path + sep + device + "_" + file_shortname + "_decompose_hour_" + date + ".csv"
            ...
            decompose_exits = True
            if not path.exists(day_file_name_motion_decompose_minute) or not path.exists(day_file_name_motion_decompose_hour):
                decompose_exits = False

            if not mims_dest_exists or not har_dest_all_exist or not decompose_exits:
                ...
                df_motion_minute = get_motion_decompose_matrix_minute(df_participant_mims)
                ...
                if df_motion_minute is not None:
                    df_motion_minute.to_csv(day_file_name_motion_decompose_minute, index=False)
                    df_motion_hour = get_motion_decompose_matrix_hour(df_motion_minute)
                    df_motion_hour.to_csv(day_file_name_motion_decompose_hour, index=False)
