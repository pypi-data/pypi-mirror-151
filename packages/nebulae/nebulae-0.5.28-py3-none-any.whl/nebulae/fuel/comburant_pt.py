...
class Random(ABC):
    ...
    def __call__(self, imgs):
        if not isinstance(self.p, (tuple, list)):
            ...
            cbr = np.random.choice(self.cbr, size=1, replace=False, p=self.p)
            return cbr(imgs)
...
class Crop(ABC):
    ...
    def __call__(self, imgs, p=None, s=None, zipped=False):
        if p is not None:
            param = p
        else:
            if isinstance(imgs, abc.Sequence):
        ...
        if isinstance(imgs, abc.Sequence):
            ...
            if isinstance(imgs[0], abc.Sequence):
                basket = []
                for i, v in enumerate(zip(*imgs)):
                    basket.append(self(v, param, scale, True))
                for r in zip(*basket):
                    ret.append(r)
            else:
                if zipped:
                    for i, v in enumerate(imgs):
                        ret.append(self.call(v, param, scale[i]))
                else:
                    for i, v in enumerate(imgs):
...
class Blur(ABC):
    ...
    def __call__(self, imgs):
        ...
        if isinstance(imgs, abc.Sequence):
            ret = []
            for i in imgs:
                ret.append(self.call(i, radius))
        else:
            ret = self.call(imgs, radius)
...
class Sharpen(ABC):
    ...
    def __call__(self, imgs):
        if isinstance(self.factor, tuple):
            factor = rand.randint(self.factor[0], self.factor[1])
        else:
            factor = self.factor
        if isinstance(imgs, abc.Sequence):
            ret = []
...
class JPEG(ABC):
    def __init__(self, quality):
        ...
        if isinstance(quality, tuple):
            ...
            raise TypeError('NEBULAE ERROR ⨷ a valid quality should be an integer or a tuple.')
    ...
    def __call__(self, imgs):
        ...
        if isinstance(imgs, abc.Sequence):
            ret = []
            for i in imgs:
                ret.append(self.call(i, qlt))
        else:
            ret = self.call(imgs, qlt)
