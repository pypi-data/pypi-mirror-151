...
GetFieldDefFn = Callable[[GraphQLSchema, GraphQLType, FieldNode], Optional[GraphQLField]]
...
class TypeInfo:
    ...
    def enter_selection_set(self, node: SelectionSetNode) -> None:
        ...
        self._parent_type_stack.append(
            cast(GraphQLCompositeType, named_type) if is_composite_type(named_type) else None
        )
    ...
    def enter_inline_fragment(self, node: InlineFragmentNode) -> None:
        ...
        output_type = (
            type_from_ast(self._schema, type_condition_ast) if type_condition_ast else get_named_type(self.get_type())
        )
        self._type_stack.append(cast(GraphQLOutputType, output_type) if is_output_type(output_type) else None)
    ...
    def enter_variable_definition(self, node: VariableDefinitionNode) -> None:
        ...
        self._input_type_stack.append(cast(GraphQLInputType, input_type) if is_input_type(input_type) else None)
    ...
    def enter_argument(self, node: ArgumentNode) -> None:
        ...
        self._default_value_stack.append(arg_def.default_value if arg_def else Undefined)
    ...
    def enter_list_value(self, node: ListValueNode) -> None:
        ...
        item_type = cast(GraphQLList, list_type).of_type if is_list_type(list_type) else list_type
    ...
    def enter_object_field(self, node: ObjectFieldNode) -> None:
        ...
        if is_input_object_type(object_type):
            input_field = cast(GraphQLInputObjectType, object_type).fields.get(node.name.value)
            input_field_type = input_field.type if input_field else None
        else:
            input_field = input_field_type = None
        self._default_value_stack.append(input_field.default_value if input_field else Undefined)
        self._input_type_stack.append(input_field_type if is_input_type(input_field_type) else None)
...
def get_field_def(schema: GraphQLSchema, parent_type: GraphQLType, field_node: FieldNode) -> Optional[GraphQLField]:
