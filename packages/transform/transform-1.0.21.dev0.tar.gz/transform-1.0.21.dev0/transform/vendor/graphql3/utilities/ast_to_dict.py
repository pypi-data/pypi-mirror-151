...
def ast_to_dict(node: Any, locations: bool = False, cache: Optional[Dict[Node, Any]] = None) -> Any:
    ...
    if isinstance(node, Node):
        if cache is None:
            ...
            return cache[node]
        cache[node] = res = {}
        res.update({key: ast_to_dict(getattr(node, key), locations, cache) for key in ("kind",) + node.keys[1:]})
        if locations:
            loc = node.loc
            if loc:
