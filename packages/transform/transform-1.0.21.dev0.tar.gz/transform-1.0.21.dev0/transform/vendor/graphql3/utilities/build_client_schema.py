...
def build_client_schema(introspection: IntrospectionQuery, assume_valid: bool = False) -> GraphQLSchema:
    ...
    def build_implementations_list(
        implementing_introspection: Union[IntrospectionObjectType, IntrospectionInterfaceType],
        ...
        if maybe_interfaces is None:
            ...
            if implementing_introspection["kind"] == TypeKind.INTERFACE.name:
                return []
            raise TypeError("Introspection result missing interfaces:" f" {inspect(implementing_introspection)}.")
    ...
    def build_union_def(
        ...
        if maybe_possible_types is None:
            raise TypeError("Introspection result missing possibleTypes:" f" {inspect(union_introspection)}.")
    ...
    def build_enum_def(enum_introspection: IntrospectionEnumType) -> GraphQLEnumType:
        if enum_introspection.get("enumValues") is None:
            raise TypeError("Introspection result missing enumValues:" f" {inspect(enum_introspection)}.")
    ...
    def build_input_object_def(
        ...
        if input_object_introspection.get("inputFields") is None:
            raise TypeError("Introspection result missing inputFields:" f" {inspect(input_object_introspection)}.")
    ...
    def build_field_def_map(
        ...
        if type_introspection.get("fields") is None:
            raise TypeError(f"Introspection result missing fields: {type_introspection}.")
        return {
    ...
    def build_field(field_introspection: IntrospectionField) -> GraphQLField:
        ...
        if not is_output_type(type_):
            raise TypeError("Introspection must provide output type for fields," f" but received: {inspect(type_)}.")
        type_ = cast(GraphQLOutputType, type_)

        args_introspection = field_introspection.get("args")
        if args_introspection is None:
            raise TypeError("Introspection result missing field args:" f" {inspect(field_introspection)}.")
    ...
    def build_argument(
        ...
        if not is_input_type(type_):
            raise TypeError("Introspection must provide input type for arguments," f" but received: {inspect(type_)}.")
    ...
    def build_input_value_def_map(
        ...
        return {
            input_value_introspection["name"]: build_input_value(input_value_introspection)
    ...
    def build_input_value(
        ...
        if not is_input_type(type_):
            raise TypeError(
                "Introspection must provide input type for input fields," f" but received: {inspect(type_)}."
            )
        type_ = cast(GraphQLInputType, type_)
    ...
    def build_directive(
        ...
        if directive_introspection.get("args") is None:
            raise TypeError("Introspection result missing directive args:" f" {inspect(directive_introspection)}.")
        if directive_introspection.get("locations") is None:
            raise TypeError("Introspection result missing directive locations:" f" {inspect(directive_introspection)}.")
    ...
    for std_type_name, std_type in chain(specified_scalar_types.items(), introspection_types.items()):
    ...
    mutation_type = None if mutation_type_ref is None else get_object_type(mutation_type_ref)
    subscription_type_ref = schema_introspection.get("subscriptionType")
    subscription_type = None if subscription_type_ref is None else get_object_type(subscription_type_ref)
    ...
    directives = (
        [build_directive(directive_introspection) for directive_introspection in directive_introspections]
