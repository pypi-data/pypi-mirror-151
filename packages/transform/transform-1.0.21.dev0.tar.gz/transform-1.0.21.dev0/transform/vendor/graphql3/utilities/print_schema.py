...
def print_object(type_: GraphQLObjectType) -> str:
    return print_description(type_) + f"type {type_.name}" + print_implemented_interfaces(type_) + print_fields(type_)


def print_interface(type_: GraphQLInterfaceType) -> str:
    return (
        print_description(type_) + f"interface {type_.name}" + print_implemented_interfaces(type_) + print_fields(type_)
    )
...
def print_enum(type_: GraphQLEnumType) -> str:
    values = [
        print_description(value, "  ", not i) + f"  {name}" + print_deprecated(value.deprecation_reason)
...
def print_args(args: Dict[str, GraphQLArgument], indentation: str = "") -> str:
    ...
    return (
        "(\n"
        + "\n".join(
            print_description(arg, f"  {indentation}", not i) + f"  {indentation}" + print_input_value(name, arg)
...
def print_description(
    ...
    block_string = print_ast(StringValueNode(value=description, block=is_printable_as_block_string(description)))
