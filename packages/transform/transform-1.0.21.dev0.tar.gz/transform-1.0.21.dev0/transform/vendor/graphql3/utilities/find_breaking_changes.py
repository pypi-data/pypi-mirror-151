...
def find_breaking_changes(old_schema: GraphQLSchema, new_schema: GraphQLSchema) -> List[BreakingChange]:
    ...
    """
    return [
        change for change in find_schema_changes(old_schema, new_schema) if isinstance(change.type, BreakingChangeType)
    ]
...
def find_dangerous_changes(old_schema: GraphQLSchema, new_schema: GraphQLSchema) -> List[DangerousChange]:
    ...
    """
    return [
        change for change in find_schema_changes(old_schema, new_schema) if isinstance(change.type, DangerousChangeType)
    ]
            ...
            if is_required_argument(new_arg):
                schema_changes.append(
                    BreakingChange(
                        ...
                        f"A required arg {arg_name} on directive" f" {old_directive.name} was added.",
                    )
                )
    ...
    for type_name, old_type in types_diff.removed.items():
        schema_changes.append(
            BreakingChange(
                ...
                f"Standard scalar {type_name} was removed" " because it is not referenced anymore."
        ...
        if is_enum_type(old_type) and is_enum_type(new_type):
            ...
            schema_changes.append(
                BreakingChange(
                    ...
                    f"{type_name} changed from {type_kind_name(old_type)}" f" to {type_kind_name(new_type)}.",
                )
            )
        ...
        if is_required_input_field(new_field):
            schema_changes.append(
                BreakingChange(
                    ...
                    f"A required field {field_name} on" f" input type {old_type.name} was added.",
                )
            )
        else:
            schema_changes.append(
                DangerousChange(
                    ...
                    f"An optional field {field_name} on" f" input type {old_type.name} was added.",
                )
            )
        ...
        if not is_safe:
            schema_changes.append(
                BreakingChange(
                    ...
                    f"{old_type.name}.{field_name} changed type" f" from {old_field.type} to {new_field.type}.",
                )
            )
...
def find_enum_type_changes(old_type: GraphQLEnumType, new_type: GraphQLEnumType) -> List[Change]:
    schema_changes: List[Change] = []
            ...
            if new_arg.default_value is Undefined:
                schema_changes.append(
                    DangerousChange(
                        ...
                        f"{old_type.name}.{field_name} arg" f" {arg_name} defaultValue was removed.",
                    )
                )
            else:
        ...
        if is_required_argument(new_arg):
            schema_changes.append(
                BreakingChange(
                    ...
                    f"A required arg {arg_name} on" f" {old_type.name}.{field_name} was added.",
                )
            )
        else:
            schema_changes.append(
                DangerousChange(
                    ...
                    f"An optional arg {arg_name} on" f" {old_type.name}.{field_name} was added.",
                )
            )
...
def is_change_safe_for_object_or_interface_field(old_type: GraphQLType, new_type: GraphQLType) -> bool:
    if is_list_type(old_type):
        return (
            ...
            and is_change_safe_for_object_or_interface_field(old_type, cast(GraphQLNonNull, new_type).of_type)
        )

    if is_non_null_type(old_type):
        # if they're both non-null, make sure underlying types are compatible
        return is_non_null_type(new_type) and is_change_safe_for_object_or_interface_field(
    ...
    return (
        ...
        and cast(GraphQLNamedType, old_type).name == cast(GraphQLNamedType, new_type).name
    ) or (
        # moving from nullable to non-null of same underlying type is safe
        is_non_null_type(new_type)
        and is_change_safe_for_object_or_interface_field(old_type, cast(GraphQLNonNull, new_type).of_type)
...
def is_change_safe_for_input_object_field_or_field_arg(old_type: GraphQLType, new_type: GraphQLType) -> bool:
    ...
    if is_non_null_type(old_type):
        return (
            ...
            and is_change_safe_for_input_object_field_or_field_arg(cast(GraphQLNonNull, old_type).of_type, new_type)
        )
...
def stringify_value(value: Any, type_: GraphQLInputType) -> str:
    ...
    class SortVisitor(Visitor):
        @staticmethod
        def enter_object_value(object_value_node: ObjectValueNode, *_args: Any) -> ObjectValueNode:
