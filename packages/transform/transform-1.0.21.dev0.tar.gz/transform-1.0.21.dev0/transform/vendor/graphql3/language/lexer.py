...
class Lexer:
    ...
    def print_code_point_at(self, location: int) -> str:
        ...
        # Unicode code point
        point = ord(
            body[location : location + 2].encode("utf-16", "surrogatepass").decode("utf-16")
            if is_supplementary_code_point(body, location)
            else char
        )
        return f"U+{point:04X}"

    def create_token(self, kind: TokenKind, start: int, end: int, value: Optional[str] = None) -> Token:
        ...
        while position < body_length:
            ...
            message = (
                ...
                if char == "'"
                    ...
                    if is_unicode_scalar_value(char) or is_supplementary_code_point(body, position)
    ...
    def read_number(self, start: int, first_char: str) -> Token:
        ...
        if char == "0":
            ...
            if is_digit(char):
                raise GraphQLSyntaxError(
                    self.source,
                    position,
                    "Invalid number, unexpected digit after 0:" f" {self.print_code_point_at(position)}.",
                )
        else:
        ...
        if char and (char == "." or is_name_start(char)):
            raise GraphQLSyntaxError(
                self.source,
                position,
                "Invalid number, expected digit but got:" f" {self.print_code_point_at(position)}.",
            )
    ...
    def read_digits(self, start: int, first_char: str) -> int:
        ...
        if not is_digit(first_char):
            raise GraphQLSyntaxError(
                self.source,
                start,
                "Invalid number, expected digit but got:" f" {self.print_code_point_at(start)}.",
            )
        ...
        while position < body_length:
            ...
            if is_unicode_scalar_value(char):
                ...
                raise GraphQLSyntaxError(
                    self.source,
                    position,
                    "Invalid character within String:" f" {self.print_code_point_at(position)}.",
                )
    ...
    def read_escaped_unicode_variable_width(self, position: int) -> EscapeSequence:
        ...
        while size < max_size:
            ...
            if char == "}":
                ...
                if size < 5 or not (0 <= point <= 0xD7FF or 0xE000 <= point <= 0x10FFFF):
                    break
            ...
            if body[position + 6 : position + 8] == "\\u":
                ...
                if 0xDC00 <= trailing_code <= 0xDFFF:
                    return EscapeSequence(
                        (chr(code) + chr(trailing_code)).encode("utf-16", "surrogatepass").decode("utf-16"),
                        12,
                    )
...
def is_supplementary_code_point(body: str, location: int) -> bool:
    ...
    """
    try:
        return "\ud800" <= body[location] <= "\udbff" and "\udc00" <= body[location + 1] <= "\udfff"
    except IndexError:
        return False
