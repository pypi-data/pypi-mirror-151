...
def reason_message(reason: "ConflictReasonMessage") -> str:
    if isinstance(reason, list):
        return " and ".join(
            f"subfields '{response_name}' conflict" f" because {reason_message(sub_reason)}"
...
def find_conflict(
    ...
    are_mutually_exclusive = parent_fields_are_mutually_exclusive or (
        parent_type1 != parent_type2 and is_object_type(parent_type1) and is_object_type(parent_type2)
    )
...
def same_arguments(arguments1: Collection[ArgumentNode], arguments2: Collection[ArgumentNode]) -> bool:
...
def do_types_conflict(type1: GraphQLOutputType, type2: GraphQLOutputType) -> bool:
    ...
    if is_list_type(type1):
        return (
            do_types_conflict(cast(GraphQLList, type1).of_type, cast(GraphQLList, type2).of_type)
    ...
    if is_non_null_type(type1):
        return (
            do_types_conflict(cast(GraphQLNonNull, type1).of_type, cast(GraphQLNonNull, type2).of_type)
