...
class UniqueArgumentDefinitionNamesRule(SDLValidationRule):
    ...
    def enter_interface_type_extension(self, node: InterfaceTypeExtensionNode, *_args: Any) -> VisitorAction:
        return self.check_arg_uniqueness_per_field(node.name, node.fields)
    ...
    def check_arg_uniqueness(
        ...
        for arg_name, arg_nodes in seen_args.items():
            if len(arg_nodes) > 1:
                self.report_error(
                    GraphQLError(
                        f"Argument '{parent_name}({arg_name}:)'" " can only be defined once.",
