...
def ast_from_serialized_value_untyped(serialized: Any) -> Optional[ValueNode]:
    ...
    if isinstance(serialized, Mapping):
        field_items = ((key, ast_from_serialized_value_untyped(value)) for key, value in serialized.items())
        field_nodes = tuple(
...
def ast_from_value(value: Any, type_: GraphQLInputType) -> Optional[ValueNode]:
    ...
    if is_non_null_type(type_):
        ...
        if isinstance(ast_value, NullValueNode):
            raise GraphQLError("Received Null value for a Non-Null type " f"{inspect(inner_type)}.")
        return ast_value
...
def dsl_gql(*operations: "DSLExecutable", **operations_with_name: "DSLExecutable") -> DocumentNode:
    ...
    return DocumentNode(definitions=[operation.executable_ast for operation in all_operations])
...
class DSLSchema:
    ...
    def __init__(self, schema: GraphQLSchema):
        ...
        if not isinstance(schema, GraphQLSchema):
            raise TypeError(f"DSLSchema needs a schema as parameter. Received: {type(schema)}")
...
class DSLSelector(ABC):
    ...
    def is_valid_field(self, field: "DSLSelectable") -> bool:
        raise NotImplementedError("Any DSLSelector subclass must have a is_valid_field method")  # pragma: no cover
    ...
    def select(
        ...
        **fields_with_alias: "DSLSelectableWithAlias",
        ...
        added_fields: Tuple["DSLSelectable", ...] = DSLField.get_aliased_fields(fields, fields_with_alias)

        # Check that each field is valid
        for field in added_fields:
            if not isinstance(field, DSLSelectable):
                raise TypeError("Fields should be instances of DSLSelectable. " f"Received: {type(field)}")
        ...
        added_selections: Tuple[Union[FieldNode, InlineFragmentNode, FragmentSpreadNode], ...] = tuple(
...
class DSLExecutable(DSLSelector):
    ...
    def executable_ast(self):
        ...
        raise NotImplementedError("Any DSLExecutable subclass must have executable_ast property")  # pragma: no cover
...
class DSLVariable:
    ...
    def to_ast_type(self, type_: Union[GraphQLWrappingType, GraphQLNamedType]) -> TypeNode:
    ...
    def set_type(self, type_: Union[GraphQLWrappingType, GraphQLNamedType]) -> "DSLVariable":
...
class DSLType:
    ...
    def __getattr__(self, name: str) -> "DSLField":
        ...
        if name in self._type.fields:
            ...
            raise AttributeError(f"Field {name} does not exist in type {self._type.name}.")
...
class DSLField(DSLSelectableWithAlias, DSLFieldSelector):
    ...
    def select(self, *fields: "DSLSelectable", **fields_with_alias: "DSLSelectableWithAlias") -> "DSLField":
...
class DSLMetaField(DSLField):
    ...
    meta_type = GraphQLObjectType(
        ...
        fields={
            ...
            "__schema": GraphQLField(cast(GraphQLObjectType, introspection_types["__Schema"])),
            "__type": GraphQLField(
...
class DSLInlineFragment(DSLSelectable, DSLFragmentSelector):
    ...
    def select(self, *fields: "DSLSelectable", **fields_with_alias: "DSLSelectableWithAlias") -> "DSLInlineFragment":
        """Calling :meth:`select <gql.dsl.DSLSelector.select>` method with
        corrected typing hints
        """
        super().select(*fields, **fields_with_alias)
    ...
    def on(self, type_condition: DSLType) -> "DSLInlineFragment":
        ...
        self.ast_field.type_condition = NamedTypeNode(name=NameNode(value=self._type.name))
        return self
...
class DSLFragment(DSLSelectable, DSLFragmentSelector, DSLExecutable):
    ...
    def select(self, *fields: "DSLSelectable", **fields_with_alias: "DSLSelectableWithAlias") -> "DSLFragment":
        """Calling :meth:`select <gql.dsl.DSLSelector.select>` method with
        corrected typing hints
        """
        if self._type is None:
            raise AttributeError("Missing type condition. Please use .on(type_condition) method")
