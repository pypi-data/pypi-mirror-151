...
import websockets
from graphql3 import DocumentNode, ExecutionResult
...
class WebsocketsTransportBase(AsyncTransport):
    ...
    async def _send(self, message: str) -> None:
        ...
        if not self.websocket:
            raise TransportClosed("Transport is not connected") from self.close_exception
        ...
        try:
            while True:
                ...
                try:
                    ...
                    assert isinstance(e.query_id, int), "TransportQueryError should have a query_id defined here"
                    try:
    ...
    async def subscribe(
        ...
        query_id: int = await self._send_query(document, variable_values, operation_name)
        ...
        try:
            ...
            while True:
                ...
                # Then we will end this async generator output without errors
                    ...
                    log.debug(f"Complete received for query {query_id} --> exit without error")
                    break
    ...
    async def execute(
        ...
        generator = self.subscribe(document, variable_values, operation_name, send_stop=False)
        ...
        if first_result is None:
            raise TransportQueryError("Query completed without any answer received from the server")
    ...
    async def connect(self) -> None:
        ...
        if self.websocket is None and not self._connecting:
            ...
            # through keep-alive messages
            if self.keep_alive_timeout is not None:
                self.check_keep_alive_task = asyncio.ensure_future(self._check_ws_liveness())
    ...
    async def _fail(self, e: Exception, clean_close: bool = True) -> None:
        ...
        if self.close_task is None:
            ...
            if self.websocket is None:
                ...
                self.close_task = asyncio.shield(asyncio.ensure_future(self._close_coro(e, clean_close=clean_close)))
        else:
            log.debug(
