...
def extend_schema(
    ...
    return schema if schema_kwargs is extended_kwargs else GraphQLSchema(**extended_kwargs)
...
def extend_schema_impl(
    ...
    if not type_extensions_map and not type_defs and not directive_defs and not schema_extensions and not schema_def:
    ...
    def extend_object_type(type_: GraphQLObjectType) -> GraphQLObjectType:
        ...
        return GraphQLObjectType(
            **merge_kwargs(
                kwargs,
                interfaces=lambda: [
                    cast(GraphQLInterfaceType, replace_named_type(interface)) for interface in kwargs["interfaces"]
                ]
                + build_interfaces(extensions),
                fields=lambda: {
                    **{name: extend_field(field) for name, field in kwargs["fields"].items()},
    ...
    def extend_union_type(type_: GraphQLUnionType) -> GraphQLUnionType:
        ...
        return GraphQLUnionType(
            **merge_kwargs(
                kwargs,
                types=lambda: [
                    cast(GraphQLObjectType, replace_named_type(member_type)) for member_type in kwargs["types"]
                ]
    ...
    def build_input_field_map(
        nodes: Collection[Union[InputObjectTypeDefinitionNode, InputObjectTypeExtensionNode]],
    ) -> GraphQLInputFieldMap:
    ...
    def build_interface_type(
        ...
        extension_nodes = type_extensions_map[ast_node.name.value]
        all_nodes: List[Union[InterfaceTypeDefinitionNode, InterfaceTypeExtensionNode]] = [ast_node, *extension_nodes]
        return GraphQLInterfaceType(
            name=ast_node.name.value,
            description=ast_node.description.value if ast_node.description else None,
            interfaces=lambda: build_interfaces(all_nodes),
    ...
    def build_input_object_type(
        ...
        extension_nodes = type_extensions_map[ast_node.name.value]
        all_nodes: List[Union[InputObjectTypeDefinitionNode, InputObjectTypeExtensionNode]] = [
            ast_node,
            *extension_nodes,
        ]
        return GraphQLInputObjectType(
    ...
    def build_type(ast_node: TypeDefinitionNode) -> GraphQLNamedType:
        try:
            ...
            raise TypeError(f"Unexpected type definition node: {inspect(ast_node)}.")  # pragma: no cover
        else:
    ...
    return GraphQLSchemaKwargs(
        ...
        directives=tuple(replace_directive(directive) for directive in schema_kwargs["directives"])
        + tuple(build_directive(directive) for directive in directive_defs),
        description=schema_def.description.value if schema_def and schema_def.description else None,
        extensions={},
        ast_node=schema_def or schema_kwargs["ast_node"],
        extension_ast_nodes=schema_kwargs["extension_ast_nodes"] + tuple(schema_extensions),
...
def get_specified_by_url(node: Union[ScalarTypeDefinitionNode, ScalarTypeExtensionNode]) -> Optional[str]:
