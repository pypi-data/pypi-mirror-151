...
def default_on_error(path: List[Union[str, int]], invalid_value: Any, error: GraphQLError) -> None:
...
def coerce_input_value(
    ...
    if is_non_null_type(type_):
        ...
        on_error(
            ...
            input_value,
            GraphQLError(f"Expected non-nullable type '{inspect(type_)}' not to be None."),
        )
        return Undefined
    ...
    if is_list_type(type_):
        ...
        if is_iterable(input_value):
            ...
            for index, item_value in enumerate(input_value):
                append_item(coerce_input_value(item_value, item_type, on_error, Path(path, index, None)))
            return coerced_list
            ...
            if field_value is Undefined:
                if field.default_value is not Undefined:
                    ...
                    on_error(
                        path.as_list() if path else [],
                        input_value,
                        GraphQLError(f"Field '{field_name}' of required type '{type_str}'" " was not provided."),
                    )
                continue
        ...
        for field_name in input_value:
            if field_name not in fields:
                ...
                on_error(
                    ...
                    input_value,
                    GraphQLError(
                        f"Field '{field_name}' is not defined by type '{type_.name}'." + did_you_mean(suggestions)
                    ),
                )
    ...
    if is_leaf_type(type_):
        ...
        try:
            ...
            on_error(
                path.as_list() if path else [],
                input_value,
                GraphQLError(f"Expected type '{type_.name}'. {error}", original_error=error),
            )
            return Undefined
