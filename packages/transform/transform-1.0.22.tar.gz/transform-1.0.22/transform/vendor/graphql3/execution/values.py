...
def get_variable_values(
    ...
    def on_error(error: GraphQLError) -> None:
        if max_errors is not None and len(errors) >= max_errors:
            raise GraphQLError("Too many errors processing variables," " error limit reached. Execution aborted.")
        errors.append(error)
    ...
    for var_def_node in var_def_nodes:
        ...
        if var_name not in inputs:
            if var_def_node.default_value:
                ...
                on_error(
                    GraphQLError(
                        f"Variable '${var_name}' of required type '{var_type_str}'" " was not provided.",
                        var_def_node,
                    )
                )
            continue
        ...
        if value is None and is_non_null_type(var_type):
            ...
            on_error(
                GraphQLError(
                    f"Variable '${var_name}' of non-null type '{var_type_str}'" " must not be null.",
                    var_def_node,
                )
            )
            continue
