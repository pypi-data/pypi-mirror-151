    ...
    def enter_field(self, node: FieldNode, *_args: Any) -> None:
        ...
        if field_def:
            ...
            if deprecation_reason is not None:
                ...
                self.report_error(
                    GraphQLError(
                        f"The field {parent_name}.{node.name.value}" f" is deprecated. {deprecation_reason}",
                        node,
                    )
                )
        ...
        if is_input_object_type(input_object_def):
            ...
            if input_field_def:
                ...
                if deprecation_reason is not None:
                    ...
                    self.report_error(
                        GraphQLError(
                            f"The input field {input_object_name}.{field_name}" f" is deprecated. {deprecation_reason}",
                            node,
                        )
                    )
    ...
    def enter_enum_value(self, node: EnumValueNode, *_args: Any) -> None:
        ...
        if enum_value_def:
            ...
            if deprecation_reason is not None:  # pragma: no cover else
                ...
                self.report_error(
                    GraphQLError(
                        f"The enum value '{enum_type_name}.{node.value}'" f" is deprecated. {deprecation_reason}",
                        node,
                    )
                )
