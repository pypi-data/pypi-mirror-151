...
class ValuesOfCorrectTypeRule(ValidationRule):
    ...
    def enter_object_value(self, node: ObjectValueNode, *_args: Any) -> VisitorAction:
        ...
        for field_name, field_def in type_.fields.items():
            ...
            if not field_node and is_required_input_field(field_def):
                ...
                self.report_error(
                    GraphQLError(
                        f"Field '{type_.name}.{field_name}' of required type" f" '{field_type}' was not provided.",
                        node,
                    )
                )
        return None
        ...
        if not field_type and is_input_object_type(parent_type):
            ...
            self.report_error(
                GraphQLError(
                    ...
                    f" is not defined by type '{parent_type.name}'." + did_you_mean(suggestions),
                    node,
                )
            )
    ...
    def is_valid_value_node(self, node: ValueNode) -> None:
        ...
        if not is_leaf_type(type_):
            self.report_error(
                GraphQLError(
                    f"Expected value of type '{location_type}'," f" found {print_ast(node)}.",
                    node,
                )
            )
            return
        ...
        try:
            ...
            self.report_error(
                GraphQLError(
                    f"Expected value of type '{location_type}'," f" found {print_ast(node)}; {error}",
                    node,
