...
class PossibleTypeExtensionsRule(SDLValidationRule):
    ...
    def __init__(self, context: SDLValidationContext):
        ...
        self.defined_types = {
            def_.name.value: def_ for def_ in context.document.definitions if isinstance(def_, TypeDefinitionNode)
        }
    ...
    def check_extension(self, node: TypeExtensionNode, *_args: Any) -> None:
        ...
        if expected_kind:
            ...
            self.report_error(
                GraphQLError(
                    f"Cannot extend type '{type_name}' because it is not defined." + did_you_mean(suggested_types),
                    node.name,
                )
            )
