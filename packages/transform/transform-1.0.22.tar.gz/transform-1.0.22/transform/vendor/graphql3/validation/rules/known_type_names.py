    ...
    def enter_named_type(
        ...
        if type_name not in self.existing_types_map and type_name not in self.defined_types:
            ...
            suggested_types = suggestion_list(
                type_name,
                list(standard_type_names) + self.type_names if is_sdl else self.type_names,
            )
            self.report_error(
                GraphQLError(
...
def is_sdl_node(value: Union[Node, Collection[Node], None]) -> bool:
    return (
        ...
        and (is_type_system_definition_node(cast(Node, value)) or is_type_system_extension_node(cast(Node, value)))
    )
