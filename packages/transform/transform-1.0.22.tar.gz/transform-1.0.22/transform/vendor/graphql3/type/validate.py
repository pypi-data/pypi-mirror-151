...
class SchemaValidationContext:
    ...
    def validate_root_types(self) -> None:
        ...
        if not query_type:
            ...
            self.report_error(
                ...
                get_operation_type_node(schema, OperationType.QUERY) or query_type.ast_node,
            )

        mutation_type = schema.mutation_type
        if mutation_type and not is_object_type(mutation_type):
            self.report_error(
                "Mutation root type must be Object type if provided," f" it cannot be {mutation_type}.",
                get_operation_type_node(schema, OperationType.MUTATION) or mutation_type.ast_node,
            )

        subscription_type = schema.subscription_type
        if subscription_type and not is_object_type(subscription_type):
            self.report_error(
                "Subscription root type must be Object type if provided," f" it cannot be {subscription_type}.",
                get_operation_type_node(schema, OperationType.SUBSCRIPTION) or subscription_type.ast_node,
            )
        ...
        for directive in directives:
            ...
            for arg_name, arg in directive.args.items():
                ...
                if is_required_argument(arg) and arg.deprecation_reason is not None:
                    self.report_error(
                        f"Required argument @{directive.name}({arg_name}:)" " cannot be deprecated.",
                        [
    ...
    def validate_name(self, node: Any, name: Optional[str] = None) -> None:
        ...
        try:
            ...
            if name.startswith("__"):
                self.report_error(
                    f"Name {name!r} must not begin with '__'," " which is reserved by GraphQL introspection.",
                    ast_node,
                )
    ...
    def validate_fields(self, type_: Union[GraphQLObjectType, GraphQLInterfaceType]) -> None:
        ...
        for field_name, field in fields.items():
            ...
            if not is_output_type(field.type):
                self.report_error(
                    f"The type of {type_.name}.{field_name}" f" must be Output Type but got: {inspect(field.type)}.",
            ...
            for arg_name, arg in field.args.items():
                ...
                if is_required_argument(arg) and arg.deprecation_reason is not None:
                    self.report_error(
                        f"Required argument {type_.name}.{field_name}({arg_name}:)" " cannot be deprecated.",
                        [
                            get_deprecated_directive_node(arg.ast_node),
                            arg.ast_node and arg.ast_node.type,
                        ],
    ...
    def validate_interfaces(self, type_: Union[GraphQLObjectType, GraphQLInterfaceType]) -> None:
        ...
        for iface in type_.interfaces:
            if not is_interface_type(iface):
                self.report_error(
                    f"Type {type_.name} must only implement Interface" f" types, it cannot implement {inspect(iface)}.",
            ...
            if type_ is iface:
                self.report_error(
                    f"Type {type_.name} cannot implement itself" " because it would create a circular reference.",
                    get_all_implements_interface_nodes(type_, iface),
                )
    ...
    def validate_type_implements_interface(
        ...
        for field_name, iface_field in iface_fields.items():
            ...
            if not type_field:
                self.report_error(
                    f"Interface field {iface.name}.{field_name}" f" expected but {type_.name} does not provide it.",
                    [
    ...
    def validate_type_implements_ancestors(
        ...
        for transitive in iface_interfaces:
            if transitive not in type_interfaces:
                self.report_error(
                    f"Type {type_.name} cannot implement {iface.name}" " because it would create a circular reference."
        ...
        for member_type in member_types:
            if is_object_type(member_type):
                if member_type.name in included_type_names:
                    self.report_error(
                        f"Union type {union.name} can only include type" f" {member_type.name} once.",
    ...
    def validate_input_fields(self, input_obj: GraphQLInputObjectType) -> None:
        ...
        for field_name, field in fields.items():
            ...
            if not is_input_type(field.type):
                self.report_error(
                    f"The type of {input_obj.name}.{field_name}" f" must be Input Type but got: {inspect(field.type)}.",
                    field.ast_node.type if field.ast_node else None,
                )

            if is_required_input_field(field) and field.deprecation_reason is not None:
                self.report_error(
                    f"Required input field {input_obj.name}.{field_name}" " cannot be deprecated.",
                    [
...
def get_deprecated_directive_node(
    ...
    if directives:
        for directive in directives:
            if directive.name.value == GraphQLDeprecatedDirective.name:  # pragma: no cover else
                return directive
