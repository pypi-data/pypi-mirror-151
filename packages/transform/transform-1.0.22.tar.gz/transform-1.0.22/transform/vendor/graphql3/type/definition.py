    ...
    def __init__(
        ...
        if extension_ast_nodes:
            if not is_collection(extension_ast_nodes) or not all(
                ...
                raise TypeError(
                    f"{name} extension AST nodes must be specified" " as a collection of TypeExtensionNode instances."
                )
...
class GraphQLScalarType(GraphQLNamedType):
    ...
    def __init__(
        ...
        if specified_by_url is not None and not isinstance(specified_by_url, str):
            raise TypeError(
                f"{name} must provide 'specified_by_url' as a string," f" but got: {inspect(specified_by_url)}."
            )
        ...
        if parse_literal is not None and (
            not callable(parse_literal) or (parse_value is None or not callable(parse_value))
        ):
            raise TypeError(f"{name} must provide" " both 'parse_value' and 'parse_literal' as functions.")
        ...
        if extension_ast_nodes and not all(isinstance(node, ScalarTypeExtensionNode) for node in extension_ast_nodes):
            raise TypeError(
                f"{name} extension AST nodes must be specified" " as a collection of ScalarTypeExtensionNode instances."
            )
    ...
    def parse_literal(self, node: ValueNode, variables: Optional[Dict[str, Any]] = None) -> Any:
    ...
    def to_kwargs(self) -> GraphQLScalarTypeKwargs:
        ...
        return GraphQLScalarTypeKwargs(  # type: ignore
            super().to_kwargs(),
            serialize=None if self.serialize is GraphQLScalarType.serialize else self.serialize,
            parse_value=None if self.parse_value is GraphQLScalarType.parse_value else self.parse_value,
            parse_literal=None
            if getattr(self.parse_literal, "__func__", None) is GraphQLScalarType.parse_literal
...
class GraphQLField:
    ...
    def __init__(
        ...
        elif not all(isinstance(value, GraphQLArgument) or is_input_type(value) for value in args.values()):
            raise TypeError("Field args must be GraphQLArguments or input type objects.")
        else:
            args = {
        ...
        if resolve is not None and not callable(resolve):
            raise TypeError("Field resolver must be a function if provided, " f" but got: {inspect(resolve)}.")
        if description is not None and not is_description(description):
            raise TypeError("The description must be a string.")
...
class GraphQLArgument:
    ...
    def __init__(
        ...
        if extensions is None:
            ...
            raise TypeError("Argument extensions must be a dictionary with string keys.")
...
class GraphQLObjectType(GraphQLNamedType):
    ...
    def __init__(
        ...
        if is_type_of is not None and not callable(is_type_of):
            raise TypeError(f"{name} must provide 'is_type_of' as a function," f" but got: {inspect(is_type_of)}.")
        ...
        if extension_ast_nodes and not all(isinstance(node, ObjectTypeExtensionNode) for node in extension_ast_nodes):
            raise TypeError(
                f"{name} extension AST nodes must be specified" " as a collection of ObjectTypeExtensionNode instances."
            )
        self._fields = fields
    ...
    def fields(self) -> GraphQLFieldMap:
        ...
        if not isinstance(fields, Mapping) or not all(isinstance(key, str) for key in fields):
            raise TypeError(f"{self.name} fields must be specified" " as a mapping with field names as keys.")
        if not all(isinstance(value, GraphQLField) or is_output_type(value) for value in fields.values()):
            raise TypeError(f"{self.name} fields must be GraphQLField or output type objects.")
        return {
    ...
    def interfaces(self) -> Tuple["GraphQLInterfaceType", ...]:
        ...
        try:
            interfaces: Collection["GraphQLInterfaceType"] = resolve_thunk(self._interfaces)  # type: ignore
        except Exception as error:
            cls = GraphQLError if isinstance(error, GraphQLError) else TypeError
            raise cls(f"{self.name} interfaces cannot be resolved. {error}") from error
        if interfaces is None:
            interfaces = ()
        elif not is_collection(interfaces) or not all(isinstance(value, GraphQLInterfaceType) for value in interfaces):
            raise TypeError(
                f"{self.name} interfaces must be specified" " as a collection of GraphQLInterfaceType instances."
            )
...
class GraphQLInterfaceType(GraphQLNamedType):
    ...
    def __init__(
        ...
        if resolve_type is not None and not callable(resolve_type):
            raise TypeError(f"{name} must provide 'resolve_type' as a function," f" but got: {inspect(resolve_type)}.")
...
class GraphQLUnionType(GraphQLNamedType):
    ...
    def __init__(
        ...
        if extension_ast_nodes and not all(isinstance(node, UnionTypeExtensionNode) for node in extension_ast_nodes):
            raise TypeError(
                f"{name} extension AST nodes must be specified" " as a collection of UnionTypeExtensionNode instances."
            )
        self._types = types
        self.resolve_type = resolve_type
    ...
    def types(self) -> Tuple[GraphQLObjectType, ...]:
        ...
        if types is None:
            types = ()
        elif not is_collection(types) or not all(isinstance(value, GraphQLObjectType) for value in types):
            raise TypeError(f"{self.name} types must be specified" " as a collection of GraphQLObjectType instances.")
        return tuple(types)
...
class GraphQLEnumType(GraphQLNamedType):
    ...
    def __init__(
        ...
        try:  # check for enum
            ...
            if not isinstance(values, Mapping) or not all(isinstance(name, str) for name in values):
                try:
                    ...
                    raise TypeError(f"{name} values must be an Enum or a mapping" " with value names as keys.")
        ...
        values = {
            assert_enum_value_name(key): value if isinstance(value, GraphQLEnumValue) else GraphQLEnumValue(value)
        ...
        if extension_ast_nodes and not all(isinstance(node, EnumTypeExtensionNode) for node in extension_ast_nodes):
            raise TypeError(
                f"{name} extension AST nodes must be specified" " as a collection of EnumTypeExtensionNode instances."
            )
        self.values = values

    def to_kwargs(self) -> GraphQLEnumTypeKwargs:
        # noinspection PyArgumentList
        return GraphQLEnumTypeKwargs(super().to_kwargs(), values=self.values.copy())  # type: ignore
        ...
        try:
            ...
            for enum_name, enum_value in self.values.items():
                if enum_value.value == output_value:
                    return enum_name
        raise GraphQLError(f"Enum '{self.name}' cannot represent value: {inspect(output_value)}")
...
class GraphQLInputObjectType(GraphQLNamedType):
    ...
    def __init__(
        ...
        if ast_node and not isinstance(ast_node, InputObjectTypeDefinitionNode):
            raise TypeError(f"{name} AST node must be an InputObjectTypeDefinitionNode.")
        if extension_ast_nodes and not all(
            isinstance(node, InputObjectTypeExtensionNode) for node in extension_ast_nodes
        ):
            raise TypeError(
                f"{name} extension AST nodes must be specified"
    ...
    def to_kwargs(self) -> GraphQLInputObjectTypeKwargs:
        ...
        return GraphQLInputObjectTypeKwargs(  # type: ignore
            super().to_kwargs(),
            fields=self.fields.copy(),
            out_type=None if self.out_type is GraphQLInputObjectType.out_type else self.out_type,
        )
    ...
    def fields(self) -> GraphQLInputFieldMap:
        ...
        if not isinstance(fields, Mapping) or not all(isinstance(key, str) for key in fields):
            raise TypeError(f"{self.name} fields must be specified" " as a mapping with field names as keys.")
        if not all(isinstance(value, GraphQLInputField) or is_input_type(value) for value in fields.values()):
            raise TypeError(f"{self.name} fields must be" " GraphQLInputField or input type objects.")
        return {
            assert_name(name): value
...
class GraphQLNonNull(GraphQLWrappingType[GNT], Generic[GNT]):
    ...
    def __init__(self, type_: GNT):
        ...
        if isinstance(type_, GraphQLNonNull):
            raise TypeError("Can only create NonNull of a Nullable GraphQLType but got:" f" {type_}.")
...
def get_nullable_type(type_: Optional[Union[GraphQLNullableType, GraphQLNonNull]]) -> Optional[GraphQLNullableType]:
...
GraphQLInputType = Union[GraphQLScalarType, GraphQLEnumType, GraphQLInputObjectType, GraphQLWrappingType]
...
GraphQLNamedInputType = Union[GraphQLScalarType, GraphQLEnumType, GraphQLInputObjectType]
