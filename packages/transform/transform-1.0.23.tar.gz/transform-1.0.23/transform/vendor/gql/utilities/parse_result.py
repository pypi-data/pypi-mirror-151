...
from graphql3.language.visitor import VisitorActionEnum
...
class ParseResultVisitor(Visitor):
    ...
    def enter_operation_definition(self, node: OperationDefinitionNode, *_args: Any) -> Union[None, VisitorActionEnum]:
    ...
    @staticmethod
    def leave_operation_definition(node: OperationDefinitionNode, *_args: Any) -> Dict[str, Any]:
    ...
    def enter_field(
        ...
        if self.current_result is None:
            ...
            if (
                ...
                assert isinstance(initial_type, (GraphQLObjectType, GraphQLInterfaceType))
        ...
        raise GraphQLError(f"Invalid result for container of field {name}: {self.current_result!r}")
    ...
    def leave_field(
        ...
        if self.current_result is None:
            ...
            return_value = {name: {k: v for pr in partial_results for k, v in pr.items()}}
    ...
    def enter_fragment_definition(self, node: FragmentDefinitionNode, *_args: Any) -> Union[None, VisitorActionEnum]:
    ...
    @staticmethod
    def leave_fragment_definition(node: FragmentDefinitionNode, *_args: Any) -> Dict[str, Any]:

        selections = cast(List[Dict[str, Any]], node.selection_set)
        return {k: v for s in selections for k, v in s.items()}

    def leave_fragment_spread(self, node: FragmentSpreadNode, *_args: Any) -> Dict[str, Any]:
        ...
        log.debug(f"Result of recursive fragment visit {fragment_name}: {fragment_result}")
...
def parse_result(
    ...
    return parse_result_recursive(schema, document, document, result, operation_name=operation_name)
