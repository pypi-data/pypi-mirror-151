    ...
    async def connect(self) -> None:
        ...
        if self.session is None:
            ...
            client_session_args: Dict[str, Any] = {
                ...
                "auth": None if isinstance(self.auth, AppSyncAuthentication) else self.auth,
            }
    ...
    async def execute(
        ...
        async with self.session.post(self.url, ssl=self.ssl, **post_args) as resp:
            ...
            async def raise_response_error(resp: aiohttp.ClientResponse, reason: str):
                ...
                raise TransportProtocolError(f"Server did not return a GraphQL result: " f"{reason}: " f"{result_text}")
