...
from graphql3 import DocumentNode, ExecutionResult, print_ast
...
class WebsocketsTransport(WebsocketsTransportBase):
    ...
    async def _wait_ack(self) -> None:
        ...
        while True:
            ...
            if answer_type != "ka":
                raise TransportProtocolError("Websocket server did not return a connection ack")
    ...
    async def _send_init_message_and_wait_ack(self) -> None:
        ...
        init_message = json.dumps({"type": "connection_init", "payload": self.init_payload})
    ...
    async def _send_query(
        ...
        query_str = json.dumps({"id": str(query_id), "type": query_type, "payload": payload})
    ...
    def _parse_answer_graphqlws(
        ...
        try:
            ...
            if answer_type in ["next", "error", "complete"]:
                ...
                if answer_type == "next" or answer_type == "error":
                    ...
                    if answer_type == "next":
                        ...
                        raise TransportQueryError(str(payload[0]), query_id=answer_id, errors=payload)
            ...
            raise TransportProtocolError(f"Server did not return a GraphQL result: {json_answer}") from e

        return answer_type, answer_id, execution_result

    def _parse_answer_apollo(self, json_answer: Dict[str, Any]) -> Tuple[str, Optional[int], Optional[ExecutionResult]]:
        """Parse the answer received from the server if the server supports the
            ...
            if answer_type in ["data", "error", "complete"]:
                ...
                if answer_type == "data" or answer_type == "error":
                    ...
                    if answer_type == "data":
                        ...
                        raise TransportQueryError(str(payload), query_id=answer_id, errors=[payload])
    ...
    def _parse_answer(self, answer: str) -> Tuple[str, Optional[int], Optional[ExecutionResult]]:
        ...
        try:
            ...
            raise TransportProtocolError(f"Server did not return a GraphQL result: {answer}")
    ...
    async def _send_ping_coro(self) -> None:
        ...
        try:
            ...
            if self.close_task is None:
                await self._fail(
                    TransportServerError(f"No pong received after {self.pong_timeout!r} seconds"),
                    clean_close=False,
                )
    ...
    async def _after_initialize(self):
        ...
        if self.subprotocol == self.GRAPHQLWS_SUBPROTOCOL and self.ping_interval is not None:
