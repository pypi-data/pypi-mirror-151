...
class PhoenixChannelWebsocketsTransport(WebsocketsTransportBase):
    ...
    def _parse_answer(self, answer: str) -> Tuple[str, Optional[int], Optional[ExecutionResult]]:
        ...
        def _required_subscription_id(d: Any, label: str, must_exist: bool = False, must_not_exist=False) -> str:
        ...
        def _validate_data_response(d: Any, label: str) -> dict:
            ...
            if len(invalid) > 0:
                raise ValueError(f"{label} contains invalid items: " + ", ".join(invalid))
            return d
        ...
        try:
            ...
            if event == "subscription:data":
                ...
                subscription_id = _required_subscription_id(payload, "payload", must_exist=True)
                ...
                if status == "ok":
                    ...
                    if answer_id in self.listeners:
                        ...
                        if isinstance(response, dict) and "subscriptionId" in response:
                            ...
                            # Subscription answer
                            subscription_id = _required_subscription_id(response, "response", must_not_exist=True)
                        ...
                        if registered_subscription_id is not None:
                            ...
                            response = _required_value(payload, "response", "payload")
                            subscription_id = _required_subscription_id(response, "response")
                    ...
                    if isinstance(response, dict):
                        if "errors" in response:
                            raise TransportQueryError(str(response.get("errors")), query_id=answer_id)
                        elif "reason" in response:
                            raise TransportQueryError(str(response.get("reason")), query_id=answer_id)
            ...
            raise TransportProtocolError(f"Server did not return a GraphQL result: {e!s}") from e
    ...
    def _find_existing_subscription(self, query_id: int) -> str:
        ...
        if subscription_id is None:
            raise TransportProtocolError(f"No subscription registered for listener {query_id}")
        return subscription_id
