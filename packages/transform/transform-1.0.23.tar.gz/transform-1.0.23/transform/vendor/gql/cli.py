...
from graphql3 import GraphQLError, print_schema
from yarl import URL

from transform.vendor.gql import Client, __version__, gql
from transform.vendor.gql.transport import AsyncTransport
from transform.vendor.gql.transport.exceptions import TransportQueryError
...
def get_parser(with_examples: bool = False) -> ArgumentParser:
    ...
    parser.add_argument("server", help="the server url starting with http://, https://, ws:// or wss://")
    parser.add_argument(
        "-V",
        "--variables",
        nargs="*",
    ...
    parser.add_argument("-H", "--headers", nargs="*", help="http headers in the form key:value")
    ...
    parser.add_argument(
        ...
        help=("select the transport. 'auto' by default: " "aiohttp or websockets depending on url scheme"),
        dest="transport",
    )
...
following arguments:"""
    ...
    appsync_group = parser.add_argument_group("AWS AppSync options", description=appsync_description)
...
async def main(args: Namespace) -> int:
    ...
    async with Client(transport=transport, fetch_schema_from_transport=args.print_schema) as session:
