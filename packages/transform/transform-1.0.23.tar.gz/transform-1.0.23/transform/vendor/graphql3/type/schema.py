...
class GraphQLSchema:
    ...
    def __init__(
        ...
        if types is None:
            ...
            if not is_collection(types) or not all(isinstance(type_, GraphQLType) for type_ in types):
                raise TypeError("Schema types must be specified as a collection of GraphQL types.")
        ...
        if extension_ast_nodes:
            if not is_collection(extension_ast_nodes) or not all(
                isinstance(node, ast.SchemaExtensionNode) for node in extension_ast_nodes
            ):
                raise TypeError(
                    "Schema extension AST nodes must be specified" " as a collection of SchemaExtensionNode instances."
                )
        ...
        for named_type in all_referenced_types:
            ...
            if type_name in type_map:
                raise TypeError(
                    "Schema must contain uniquely named types" f" but contains multiple types named '{type_name}'."
                )
                ...
                for iface in named_type.interfaces:
                    if is_interface_type(iface):
                        ...
                        if iface.name in implementations_map:
                            ...
                            implementations = implementations_map[iface.name] = InterfaceImplementations(
                                objects=[], interfaces=[]
                            )
    ...
    def __deepcopy__(self, memo_: Dict) -> "GraphQLSchema":
        ...
        directives = [
            directive if is_specified_directive(directive) else copy(directive) for directive in self.directives
        ]
        return self.__class__(
            self.query_type and cast(GraphQLObjectType, type_map[self.query_type.name]),
            self.mutation_type and cast(GraphQLObjectType, type_map[self.mutation_type.name]),
            self.subscription_type and cast(GraphQLObjectType, type_map[self.subscription_type.name]),
            types,
            directives,
            self.description,
    ...
    def get_possible_types(self, abstract_type: GraphQLAbstractType) -> List[GraphQLObjectType]:
        ...
        return (
            ...
            else self.get_implementations(cast(GraphQLInterfaceType, abstract_type)).objects
        )
