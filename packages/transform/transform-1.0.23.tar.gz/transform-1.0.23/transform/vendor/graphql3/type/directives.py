...
class GraphQLDirective:
    ...
    def __init__(
        ...
        try:
            locations = tuple(
                value if isinstance(value, DirectiveLocation) else DirectiveLocation[cast(str, value)]
                for value in locations
            )
        except (KeyError, TypeError):
            raise TypeError(f"{name} locations must be specified" " as a collection of DirectiveLocation enum values.")
        if args is None:
            args = {}
        elif not isinstance(args, dict) or not all(isinstance(key, str) for key in args):
            ...
            raise TypeError(f"{name} args must be GraphQLArgument or input type objects.")
        else:
            args = {
...
GraphQLIncludeDirective = GraphQLDirective(
    ...
    args={"if": GraphQLArgument(GraphQLNonNull(GraphQLBoolean), description="Included when true.")},
...
GraphQLSkipDirective = GraphQLDirective(
    ...
    args={"if": GraphQLArgument(GraphQLNonNull(GraphQLBoolean), description="Skipped when true.")},
