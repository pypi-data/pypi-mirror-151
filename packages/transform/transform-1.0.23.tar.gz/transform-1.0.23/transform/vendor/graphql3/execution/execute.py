...
class ExecutionResult:
    ...
    def __eq__(self, other: Any) -> bool:
        if isinstance(other, dict):
            ...
            return other == dict(data=self.data, errors=self.errors, extensions=self.extensions)
...
class ExecutionContext:
        ...
        for definition in document.definitions:
            if isinstance(definition, OperationDefinitionNode):
                if operation_name is None:
                    if operation:
                        return [GraphQLError("Must provide operation name" " if query contains multiple operations.")]
    ...
    def execute_operation(self, operation: OperationDefinitionNode, root_value: Any) -> Optional[AwaitableOrValue[Any]]:
        ...
        if root_type is None:
            raise GraphQLError(
                "Schema is not configured to execute" f" {operation.operation.value} operation.",
                operation,
            )
        ...
        return (self.execute_fields_serially if operation.operation == OperationType.MUTATION else self.execute_fields)(
        ...
        for response_name, field_nodes in fields.items():
            ...
            if is_awaitable(results):
                ...
                async def await_and_set_result(
                    ...
                    awaited_results[response_name] = await result if is_awaitable(result) else result
                    return awaited_results

                results = await_and_set_result(cast(Awaitable, results), response_name, result)
                ...
                results = set_result(cast(Dict[str, Any], results), response_name, result)
            else:
        ...
        try:
            ...
            if self.is_awaitable(result):
                ...
                async def await_result() -> Any:
                    try:
                        completed = self.complete_value(return_type, field_nodes, info, path, await result)
    ...
    def complete_value(
        ...
        if is_non_null_type(return_type):
            ...
            if completed is None:
                raise TypeError(
                    "Cannot return null for non-nullable field" f" {info.parent_type.name}.{info.field_name}."
                )
            return completed
        ...
        if is_list_type(return_type):
            return self.complete_list_value(cast(GraphQLList, return_type), field_nodes, info, path, result)
        ...
        if is_abstract_type(return_type):
            return self.complete_abstract_value(cast(GraphQLAbstractType, return_type), field_nodes, info, path, result)

        # If field type is Object, execute and complete all sub-selections.
        if is_object_type(return_type):
            return self.complete_object_value(cast(GraphQLObjectType, return_type), field_nodes, info, path, result)

        # Not reachable. All possible output types have been considered.
        raise TypeError(  # pragma: no cover
            "Cannot complete value of unexpected output type:" f" '{inspect(return_type)}'."
        )
    ...
    def complete_list_value(
        ...
        if not is_iterable(result):
            ...
            if isinstance(result, AsyncIterable):
                ...
                async def async_iterable_to_list(
                    ...
                    return self.complete_list_value(return_type, field_nodes, info, path, sync_result)

                return async_iterable_to_list(result)

            raise GraphQLError(
                "Expected Iterable, but did not find one for field" f" '{info.parent_type.name}.{info.field_name}'."
            )
        ...
        for index, item in enumerate(result):
            ...
            if is_awaitable(item):
                ...
                async def await_completed(item: Any, item_path: Path) -> Any:
                    try:
                        completed = self.complete_value(item_type, field_nodes, info, item_path, await item)
                ...
                try:
                    completed_item = self.complete_value(item_type, field_nodes, info, item_path, item)
        ...
        async def get_completed_results() -> List[Any]:
            for index, result in zip(
                awaitable_indices,
                await gather(*(completed_results[index] for index in awaitable_indices)),
            ):
    ...
    def complete_abstract_value(
        ...
        return self.complete_object_value(
            self.ensure_valid_runtime_type(runtime_type, return_type, field_nodes, info, result),
            field_nodes,
            info,
            path,
            result,
    ...
    def ensure_valid_runtime_type(
        ...
        if not is_object_type(runtime_type):
            raise GraphQLError(
                f"Abstract type '{return_type.name}' was resolved" f" to a non-object type '{runtime_type_name}'.",
                field_nodes,
            )
        ...
        if not self.schema.is_sub_type(return_type, runtime_type):
            raise GraphQLError(
                f"Runtime Object type '{runtime_type.name}' is not a possible" f" type for '{return_type.name}'.",
                field_nodes,
            )
        ...
        if return_type.is_type_of:
            ...
            if self.is_awaitable(is_type_of):
                ...
                async def execute_subfields_async() -> Dict[str, Any]:
                    ...
                    return self.execute_fields(return_type, result, path, sub_field_nodes)  # type: ignore
