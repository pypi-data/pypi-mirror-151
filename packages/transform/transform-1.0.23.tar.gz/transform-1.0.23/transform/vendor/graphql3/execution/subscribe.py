...
async def create_source_event_stream(
    ...
    try:
        ...
        if not isinstance(event_stream, AsyncIterable):
            raise TypeError("Subscription field must return AsyncIterable." f" Received: {inspect(event_stream)}.")
        return event_stream
...
async def execute_subscription(context: ExecutionContext) -> AsyncIterable[Any]:
    ...
    # Implements the "ResolveFieldEventStream" algorithm from graphql3 specification.
