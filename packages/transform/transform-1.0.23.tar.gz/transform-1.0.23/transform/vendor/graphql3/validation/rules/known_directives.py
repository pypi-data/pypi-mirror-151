...
class KnownDirectivesRule(ASTValidationRule):
    ...
    def __init__(self, context: Union[ValidationContext, SDLValidationContext]):
        ...
        defined_directives = schema.directives if schema else cast(List, specified_directives)
        ...
        for def_ in ast_definitions:
            if isinstance(def_, DirectiveDefinitionNode):
                locations_map[def_.name.value] = tuple(DirectiveLocation[name.value] for name in def_.locations)
    ...
    def enter_directive(
        ...
        if locations:
            ...
            if candidate_location and candidate_location not in locations:
                self.report_error(
                    GraphQLError(
                        f"Directive '@{name}'" f" may not be used on {candidate_location.value}.",
                        node,
                    )
                )
        else:
