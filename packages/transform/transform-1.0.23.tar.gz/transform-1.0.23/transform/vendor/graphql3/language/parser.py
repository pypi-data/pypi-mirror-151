...
class Parser:
    ...
    def parse_definition(self) -> DefinitionNode:
        ...
        keyword_token = self._lexer.lookahead() if has_description else self._lexer.token

        if keyword_token.kind is TokenKind.NAME:
            token_name = cast(str, keyword_token.value)
            method_name = self._parse_type_system_definition_method_names.get(token_name)
            if method_name:
    ...
    def parse_variable_definitions(self) -> List[VariableDefinitionNode]:
        ...
        return self.optional_many(TokenKind.PAREN_L, self.parse_variable_definition, TokenKind.PAREN_R)
    ...
    def parse_variable_definition(self) -> VariableDefinitionNode:
        ...
        return VariableDefinitionNode(
            ...
            default_value=self.parse_const_value_literal() if self.expect_optional_token(TokenKind.EQUALS) else None,
    ...
    def parse_selection_set(self) -> SelectionSetNode:
        ...
        return SelectionSetNode(
            selections=self.many(TokenKind.BRACE_L, self.parse_selection, TokenKind.BRACE_R),
            loc=self.loc(start),
        )

    def parse_selection(self) -> SelectionNode:
        """Selection: Field or FragmentSpread or InlineFragment"""
        return (self.parse_fragment if self.peek(TokenKind.SPREAD) else self.parse_field)()
    ...
    def parse_field(self) -> FieldNode:
        ...
        return FieldNode(
            ...
            directives=self.parse_directives(False),
            selection_set=self.parse_selection_set() if self.peek(TokenKind.BRACE_L) else None,
            loc=self.loc(start),
        )
    ...
    def parse_argument(self, is_const: bool = False) -> ArgumentNode:
        ...
        return ArgumentNode(name=name, value=self.parse_value_literal(is_const), loc=self.loc(start))
    ...
    def parse_object_field(self, is_const: bool) -> ObjectFieldNode:
        ...
        return ObjectFieldNode(name=name, value=self.parse_value_literal(is_const), loc=self.loc(start))
    ...
    def parse_schema_definition(self) -> SchemaDefinitionNode:
        ...
        directives = self.parse_const_directives()
        operation_types = self.many(TokenKind.BRACE_L, self.parse_operation_type_definition, TokenKind.BRACE_R)
    ...
    def parse_operation_type_definition(self) -> OperationTypeDefinitionNode:
        ...
        self.expect_token(TokenKind.COLON)
        type_ = self.parse_named_type()
        return OperationTypeDefinitionNode(operation=operation, type=type_, loc=self.loc(start))
    ...
    def parse_enum_value_name(self) -> NameNode:
        ...
        if self._lexer.token.value in ("true", "false", "null"):
            raise GraphQLSyntaxError(
                self._lexer.source,
                self._lexer.token.start,
                f"{get_token_desc(self._lexer.token)} is reserved" " and cannot be used for an enum value.",
            )
        return self.parse_name()
    ...
    def unexpected(self, at_token: Optional[Token] = None) -> GraphQLError:
        ...
        return GraphQLSyntaxError(self._lexer.source, token.start, f"Unexpected {get_token_desc(token)}.")

    def any(self, open_kind: TokenKind, parse_fn: Callable[[], T], close_kind: TokenKind) -> List[T]:
    ...
    def many(self, open_kind: TokenKind, parse_fn: Callable[[], T], close_kind: TokenKind) -> List[T]:
...
def get_token_desc(token: Token) -> str:
    ...
    value = token.value
    return get_token_kind_desc(token.kind) + (f" '{value}'" if value is not None else "")
