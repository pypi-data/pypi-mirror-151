...
def value_from_ast_untyped(value_node: ValueNode, variables: Optional[Dict[str, Any]] = None) -> Any:
    ...
    raise TypeError(f"Unexpected value node: {inspect(value_node)}.")  # pragma: no cover
...
def value_from_list(value_node: ListValueNode, variables: Optional[Dict[str, Any]]) -> Any:
    return [value_from_ast_untyped(node, variables) for node in value_node.values]


def value_from_object(value_node: ObjectValueNode, variables: Optional[Dict[str, Any]]) -> Any:
    return {field.name.value: value_from_ast_untyped(field.value, variables) for field in value_node.fields}
