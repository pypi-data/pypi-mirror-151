...
from .sql_toolset import format_sql
...
def parse_pipe(filename):
    ...
    try:
        ...
        for node in doc.nodes:
            sql = node.get('sql', '')
            if sql.strip()[0] == '%':
                sql = render_sql_template(sql[1:], test_mode=True)
            format_sql(sql)
...
async def check_pipe(
    ...
    params = {
        'q': f"""
            SELECT extractURLParameterNames(assumeNotNull(url)) as params, groupArraySample({limit})(url) as endpoint_url
    ...
    await new_pipe(checker_pipe, cl, replace=True, check=False, populate=populate, skip_tokens=True, skip_table_checks=True)
...
async def check_materialized(pipe, host, token, cl):
    ...
    try:
        pipe_created = False
        await new_pipe(checker_pipe, cl, replace=True, check=False, populate=False, skip_tokens=True, skip_table_checks=True, ignore_sql_errors=False)
        pipe_created = True
...
async def new_ds(ds, client: TinyB, replace=False, skip_table_checks=False, skip_confirmation=False):
                    ...
                    if name not in remote_resource_names or resource_versions.get(name.replace(".", "_"), '') != latest_datasource_versions.get(name, '') or force or run_tests:
                        ...
                        try:
                            ...
                            if '[CLI] skip_table_checks_error' in exception:
                                skip_table_checks_error = FeedbackManager.error_skip_table_checks_path()
        ...
        if csv_test_file.exists():
            ...
            try:
                with open(csv_test_file, 'rb') as file:
                    res = await cl.datasource_append_data(
                        datasource_name=r['params']['name'],
                        f=file,
                        mode='replace',
                        sql_condition='1=1')
