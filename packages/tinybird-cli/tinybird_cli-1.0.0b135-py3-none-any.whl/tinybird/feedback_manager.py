...
class FeedbackManager:
    ...
    error_datasource_does_not_exist = error_message("'{datasource}' Data Source does no exist")
    ...
    error_while_check_materialized = error_message("Invalid results, read description below to fix the error. You can skip this error using --skip-table-checks although it might have undesired effects: {error}")
    ...
    error_skip_table_checks_path = simple_error_message("you can skip this error using --skip-table-checks")
    ...
    error_workspace = error_message("Workspace not found. use 'tb workspace ls' to list your workspaces")
    error_getting_region_by_index = error_message("Unable to get region by index, list available regions using 'tb auth ls'")
    error_region_index = error_message("Error selecting region '{host_index}', available options are: {available_options}")
    ...
    warning_beta_tester = prompt_message("This feature is under development and released as a beta version. You can report any feedback (bugs, feature requests, etc.) to support@tinybird.co")
    warning_confirm_file_override = prompt_message("Local file {file} already exists, do you want to override?")
    warning_choose = prompt_message("=> Choose one of the above options to continue... ")
    warning_choose_node = prompt_message("=> Please, select a node to materialize... ")
    warning_populate = prompt_message("Do you want to populate the materialized view with existing data? (It'll truncate the materialized view before population starts)")
...
**    $ tb datasource rm {datasource}""")
    ...
    warning_confirm_delete_datasource = prompt_message("Do you want to delete {datasource}? Once deleted, it can't be recovered")
    ...
    info_materialize_push_pipe_skip = info_message('  [1] Push {name} and override if it exists (tb push --force)')
    info_materialize_push_pipe_override = info_message('  [2] Push {name} and override if it exists with no checks (tb push --force --skip-table-checks)')
    info_materialize_populate_partial = info_message('  [1] Partially populate: Uses a 10 percent subset of the data (quicker, useful to validate the materialized view)')
    ...
    info_pipe_exists = prompt_message("** A pipe with name {name} already exists, do you want to override it?")
    ...
    info_materialized_datasource_used = info_message("** Materialized node '{node}' using the Data Source '{datasource}'")
