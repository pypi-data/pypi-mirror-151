...
def checkCoordinates(guess, answer, tolerance):
    ...
    if error < 1000:
        print(f'{color}{guesser}\'s {hyperlink("guess", commentlink)} was {error}m {hyperlink("off", mapslink)}{fg.rs}')
    else:
        print(f'{color}{guesser}\'s {hyperlink("guess", commentlink)} was {round(error/1000,2)}km {hyperlink("off", mapslink)}{fg.rs}')
