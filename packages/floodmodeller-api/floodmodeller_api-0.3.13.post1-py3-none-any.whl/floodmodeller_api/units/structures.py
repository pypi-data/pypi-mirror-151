    ...
    **Outlet Loss Type (``CULVERT.subtype == 'OUTLET'``)**
    ...
    def _write(self):
        ...
        if self.subtype == "INLET":
            ...
            params1 = join_10_char(self.screen_width, self.bar_proportion, self.debris_proportion,
                    self.loss_coefficient, self.reverse_flow_mode, self.headloss_type, self.max_screen_height, dp = 4)
...
class BLOCKAGE(Unit):
    ...
    def _write(self):
        ...
        if self.data.max() > 1 or self.data.min() < 0:
            raise ValueError(f'Parameter error with {self.__repr__()} - blockage percentage must be between 0 and 1')
        ...
        if self.timeunit == 'DATES':
            blockage_data = [f'{t:<20}{join_10_char(b)}' for t, b in self.data.iteritems()]
        else:
...
class BRIDGE(Unit):
    ...
    **PIERLOSS Type (``BRIDGE.subtype == 'PIERLOSS'``)**
    ...
    def _read(self, br_block):
        ...
        labels = split_n_char(
            f'{br_block[2]:<{4*self._label_len}}', self._label_len)
        self.name = labels[0]
        self.ds_label = labels[1]
        self.us_remote_label = labels[2]
        self.ds_remote_label = labels[3]
        ...
        if self.subtype == 'ARCH':
            ...
            self.opening_nrows = int(split_10_char(
                br_block[6+self.section_nrows])[0])
            data_list = []
            ...
            self.opening_data = pd.DataFrame(
                data_list, columns=['Start', 'Finish', 'Springing Level', 'Soffit Level'])
        ...
        # Read USBPR type unit
            ...
            # Read bridge opening data
            self.opening_nrows = int(split_10_char(
                br_block[9+self.section_nrows])[0])
            data_list = []
            ...
            self.culvert_nrows = int(split_10_char(
                br_block[9 + self.section_nrows + self.opening_nrows + 1])[0])
            data_list = []
            ...
            for row in br_block[start_row:end_row]:
                ...
                data_list.append(
                    [invert, soffit, area, cd_part, cd_full, drown])
            self.culvert_data = pd.DataFrame(data_list, columns=[
                                             'Invert', 'Soffit', 'Section Area', 'Cd Part Full', 'Cd Full', 'Drowning Coefficient'])
        ...
        # Read Pierloss type bridge
            ...
            self.pier_locs_data = pd.DataFrame(
                data_list, columns=['Left X', 'Left Top Level', 'Right X', 'Right Top Level'])
    ...
    def _write(self):
        ...
        _validate_unit(
            self)  # Function to check the params are valid for BRIDGE unit
        header = 'BRIDGE '+self.comment
        labels = join_n_char_ljust(
            self._label_len, self.name, self.ds_label, self.us_remote_label, self.ds_remote_label)
        br_block = [header, self.subtype, labels]

        if self.subtype == 'ARCH':
            orifice = 'ORIFICE' if self.orifice_flow else ''
            params = join_10_char(self.calibration_coefficient, self.skew, self.bridge_width_dual, self.bridge_dist_dual, '',
                                  orifice, self.orifice_lower_transition_dist, self.orifice_upper_transition_dist, self.orifice_discharge_coefficient)
            self.section_nrows = len(self.section_data)
            br_block.extend(
                ['MANNING', params, f'{str(self.section_nrows):>10}'])
            ...
            orifice = 'ORIFICE' if self.orifice_flow else ''
            params = join_10_char(self.calibration_coefficient, self.skew, self.bridge_width_dual, self.bridge_dist_dual, self.total_pier_width,
                                  orifice, self.orifice_lower_transition_dist, self.orifice_upper_transition_dist, self.orifice_discharge_coefficient)
            ...
            self.section_nrows = len(self.section_data)
            br_block.extend(['MANNING', params, f'{str(self.abutment_type):>10}',
                            pier_params, self.abutment_alignment, f'{str(self.section_nrows):>10}'])
            ...
            culvert_data = []
            for _, invert, soffit, area, cd_part, cd_full, drown in self.culvert_data.itertuples():
                row = join_10_char(invert, soffit, area,
                                   cd_part, cd_full, drown)
            ...
            orifice = 'ORIFICE' if self.orifice_flow else ''
            params = join_10_char(self.calibration_coefficient, orifice, self.orifice_discharge_coefficient,
                                  self.orifice_lower_transition_dist, self.orifice_upper_transition_dist)
            additional_params = join_10_char(
                self.pier_coefficient, self.bridge_width)
            ...
            pier_locs_data = []
            for _, l_x, l_top_level, r_x, r_top_level in self.pier_locs_data.itertuples():
...
class CONDUIT(Unit):
    ...
    **Circular Type (``CONDUIT.subtype == 'CIRCULAR'``)**
    ...
    def _read(self, c_block):
        ...
        # Extends label line to be correct length before splitting to pick up blank labels
        labels = split_n_char(
            f'{c_block[2]:<{2*self._label_len}}', self._label_len)
        self.name = labels[0]
        self.spill = labels[1]
    ...
    def _write(self):
        ...
        _validate_unit(
            self)  # Function to check the params are valid for CONDUIT unit
        header = 'CONDUIT ' + self.comment
        labels = join_n_char_ljust(self._label_len, self.name, self.spill)
        c_block = [header, self.subtype, labels]

        if self.subtype == 'CIRCULAR':
            params = join_10_char(self.invert, self.diameter, self.use_bottom_slot, self.bottom_slot_dist,
                                  self.bottom_slot_depth, self.use_top_slot, self.top_slot_dist, self.top_slot_depth)
            friction_params = f'{self.friction_below_axis:>10.4f}{self.friction_below_axis:>10.4f}'
            c_block.extend([f'{self.dist_to_next:>10.3f}',
                           self.friction_eq, params, friction_params])
            return c_block

        elif self.subtype == 'RECTANGULAR':
            params = join_10_char(self.invert, self.width, self.height, self.use_bottom_slot, self.bottom_slot_dist,
                                  self.bottom_slot_depth, self.use_top_slot, self.top_slot_dist, self.top_slot_depth)
...
class SLUICE(Unit):
    ...
    **Radial Type (``SLUICE.subtype == 'RADIAL'``)**
    ...
    def _read(self, block):
        ...
        # Extends label line to be correct length before splitting to pick up blank labels
        labels = split_n_char(
            f'{block[2]:<{3*self._label_len}}', self._label_len)
        self.name = labels[0]
        self.ds_label = labels[1]
        ...
        if self.subtype == 'RADIAL':
            ...
            self.modular_limits = {'weir_flow': _to_float(params2[4]), 'under_gate_flow': _to_float(params2[5], 1.0), 'over_gate_flow': _to_float(params2[6], 1.0)}
        ...
        if self.control_method == 'TIME':
            ...
            print(f'Note: Sluice control using method: {self.control_method} is not currently supported.')
    ...
    def _write(self):
        ...
        # First parameter line
        params1 = join_10_char(self.weir_flow_coefficient, self.under_gate_flow, self.weir_breadth, self.crest_elevation, 
                                self.gate_height_or_chord, self.weir_length) 
        if self.subtype == 'RADIAL':
            params1 += f'{"DEGREES":<10}' if self.use_degrees else f'{"":<10}'
            params1 += 'FREESLUICE' if self.allow_free_flow_under else f'{"":<10}'

        # Second parameter line
        params2 = join_10_char(self.us_weir_height, self.ds_weir_height, self.bias_factor, self.over_gate_flow)

        if self.subtype == 'RADIAL':
            params2 += join_10_char(self.pivot_height, self.gate_radius)
        else:
            params2 += join_10_char(self.modular_limits['weir_flow'], self.modular_limits['under_gate_flow'], self.modular_limits['over_gate_flow'])
        ...
        if self.control_method == 'TIME':
            ...
            # ADD GATES
            block.append(join_10_char(f'{"LOGICAL":<10}', self.max_movement_rate, self.max_setting, self.min_setting))
            n = 1
            for gate in self.gates:
                block.append(f'GATE {n}')
                nrows = len(gate)
                block.append(f'{nrows:>10}')
                gate_data = [f'{t:>10.3f}{m:>10}{o:>10.3f}' for t, m, o in gate.itertuples()]
                block.extend(gate_data)
                n += 1
            ...
            time_rule_data = [f'{t:>10.3f}{o_r:<10}' for t, o_r in self.time_rule_data.iteritems()]
...
class ORIFICE(Unit):
    ...
    def _write(self):
        ...
        # First parameter line
        params1 = join_10_char(self.invert, self.soffit, self.bore_area, self.upstream_sill, 
                                self.downstream_sill, self.shape)
    ...
    def _create_from_blank(self, name='new_orifice', flapped=False, ds_label='', comment='', invert=0.0, 
                            soffit=0.0, bore_area=1.0, upstream_sill=0.0, downstream_sill=0.0, shape='RECTANGLE', 
                            weir_flow=1.0, surcharged_flow=1.0, modular_limit=0.7):
        
        for param, val in {'name': name, 'flapped': flapped, 'ds_label': ds_label, 'comment': comment, 
            'invert': invert, 'soffit': soffit, 'bore_area': bore_area, 'upstream_sill': upstream_sill, 
            'downstream_sill': downstream_sill, 'shape': shape, 'weir_flow': weir_flow, 
            'surcharged_flow': surcharged_flow, 'modular_limit': modular_limit}.items():
            setattr(self, param, val)
...
class SPILL(Unit):
    ...
    def _create_from_blank(self, name='new_spill', ds_label='', comment='', weir_coefficient=1.2, modular_limit=0.9, data=None):
        
        for param, val in {'name': name, 'ds_label': ds_label, 'comment': comment, 
             'weir_coefficient': weir_coefficient, 'modular_limit': modular_limit}.items():
            setattr(self, param, val)
        
        self.data = data if isinstance(data, pd.DataFrame) else pd.DataFrame(
            [[0.,0.,0.,0.]], columns=['X', 'Y', 'Easting', 'Northing']) 