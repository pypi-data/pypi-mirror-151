...
class RIVER(Unit):
    ...
    def _read(self, riv_block):
        ...
        if self.subtype == 'SECTION':
            ...
            for row in riv_block[5:]:
                ...
                data_list.append([x, y, n, panel, rpl, marker,
                                 easting, northing, deactivation, sp_marker])
            self.data = pd.DataFrame(data_list, columns=[
    ...
    def _write(self):
        ...
        if self.subtype == 'SECTION':
            ...
            labels = join_n_char_ljust(self._label_len, self.name, self.spill1,
                                       self.spill2, self.lat1, self.lat2, self.lat3, self.lat4)
            # Manual so slope can have more sf
            params = f'{self.dist_to_next:>10.3f}{"":>10}{self.slope:>10.6f}{self.density:>10.3f}'
            self.nrows = len(self.data)
            riv_block = [header, self.subtype, labels,
                         params, f'{str(self.nrows):>10}']

            riv_data = []
            for _, x, y, n, panel, rpl, marker, easting, northing, deactivation, sp_marker in self.data.itertuples():
