...
class IEF(FMFile):
    ...
    def _create_from_blank(self):
        ...
        blank_ief = ['[ISIS Event Header]', 'Title=""', 'Datafile=""', 'Results=""',
                     '[ISIS Event Details]', 'RunType=Steady', 'Start=0', 'ICsFrom=1']
    ...
    def _update_ief_properties(self):
        ...
        for prop, val in self.__dict__.copy().items():
            if (prop not in self._ief_properties) and (not prop.startswith('_')):
                ...
                if prop.upper() not in flags:
                    ...
                    if not prop == 'EventData':  # In case of EventData being added with correct case where it doesn't already
                    ...
                    for idx, itm in enumerate(self._ief_properties[:]):
                        if itm.upper() == prop.upper():  # Check if existing property exists with different casing (e.g. Title instead of TITLE)
        ...
        if hasattr(self, 'EventData'):
            ...
            event_properties = self._ief_properties.count(
                'EventData')  # Number of 'EventData' flags in ief
        ...
        self._ief_properties = [line for line in self._ief_properties if (
            line.startswith('[') or (line in dir(self)) or line.lstrip().startswith(';'))]
    ...
    def simulate(self, method: Optional[str] = 'WAIT', raise_on_failure: Optional[bool] = True, 
        precision: Optional[str] ='DEFAULT', 
        enginespath: Optional[str] = '') -> Optional[subprocess.Popen]:
        ...
        if (enginespath == ''):
            ...
            if not Path(_enginespath).exists:
                raise Exception (f"Flood Modeller non-default engine path not found! {str(_enginespath)}")
        ...
        if method.upper() == 'WAIT':
            ...
            process = Popen(run_command, cwd=os.path.dirname(
                self._filepath))  # execute simulation
    ...
    def get_results(self) -> ZZN:
        ...
        if hasattr(self, 'Results'):
            ...
            if not result_path.is_absolute():
                ...
                result_path = Path(self._filepath.parent,
                                   result_path).resolve()
    ...
    def _summarise_exy(self):
        ...
        if exy_path.exists():
            ...
            raise FileNotFoundError(
                "Simulation results error log (.exy) not found")
