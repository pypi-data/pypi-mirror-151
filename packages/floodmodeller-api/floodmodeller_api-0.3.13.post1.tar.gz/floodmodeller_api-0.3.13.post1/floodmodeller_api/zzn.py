...
class ZZN(FMFile):
    ...
    def to_dataframe(self, result_type: Optional[str] ='all', variable: Optional[str] ='all', include_time: Optional[bool] =False, 
        ...
        if result_type == 'all':
            ...
            if multilevel_header:
                ...
                df = pd.DataFrame(arr.reshape(nz, nx*ny),
                              index=time_index, columns=pd.MultiIndex.from_product(col_names))
                ...
                df = pd.DataFrame(arr.reshape(nz, nx*ny),
                                index=time_index, columns=col_names)
                df.index.name = 'Time (hr)'
                if not variable == 'all':
                    use_cols = [col for col in df.columns if col.endswith(
                        variable.capitalize())]
                    return df[use_cols]
            return df
            ...
            col_names = [result_type.capitalize()+lbl for lbl in [' Flow', ' Stage', ' Froude',
                         ' Velocity', ' Mode', ' State']]
            ...
            if include_time:
                ...
                times = (
                    (times - self.meta['timestep0']) * self.meta['dt'])/3600
                time_col_names = [name+' Time(hrs)' for name in col_names]
                time_df = pd.DataFrame(
                    times, index=node_index, columns=time_col_names)
                time_df.index.name = 'Node Label'
                df = pd.concat([df, time_df], axis=1)
                new_col_order = [x for y in list(
                    zip(col_names, time_col_names)) for x in y]
                df = df[new_col_order]
                if not variable == 'all':
                    return df[[f'{result_type.capitalize()} {variable.capitalize()}', f'{result_type.capitalize()} {variable.capitalize()} Time(hrs)']]
                return df
    ...
    def export_to_csv(self, save_location: Optional[Union[str, Path]] ='default', result_type: Optional[str] ='all', 
        variable: Optional[str] ='all', include_time: Optional[bool] = False) -> None:
        ...
        if save_location == 'default':
            ...
            if not save_location.is_absolute():
                ...
                save_location = Path(
                    Path(self.meta['zzn_name']).parent, save_location)

        if not save_location.suffix == '.csv':  # Assumed to be pointing to a folder
            # Check if the folder exists, if not create it
            if not save_location.exists():
                Path.mkdir(save_location)
            save_location = Path(save_location, Path(
                self.meta['zzn_name']).with_suffix('.csv').name)
        ...
        df = self.to_dataframe(result_type=result_type,
                               variable=variable, include_time=include_time)
