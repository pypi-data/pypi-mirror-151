"""Utilizario de funciones usadas en los procesos de robotizacion
   con Python."""
        ...
        with paramiko.SFTPClient.from_transport(transport) as server:
            ...
            if type(filename) is list:
                ...
                for file in dir_local.iterdir():
                    ...
                    if fnmatch.fnmatch(file.name, filename.name):
                        flag_exists = True
                        log.debug ("Subiendo archivo: " + str(file))
                        server.put(
                            str(file), 
                            file.name)
    ...
    with paramiko.Transport((credentials["hostname"], puerto)) as transport:
        ...
        with paramiko.SFTPClient.from_transport(transport) as server:
            ...
            for origin, destine in filenames.items():
                try:
                    pendientes[str(PurePosixPath(origin))] = str(PurePosixPath(destine))
                except:
...
def sqlcmd(file_sql:StrOrPath, 
    ...
    if file_sql_log.exists():
        log.debug(open(file_sql_log, "r", encoding=decode).read())
...
def notifica_exception(exception=None, 
                       notifica=None, 
                       lista_exceptions_bl=[], 
                       subject_replace=None, 
                       body_replace=None):
    ...
    if str(type(exception).__name__).startswith("ID"):
        if type(exception).__name__ not in lista_exceptions_bl:
            if type(subject_replace) is dict:
                subject=subject_replace
            else:
                subject={'[TEXTO]':subject_replace}
                
            notifica.enviar_correo(subject,
                                   {'[TEXTO]':str(exception)})
    else:
        if type(subject_replace) is dict:
            subject=subject_replace
        ...
        notifica.enviar_correo(subject,
                               {'[TEXTO]':body_replace + str(exception)})
