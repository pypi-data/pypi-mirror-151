...
class BillLinkModel():
    def __init__(self,token="AD64F20D-6063-4E87-81E8-A24C1751D758",FFormId='BD_MATERIAL',FActionName='Save'):
        ...
        #用于查询元数据信息
        self.metaClient = RdClient(token=self.token)
    ...
    def getBillLinkStat(self, FBillNo='so-api-002'):
        '''
        get the actual statistics info from business database.
        :param FBillNo:
        :return:
        ...
        sql_head = "select  FEntityName,FListCount   from  " + self.viewNameStat
        sql_where1 = " where  " + self.tableKey + " = '" + self.FBillNo + "' and FFormId ='" + self.FFormId + "' and FActionName ='" + self.FActionName + "'"
        sql_where2 = " and FOwnerName ='" + self.FOwnerName + "' and FType ='entryListLink'"
        sql_all = sql_head + sql_where1 + sql_where2
        data = self.dataClient.select(sql_all)
        return (data)
    ...
    def billLinkGetMeta(self, FBillNo='so-api-002'):
        '''
        get metainfo for head part.
        :param FBillNo:
        :return:
        '''
        self.FBillNo = FBillNo
        res = self.billTableSqlGenerator(FBillNo=self.FBillNo, Ftype='entryListLink', FEntityName='FEntity_Link')
        return (res)
    ...
    def billHeadGetData(self,FBillNo='so-api-002'):
        ...
        if sql != '':
            ...
            data = ''
        return(data)
    def billLinkGetData(self,FBillNo='so-api-002'):
        '''
        get business data for head part
        :param FBillNo:
        :return:
        '''
        self.FBillNo = FBillNo
        sql = self.billLinkGetMeta(FBillNo=self.FBillNo)
        if sql != '':
            data = self.dataClient.select(sql)
        else:
            data = ''
    ...
    def setLinkValue(self,option,FMainKey='FIssueType',FListCount=0, FValue=""):
        '''
        set the value for each key
        which has a bug for the key has more than one row.
        to do list: add the parameter FListCount
        ...
        self.FValue = FValue
        node_count = FListCount - 1
        entryname = self.getEntryEntityNameByLink()
        entryLinkName = self.getLinkEntityNameByLink()
        data = self.queryMeta(FMainKey=self.FMainKey)
        ...
        if ncount > 0:
            for i in data:
                ...
                node_datatype = i['FDataType']
                node_valueType = i['FValueType']
                if node_type == 'entryListLink':
                    if node_valueType == 'simple':
                        option[node_model][entryname][node_count][entryLinkName][0][node_main] = valueConverter(
                                self.FValue, node_datatype)
        ...
        #print('result-'+str(FListCount)+' param:'+FMainKey)
        #print(option['Model']['FEntity'][0]['FEntity_Link'][0])
        #print(option['Model']['FEntity'][1]['FEntity_Link'][0])
        return (option)
    ...
    def tailSet(self,option,FBillNo='so-api-002'):
        ...
        for i in tailSheet:
            ...
            #print(self.option)
        return (self.option)
    def bodyLink(self,option,FBillNo='FHTZD22050032'):
        self.FBillNo = FBillNo
        data = self.queryData(Ftype='entryListLink', FEntityName='FEntity_Link', FListCount=1)
        node_link_each = self.unityLink(data=data)
        node_link_item = node_link_each[0]
        #注意由于python的特性，如果引用2次则共享内存
        #一次修改，处处修改
        entryname = self.getEntryEntityNameByLink()
        #print(entryname)
        entryLinkName = self.getLinkEntityNameByLink()
        #print(entryLinkName)
        linkStat = self.getBillLinkStat(FBillNo=self.FBillNo)
        ncount = len(linkStat)
        if ncount >0:
            FListCount = linkStat[0]['FListCount']
            for i in range(FListCount):
                #print('bug link:')
                #注意此处必须使用copy，由系统单独分配内存
                #copy也是不行的，需要重新装载数据才行
                res_list=[]
                res_dict={}
                for key, value in node_link_item.items():
                    res_dict[key] = value
                res_list.append(res_dict)
                option['Model'][entryname][i][entryLinkName] = res_list
        return(option)

    def bodyLinkSetvalue(self, option, FBillNo='FHTZD22050032'):
        self.FBillNo = FBillNo
        linkData =self.billLinkGetData(FBillNo=self.FBillNo)
        ncount = len(linkData)
        if ncount >0:
            for i in range(ncount):
                ...
                ncount_keys = len(FMainKeys)
                if ncount_keys > 0:
                    for k in range(ncount_keys):
                        FMainKey = FMainKeys[k]
                        FValue = FValues[k]
                        option = self.setLinkValue(option=option, FMainKey=FMainKey, FValue=FValue,FListCount=FListCount)
                        #print('setLinkValueCall:')
                        #print(option)
        #print('setLinkLastCall:')
        #print(option)
    ...
    def getEntryEntityNameByLink(self):
        sql_head = "select  left(FEntityName,len(FEntityName)-len('_Link')) as FEntityName   from   t_api_kdc_entity "
        sql_where =" where Ftype = 'entryListLink' and FOwnerName ='"+self.FOwnerName+"' and FFormId ='"+self.FFormId+"' and FActionName ='"+self.FActionName+"'"
        sql_all = sql_head + sql_where
        data = self.metaClient.select(sql_all)
        ncount = len(data)
        if ncount >0:
            res = data[0]['FEntityName']
        else:
            res =""
        return(res)
    def getLinkEntityNameByLink(self):
        sql_head = "select   FEntityName   from   t_api_kdc_entity "
        sql_where =" where Ftype = 'entryListLink' and FOwnerName ='"+self.FOwnerName+"' and FFormId ='"+self.FFormId+"' and FActionName ='"+self.FActionName+"'"
        sql_all = sql_head + sql_where
        data = self.metaClient.select(sql_all)
        ncount = len(data)
    ...
    def dataGen(self,FBillNo='so-api-002'):
        ...
        #设置关联信息
        option = self.bodyLink(option=option,FBillNo=self.FBillNo)
        ...
        option = self.bodyLinkSetvalue(option=option,FBillNo=self.FBillNo)
        #更新关联信息
        #set value
