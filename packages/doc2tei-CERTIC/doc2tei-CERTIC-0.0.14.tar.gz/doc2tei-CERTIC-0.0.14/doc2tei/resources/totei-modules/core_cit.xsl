...
<xsl:template match="text:p[starts-with(@text:style-name,'TEI_quote') and not(contains(@text:style-name,'nested')) and not(contains(@text:style-name,'continuation'))]">
    ...
    <cit>
        <xsl:for-each-group select="node()" group-ending-with="text:line-break">
            <quote>
                ...
                <xsl:if test="contains($quoteType,':')">
                    <xsl:attribute name="xml:lang"><xsl:value-of select="substring-after($quoteType,':')"/></xsl:attribute>
                </xsl:if>
        ...
        <xsl:if test="following-sibling::text:*[1][@text:style-name='TEI_quote_nested']">
            <xsl:apply-templates select="following-sibling::text:*[1][@text:style-name='TEI_quote_nested']" mode="inCit"/>
        </xsl:if>
        <xsl:if test="following-sibling::text:*[2][@text:style-name='TEI_quote_continuation']">
            <xsl:apply-templates select="following-sibling::text:*[2][@text:style-name='TEI_quote_continuation']" mode="inCit"/>
        </xsl:if>

    </cit>
</xsl:template>
    
<xsl:template match="text:p[@text:style-name='TEI_bibl_citation']" mode="inCit">
    <bibl><xsl:apply-templates/></bibl>
</xsl:template>
    
<xsl:template match="text:p[@text:style-name='TEI_quote_nested']" mode="inCit">
    <cit>
        <xsl:for-each-group select="node()" group-ending-with="text:line-break">
            <quote>
                <xsl:apply-templates select="current-group()"/>
...
<xsl:template match="text:p[@text:style-name='adquestion']">
    <sp>
