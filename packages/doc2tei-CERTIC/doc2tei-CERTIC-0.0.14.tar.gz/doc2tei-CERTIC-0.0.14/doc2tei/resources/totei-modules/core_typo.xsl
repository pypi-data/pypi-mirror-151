...
<!-- enrichissement simple, une valeur dans la liste suivante -->
<!--<hi rend="bold|italic|small-caps|sup|sub|underline|strikethrough|uppercase">…</hi>-->
<!-- double enrichissements, cumul de valeurs -->
<!--<hi rend="bold italic">…</hi>-->

<xsl:template match="text:span[@rendition]">
    <hi rend="{@rendition}">
        <xsl:apply-templates/>
    </hi>
</xsl:template>
...
<xsl:template match="text:line-break">
    <xsl:choose>
        <xsl:when test="parent::text:p[@text:style-name='TEI_code']"/>
        <xsl:when test="parent::text:p[starts-with(@text:style-name,'TEI_quote')]"/>
