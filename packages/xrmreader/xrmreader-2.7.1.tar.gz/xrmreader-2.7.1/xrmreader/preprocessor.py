...
def smooth_reference(reference, kernel_size=15):
    ...
    # reference_smooth = convolve2d(reference, kernel, mode='same', boundary='symm')
...
def read_and_preprocess_txrm(projections_file, downsample_factor=1, angular_factor=1, do_truncation_correction=True,
    ...
    x_shifts_detector = metadata['x-shifts'][::angular_factor]
    y_shifts_detector = metadata['y-shifts'][::angular_factor]
    x_shifts_temperature = metadata['temperature-x-shifts'][::angular_factor]
    y_shifts_temperature = metadata['temperature-y-shifts'][::angular_factor]
    x_shifts_amc = metadata['AMC-x-shifts'][::angular_factor]
    y_shifts_amc = metadata['AMC-y-shifts'][::angular_factor]
    x_shifts_source = metadata['source-x-shifts'][::angular_factor]
    y_shifts_source = metadata['source-y-shifts'][::angular_factor]
    x_shifts = x_shifts_detector + x_shifts_amc - x_shifts_amc[0] / 2 * np.ones_like(x_shifts_amc)
    y_shifts = y_shifts_detector + y_shifts_amc - y_shifts_amc[0] / 2 * np.ones_like(y_shifts_amc)
