...
class PostgresShell:
    ...
    * dropdb
    ...
    def dump(self, database, destiny_file, jobs=False, exclude=False):
        ...
        dump_cmd = 'pg_dump {dbname} -w -O -x -f {0}'
        if jobs:
            self.__config.update({'num': jobs})
            dump_cmd = 'pg_dump -w  -j {num} -F d {dbname} -f {0} -x'
        if exclude:
    ...
    def restore(self, database_name, dump_name, jobs=False):
        ...
        :type dump_name: str
        :param jobs: How many jobs will be used to restore the database, none by default
        :type dump_name: int
        :returns: If succeeded (True) or not (None).
        :rtype: bool or None
        """
        ...
        restore_cmd = 'psql {dbname} -w -f {0}'
        if jobs:
            restore_cmd = 'pg_restore -w -j {num} -F d {0} -d {dbname} -x -O -n public'
    ...
    def create_extension_unaccent(self, database_name):
        ...
        Keeping this for not braking anything unexpected
        ...
        self.create_extension(database_name, 'unaccent')

    def create_extension(self, database_name, extension_name):
        """ Creates the given extension_name in the database_name.
        The credentials must have permission to create extensions, generally a superuser.

        :param database_name: The database where you want to create the extension
        :param extension_name: The extension name
        :return: None
        """
        config = self.__config.copy()
        config.update({'dbname': database_name})
        logger.info('Creating extension %s in the database %s', extension_name, database_name)
        try:
            with PostgresConnector(config) as db:
                db.execute("create extension {};".format(extension_name))
            ...
            if any(err in error_msg for err in allowed_errors):
                logger.error(
                    'Unable to create the %s extension, authentication for the'
                    ' postgres user in the deployv.conf file failed or the'
                    ' user is not superuser', extension_name
                )
                return
            raise
