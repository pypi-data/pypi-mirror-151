...
class DockerV:
    ...
    def pull(self, image_name=False, raise_exception=False):
        """Make a pull of image with provided name.

        :param image_name: Name of image to pull.
        :type image_name: str
        :param raise_exception: Indicates if raises an exception when no image got from cli's pull
        :type raise_exception: bool
        """
        image_name = image_name or self.config.get('image_name')
        try:
            self.cli_pull_wrapper(image_name=image_name)
        except errors.NoSuchImage as exc:
            if raise_exception:
                raise exc
            image = self.inspect_image(image_name)
            logger.debug('Image info %s', image)

    def cli_pull_wrapper(self, image_name=False):
        ...
        try:
            ...
            if (isinstance(error, docker.errors.NotFound) or
                    'was deleted or has expired' in msg_error):
                logger.debug('Image not found in hub')
                error = errors.NoSuchImage(image_name)
            raise error
    ...
    def inspect_image(self, image_name):
        """Inspect image with provided name, returning its params.

        :param image_name: The image name that will be inspected.
        :type image_name: str
        :returns: The result of docker inspect_image.
        :raises: :class:`~deployv.base.errors.NoSuchImage` when image doesn't exist
        """
        try:
            self.cli_pull_wrapper(image_name)   # just in case does not exist in local
        except errors.NoSuchImage:
            pass
        res = self.cli_inspect_image_wrapper(image_name)
        return res

    def cli_inspect_image_wrapper(self, image_name):
    ...
    def create_container(self, env_vars, volume_binds, volumes):
        ...
        container_config = {
            ...
            "volumes": [
                volume.split(':')[1] if len(volume.split(':')) > 1 else
        ...
        if "5432/tcp" in hconfig.get('PortBindings'):
            default_network_info = self.__cli.inspect_network('bridge')
            default_gateway = default_network_info.get('IPAM').get('Config')[0].get('Gateway')
            host_port = hconfig.get('PortBindings').get("5432/tcp")[0].get('HostPort')
            hconfig.get('PortBindings').update({
                "5432/tcp": [
                    {'HostIp': '127.0.0.1', 'HostPort': host_port},
                    {'HostIp': default_gateway, 'HostPort': host_port}
                ]})
