...
def isConnected():
    import requests
    try:
        html = requests.get("https://pypi.org/project/Fishconsole/", timeout=2)
    except:
        logs.系统日志("》强制更新系统》官网连接失败", UPI)
        return False
    logs.系统日志("》强制更新系统》官网连接成功", UPI)
    return True
    ...
    if isConnected():
        ...
        if res_v != v:
            if sys:
                ...
                print(logs.日志(f"检测到最新版本号发生了变化({res_v}>{v})，请在终端键入pip install --upgrade Fishconsole更新至最新版本", "红色"))
                print(logs.日志(f"Fishconsole 模块已被禁用，解决方式为更新该模块", "红色"))
            else:
                print(logs.日志(f"f_debug已允许Fishconsole以开发者模式运行", "绿色"))
            if sys:
                ...
                if fupdata:
                    ...
                    Fishsys.缓存(1, config)
                Fishsys.缓存(1, config)
                logs.安全退出(f"Fishconsole》help》强制更新启动")
            else:
                logs.日志(f"Fishconsole》f_debug》已取消强制更新", "绿色")
                # 尝试获取缓存中的值
                config = Fishsys.缓存(2)
                fupdata = config.get("updata")
                if fupdata:
        ...
        print(logs.日志(f"Fishconsole{fcv.version()} 检查更新结束"))
        print(logs.日志("你的设备无法访问互联网，请检查网络连接"))

    help_mod = logs.输入(f"这是Fishconsole的开发文档，如果想查看，请按任意字符开始")
    if True:
        res = window.列表选择对话框(
            "我如何重新进入？\n"
            ...
            f"Fishconsole {fcv.version()} 帮助文档",
            ["Matplotlib中文辅助模块", "easygui中文辅助模块", "logs控制台输出辅助模块", "Fishsys模块"])
        logs.系统日志(f"用户请求启动{res}")
        if res == None:
            logs.安全退出("用户取消了操作")
        if res is not None:
            for a in res:
                    ...
                    for a in res:
                        ...
                        if a == "讯飞云控":
                            window.弹窗(
                                ...
                                "语法就是\n"
                                "Fishsys.讯飞云控(fid)\n\n"
                                "讯飞语记fid（如何获得？使用检查，找到相关的json数据就可以拿走fid）"
                                , "讯飞云控", "ok")
                        if a == "密码模块":
                            window.弹窗(
                        ...
                        if a == "缓存模块":
                            window.弹窗(
                                ...
                                "\n-----------------------------\n\n"
                                "语法就是\n\n"
                                "Fishsys.缓存(模式, '字典（请注意，字符串用单引号括起来，千万不要用双引号，否则将过不了加密',缓存文件的名字)\n\n"
                                "模式有两种\n\n1:写\n覆写模式就是创建缓存文件，你需要两个模式和缓存一起使用\n\n2：读\n你只需要两个参数（模式，文件名），他会返回给你一个列表\n但如果你在之前没有创建文件，那么我们将会返回一个布尔值（False）\n\n"
                                "当然，不可能我们在任何时候都会把所有的缓存文件放在一起，所以我允许指定文件的名字\n"
                                "但是每次都在后面指定文件的名字的话就有点废手了，为了避免这种情况，我们可以在开头就指定好文件的名字，这样就可以了\n\n"
                                "示例语法\n"
                                "Filename='1.txt'\n"
                                "a = 缓存(2, '{'a':123,'b':456}',Filename)"
                                , "缓存模块", "ok")
                        if a == "文本转Unicode模块":
                            window.弹窗(
