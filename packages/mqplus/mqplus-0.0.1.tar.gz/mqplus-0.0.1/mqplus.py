...
class RabbitProducer:
    ...
    def __init__(self, host: str, username: str, password: str, port: int = 5672, virtual_host: str = '/',
                 ...
                 exchange: str = None, exchange_type: str = 'direct',
                 queue: str = None,
                 durable: bool = True,
                 **kwargs) -> None:
        ...
        # 绑定交换机和队列
        self.registerQueue(queue=queue, durable=durable)
        self.registerExchange(exchange=exchange, exchange_type=exchange_type, durable=durable)
        self.bind(exchange=exchange, queue=queue)
    ...
    def send(self, message, exchange: str = None, routing_key: str = '#', delivery_mode: int = 2,
    ...
    def sendMessage(self, message, exchange: str = None, routing_key: str = '#', **kwargs):
        """
        发送消息
        """
        return self.send(message=message, exchange=exchange, routing_key=routing_key, **kwargs)

    def put(self, message, queue: str = None, delivery_mode: int = 2, mandatory: bool = False, **kwargs):
        """
        发送消息
        """
        if queue is None:
            queue = self.__queue
        assert queue is not None
        return self.send(exchange='', routing_key=queue, message=message, delivery_mode=delivery_mode,
                         mandatory=mandatory, **kwargs)

    def putMessage(self, message, queue: str = None, **kwargs):
        """
        发送消息
        """
        return self.put(message=message, queue=queue, **kwargs)
