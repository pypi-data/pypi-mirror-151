...
from upswingutil.channel.email import TriggerWelcomeEmailModel
...
def _trigger_email(record: ReservationModelV2):
    try:
        if record.status == Status.CONFIRMED or record.status == Status.CREATED:
