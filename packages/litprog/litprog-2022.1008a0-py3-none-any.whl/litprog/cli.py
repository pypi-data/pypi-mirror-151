...
LOG_FORMAT_VERBOSE = ('%(asctime)s.%(msecs)03d %(levelname)-7s ' +
    '%(name)-16s - %(message)s')
...
def _configure_logging(verbosity: int=0) ->None:
    ...
    logging.basicConfig(level=log_cfg.lvl, format=log_cfg.fmt, datefmt=
        '%Y-%m-%dT%H:%M:%S')
...
def _build(input_paths: InputPaths, html: typ.Optional[str], pdf: typ.
    Optional[str], exitfirst: bool=False, in_place_update: bool=False,
    cache_enabled: bool=True, concurrency: int=DEFAULT_CONCURRENCY) ->None:
    import litprog.build as lp_build
    import litprog.parse as lp_parse
    build_opts = lp_build.BuildOptions(exitfirst=exitfirst, in_place_update
        =in_place_update, cache_enabled=cache_enabled, concurrency=concurrency)
...
_opt_existfirst = click.option('-e', '--exitfirst/--no-exitfirst', default=
    False, help='Exit instantly on first error.')
_opt_in_place = click.option('-i', '--in-place-update', is_flag=True,
    default=False, help=
    'In place update of lp_out and lp_run blocks in markdown files.')
_opt_concurrency = click.option('-n', '--concurrency', default=
    DEFAULT_CONCURRENCY, help='Number of concurrent processes to execute.')
_opt_cache_enabled = click.option('--cache-enabled/--no-cache', is_flag=
    True, default=True, help=
    'Enable/disable block result cache. Default: enabled')
_opt_verbose = click.option('-v', '--verbose', count=True, help=
...
@_opt_cache_enabled
@_opt_verbose
def build(input_paths: InputPaths, html: typ.Optional[str], pdf: typ.
    Optional[str], exitfirst: bool=False, in_place_update: bool=False,
    concurrency: int=DEFAULT_CONCURRENCY, cache_enabled: bool=True, verbose:
    int=0) ->None:
    _configure_logging(verbose)
    import litprog.build as lp_build
    try:
        _build(input_paths, html, pdf, exitfirst, in_place_update,
            cache_enabled, concurrency)
...
@_opt_cache_enabled
@_opt_verbose
def watch(input_paths: InputPaths, html: typ.Optional[str], pdf: typ.
    Optional[str], exitfirst: bool=False, in_place_update: bool=False,
    concurrency: int=DEFAULT_CONCURRENCY, cache_enabled: bool=True, verbose:
    int=0) ->None:
    _configure_logging(verbose)
...
MARKDOWN_FILE_EXTENSIONS = {'markdown', 'mdown', 'mkdn', 'md', 'mkd',
