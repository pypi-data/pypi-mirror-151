...
def init_manifest_entry(task: ct.BlockTask, task_key: str, capture: session
    .Capture) ->tuple[ManifestEntry, CaptureData]:
    ...
    if content_lines:
        ...
        task_info += ' - ' + firstline
    entry = ManifestEntry(created, runtime_ms, capture_size, capture_digest,
        task_key, md_filename, task_info)
...
def _manifest_lines(manifest_entries: list[ManifestEntry]) ->typ.Iterable[str]:
    ...
    for entry in manifest_entries:
        line = ''.join(f'{val:<{colwidths[i]}}  ' if isinstance(val, str) else
            f'{val:>{colwidths[i]}}  ' for i, val in enumerate(entry))
        yield line.rstrip()
...
def parse_manifest(manifest_text: str) ->typ.Iterable[ManifestEntry]:
    ...
    for manifest_entry in manifest_text.splitlines():
        (created, runtime_ms, capture_size, capture_digest, task_key,
            md_path, task_info) = re.split('\\s+', manifest_entry, max_split)
        yield ManifestEntry(created, int(runtime_ms), int(capture_size),
            capture_digest, task_key, md_path, task_info)


def _path_parser(task: ct.BlockTask) ->typ.Iterable[str]:
    for maybe_path in shlex.split(task.command):
        yield maybe_path
    module_match = re.match('python[23]? -m ([\\w\\.]+)', task.command)
    if module_match:
...
class ResultCache:
    ...
    def _reset_task_keys(self, task: ct.BlockTask, entry: ManifestEntry
        ) ->None:
    ...
    def write_capture(self, entry: ManifestEntry, capture_data: CaptureData
        ) ->None:
...
class LocalResultCache(ResultCache):
    ...
    def __init__(self, orig_files: Chapters) ->None:
        cache_subdir = parse_cache_id(orig_files)
        cache_dir = config.CACHE_DIR / cache_subdir
        if not cache_dir.exists():
            cache_dir.mkdir(parents=True)
        self._manifest_file = (cache_dir /
            f'build_cache.manifest_v{SERIAL_VERSION_ID}')
