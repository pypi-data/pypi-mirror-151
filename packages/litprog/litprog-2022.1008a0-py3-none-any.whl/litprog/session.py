...
def dumps_capture(capture: Capture, pretty: bool=False) ->CaptureData:
    ...
    if pretty:
        capture_obj = {'command': capture.command, 'exit_status': capture.
            exit_status, 'runtime': capture.runtime, 'lines_args': line_args}
        capture_json = json.dumps(capture_obj, indent=2)
    else:
        capture_list = [capture.command, capture.exit_status, capture.
            runtime, line_args]
...
def _gen_captured_lines(raw_lines: typ.Iterable[bytes], encoding: str=
    'utf-8', debug_log: bool=False) ->typ.Iterable[RawCapturedLine]:
...
def _read_loop(sp_output_pipe: typ.IO[bytes], captured_lines: list[
    RawCapturedLine], encoding: str='utf-8') ->None:
...
def _start_reader(sp_output_pipe: typ.IO[bytes], encoding: str='utf-8'
    ) ->CapturingThread:
    captured_lines: list[RawCapturedLine] = []
    read_loop_thread = threading.Thread(target=_read_loop, args=(
        sp_output_pipe, captured_lines, encoding))
...
class InteractiveSession:
    ...
    def __init__(self, cmd: AnyCommand, *, env: typ.Optional[Environ]=None,
        encoding: str='utf-8', debug_log: bool=False) ->None:
        _env: Environ
        if env is None:
        ...
        self._proc = sp.Popen(cmd_parts, stdin=sp.PIPE, stdout=sp.PIPE,
            stderr=sp.PIPE, env=_env)
    ...
    def _assert_retcode(self) ->None:
        if self._retcode is None:
            cls = type(self)
            msg = (
                f"'{cls}.wait()' must be called before accessing captured output."
                )
