...
PAPER_FORMATS_PT = {'A6': (105 * PT_PER_MM, 148 * PT_PER_MM), 'A5': (148 *
    PT_PER_MM, 210 * PT_PER_MM), 'A4': (210 * PT_PER_MM, 297 * PT_PER_MM),
    'A3': (297 * PT_PER_MM, 420 * PT_PER_MM), 'Letter': (8.5 * PT_PER_INCH,
    11 * PT_PER_INCH)}
Points = float


def parse_to_pt(size: str, w_pt: Points, h_pt: Points) ->typ.Tuple[Points,
    Points]:
    if size.endswith('%'):
...
def _calc_op_coords(pdf_doc: PDFDocument, op: str, raw_ops: typ.Iterator[str]
    ) ->OpCoordinantes:
    ...
    print(
        f'{op} w:{next_w_pt} h:{next_h_pt} s:{scale_factor} x:{translate_x} y:{translate_y}'
        )
    return OpCoordinantes(next_w_pt, next_h_pt, scale_factor, translate_x,
        translate_y)
...
def main(args: typ.Sequence[str]=sys.argv[1:]) ->int:
    ...
    for op in raw_ops:
        ...
        for cur_page in curr_pdf.pages:
            next_page = next_pdf.addBlankPage(width=op_coords.next_w_pt,
                height=op_coords.next_h_pt)
            next_page.mergeScaledTranslatedPage(cur_page, op_coords.
                scale_factor, op_coords.translate_x, op_coords.translate_y,
                expand=False)
