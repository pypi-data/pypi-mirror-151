...
BOOKLET_FORMAT_MAPPING = {'A6-Portrait': ('A5-Landscape', 'booklet', 0 *
    PT_PER_MM), 'A5-Portrait': ('A4-Landscape', 'booklet', 0 * PT_PER_MM),
    ...
    '1of2col-A4': ('A4-Portrait', 'paper', 0 * PT_PER_MM), '1of2col-Letter':
    ('Letter-Portrait', 'paper', 0 * PT_PER_MM), 'Letter-Portrait': (
    'A4-Landscape', 'booklet', -5 * PT_PER_MM)}
PAPER_FORMATS_PT = {'A6-Portrait': (105 * PT_PER_MM, 148 * PT_PER_MM), 'A6':
    (105 * PT_PER_MM, 148 * PT_PER_MM), 'A5-Landscape': (210 * PT_PER_MM, 
    148 * PT_PER_MM), 'A5-Portrait': (148 * PT_PER_MM, 210 * PT_PER_MM),
    'A5': (148 * PT_PER_MM, 210 * PT_PER_MM), 'A4-Landscape': (297 *
    PT_PER_MM, 210 * PT_PER_MM), 'A4-Portrait': (210 * PT_PER_MM, 297 *
    PT_PER_MM), 'A4': (210 * PT_PER_MM, 297 * PT_PER_MM), 'A3-Portrait': (
    297 * PT_PER_MM, 420 * PT_PER_MM), 'A3': (297 * PT_PER_MM, 420 *
    PT_PER_MM), '1of2col-A4': (105 * PT_PER_MM, 297 * PT_PER_MM),
    '1of2col-Letter': (4.25 * PT_PER_INCH, 11 * PT_PER_INCH), 'Half-Letter':
    (5.5 * PT_PER_INCH, 8.5 * PT_PER_INCH), 'Letter-Landscape': (11 *
    PT_PER_INCH, 8.5 * PT_PER_INCH), 'Letter-Portrait': (8.5 * PT_PER_INCH,
    11 * PT_PER_INCH), 'Letter': (8.5 * PT_PER_INCH, 11 * PT_PER_INCH)}
_BOOKLET_PAGE_NUMBERING_ILLUSTRATION = """
...
def get_format_id(page_width_pt: float, page_height_pt: float, epsilon_pt:
    float=2.0) ->typ.Optional[str]:
    best_format_id: typ.Optional[str] = None
    best_delta = 999999
    for format_id, (fmt_width_pt, fmt_height_pt) in PAPER_FORMATS_PT.items():
        delta = abs(fmt_width_pt - page_width_pt) + abs(fmt_height_pt -
            page_height_pt)
        if delta < best_delta:
...
def parse_output_parameters(in_page_width: float, in_page_height: float,
    out_format: str) ->OutputParameters:
    ...
    logger.info(
        f'         in: {_iw_mm}mm x {_ih_mm}mm -> {_sw_mm}mm x {_sh_mm}mm (2x)'
        )
    logger.info(f'        out: {_ow_mm}mm x {_oh_mm}mm')
    logger.info(f'    offsets: {lx_offset}mm {rx_offset}mm')
    return OutputParameters(out_sheet_width, out_sheet_height, scale,
        pad_center, lx_offset, rx_offset)
...
def fit_pdf(in_path: pl.Path, crop_width: int, crop_height: int, out_format:
    str) ->typ.Iterator[pl.Path]:
    ...
    with in_path.open(mode='rb') as in_fobj:
        ...
        for in_page in pages:
            ...
            if abs(1 - scale) > 0.02:
                out_page.mergeScaledTranslatedPage(in_page, scale=scale, tx
                    =translate_x, ty=translate_y, expand=False)
            else:
                out_page.mergeTranslatedPage(in_page, tx=translate_x, ty=
                    translate_y, expand=False)
...
def _create(in_path: pl.Path, out_path: pl.Path, out_format: str,
    max_section_pages: int) ->pl.Path:
    ...
    assert _x0 == 0
    assert _y0 == 0
    out_params = parse_output_parameters(in_page_width, in_page_height,
        out_format)
...
def create(in_path: pl.Path, out_path: typ.Optional[pl.Path]=None,
    out_format: str='A4-Landscape', max_section_sheets: int=
    MAX_SECTION_SHEETS, crop_width: int=0, crop_height: int=0, autofit:
    bool=True) ->pl.Path:
    assert in_path.exists(), f'No such file: {in_path}'
    if out_format not in PAPER_FORMATS_PT:
        valid_formats = list(PAPER_FORMATS_PT)
        errmsg = (
            f'Invalid out_format {out_format}. Valid formats: {valid_formats}')
