...
class Query:
    ...
    def filter(self, cols: List[str], **conditions) -> 'Query':
        if not self.apply_range:
            raise LogicException('Must apply range first')
        self.cols = cols
        if len(cols) < 2:
            self.lines.append(f'  |> filter(fn: (r) => r["_field"] == "{cols[0]}")')
        else:
            conds = " or ".join([f'r["_field"]  == "{col}"' for col in cols])
            self.lines.append(f'  |> filter(fn: (r) => {conds})')
        if conditions:
