import time, pyperclip, os, darkdetect, threading, base64, winshell
...
'''
    ...
    def create_shortcut(self, source=__file__, target=None, name=None, only_exe=False):

        if only_exe and not source.endswith('.exe'):
            return 'Source file is not exe'

        if not target:
            target = self.get_shell_startup()

        if not name:
            name = self.app_name

        try:
            path = rf"{target}\{name}.lnk"
            ...
            shortcut.save()
            return 'Successfully created shortcut'
    ...
    @thread
    def writeBase64(self, file, name=None):
        if name is None:
            name = os.path.basename(file).split('.')[0]

        with open(file, 'rb') as b:
            with open('bases.py', 'a') as f:
                f.write(f'\n\n{name} = ["{base64.b64encode(b.read()).decode()}", "{os.path.basename(file)}"]')


    def getBase64(self, file, name=None):
        ...
        base, full_name = eval(f'bases.{name}')

        with open(self.appdata_path + '/' + full_name, 'wb') as f:
            f.write(base64.b64encode(base.encode()))
