...
def daily_factor_on300500(fac,hs300=False,zz500=False,zz1000=False,other=False):
    ...
    if fac.shape[0]/last.shape[0]>2:
        if hs300:
            ...
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index')
            df=df*fac
            df=df.dropna(how='all')
        elif other:
            df1=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date').fillna(0)
            df2=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date').fillna(0)
            df3=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').fillna(0)
            df=np.sign(df1-1)*np.sign(df2-1)*np.sign(df3-1)
        ...
        if hs300:
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date').resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif zz500:
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date').resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif zz1000:
            df=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif other:
            df1=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/沪深300成分股.feather').set_index('date').resample('M').last().fillna(0)
            df2=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证500成分股.feather').set_index('date').resample('M').last().fillna(0)
            df3=pd.read_feather('/Users/chenzongwei/pythoncode/数据库/中证1000成分股.feather').set_index('index').resample('M').last().fillna(0)
            df=np.sign(df1-1)*np.sign(df2-1)*np.sign(df3-1)
            df=df.replace(0,np.nan)*fac
            df=df.dropna(how='all')
        else:
...
def boom_four(df,minus=None,backsee=20,daily=False,min_periods=20):
    '''使用20天均值和标准差，生成4个因子'''
    if not daily:
...
def read_index_three(day=False):
    ...
    def read(file):
        file1=f'/Users/chenzongwei/pythoncode/数据库/日频数据/{file}日行情.xlsx'
...
def coin_reverse(ret20,vol20,mean=0,positive_negtive=1):
    '''根据vol20的大小，翻转一半ret20，把vol20较大的部分，给ret20添加符号'''
    if positive_negtive:
        if not mean:
    ...
    def run(self, groups_num=10, neutralize=False, boxcox=False, value_weighted=False,y2=False, plt_plot=True,
        ...
        if sheetname:
            ...
            if net_values_writer:
                ...
                groups_net_values.columns=[f'分组{i}' for i in range(1,len(list(groups_net_values.columns)))]+['多空对冲']
            ...
            if rets_writer:
                ...
                group_rets.columns=[f'分组{i}' for i in range(1,len(list(group_rets.columns)))]+['多空对冲']
            ...
            if comments_writer and comments_sheetname:
                ...
                tc[9]=str(round(tc[9]*100,2))+'%'
                new_total_comments=pd.DataFrame({sheetname:tc},index=total_comments.index)
                new_total_comments.to_excel(comments_writer,sheet_name=comments_sheetname)
