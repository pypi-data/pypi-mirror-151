...
from explorer.client import exceptions
...
class _OAuth2KeyRingTokenCache(_OAuth2TokenCache):
    ...
    def get_token_from_cache(self, host: str) -> Optional[str]:
        try:
            ...
            raise explorer.exceptions.NotSupportedError("Although keyring module is installed no backend has been "
...
class _OAuth2TokenBearer(AuthBase):
    """
    Custom implementation of Explorer Oauth2 based authorization to get the token
    """
    ...
    def _attempt_oauth(self, response, **kwargs):
        ...
        if not auth_info:
            raise exceptions.ExplorerAuthError("Error: header WWW-Authenticate not available in the response.")

        if not _OAuth2TokenBearer._BEARER_PREFIX.search(auth_info):
            raise exceptions.ExplorerAuthError(f"Error: header info didn't match {auth_info}")
        ...
        if auth_server is None:
            raise exceptions.ExplorerAuthError("Error: header info didn't have x_redirect_server")

        token_server = auth_info_headers.get('x_token_server')
        if token_server is None:
            raise exceptions.ExplorerAuthError("Error: header info didn't have x_token_server")
        ...
        while attempts < self.MAX_OAUTH_ATTEMPTS:
            ...
            with response.connection.send(Request(method='GET', url=token_server).prepare(), **kwargs) as response:
                if response.status_code == 200:
                    ...
                    if error:
                        raise exceptions.ExplorerAuthError(f"Error while getting the token: {error}")
                    else:
                        token_server = token_response.get('nextUri')
                        logger.debug(f"nextURi auth token server: {token_server}")
                else:
                    raise exceptions.ExplorerAuthError(
                        f"Error while getting the token response "
                        f"status code: {response.status_code}, "
                        f"body: {response.text}")

        raise exceptions.ExplorerAuthError("Exceeded max attempts while getting the token")
