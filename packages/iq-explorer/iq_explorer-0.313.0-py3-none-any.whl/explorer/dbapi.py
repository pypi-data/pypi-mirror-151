...
from explorer import constants
import explorer.exceptions
import explorer.client
import explorer.logging
from explorer.transaction import Transaction, IsolationLevel, NO_TRANSACTION
from explorer.exceptions import (
    Warning,
    Error,
    InterfaceError,
    DatabaseError,
...
class Connection(object):
    """Explorer supports transactions and the ability to either commit or rollback
    ...
    def __init__(
        ...
        if http_session is None:
            self._http_session = explorer.client.ExplorerRequest.http.Session()
    ...
    def close(self):
        """Explorer does not have anything to close"""
    ...
    def _create_request(self):
        return explorer.client.ExplorerRequest(
            self.host,
            self.port,
            self.user,
            self.source,
...
class Cursor(object):
    ...
    def rowcount(self):
        ...
        Explorer cannot reliablity determine the number of rows returned by an
    ...
    def setinputsizes(self, sizes):
        raise explorer.exceptions.NotSupportedError
    ...
    def _prepare_statement(self, operation, statement_name):
        ...
        :raises explorer.exceptions.FailedToObtainAddedPrepareHeader: Error raised
        ...
        query = explorer.client.ExplorerQuery(copy.deepcopy(self._request), sql=sql,
        ...
        raise explorer.exceptions.FailedToObtainAddedPrepareHeader

    def _get_added_prepare_statement_explorer_query(
        self,
        statement_name,
        params
    ):
        ...
        # operation
        return explorer.client.ExplorerQuery(self._request, sql=sql, experimental_python_types=self._experimental_pyton_types)
    ...
    def _format_prepared_param(self, param):
        ...
        raise explorer.exceptions.NotSupportedError("Query parameter of type '%s' is not supported." % type(param))
    ...
    def _deallocate_prepare_statement(self, added_prepare_header, statement_name):
        ...
        raise explorer.exceptions.FailedToObtainDeallocatedPrepareHeader
    ...
    def execute(self, operation, params=None):
        if params:
            ...
            try:
                ...
                self._query = self._get_added_prepare_statement_explorer_query(
            ...
            self._query = explorer.client.ExplorerQuery(self._request, sql=operation,
                                                  experimental_python_types=self._experimental_pyton_types)
            result = self._query.execute()
    ...
    def fetchone(self) -> Optional[List[Any]]:
        ...
        try:
            ...
            return None
        except explorer.exceptions.HttpError as err:
            raise explorer.exceptions.OperationalError(str(err))
    ...
    def cancel(self):
        if self._query is None:
            raise explorer.exceptions.OperationalError(
...
ROWID = DBAPITypeObject()  # nothing indicates row id in Explorer
