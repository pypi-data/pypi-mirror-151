...
This module implements the Explorer protocol to submit SQL statements, track
their state and retrieve their result as described in
https://github.com/explorerdb/explorer/wiki/HTTP-Protocol
and Explorer source code.
...
Explorer queries are managed by the ``ExplorerQuery`` class. HTTP requests are
managed by the ``ExplorerRequest`` class. the status of a query is represented
by ``ExplorerStatus`` and the result by ``ExplorerResult``.


The main interface is :class:`ExplorerQuery`: ::

    >> request = ExplorerRequest(host='coordinator', port=8080, user='test')
    >> query =  ExplorerQuery(request, sql)
...
import explorer.logging
from explorer import constants, exceptions
from explorer.transaction import NO_TRANSACTION

__all__ = ["ExplorerQuery", "ExplorerRequest", "PROXIES"]

logger = explorer.logging.get_logger(__name__)
...
class ExplorerRequest(object):
    """
    Manage the HTTP requests of a Explorer query.
    ...
    :param catalog: to query. The *catalog* is associated with a Explorer
    ...
    :param session_properties: set specific Explorer behavior for the current
    ...
    Please refer to :class:`ExplorerStatus` to access the status returned by
    :meth:`ExplorerRequest.process`.
    ...
    def _process_error(self, error, query_id):
        ...
        if error_type == "EXTERNAL":
            raise exceptions.ExplorerExternalError(error, query_id)
        elif error_type == "USER_ERROR":
            return exceptions.ExplorerUserError(error, query_id)

        return exceptions.ExplorerQueryError(error, query_id)
    ...
    def process(self, http_response) -> ExplorerStatus:
...
class ExplorerResult(object):
    """
    Represent the result of a Explorer query as an iterator on rows.
    ...
    def _map_to_python_type(cls, item: Tuple[Any, Dict]) -> Any:
        ...
        try:
            ...
            raise explorer.exceptions.ExplorerDataError(error_str) from e
...
class ExplorerQuery(object):
    """Represent the execution of a SQL statement by Explorer."""
    ...
    def __init__(
        ...
        self._sql = sql
        self._result = ExplorerResult(self, experimental_python_types=experimental_python_types)
    ...
    def execute(self, additional_http_headers=None) -> ExplorerResult:
        """Initiate a Explorer query by sending the SQL statement
        ...
        """
        if self.cancelled:
            raise exceptions.ExplorerUserError("Query has been cancelled", self.query_id)
        ...
        if status.next_uri is None:
            self._finished = True
        self._result = ExplorerResult(self, status.rows, self._experimental_python_types)
        return self._result
