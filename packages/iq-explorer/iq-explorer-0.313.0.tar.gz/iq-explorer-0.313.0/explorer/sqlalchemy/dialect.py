...
from explorer import dbapi as explorer_dbapi, logging
from explorer.auth import BasicAuthentication, JWTAuthentication
from explorer.dbapi import Cursor
from explorer.exceptions import ExplorerUserError
from explorer.sqlalchemy import compiler, datatype, error

logger = logging.get_logger(__name__)


class ExplorerDialect(DefaultDialect):
    name = "explorer"
    driver = "rest"

    statement_compiler = compiler.ExplorerSQLCompiler
    ddl_compiler = compiler.ExplorerDDLCompiler
    type_compiler = compiler.ExplorerTypeCompiler
    preparer = compiler.ExplorerIdentifierPreparer
    ...
    # DML
    # Queries of the form `INSERT () VALUES ()` is not supported by Explorer.
    ...
    def get_pk_constraint(self, connection: Connection, table_name: str, schema: str = None, **kw) -> Dict[str, Any]:
        """Explorer has no support for primary keys. Returns a dummy"""
    ...
    def get_foreign_keys(
        ...
        """Explorer has no support for foreign keys. Returns an empty list."""
        return []
    ...
    def get_temp_table_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        """Explorer has no support for temporary tables. Returns an empty list."""
        return []
    ...
    def get_temp_view_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        """Explorer has no support for temporary views. Returns an empty list."""
        return []
    ...
    def get_sequence_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        """Explorer has no support for sequences. Returns an empty list."""
        return []

    def get_unique_constraints(
        self, connection: Connection, table_name: str, schema: str = None, **kw
    ) -> List[Dict[str, Any]]:
        """Explorer has no support for unique constraints. Returns an empty list."""
        return []

    def get_check_constraints(
        self, connection: Connection, table_name: str, schema: str = None, **kw
    ) -> List[Dict[str, Any]]:
        """Explorer has no support for check constraints. Returns an empty list."""
        return []
    ...
    def get_table_comment(self, connection: Connection, table_name: str, schema: str = None, **kw) -> Dict[str, Any]:
        ...
        except error.ExplorerQueryError as e:
            if e.error_name in (
    ...
    def has_sequence(self, connection: Connection, sequence_name: str, schema: str = None, **kw) -> bool:
        """Explorer has no support for sequence. Returns False indicate that given sequence does not exists."""
        return False
    ...
    def _get_default_schema_name(self, connection: Connection) -> Optional[str]:
        dbapi_connection: explorer_dbapi.Connection = connection.connection
    ...
    def do_execute(
        ...
        if context and context.should_autocommit:
            # SQL statement only submitted to Explorer server when cursor.fetch*() is called.
    ...
    def do_rollback(self, dbapi_connection: explorer_dbapi.Connection):
        if dbapi_connection.transaction is not None:
            dbapi_connection.rollback()

    def set_isolation_level(self, dbapi_conn: explorer_dbapi.Connection, level: str) -> None:
        dbapi_conn._isolation_level = explorer_dbapi.IsolationLevel[level]

    def get_isolation_level(self, dbapi_conn: explorer_dbapi.Connection) -> str:
        return dbapi_conn.isolation_level.name

    def get_default_isolation_level(self, dbapi_conn: explorer_dbapi.Connection) -> str:
        return explorer_dbapi.IsolationLevel.AUTOCOMMIT.name
