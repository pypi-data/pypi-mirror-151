...
# https://explorer.io/docs/current/language/types.html
_type_map = {
    # === Boolean ===
...
def parse_sqltype(type_str: str) -> TypeEngine:
    ...
    if type_name == "array":
        ...
        if isinstance(item_type, sqltypes.ARRAY):
            ...
            # `ARRAY(ARRAY(INT))` in Explorer SQL will become `ARRAY(INT(), dimensions=2)` in SQLAlchemy
    ...
    if type_name in ("time", "timestamp"):
        ...
        # TODO: support parametric timestamps (https://github.com/explorerdb/explorer-python-client/issues/107)
