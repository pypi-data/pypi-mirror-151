...
class Transaction(object):
    ...
    def begin(self):
        ...
        if not response.ok:
            raise explorer.exceptions.DatabaseError(
    ...
    def commit(self):
        query = explorer.client.ExplorerQuery(self._request, COMMIT)
        try:
            list(query.execute())
    ...
    def rollback(self):
        query = explorer.client.ExplorerQuery(self._request, ROLLBACK)
        try:
            list(query.execute())
        except Exception as err:
            raise explorer.exceptions.DatabaseError(
