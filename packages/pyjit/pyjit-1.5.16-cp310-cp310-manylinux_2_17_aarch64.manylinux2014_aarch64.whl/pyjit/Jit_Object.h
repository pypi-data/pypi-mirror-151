...
public:
	...
	static Object CreateJitObject(void* lib, const char* moduleName,
		const char* objTypeName, GalaxyJitPtr args)
	{
		return g_pHost->CreateJitObject(lib, moduleName, objTypeName, args);
	}
	bool Pack(std::vector<GalaxyJitPtr>& objList)
	{
		m_p = g_pHost->Pack(objList);
		return true;
	}
	bool Unpack(std::vector<GalaxyJitPtr>& objList)
	{
		return g_pHost->Unpack(m_p, objList);
	}
	bool PackTo(JitStream* pStream)
	{
		return g_pHost->PackTo(m_p, pStream);
	}
	void UnpackFrom(JitStream* pStream)
	{
		m_p = g_pHost->UnpackFrom(pStream);
	}
	...
	Object(GalaxyJitPtr p)
	{
		m_p = p;
	}
	Object(GalaxyJitPtr p, bool bAddRef)
	{
		m_p = p;
	...
	Object(GalaxyJitPtr p, Action act)
	{
		m_p = p;
		switch (act)
		{
		case PyJit::Action::AddRef:
			if (m_p != nullptr)
			{
				g_pHost->AddRef(m_p);
			}
			break;
		case PyJit::Action::ConvertToPythonObject:
			if(p!=nullptr)
			{
	...
	inline Object Call(int size, GalaxyJitPtr* ptrs)
	{
		return g_pHost->Call(m_p, size, ptrs);
	}
	inline Object Call(int size, GalaxyJitPtr* ptrs, GalaxyJitPtr kwargs)
	{
		return g_pHost->Call(m_p, size, ptrs, kwargs);
	}
	inline Object Call(GalaxyJitPtr args,GalaxyJitPtr kwargs)
	{
	...
	explicit operator TClass* () const
	{
		return (TClass*)g_pHost->GetClassProxyNative(m_p);
	}
	void Empty()
	{
		if (m_p)
...
{
public:
	Dynamic(GalaxyJitPtr p)
	{
		g_pHost->ParseModule(p);
	}
	~Dynamic()
	{
...
public:
	...
	Native(GalaxyJitPtr p, unsigned int secret=0)
	{
		m_secret = secret;
		if (p)
		{
			m_point = (T*)g_pHost->GetClassProxyNative(p);
		}
	}
	~Native()
	{
...
private:
	T* m_point;
	unsigned int m_secret=0;//encoded by PyJit Compiler, the hightest byte: 1=prop.get,2=prop.set,3=method, 
	//other bytes for order
};
class None:
	public Object
{
...
Object Extract(GalaxyJitPtr self,const char* class_name,Native_Class* pNativeObj)
{
	if (hasPythonProxy)
	{
	...
	{
		return g_pHost->QueryOrCreate(self,class_name,pNativeObj);
	}
}
...
public:
	...
	Stream(unsigned long long streamId):
		Stream()
	{
		m_id = streamId;
	}
	inline bool Write(char* data, long long size)
	{
		return g_pHost->StreamWrite(m_id, data, size);
	}
	inline bool Read(char* data, long long size)
	{
		return g_pHost->StreamRead(m_id, data, size);
	}
	template<typename T>
	Stream& operator<<(T v)
	{
		g_pHost->StreamWrite(m_id,(char*)&v, sizeof(v));
		return *this;
	}
	Stream& operator<<(const char c)
	{
		g_pHost->StreamWriteChar(m_id, c);
		return *this;
	}
	Stream& operator<<(std::string v)
	{
		g_pHost->StreamWriteString(m_id,v);
		return *this;
	}
	Stream& operator<<(const char* str)
	{
		g_pHost->StreamWrite(m_id, (char*)str, strlen(str) + 1);
		return *this;
	}
	template<typename T>
	Stream& operator>>(T& v)
	{
		g_pHost->StreamRead(m_id, (char*)&v, sizeof(v));
		return *this;
	}
	Stream& operator>>(std::string& v)
	{
		g_pHost->StreamReadString(m_id, v);
		return *this;
	}
	Stream& operator>>(char& c)
	{
		g_pHost->StreamReadChar(m_id, c);
		return *this;
	}
private:
