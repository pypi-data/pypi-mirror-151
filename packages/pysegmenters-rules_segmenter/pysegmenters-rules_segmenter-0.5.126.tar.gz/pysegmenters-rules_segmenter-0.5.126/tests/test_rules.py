...
from pysegmenters_rules_segmenter.rules_segmenter import RulesSegmenter, RulesSegmenterParameters, get_sentencizer_from_params, RuleEngine
...
The company doesn’t expect to meet analysts’ profit estimates of $3.9 to $4 million, or 76 cents a share to 79 cents a share, for its year ending Sept. 24, according to Pres. John Doe."""
    ...
    parameters = RulesSegmenterParameters(lang='en', engine=RuleEngine.Custom, join_rules=[])
    ...
    # With abbrev
    parameters = RulesSegmenterParameters(lang='en', engine=RulesEngine.Custom)
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    ...
    # With PySDB
    parameters = RulesSegmenterParameters(lang='en', engine=RulesEngine.PySBD)
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    assert len(doc0.sentences) == 4
    sents = [doc0.text[s.start:s.end] for s in doc0.sentences]
...
रईसी ने कहा कि ईरान के रक्षा सिद्धांत में परमाणु हथियार और प्रतिरोधक नीति (डेटरेंस पॉलिसी) की कोई जगह नहीं है. अमेरिकी राष्ट्रपति जो बाइडन ने यूएन महासभा को संबोधित करते मंगलवार को कहा था कि अमेरिका अपने रुख़ पर कायम है कि वो ईरान को परमाणु हथियार हासिल नहीं करने देगा."""
    ...
    assert model_class == RulesSegmenterParameters
    segmenter = RulesSegmenter()
    parameters = RulesSegmenterParameters(lang='hi', engine=RuleEngine.Custom)
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    assert len(doc0.sentences) == 10
    # sents = [doc0.text[s.start:s.end] for s in doc0.sentences]
    # With PySDB
    parameters = RulesSegmenterParameters(lang='en', engine=RuleEngine.PySBD)
    docs: List[Document] = segmenter.process([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    assert len(doc0.sentences) == 26
    # sents = [doc0.text[s.start:s.end] for s in doc0.sentences]
...
"""
    ...
    assert model_class == RulesSegmenterParameters
    segmenter = RulesSegmenter()
    parameters = RulesSegmenterParameters(lang='fr', engine=RuleEngine.Custom, split_rules=SPLIT_DEFAULT, join_rules=[])
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    ...
    # With PySBD
    parameters = RulesSegmenterParameters(lang='fr', engine=RuleEngine.PySBD)
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    assert len(doc0.sentences) == 10
    sents = [doc0.text[s.start:s.end] for s in doc0.sentences]
...
def test_cached_nlp():
    parameters1 = RulesSegmenterParameters(engine=RuleEngine.Custom)
    nlp1, enable_pipes1 = get_sentencizer_from_params(parameters1)
    parameters2 = RulesSegmenterParameters(engine=RuleEngine.Custom, split_rules=[[{}]])
