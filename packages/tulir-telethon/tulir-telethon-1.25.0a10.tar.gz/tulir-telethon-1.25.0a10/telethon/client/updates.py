...
from .._updates.messagebox import ENTRY_ACCOUNT
            ...
            while self.is_connected():
                ...
                if get_diff:
                    self._log[__name__].info(
                        'Getting difference for account updates (seq: %d, pts: %d, date: %s)',
                        self._message_box.seq, get_diff.pts, get_diff.date
                    )
                    diff = await self(get_diff)
                    updates, users, chats = self._message_box.apply_difference(diff, self._mb_entity_cache)
                    self._log[__name__].info(
                        'New account state after applying difference (%s): seq: %d, pts: %d, date: %s',
                        type(diff).__name__, self._message_box.seq,
                        self._message_box.map[ENTRY_ACCOUNT].pts, self._message_box.date
                    )
