...
from docums import __version__
from docums import utils
from docums import config
from docums.commands import build, gh_deploy, new, serve
...
class State:
    ...
    def __init__(self, log_name='docums', level=logging.INFO):
        ...
        self.stream.name = 'DocumsStreamHandler'
...
config_help = "Provide a specific Docums config"
dev_addr_help = ("IP address and port to serve documentation locally (default: "
                 "localhost:8000)")
strict_help = ("Enable strict mode. This will cause Docums to abort the build "
               "on any warnings.")
...
commit_message_help = ("A commit message to use when committing to the "
                       "Github Pages remote branch. Commit {sha} and Docums {version} are available as expansions")
...
ignore_version_help = "Ignore check that build is not being deployed with an older version of Docums."
...
def cli():
    """
    Docums - Project documentation with Markdown.
    """
...
def build_command(clean, **kwargs):
    """Build the Docums documentation"""
