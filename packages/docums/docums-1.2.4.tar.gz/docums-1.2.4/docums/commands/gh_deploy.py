...
import docums
import ghp_import
from docums.exceptions import Abort

log = logging.getLogger(__name__)

default_message = """Deployed {sha} with Docums version: {version}"""
...
def _check_version(branch):
    ...
    currentv = version.parse(docums.__version__)
    if not previousv:
        log.warning('Version check skipped: No version specified in previous deployment.')
    elif currentv > previousv:
        log.info(
            f'Previous deployment was done with Docums version {previousv}; '
            f'you are deploying with a newer version ({currentv})'
        )
    elif currentv < previousv:
        log.error(
            f'Deployment terminated: Previous deployment was made with Docums version {previousv}; '
...
def gh_deploy(config, message=None, force=False, ignore_version=False, shell=False):
    ...
    message = message.format(version=docums.__version__, sha=sha)
