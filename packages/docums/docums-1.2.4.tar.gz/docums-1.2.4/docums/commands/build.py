...
from docums.exceptions import BuildError, Abort
from docums.structure.files import Files, get_files
from docums.structure.nav import get_navigation
import docums
...
def build(config, live_server=False, dirty=False):
    ...
    logger = logging.getLogger('docums')

    # Add CountHandler for strict mode
    warning_counter = utils.CountHandler()
    warning_counter.setLevel(logging.WARNING)
    if config['strict']:
        logging.getLogger('docums').addHandler(warning_counter)
