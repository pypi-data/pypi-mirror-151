"""
Implements the plugin API for Docums.
...
from docums.config.base import Config


log = logging.getLogger('docums.plugins')
...
def get_plugins():
    ...
    plugins = importlib_metadata.entry_points(group='docums.plugins')

    # Allow third-party plugins to override core plugins
    pluginmap = {}
    for plugin in plugins:
        if plugin.name in pluginmap and plugin.value.startswith("docums.contrib."):
            continue
