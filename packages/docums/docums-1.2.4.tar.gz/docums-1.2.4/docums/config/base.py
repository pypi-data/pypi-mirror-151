...
log = logging.getLogger('docums.config')
...
class Config(UserDict):
    ...
    def load_file(self, config_file):
        ...
        try:
            ...
            # Docums knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                f"Docums encountered an error parsing the configuration file: {e}"
            )
...
def _open_config_file(config_file):
    ...
    When None, it defaults to `docums.yml` in the CWD. If a closed file descriptor
    ...
    if config_file is None:
        paths_to_try = ['docums.yml', 'docums.yaml']
...
def load_config(config_file=None, **kwargs):
    ...
    the default `docums.yml` filename will loaded.
    ...
    with _open_config_file(config_file) as fd:
        ...
        from docums.config.defaults import get_schema
