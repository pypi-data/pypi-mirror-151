...
hub.exec.boto3.client.lambda.create_function
hub.exec.boto3.client.lambda.delete_function
hub.exec.boto3.client.lambda.get_function
hub.exec.boto3.client.lambda.update_function_code
...
async def present(
    ...
    Args:
        ...
        code(dict): The code for the function.
            ZipFile (bytes) --
                The base64-encoded contents of the deployment package. Amazon Web Services SDK and Amazon Web
        ...
        image_config(dict, optional): Container image configuration values that override the values in the container
                image Dockerfile.
        file_system_configs(list, optional): Connection settings for an Amazon EFS file system.
        ...
        tags(dict, optional): A list of tags to apply to the function.
        tracing_config(dict, optional): Set Mode to Active to sample and trace a subset of incoming requests with X-Ray.
        ...
        environment(dict, optional): Environment variables that are accessible from function code during execution.
        dead_letter_config(dict, optional): A dead letter queue configuration that specifies the queue or topic where
        ...
        package_type(Text, optional): The type of deployment package. Set to Image for container image and set Zip for
                ZIP archive.
        vpc_config(dict, optional): For network connectivity to Amazon Web Services resources in a VPC, specify a list
                of security groups and subnets in the VPC. When you connect a function to a VPC, it can only access
                resources and the internet through that VPC.
        ...
        timeout(Dict, optional): Timeout configuration for create/update/deletion of AWS Lambda Function.
            * create (Dict) -- Timeout configuration for creating AWS Lambda Function
            ...
            * update (Text) -- Timeout configuration for updating AWS Lambda Function
                * delay -- The amount of time in seconds to wait between attempts.
                * max_attempts -- Customized timeout configuration containing delay and max attempts.
        hub:
        ctx:
    ...
    if before["result"] and before["ret"].get("Configuration"):
        ...
        code_config = before["ret"].get("Code")
        function_arn = function.get("FunctionArn")
        tags_return = await hub.exec.boto3.client["lambda"].list_tags(
            ctx, Resource=function_arn
        )
        ...
        if result["result"]:
            ...
            return result
        resource_converted = await hub.tool.aws.lambda_.conversion_utils.convert_raw_lambda_function_to_present(
            ctx,
        ...
        update_return = await hub.exec.aws.lambda_.function.compare_inputs_and_update_lambda_function(
            ctx=ctx,
            name=name,
            function=function,
            update=update,
        ...
        if tags is not None:
            # Update tags
            update_tags_result = await hub.exec.aws.lambda_.tag.update_tags(
                ctx=ctx,
...
async def absent(
    ...
    if not before["result"]:
        ...
        tags = await hub.exec.boto3.client["lambda"].list_tags(
            ctx, Resource=before["ret"].get("Configuration").get("FunctionArn")
        )
        if tags.get("result"):
        ...
        for function in ret["ret"].get("Functions"):
            ...
            if not resource_converted["result"]:
                ...
                result[function.get("FunctionName")] = {
                    "aws.lambda.function.present": [resource_converted["ret"]]
                }
    return result
