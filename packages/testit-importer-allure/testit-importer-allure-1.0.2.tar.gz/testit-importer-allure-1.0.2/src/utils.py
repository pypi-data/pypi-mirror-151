...
def form_setup_teardown(data_before_after, uuid, requests, path):
    ...
    if uuid:
        for time in data_before_after:
            if 'children' in data_before_after[time]:
                for child in data_before_after[time]['children']:
                    if child == uuid:
                        steps, results_steps = form_steps(data_before_after[time].get('befores'), requests, path)
                        setup += steps
                        results_setup += results_steps
                        steps, results_steps = form_steps(data_before_after[time].get('afters'), requests, path)
                        teardown += steps
