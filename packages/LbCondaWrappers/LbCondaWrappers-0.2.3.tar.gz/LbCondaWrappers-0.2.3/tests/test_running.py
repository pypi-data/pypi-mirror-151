...
from LbCondaWrappers import (
    CONDA_ENVIRONMENTS,
    DISPLAY_VERSIONS,
    cvmfs_lhcbdev_available,
)
...
def get_random_env():
    ...
    raise ValueError("Failed to find a version to test with")
...
def test_cvmfs_lhcbdev_available():
    with TemporaryDirectory() as td:
        assert not cvmfs_lhcbdev_available(td)
        with NamedTemporaryFile(dir=td):
            assert cvmfs_lhcbdev_available(td)
    assert not cvmfs_lhcbdev_available(td)
