...
def conda_cmd():
    ...
    return join(MINICONDA_DIR, f"{conda_subdir}/prod/bin/conda")
...
MINICONDA_DIR = join(INSTALL_ROOT, "miniconda")
if isdir(MINICONDA_DIR):
    KNOWN_CONDA_SUBDIRS = [d for d in os.listdir(MINICONDA_DIR) if "-" in d]
else:
    KNOWN_CONDA_SUBDIRS = []
ENV_VAR_WHITELIST = [
    # General unix
