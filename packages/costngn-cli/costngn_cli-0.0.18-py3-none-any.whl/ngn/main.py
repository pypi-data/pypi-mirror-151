...
from ngn.het.actions import *
from ngn.lin.data import *
from ngn.lin.actions import *
import pandas as pd
import csv
...
def het(nick:str,
    ...
    rep= het_main(acc, nick,tag,do_prices)
    if stop or start or reboot:        
        het_action(acc,rep,action)
    else:   
        rep_table(nick, rep, company)
...
def lin(nick:str,
    ...
    """
    Gets/manages Linode instances data \n 'costngn-cli lin --help' for options
    """
    company='lin'
    #nick=choose_acc(company)
    nick_ok(company, nick)
    ...
    do_prices= r_action[1]
    prof=toml.load(config_file); acc=prof[nick]
    rep= lin_main(acc,nick,tag,do_prices)
    if stop or start or reboot:        
        lin_action(acc,rep,action)        
    else:   
        rep_table(nick, rep, company)
