...
class Position():
  ...
  def for_each_trading_condition(self, p1, p2, operator):
    ...
    for oc in [ OrderCondition.CASH,
      ...
      ret += [{'stock_id': sid, 'quantity': int(qty), 'order_condition': oc} for sid, qty in ps.items()]

    return Position.from_dict(ret)

  @staticmethod
  def op(position1, position2, operator):
    position1 = pd.Series(position1, dtype='int64')
    position2 = pd.Series(position2, dtype='int64')
