...
def _create_ssl_context(
    *,
    ...
    elif client_certificate and client_certificate.certificate_authority is not None:
        context.load_verify_locations(
...
def _handle_error(error: HTTPError, body: Any) -> None:
    if isinstance(body, dict) and body.get("status") == "error":
        stack_trace = body.get("error", {}).get("stackTrace")
...
def _serialize_condition(condition: BaseCondition) -> Dict[str, Any]:
    ...
    equal_conditions = {
        coordinates_to_java_description(
            level_condition.level_coordinates
...
def _enrich_context(
    ...
    *,
    ...
    return {
        ...
        **{normalize_context_value_key(name): value for name, value in context.items()},
    }
...
class QuerySession(BaseSession[QueryCubes]):
    ...
    def _discovery(self) -> Discovery:
        ...
        return cast(Discovery, body["data"])
