...
from atoti_core import PathLike, deprecated, keyword_only_dataclass
...
class ClientCertificate:
    ...
    """Deprecated in favor of `HttpsConfig()`'s parameter with the same name.
    ...
    def __post_init__(self) -> None:
        if self.certificate_authority is not None:
            deprecated(
                "Setting `certificate_authority` is deprecated. Use `QuerySession()` or `HttpsConfig()`'s parameter with the same name instead."
            )
