...
@LastEditTime: 2022-05-17 12:54:47
...
class StatConsoleModel():
    ...
    def console_stat_queue(self, mod_count=1):
        ...
        clea_data_work.start_hours = 1
        clea_data_work.end_hours = 1
    ...
    def _process_redis_stat_queue(self, mod_value):
        ...
        while True:
            try:
                ...
                db_transaction = DbTransaction(db_config_dict=config.get_value("db_cloudapp"))
                stat_orm_model = StatOrmModel()
                stat_queue_json = SevenHelper.redis_init().lpop(f"stat_queue_list:{mod_value}")
                ...
                try:
                    ...
                    if stat_queue_dict["process_count"] <= 10:
                        SevenHelper.redis_init().rpush(f"stat_queue_list:{mod_value}", SevenHelper.json_dumps(stat_queue_dict))
                    else:
