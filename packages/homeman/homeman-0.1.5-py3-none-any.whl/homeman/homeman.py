...
class HomeMan:
    ...
    def __scan_data(self, d: Dict) -> List[Dict]:
    ...
    def __pull(self, data: Dict, force: bool):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.__auto_sync(data, [data['homeman']])
    ...
    def __sync(self, data: Dict, reverse: bool):
    ...
    def __auto_sync(self, data: Dict, global_routes: List[str]):
        ...
        for slave_route in data['auto']:
            ...
            if os.path.isfile(slave_man_path):
                os.remove(slave_man_path)
            elif os.path.isdir(slave_man_path):
                shutil.rmtree(slave_man_path)
            elif os.path.exists(slave_man_path):
                raise HomeManException(f'cannot remove: {slave_man_path}')
            os.makedirs(slave_man_path)
            os.rmdir(slave_man_path)
            if os.path.isfile(man_path):
                shutil.copy(man_path, slave_man_path)
            elif os.path.isdir(man_path):
                shutil.copytree(man_path, slave_man_path)
    ...
    def __backup(self, data: Dict, force: bool):
        route = data['homeman']
    ...
    def check(self, route: str, show_diff: bool):
        logger.debug('========< check >========')
        data = self.get_data(route)
        if data is None:
            logger.debug('route not exists: %s', route)
