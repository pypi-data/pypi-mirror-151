...
def troughs_all(ohlc: pd.DataFrame,
    ...
    - returns distance to last low price in the window (normalized by current price)
    """
...
def peaks_all(ohlc: pd.DataFrame,
    ...
    """
        ...
        - returns distance to last high price in the window (normalized by current price)
    """
...
def find_extreme(data: pd.DataFrame,
    ...
    def _find_extreme(win: pd.DataFrame,
        ...
        peaks = (peaks - win_data.iloc[-1])/win_data.iloc[-1]
