...
# Otherwise call without, so suitable for app builds.
        if ngBuild:
            ...
            local('cp {package.json,yarn.lock,knexfile.js,requirements.txt} dist/')
...
def update_app_utils(appname, branch, keys=False, reboot=False):
    ...
    cmdToRun = "sudo --preserve-env=HOME /home/ubuntu/" + appname + "/" + APP_UTILS_DIR + "/config/" + ENVIRONMENT + "/${dc_TAG_Type}-commands.sh"
    run(cmdToRun)
