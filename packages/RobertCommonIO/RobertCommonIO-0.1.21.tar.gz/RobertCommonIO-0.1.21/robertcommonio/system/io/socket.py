...
from robertcommonbasic.basic.data.frame import FRAME, FRAMEFLAG, PACKAGETYPE, FRAMETYPE, TRANSFERTYPE, pack_frame, unpack_frame, get_pack_start_length, get_pack_header_length, convert_bytes_to_int
...
class SocketIPHandler():
    ...
    def recv_bytes(self, length: int) -> bytes:
        ...
        if data is None or len(data) == 0:
            self.handle_close(f"remote close")
        ...
        if data_start == FRAMEFLAG.START_FLAG:  #
            ...
            if data_header is not None:
                ...
                if data_header is not None:
                    if data_end[-2:] == FRAMEFLAG.END_FLAG:  # 包结尾
                        try:
                            package = unpack_frame(data_start + data_header + data_end)
