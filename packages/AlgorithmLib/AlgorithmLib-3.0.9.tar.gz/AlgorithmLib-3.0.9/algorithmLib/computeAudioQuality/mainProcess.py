...
from timeAligment.time_align import cal_delay
import os
import wave
import numpy as np
from ctypes import  *
...
from FUNCTION.audioFunction import isSlience,audioFormat,get_rms_level
...
allMetrics = ['G160','P563','POLQA','PESQ','STOI','STI','PEAQ','SDR',
              ...
              'MATCHAEC','ERLE','AECMOS','AIMOS','TRMS','ARMS','NOISE']
...
class computeAudioQuality():
    ...
    def POLQA(self):
        ...
        result =  polqa_client_test(wav2pcm(self.refFile),wav2pcm(self.testFile),curSR)
        time.sleep(2)
        return  result
    ...
    def ELRE(self):
        ...
        if self.__check_format(self.refFile) != self.__check_format(self.micFile) or \
            self.__check_format(self.micFile) != self.__check_format(self.testFile):
            raise TypeError('there are different parametre in inputfiles!')
        return cal_erle(pcm2wav(self.refFile,sample_rate=self.samplerate),pcm2wav(self.micFile,sample_rate=self.samplerate),pcm2wav(self.testFile,sample_rate=self.samplerate),targetType=self.aecTargetType)
    ...
    def MATCHAEC(self):
        ...
        """
        if self.caliFile is None or self.micFile is None or self.testFile is None:
            raise EOFError('lack of inputfiles!')
        if self.__check_format(self.caliFile) != self.__check_format(self.micFile) or \
            self.__check_format(self.caliFile) != self.__check_format(self.testFile):
            raise TypeError('there are different parametre in inputfiles!')
        return MATCH_AEC(pcm2wav(self.micFile,sample_rate=self.samplerate),pcm2wav(self.testFile,sample_rate=self.samplerate),pcm2wav(self.caliFile,sample_rate=self.samplerate),self.outFile,targetType=self.aecTargetType)
    ...
    def SLIENCE(self):
        ...
        if self.testFile is None:
            raise EOFError('lack of inputfiles!')
        return isSlience(self.testFile)
