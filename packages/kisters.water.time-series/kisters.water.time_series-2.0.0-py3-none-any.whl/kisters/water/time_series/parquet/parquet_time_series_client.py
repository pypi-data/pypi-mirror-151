...
from kisters.water.time_series.memory.memory_time_series_client import MemoryTimeSeriesClient
...
class ParquetTimeSeriesClient(MemoryTimeSeriesClient):
    ...
    def _load_table(self) -> None:
        ...
        for path, metadata in master_metadata.items():
            self._ts_metadata[path] = self.sanitize_metadata(metadata)
        ...
        for (path, t0, member, dispatch_info), group in master_df.groupby(level=self.categorical_columns):
            df_ts = (
                group.reset_index()
            ...
            self._ts_data[path][
                EnsembleMember(t0=pd.to_datetime(t0), dispatch_info=dispatch_info, member=member)
            ] = df_ts
    ...
    def _save_table(self) -> None:
        ...
        for ts_path, ts_metadata in self._ts_metadata.items():
            metadata[ts_path] = ts_metadata.dict()
        ...
        table = table.replace_schema_metadata(
            {
                **table.schema.metadata,
                self.ts_metadata_field: orjson.orjson.dumps(
    ...
    async def create_time_series(
        ...
        **kwargs: Any,
        ...
        return await super().create_time_series(path=path, metadata=metadata, **kwargs)

    async def update_time_series(
        self, *, path: str, metadata: dict[str, Any] | TimeSeriesMetadata, **kwargs: Any
    ) -> None:
        self._data_changed = True
        return await super().update_time_series(path=path, metadata=metadata, **kwargs)

    async def delete_time_series(self, *, path: str, **kwargs: Any) -> None:
        self._data_changed = True
        return await super().delete_time_series(path=path, **kwargs)
