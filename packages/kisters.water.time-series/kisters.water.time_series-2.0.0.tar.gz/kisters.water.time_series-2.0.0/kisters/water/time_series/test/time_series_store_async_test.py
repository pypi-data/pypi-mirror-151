...
class TimeSeriesStoreAsyncTest:
    ...
    START_00 = datetime(year=2020, month=1, day=1, tzinfo=utc)
    START_12 = datetime(year=2020, month=1, day=1, hour=12, tzinfo=utc)
    ENSEMBLE_MEMBERS = [
    ...
    END_00 = datetime(year=2020, month=1, day=3, tzinfo=utc)
    END_12 = datetime(year=2020, month=1, day=3, hour=12, tzinfo=utc)
    ...
    async def test_02_read_bulk_mixed(self):
        start = datetime(year=2020, month=1, day=2, tzinfo=utc)
        end = datetime(year=2020, month=1, day=2, hour=12, tzinfo=utc)
        ...
        async with self.STORE.client as client:
            missing_df, scalar_df = (
                await client.read_data_frames(
                    ...
                    t0=[datetime(year=2018, month=1, day=1, tzinfo=utc), None],
                )
            ).values()
    ...
    async def test_07_partial_writes(self):
    ...
    async def test_08_async_iters(self):
        async with self.STORE.client as client:
    ...
    async def test_09_delete_data(self):
        async with self.STORE.client as client:
            start = datetime(year=2020, month=1, day=2, tzinfo=utc)
            ...
            assert df.index[-1] == datetime(year=2020, month=1, day=1, hour=23, minute=55, tzinfo=utc)

    async def test_10_delete_time_series(self):
        async with self.STORE.client as client:
