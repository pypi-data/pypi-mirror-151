...
class Storage:
    ...
    def from_json(self, data):
        if 'pki' in data:
            self.pki = data['pki']
        if 't_created' in data:
            self.t_created = data['t_created']
        if 't_modified' in data:
            self.t_modified = data['t_modified']
        if 'base64_bytes_array' in data:
            # converts to image on load
            self.image = self._b64_to_image(data['base64_bytes_array'])
        if 'meta' in data:
            self.meta = data['meta']
        if 'obj_sourcedata' in data:
            self.obj_sourcedata = data['obj_sourcedata']
        if 'fki_record_id' in data:
            self.fki_record_id = data['fki_record_id']
