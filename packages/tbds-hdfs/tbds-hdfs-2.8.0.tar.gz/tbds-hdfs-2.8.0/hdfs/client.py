...
class Client(object):
  ...
  """
    ...
    def __init__(self, url, root=None, proxy=None, timeout=None, session=None, header=None):
        self.root = root
        self.url = url
    ...
    """
        if self._header:
            return rq.request(
                ...
                headers=self._header,  # For HttpFS.
                **kwargs
            )
        else:
...
def generateSignature(secureid, securekey):
    ...
    seed = ''.join([secureid, timestamp, randomInt])
    hashed = hmac.new(securekey.encode('utf-8'), seed.encode('utf-8'), sha1).digest()
    my_sign = base64.b64encode(hashed).decode('utf-8')
    signature = parse.quote_plus(str(my_sign))
    return "{0} {1} {2} {3}".format(secureid, timestamp, randomInt, signature)

class TbdsClient(Client):
    r"""HDFS web client using Hadoop tbds security.
  :param url: Hostname or IP address of HDFS namenode, prefixed with protocol,
    followed by WebHDFS port on namenode
  """

    def __init__(self, url, secureid, securekey, **kwargs):
        # session = kwargs.setdefault('session', rq.Session())
        signStr = generateSignature(secureid, securekey)
        header = {"tbds-auth": signStr,'content-type':'application/octet-stream' }
        kwargs['header'] = header
        super(TbdsClient, self).__init__(url, **kwargs)
