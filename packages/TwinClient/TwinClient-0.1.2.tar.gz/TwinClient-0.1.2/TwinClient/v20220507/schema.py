...
# FilePath: /05 代码/Users/shihua/Documents/武汉东西湖社会治理指挥中心/04接口文档/01孪生体创建/python-sdk/TwinClient/v20220507/schema.py
# Create: 2022-05-07 12:57:40
# LastAuthor: Please set LastEditors
# lastTime: 2022-05-12 17:16:17
# --------------------------------------------------------
...
class AssetTypeModel(BaseModel):
    ...
    pid: Optional[str]                  # AssetTwinSubtypeModel 合并
...
class AssetRelationModel(BaseModel):
    ...
    # @validator('from_asset', 'to_asset', pre=True)
    # def from_asset_uuid(cls, v):
    #     asset_version= uuid.UUID(v).version
    #     if not asset_version:
    #         raise ValueError('Error: asset uuid version!')
    #     return v
    
class AssetCodeRelationModel(AssetRelationModel):
    @validator('from_asset', 'to_asset', pre=True)
    def from_asset_uuid(cls, v):
        return v
...
class TwinTypeModel(BaseModel):
    ...
    featuresAdditions: bool = True
    attributesAddition: bool = True
    command: List = []
    features: List = []  # 动态属性
    category: str       # 动态属性
    twinType: str
...
class TwinBaseModel(BaseModel):
    ...
    """
    name: str           # name 全局唯一
    thing: str          # thing 全局唯一
    category: str       # 资产类别 code
    type: str           # 资产 twin type code
    subType: str        # 资产 subType code
    definitionId: str   # twin 模型的 id
    version: str        # 版本
