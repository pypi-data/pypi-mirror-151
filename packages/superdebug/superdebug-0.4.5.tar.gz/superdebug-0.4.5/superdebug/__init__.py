...
CALL_CHAIN_LEN = 2 # 标准为0，即只显示调用debug()的位置，设置成>0则可显示更长的调用链
...
def get_pos(level=1):
    position = _get_pos(level=level)
    if CALL_CHAIN_LEN > 0:
        for deeper_level in range(level + 1, level + CALL_CHAIN_LEN + 1):
            try:
                position += "\nFrom " + _get_pos(level=deeper_level)
            except ValueError:
                break
    return position
...
def debug(*args, **kwargs):
    ...
    if len(args) + len(kwargs) == 0:
        logging(f"\033[0m\033[1;32mMARK:\033[0m\033[1;33m at \033[0m\033[1;32m{get_pos(level=3)}\033[0m\033[1;33m")
    elif len(args) == 1 and len(kwargs) == 0 and type(args[0]) == str:
        logging(f"\033[0m\033[1;32mDEBUG:\033[0m\033[1;33m at \033[0m\033[1;32m{get_pos(level=3)}\033[0m\033[1;33m")
        logging(f"\033[0m\033[1;36m{args[0]}\033[0m\033[1;33m")
    else:
        logging(f"\033[0m\033[1;32mDEBUG:\033[0m\033[1;33m {len(args) + len(kwargs)} vars: {['?' for _ in args] + keys}, at \033[0m\033[1;32m{get_pos(level=3)}\033[0m\033[1;33m")
        for var in args:
...
def mail(subject = "Progress Notification", message = ""):
    ...
    message = f"{message}\nThis email is sent at   {get_pos(level=3)}"
