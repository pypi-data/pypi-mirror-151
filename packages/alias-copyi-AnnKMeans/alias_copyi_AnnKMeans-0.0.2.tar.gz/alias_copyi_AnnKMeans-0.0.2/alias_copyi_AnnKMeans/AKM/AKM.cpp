...
AKM::AKM(vector<vector<double>> &X, int c_true, bool debug){
    this->N = X.size();
    this->dim = X[0].size();
    this->c_true = c_true;
    this->debug = debug;
...
void AKM::opt(vector<vector<vector<double>>> &Cen, int ITER){
...
Veci_int AKM::opt_once(vector<vector<double>> &Cen_vec, int ITER){
