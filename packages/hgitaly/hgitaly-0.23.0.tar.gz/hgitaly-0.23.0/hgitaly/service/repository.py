...
class RepositoryServicer(RepositoryServiceServicer, HGitalyServicer):
    ...
    def CreateBundleFromRefList(self, request: CreateBundleFromRefListRequest,
        ...
        patterns = list(first_req.patterns)
        for req in request:
            for pat in req.patterns:
                patterns.append(pat)
        revs = [gitlab_revision_changeset(repo, pat).rev()
                for pat in patterns if gitlab_revision_changeset(repo, pat)]
        rev = [pycompat.sysbytes(str(rev)) for rev in revs]
