...
def files_from(args=None, *,
               # keyword-only arg names intentionally same as 'dest' defaults
    ...
    with getattr().
    """
    # Capture explicit keyword arguments (excluding 'args').
    kwds = locals().copy()
    kwds.pop('args')
    ...
    files = [(f for f in Path('.').glob(wild) if f.is_file())
             for wild in kwds['files']]
    dirs  = [(d for d in Path('.').glob(wild) if d.is_dir())
             for wild in kwds['dirs']]
    trees = [(d for d in Path('.').glob(wild) if d.is_dir())
    ...
    files_or_dirs  = tuple(itertools.chain.from_iterable(
        (f_d for f_d in Path('.').glob(wild)) for wild in kwds['files_or_dirs']))
    files_or_trees = tuple(itertools.chain.from_iterable(
        (f_t for f_t in Path('.').glob(wild)) for wild in kwds['files_or_trees']))
    ...
    dirs .append( (d for d in files_or_dirs  if d.is_dir()) )
    trees.append( (d for d in files_or_trees if d.is_dir()) )
...
def files_in_tree(dir):
    """
    Generate all the files at any depth under specified dir.
    """
