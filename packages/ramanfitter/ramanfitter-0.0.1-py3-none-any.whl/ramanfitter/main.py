...
class RamanFitter:
    ...
    def __init__( self, x, y, autorun = True, threshold = 12., PercentRange = 0.2, Sigma = 15, SigmaMin = 3, SigmaMax = 100 ):
        ...
        # Ensure that the values are in ascending order (cause I like it)
        ...
        if autorun:
            ...
            # Denoise Data
            self.x, self.y                          = self.Denoise( x = self.x, y = self.y )

            # Find Peaks
            self.peaks_x, self.peaks_y, self.npeaks = self.FindPeaks( Threshold = self.threshold)
    ...
    def NormalizeData( self ):
        ...
        min_y           = np.min( self.y )
        max_y           = np.max( self.y )
        self.scale      = 1./( max_y - min_y )

        self.y          = ( self.y - min_y )*self.scale
    ...
    def Denoise( self,
        ...
        '''
            Denoise( self, x, y, UseFFT = True, FFT_PS_Cutoff = 1./1200., UseSavgol = True, SavgolWindow = 25, SavgolOrder = 3, ShowPlot = False )
        ...
        if UseFFT:
            # Get points and dx
            x_pnts      = len( x )                                                  # Number of points
            Δx          = x[1] - x[0]                                               # Δx


            fhat        =  np.fft.fft( y, x_pnts )                                  # Computes the fft
            psd         = fhat * np.conj( fhat )/x_pnts                             # Compute Power Spectrum
            freq        = ( 1./( Δx*x_pnts ) )*np.arange( x_pnts )                  # Frequency array
            ...
            y           = np.fft.ifft( fhat )                                       # Calculate Inverse FFT

        # If
        if UseSavgol:
            y   = signal.savgol_filter( y, SavgolWindow, SavgolOrder )

        # If ShowPlot, build plots with matplotlib
        if ShowPlot:
            plt.plot( x, old_y, color = 'c', label = 'Noisy' )
            plt.plot( x, y, color = 'k', label = 'Filtered' )
            plt.xlim( x[0], x[-1] )
            plt.legend()
            plt.show()
    ...
    def FindPeaks( self, DistBetweenPeaks = 50, Threshold = 12., showPlot = False ):

        '''
            FindPeaks( self, DistBetweenPeaks = 50, Threshold = 12., showPlot = False )
            ...
            Threshold : float
                `Threshold` helps the code determine what is a worthy peak and what isn't.
            ShowPlot : bool, optional, default: False
            ...
            -------
            peaks_x : numpy.array
                1D array of the x-values associated with each peak
            peaks_y : numpy.array
                1D array of the y-values associated with each peak
            n_peaks :
                1D array of the raman data indices associated with each peak 
        ...
        #Set Prominent Threshold
        prom_thresh = Threshold

        rem_ind     = [ i for ( i, p ) in enumerate( prominence ) if p < prom_thresh]
        n_peaks     = np.delete( peaks, rem_ind )
        peaks_y     = np.array( [ self.y[i] for i in n_peaks ] )
        peaks_x     = np.array( [ self.x[i] for i in n_peaks ] )

        if showPlot:
            plt.plot( self.x, self.y, color = 'c', label = 'Data' )
            plt.scatter( peaks_x, peaks_y, color = 'k', label = 'Peaks' )
            plt.xlim( x[0], x[-1] )
            plt.legend()
            plt.show()
