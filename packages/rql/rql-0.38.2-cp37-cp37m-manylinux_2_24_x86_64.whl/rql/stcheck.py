    ...
    * IN function with a single child
        ...
        for var in path:
            ...
            for rdef in rschema.rdefs.values():
                ...
                if not (
                    ...
                    or (
                        rdef.cardinality[cardidx] in "?1"
                        and (var == tovar or not rschema.final)
                    )
                ):
                    return False
            fromvar = var
    ...
    def visit_relation(self, relation, state):
        if relation.optional and state.under_not:
            state.error(
                "can't use optional relation under NOT (%s)" % relation.as_string()
            )
        lhsvar = relation.children[0].variable
        if relation.is_types_restriction():
            if relation.optional:
                state.error(
                    'can\'t use optional relation on "%s"' % relation.as_string()
                )
