...
SPEC_QUERIES = (
    ...
    "Any T2" " GROUPBY T2" " WHERE T1 relation T2" " HAVING COUNT(T1) IN (1,2);",
    "Any T2"
    " GROUPBY T2"
    ...
    " WHERE T1 relation T2"
    " HAVING (1 < COUNT(T1) OR COUNT(T1) IN (3,4));",
    "Any T2" " GROUPBY T2" " WHERE T1 relation T2" " HAVING 1+2 < COUNT(T1);",
    "Any X,Y,A ORDERBY Y "
...
class ParserHercule(unittest.TestCase):
    ...
    def test_precedence_1(self):
        tree = self.parse(
            "Any X WHERE X firstname 'lulu' AND X name 'toto' OR X name 'tutu';"
        )
        base = tree.children[0].where
    ...
    def test_precedence_4(self):
        tree = self.parse(
            "Any X WHERE X firstname 'lulu' OR X name 'toto' AND X name 'tutu';"
        )
        base = tree.children[0].where
        self.assertEqual(isinstance(base, nodes.Or), 1)
        self.assertEqual(isinstance(base.children[0], nodes.Relation), 1)
    ...
    def test_not_precedence_2(self):
        ...
        self.assertEqual(
            str(tree), 'Any X WHERE (NOT X firstname "lulu") OR (X name "toto")'
        )
