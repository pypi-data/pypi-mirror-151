...
def encode(data: bytes, data_rate=24000, *, sample_rate=24000) -> bytes:
    ...
    return silkEncode(data, sample_rate, data_rate)


def encode_file(target: Union[str, BinaryIO], data_rate=24000, *, sample_rate=24000) -> bytes:
    fd = get_file(target)
    try:
        return encode(fd.read(), data_rate, sample_rate=sample_rate)
    finally:
        fd.close()
