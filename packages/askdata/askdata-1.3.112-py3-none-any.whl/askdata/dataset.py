...
class Dataset():
    ...
    def __init__(self, env, token, agent_id, slug=''):
    ...
    def _get_info_dataset_by_slug(self, slug: str):
        ...
        self._domain = dataset.iloc[0]['domain']
    ...
    def set_data_last_update(self):

        now = datetime.now()
        body = {
            "dataLastUpdate": now.strftime("%Y-%m-%dT%H:%M:%S")
        }
        ...
        authentication_url2 = self._base_url_askdata + '/smartdataset/datasets/' + self._dataset_id + '/dataLastUpdate'
        r = s.put(url=authentication_url2, headers=self._headers, json=body)
    ...
    def set_synonym_transformers(self, column_code, synonym_transformers):
        if(type(synonym_transformers)==str):
            synonym_transformers_list = []
            synonym_transformers_list.append(synonym_transformers)
        else:
            synonym_transformers_list = synonym_transformers

        column_code_settings = self.__retrive_entity(entity_code=column_code, dataset_id=self._dataset_id,
                                                     dataset_type=self._dataset_type)
        column_code_settings['synonymTransformers'].extend(synonym_transformers_list)
    ...
    def set_parameter_name(self, column_code: str, name: str):
        column_code_settings = self.__retrive_entity(entity_code=column_code, dataset_id=self._dataset_id,
                                                     dataset_type=self._dataset_type)

        column_code_settings['name'] = name
    ...
    def add_synonym_to_value(self, column_code: str, row_code, synonyms: list):

        value_object = self.__get_values_from_entity(column_code, row_code)
        value_object["synonyms"].extend(synonyms)

        url = self._base_url_askdata+"/smartmanager/data/"+self._domain+"/entity/"+column_code
        ...
        response = s.put(url=url, headers=self._headers, json=value_object)
        response.raise_for_status()

    def __get_values_from_entity(self, column_code, value_code):
        ...
        dataset_url = self._base_url_askdata + '/smartmanager/data/'+self._domain+'/entity/'+column_code+'?filter=%257B%257D'
        response = s.get(url=dataset_url, headers=self._headers)
        response.raise_for_status()
        data=response.json()["payload"]["data"]
        #print(data)
        for value in data:
            if(value_code==value["code"]):
                return value
