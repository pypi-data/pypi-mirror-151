...
class Insight_Definition:
    ...
    def add_widget_test(self, widget_type: str, query_ids:list=[], settings:dict={}, values:dict={}, texts:dict={}, charts:list=[]):
        position = (len(self.components))
        widget_id = self.__create_widget(position, widget_type)
        if(query_ids!=[]):
            self.edit_widget_test(widget_id, widget_type,query_ids, settings, values, texts, charts)
        return widget_id
