...
from cnvrgv2.config.routes import LIBRARY_BASE, LIBRARY_VERSION_BASE, USE_LIBRARY_SUFFIX
...
class LibraryVersion(DynamicAttributes):
    ...
    def clone(self, progress_bar_enabled=False):
        destination = "./{0}".format(self._context.library)
        ...
        for chunk in response.iter_content(chunk_size=32000):
            ...
            file.write(chunk)
            if progress_bar_enabled:
                self.progress_bar.throttled_next(dl)
