...
from statute_utils import StatuteMatcher
...
class CodificationItem:
    ...
    def populate_data_from_path(self) -> NoReturn | dict:
        ...
        self.format_histories(raw_data["units"])  # self-adjust
        set_node_ids(raw_data["units"])  # adds id to each node

        return raw_data

    def format_histories(self, nodes: list[dict]) -> None:
        """Recursive function to check units pre-database ingestion."""
        for node in nodes:
            if histories := node.get("history", None):
                for history in histories:
                    ...
                    if history.get("date", None):
                        iso_date = parse(history["date"]).date().isoformat()
                        history["date"] = iso_date
            if new_nodes := node.get("units", None):
                self.format_histories(new_nodes)  # call self
...
def extract_codifications(codifications_folder: Path) -> Iterator[dict]:
    for code_path in codifications_folder.glob("**/*.yaml"):
        try:
            item = CodificationItem(code_path)
            yield item.extracted_fields
        except Exception as e:
            print(f"{code_path=} issue: {e=}")
