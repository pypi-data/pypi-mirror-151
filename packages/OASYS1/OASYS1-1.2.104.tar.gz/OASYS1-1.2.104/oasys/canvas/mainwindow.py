        ...
        if not items is None:
            ...
            for internal_library in self.__pypi_internal_libraries:
                try:
                    ...
                    entry_points.append(EntryPoint(internal_library.name, get_latest_version(internal_library.name)))

            items = addons.installable_items(self.__pypi_internal_libraries, entry_points)

            self.__internal_library_to_update = []
            for item in items:
                if internal_libraries.is_updatable(item) or item: self.__internal_library_to_update.append(item)
