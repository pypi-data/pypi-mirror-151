...
from ..const import SERVICE_DEF_FILE_NAME, TEMPLATE_DEF_FILE_NAME
from ..exceptions import ConfigurationException
from ..util import create_file_from_content, read_text_from_file
...
class SfTemplate:
    ...
    def write(
        ...
        if not overwrite and len(conflicting_files) > 0:
            if not (
                input_hook
                and input_hook.ask_boolean(
                    f"Following files already exist: [{conflicting_files_str}]. Do you want to overwrite them?"
                )
            ):
        ...
        for file in files:
            if file == SERVICE_DEF_FILE_NAME:
                ...
                new_content = self.file_interceptor[file](old_content)
                create_file_from_content(join(out_folder, file), new_content)
            else:
