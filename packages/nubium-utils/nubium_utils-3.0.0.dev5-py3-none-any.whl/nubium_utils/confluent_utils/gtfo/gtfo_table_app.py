...
from .gtfo_app import GtfoApp, Transaction, FailedTransaction
import logging
...
class GtfoTableApp(GtfoApp):
    ...
    def _table_db_assignments(self, max_retry=4):
            ...
            try:
                if partition not in self.tables:
                    if partition_attempts[partition]:
                        ...
                        LOGGER.debug(f'Sleeping for {sleep_amt} seconds before retrying db p{partition} init...')
