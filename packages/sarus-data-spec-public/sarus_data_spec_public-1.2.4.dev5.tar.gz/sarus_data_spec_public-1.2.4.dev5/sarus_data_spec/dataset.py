...
class Dataset(Referring[sp.Dataset]):
    ...
    def sql(self, query: str) -> List[Dict[str, Any]]:
        ...
        return self.manager().sql(self, query)
    ...
    def to_arrow(self) -> Iterator[pa.RecordBatch]:
        return self.manager().to_arrow(self, self._batch_size)

    async def async_to_arrow(self) -> AsyncIterator[pa.RecordBatch]:
        return await self.manager().async_to_arrow(self, self._batch_size)
