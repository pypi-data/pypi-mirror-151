...
try:
    ...
    print('tensorflow not found, tensorflow datasets not available')
...
class Dataset(DataSpec[sp.Dataset], t.Protocol):
    ...
    def batch(self, batch_size: int) -> Dataset:
        """sets the batch size of the dataset"""
        ...
...
if t.TYPE_CHECKING:
    ...
    import sarus_data_spec.path as spa
    import sarus_data_spec.schema as sc
    import sarus_data_spec.statistics as sdstat
    import sarus_data_spec.transform as st
    import sarus_data_spec.type as sty

    test_path: Path = spa.Path(sp.Path())
    test_type: Type = sty.Type(sp.Type())
    test_schema: Schema = sc.schema(sd.sql(uri='sqlite:///:memory:'))
    test_transform: Transform = st.Transform(sp.Transform())
    test_sql: Dataset = sd.sql(uri='sqlite:///:memory:')
    test_file: Dataset = sd.file(format='', uri='')
    test_csv_file: Dataset = sd.csv_file(uri='')
    test_files: Dataset = sd.files(name='', uri_pattern='', format='')
    test_csv_files: Dataset = sd.csv_files(name='', uri_pattern='')
    test_transformed: DataSpec = sd.transformed(
        st.protect(protected_entities=[]), sd.sql(uri='sqlite:///:memory:')
    )
    test_stat: Statistics = sdstat.Statistics(sp.Statistics())
