...
from wfGenes_exe.initial_wfgenes import BasewfGenes
from wfGenes_exe.pywrapper_wfgenes import PywrapperwfGens
from wfGenes_exe.simstack_wfgenes import SimstackwfGenes
from wfGenes_exe.fireworks_wfgenes import FireworkwfGenes
from wfGenes_exe.dot_wfgenes import DotwfGenes
from wfGenes_exe.clean_wfgenes import CleanwfGenes
from wfGenes_exe.dask_wfgenes import DaskwfGenes
from wfGenes_exe.parsl_wfgenes import ParslwfGenes
import time
import signal
import jsonschema
...
def run_wfgenes(args):
    ...
    if args['wms'] in valid_wms:
        ...
        exit()    
    while True:
        # while user using wfGenes
        time.sleep(1) # Avoid extra cpu load
        if modified(wconfig, wconfig_stat) == True:
            #init= BasewfGenes (args)
            validator = wconfig_validator(BasewfGenes, args ,wconfig)
            wfgenes_init = validator[0]
            verified =  validator[1]
            i =  validator[2]
            if verified == True:
                model = wfgenes_init.wconfig_checker()
                wconfig_status = model[0]
                wconfig_version = model[2]
                if wconfig_status == 'active': 
                    # modeling phase
                    print('Model is verified and Graph generation is active. Remove "active" suffix to proceed with code generation or continue with modling')
                    i = 'null'
