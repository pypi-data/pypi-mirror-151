...
class PostgresCfg(BaseSettings):
    url: PostgresDsn
    class Config:
...
def create_app_table():
    engine = sa.create_engine(PostgresCfg().url)
    with engine.begin() as conn:
        td_app_table.create(conn, checkfirst=True)
    engine.dispose()
    
def print_app_config():
    engine = sa.create_engine(PostgresCfg().url)
    with engine.begin() as conn:
        cfg = dict(conn.execute(sa.select(td_app_table)).fetchone())
    pprint(cfg)
    engine.dispose()
    return cfg
...
class TDAuth:
    ...
    def __init__(self):
        self.engine = sa.create_engine(PostgresCfg().url)
    ...
    def _get_user_principles(self):
        ...
        if resp.status_code == 200:
            self.logger.debug(f"Successfully got userprincipals")
            return resp.json()
        self.logger.debug(
    ...
    def _set_access_token_from_refresh_token(self):
        ...
        if resp.status_code == 200:
            self.logger.debug(f"Successfully got new access token from refresh token.")
            data = resp.json()
    ...
    def _set_access_and_refresh_tokens(self):
        ...
        if resp.status_code == 200:
            self.logger.debug(f"Successfully got new access token from Code.")
            data = resp.json()
            self._update_cfg(
