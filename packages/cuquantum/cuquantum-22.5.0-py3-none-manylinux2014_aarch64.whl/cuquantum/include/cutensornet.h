...
#define CUTENSORNET_MINOR 0 //!< cuTensorNet minor version.
 ...
 * \param[in] stridesOut Array of size \p numModesOut; ``stridesOut[j]`` (``j`` < ``numModesOut``) corresponding to the offset between two logically-neighboring elements within the j-th mode of the output tensor.
 ...
 * \param[in] extentsOut Array of size \p numModesOut; ``extentsOut[j]`` denotes the extent of the j-th mode of the output tensor.
 * \param[in] stridesIn Array of size \p numOutputs; ``stridesOut[j]`` corresponding to the linearized offset -- in physical memory -- between two logically-neighboring elements w.r.t the j-th mode of the output tensor.
 ...
 * \param[in,out] optimizerInfo On return, this object will hold all necessary information about the optimized path and the related slicing information. optimizerInfo will hold information about (see ::cutensornetContractionOptimizerInfoAttributes_t):
