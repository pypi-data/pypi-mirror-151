import re
def validateField(field, capitalization = None, spaces = False, email = False, length = 0, document = False, integer = False, string =  False):
    validateField = "{field}".format(field = field)
    validatestrip = validateField.strip()

    if capitalization == 'lowercase' and spaces is True :
        space = re.sub(r"\s+", "", field)
        data = space.lower()
        return data

    if capitalization == 'uppercase' and spaces is True :
        space = re.sub(r"\s+", "", field)
        data = space.upper()
        return data

    if capitalization == 'lowercase' and string is True :
        r1 = (r'^[a-zA-Z-ñÑ]')
