...
class TestAsynchronousS3Interactions:
    ...
    async def test_async_upload_to_s3_returns_bad_response(
        ...
        with pytest.raises(
            HTTPXError, match=re.escape("Issue uploading object to S3: (500")
        ):
            await _async_upload_to_s3(
                s3_upload_url,
                s3_upload_fields,
    ...
    async def test_async_upload_to_s3_returns_unacceptable_response(
        ...
        """
        httpx_mock.add_response(status_code=206)  # 206 is Partial content

        with pytest.raises(
            HTTPXError, match=re.escape("Issue uploading object to S3: (206)")
        ):
            await _async_upload_to_s3(
                s3_upload_url,
                s3_upload_fields,
    ...
    async def test_async_upload_to_s3_raises_exception(
        ...
        with pytest.raises(HTTPXError, match="Issue uploading object to S3"):
            await _async_upload_to_s3(
                s3_upload_url,
                s3_upload_fields,
                to_upload=packed_data,
    ...
    async def test_async_download_from_s3(
        self, httpx_mock: HTTPXMock, s3_download_url: _S3PresignedURL
    ) -> None:
    ...
    async def test_async_download_from_s3_returns_bad_response(
        ...
        with pytest.raises(
            HTTPXError, match=re.escape("Issue whilst retrieving data from S3: (500)")
        ):
            await _async_download_from_s3(s3_download_url)
    ...
    async def test_async_download_from_s3_returns_unacceptable_response(
        ...
        with pytest.raises(
            HTTPXError, match=re.escape("Issue whilst retrieving data from S3: (206)")
        ):
            await _async_download_from_s3(s3_download_url)
    ...
    async def test_async_download_from_s3_raises_exception(
        ...
        with pytest.raises(HTTPXError, match="Issue whilst retrieving data from S3"):
            await _async_download_from_s3(s3_download_url)
