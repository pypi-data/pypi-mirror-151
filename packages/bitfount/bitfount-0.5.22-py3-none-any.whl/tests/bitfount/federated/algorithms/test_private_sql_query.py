...
class TestPrivateSqlQuery:
    ...
    @fixture
    def datasource(self) -> _BaseSource:
    ...
    def test_worker_init_datasource(
        self, column_ranges: dict, datasource: _BaseSource
    ) -> None:
    ...
    @unit_test
    def test_no_data(
        self, caplog: LogCaptureFixture, column_ranges: dict, mocker: MockerFixture
    ) -> None:
        """Tests that having no data raises an error."""
        algorithm_factory = PrivateSqlQuery(
        ...
        worker = algorithm_factory.worker(hub=Mock(spec=BitfountHub))
        worker.datasource = Mock(spec=_BaseSource, data=None)

        worker.run()
        worker.datasource.load_data.assert_called_once()
        # This is raised as a ValueError but caught higher up and logged
        assert "No data on which to execute SQL query." in caplog.text
    ...
    def test_worker_gets_sql_results(
        self, column_ranges: dict, datasource: _BaseSource, pod_schema: BitfountSchema
    ) -> None:
