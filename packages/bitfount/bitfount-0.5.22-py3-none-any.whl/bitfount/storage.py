...
def _handle_request_exception(
    ...
    # If the exception is a HTTPXError (or subclass), we raise an HTTPXError.
    # This is because the subclasses of HTTPXError cannot be re-raised without
    # the original request.
    if isinstance(exc, HTTPXError):
        raise HTTPXError(err_message) from exc
...
async def _async_upload_to_s3(
    ...
    try:
        ...
        if response.status_code not in _OK_UPLOAD_STATUS_CODES:
            raise HTTPXError(f"Response not OK; not one of {_OK_UPLOAD_STATUS_CODES}")
...
async def _async_download_from_s3(download_url: _S3PresignedURL) -> bytes:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(url=download_url, timeout=None)
