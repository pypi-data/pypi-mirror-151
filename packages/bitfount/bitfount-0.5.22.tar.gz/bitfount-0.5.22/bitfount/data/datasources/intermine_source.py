...
from intermine.webservice import (  # type: ignore[import] # Reason: missing library stubs or py.typed marker  # noqa: B950
    Service,
)
import numpy as np
import pandas as pd

from bitfount.data.datasources.base_source import _MultiTableSource
...
class IntermineSource(_MultiTableSource):
    ...
    def _template_to_df(self, template_name: str) -> pd.DataFrame:
        ...
        return pd.DataFrame(template.results(row="list"), columns=template.views)
    ...
    def get_dtypes(self, table_name: Optional[str] = None, **kwargs: Any) -> _Dtypes:
        ...
        dtypes = {
            k: _convert_python_dtypes_to_pandas_dtypes(INTERMINE_TYPE_MAPPING[v], k)
