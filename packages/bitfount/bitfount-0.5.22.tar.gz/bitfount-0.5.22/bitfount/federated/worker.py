...
class _Worker:
    ...
    def __init__(
        ...
        **_kwargs: Any,
        ...
        self.protocol = cast(
            ...
            bf_load(cast(_JSONDict, serialized_protocol), _PROTOCOLS),
        )
    ...
    async def run(self) -> None:
        ...
        if isinstance(self.protocol.algorithm, _BaseModelAlgorithmFactory):
            datastructure = self.protocol.algorithm.model.datastructure

        if not isinstance(self.protocol.algorithm, SqlQuery):
        ...
        worker_protocol = self.protocol.worker(mailbox=self.mailbox, hub=self.hub)
        ...
        logger.info("Task complete. Ready for next task...")
    ...
    def _load_data_for_worker(
        ...
        # This call loads the data for a multi-table _BaseSource as specified by the
