...
def initiate_cfg(cfg_file=None, fpath_model_pth=None, use_cpu=True, conf_threshold=0.6):
    """DEPRECATED"""
    if cfg_file is None and fpath_model_pth is None:
        raise FileNotFoundError("No cfg file or model path provided.")
    cfg = get_cfg()
...
def load_previous_cfg(model_results_dir: str, use_cpu: bool = True, conf_threshold: float = 0.6,
                      color_mode: str = "RGB"):
    cfg = get_cfg()
    ...
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = conf_threshold
    cfg.MODEL.DEVICE = "cpu" if use_cpu else "gpu"
    return cfg
...
def predict_single_img(fpath_img, model_dir, materials_dict,
                       conf_threshold=0.6, nms_iou_threshold=0.7,
                       inference_output_dims_wh=None, use_cpu=True,
                       color_mode="RGB"):
    # Create input dict
    input_img = create_img_dict(fpath_img)
    # Load cfg
    cfg = load_previous_cfg(model_dir, use_cpu=use_cpu, conf_threshold=conf_threshold, color_mode=color_mode)
    cfg.MODEL.ROI_HEADS.NMS_THRESH_TEST = nms_iou_threshold
    # Run prediction
...
def predict_multiple_imgs(fdir_imgs: str, fdir_model: str, materials_dict: dict = None, save_predictions=False, save_visualisations=False,
    ...
    fdir_predictions_out = f"{fdir_imgs}/output"
    ...
    cfg = load_previous_cfg(fdir_model, use_cpu=use_cpu, conf_threshold=conf_threshold, color_mode=color_mode)
    cfg.MODEL.ROI_HEADS.NMS_THRESH_TEST = nms_iou_threshold
    ...
    for img_path in input_img_paths:
        ...
        if save_visualisations:
            ...
            detectron.visualize_detectron_prediction(result, save_as=img_pred_name, show_img=False)
