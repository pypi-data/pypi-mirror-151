...
def count_yolo_annotations(fdir, yolo_format=True) -> int:
    ...
    # annotation_count = 0
    # file_list = sorted([file for file in os.listdir(fdir) if file.endswith('.txt')])
    # for file in file_list:
    #     file_path = os.path.join(fdir, file)
...
def create_fallback_yolo_materials_dict(yolo_root_dir) -> dict:
    ...
    """
    materials_dict = {}
    unique_material_ids = extract_material_ids_from_yolo_ds(yolo_root_dir)
    for i in unique_material_ids:
        materials_dict[int(i)] = "unspecified_" + str(i)
    return materials_dict


def create_materials_dict(materials: Set[str]) -> dict:
    """
    Takes a materials set and returns a dictionary with an index for each unique material: {material: index}
    :param materials:
    :return:
    """
    mats_dict = dict(enumerate(materials))
    # "Flip" the dictionary, so that numbers are values, and material names are keys.
    mats_dict = {material: index for index, material in mats_dict.items()}
    return mats_dict


def create_materials_dict_omnidirectional(materials: Set[str]) -> dict:
    """
    Creates an "omnidirectional" materials dict that works in both ways, i.e. you can access the index using the material
    string, but you can also access the material string using the index.
    Example: {0: 'plastic', 'plastic': 0}
    :param materials:
    :return:
    """
...
def extract_all_fpaths_by_extension(root_fdir: str, ext: str) -> List[str]:
    """
...
def extract_annotation_from_line(line: str):
    """
...
def extract_bboxes_from_labelme_json(fpath_json: str) -> List[BBox]:
    """
...
def extract_materials_from_labelme_jsons(json_fdir) -> Set[str]:
...
def extract_material_ids_from_yolo_ds(yolo_root_fdir) -> set:
    """
    Recursively extracts the material ids from txt files in a yolo-structured folder.
    :param yolo_root_fdir:
    :return:
    """
    fpaths = extract_all_fpaths_by_extension(yolo_root_fdir, ".txt")
    id_set = set()
    for fpath in fpaths:
...
def find_bg_img(fpath_annotations_json: str, img_dir: str):
...
def find_max_annotation_count(fdir_annotations) -> int:
    """
...
def load_labelme_annotation_file(fpath_json) -> LabelmeAnnotation:
    ...
    _loaded_bboxes = extract_bboxes_from_labelme_json(fpath_json)
    ...
    try:
        ...
        if annotations_raw:  # if there are any lines in the file
            for line in annotations_raw:
                ...
                logging.debug(line)
                current_box = extract_annotation_from_line(line)
...
def visualize_coco_annotations(coco_json_fpath: str):
    ...
    # img_fnames = sorted(annotations_df['file_name'].unique())
    # logging.debug(annotations_df.columns)
    # img_count = len(img_fnames)
    # _checked_ids = []
    #
    # # Check if all images show up in annotation file:
    # img_files = [fname for fname in sorted(os.listdir(img_directory)) if not fname.startswith(".")]
    # img_count = len(img_files)
    # for i, img in enumerate(img_files):
    #     # if img not in annotations_df["file_name"].unique():
    #     #     print(f"File {img} does not seem to be annotated.")
    #     show_img_annotations_coco(img, img_directory, annotations_df, prefix_text=f"{i + 1}/{img_count}")


def show_img_annotations_coco(img_name: str, img_dir: str, annotations_df: pd.DataFrame, prefix_text: str = ""):
...
def show_planned_cuts(fpath_img: str, vcut_locations: List[int], hcut_locations: List[int], fpath_annot: str = None):
    if fpath_annot is not None:
        visualize_img_annotations_yolo(fpath_img, fpath_annot)
    else:
...
def visualize_img_annotations_yolo(fpath_img: str, fpath_annotations: str, prefix_text: str = '', color="red",
                              materials_dict=None):
    """

    :param fpath_img:
    :param fpath_annotations:
    :param prefix_text:
    :param color:
    :param materials_dict: {i: material}
    :return:
    ...
    img_basename = str(os.path.split(fpath_img)).split(".")[0]
    lab_basename = str(os.path.split(fpath_annotations)).split(".")[0]
    if not img_basename == lab_basename:
        print(f"WARNING: differing names: label: {lab_basename}, image: {img_basename}")
    # Read Image
    try:
        ...
        print_text = prefix_text + f'  |  {img_name} |  Annotations: {annotation_count}'
        plt.text(0, -30, print_text)
        fig.canvas.manager.full_screen_toggle()
        # plt.show()
...
def update_material_in_labelme_file(json_fpath: str, old_mat: str, new_mat: str, backup_dir=None) -> None:
    ...
    :param json_dir: File path to directory containing labelme jsons
    :param old_mat: old material name (to be overwritten)
    :param new_mat: new material name to be used when overwriting
    :return: None. Overwrites json files in place.
    """
...
if __name__ == '__main__':
    ...
    # img = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/images/train/Gnejna_DJI_0721_0-1.tif"
    # img = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/images/train/Gnejna_DJI_0721_1-2.tif"
    # annots = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/labels/train/Gnejna_DJI_0721_0-1.txt"
    # annots = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/labels/train/Gnejna_DJI_0721_1-2.txt"
    # visualize_img_annotations_yolo(img, annots, materials_dict={0: "trash"})
    # plt.show()

    # mini_gejna_coco = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_05_mini_gnejna_coco/"
    # fpath_train_json = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_05_mini_gnejna_coco/coco/annotations/instances_train2017.json"
    # fpath_img_dir = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_05_mini_gnejna_coco/coco/train2017/"
    # img_df, labels_df = load_coco_ds(fpath_train_json)
    # train_df = merge_img_info_into_labels(img_df, labels_df)
    # visualize_coco_annotations(fpath_img_dir, train_df)

    fpath_json_gnejna_full_train = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates_coco/coco/annotations/instances_train2017.json"
    fpath_gnejna_full_imgs = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates_coco/coco/train2017/"
    # img_df, labels_df = load_coco_ds(fpath_json_gnejna_full_train)
    # train_gnejna_full_df = merge_img_info_into_labels(img_df, labels_df)
    # train_gnejna_full_df.to_csv("/home/findux/Desktop/tmp/gnejna_full.csv")
    # visualize_coco_annotations(fpath_gnejna_full_imgs, train_gnejna_full_df)

    # visualize_coco_annotations(fpath_json_gnejna_full_train)

    json_fpath = "/media/findux/DATA/Documents/Malta_II/results/5148_2022-05-01_225524/instances_detectron_val_coco_format.json"
    json_fpath = "/home/findux/Desktop/out/coco_instances_results.json"
    make_json_readable(json_fpath, False)
