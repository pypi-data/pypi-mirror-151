...
class Tile(object):
    def __init__(self, array: np.array,
        ...
        if self.yaw_angle_deg is None:
            self.yaw_angle_deg = geoprocessing.extract_uav_yaw(self.fpath_orig_img)
...
def cut_img(fpath_img, tile_target_dir, altitude_m, location: str,
    ...
    try:
        ...
        if use_gps:
            original_coordinates_lat_long = geoprocessing.extract_coordinates_from_img(fpath_img)
...
def create_tiles(img_dir: str, horizontal_tile_count, vertical_tile_count, altitude_m, location: str,
    ...
    for i, img_fname in enumerate(img_list):
        ...
        # Trying to fi memory leak:
        del current_tiles, current_tiles_df, tiles_df
        gc.collect()
...
if __name__ == "__main__":
    ...
    FDIR = '/media/findux/HDD/drone_surveys_malta/paradise_bay/rgb/images/RGB images and video/set_4'
    FDIR = '/media/findux/HDD/drone_surveys_malta/paradise_bay/RGB images and video/'
    FDIR = '/media/findux/HDD/drone_surveys_malta/paradise_bay/RGB images and video/'
    FDIR = '/home/findux/Desktop/img_cutting/'
    create_tiles(FDIR, 3, 2, 5, 'Gnejna')
