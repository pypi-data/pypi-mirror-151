...
from indico_install.config import SERVICES_YAML, IMAGE_REGISTRY, ConfigsHolder
...
def push_all(ctx, *, yes, forwarded=True):
    ...
    for (
        ...
        if yes or confirm("Push new image?"):
            secho(
                run_cmd(
                    f"kubectl set image {resource}/{deployment} "
                    f"{deployment}={IMAGE_REGISTRY}/{saved_image}"
                ),
                fg="green",
            )
    ...
    for resource_type, resource, image in resources:
        ...
        if yes or confirm("Push new image?"):
            secho(
                run_cmd(
                    "kubectl set image "
                    f"{resource_type}/{resource} {resource}={IMAGE_REGISTRY}/{image}",
                    silent=True,
                )
            )
