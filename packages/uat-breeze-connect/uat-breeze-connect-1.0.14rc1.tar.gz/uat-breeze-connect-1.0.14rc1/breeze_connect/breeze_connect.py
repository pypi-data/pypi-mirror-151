        ...
        try:
            ...
            if exchange_code_name == False:
                ...
                if stock_data == False:
                    ...
                    if stock_data == False:
                        raise Exception("Stock_Token does not exist in both exchange-code i.e. NSE or NFO.")
                    else:
            ...
            if exchange_code_name.lower() not in ["nse", "bse"]:
                ...
                for date in stock_data[0].split("-")[2:5]:
                    date_string += date + "-"
                output_data["strike_date"] = date_string[:-1]
        ...
        try:
            if interval == "" or interval == None:
                ...
                if product_type == "" or product_type == None:
                    ...
                    return response
                elif product_type.lower() not in ["futures", "options", "furtureplus", "optionplus"]:
                    response = {"Success": "", "Status": 500, "Error": "Product-type should be either 'futures', 'options', 'furtureplus', or 'optionplus' for Exchange-Code 'NFO'"}
                    return response
    ...
    def add_margin(self, product_type="", stock_code="", exchange_code="", settlement_id="", add_amount="", margin_amount="", open_quantity="", cover_quantity="", category_index_per_stock="", expiry_date="", right="", contract_tag="", strike_price="", segment_code=""):
        try:
            if exchange_code == "" or exchange_code == None:
                response = {"Success": "", "Status": 500, "Error": "Exchange-Code cannot be empty"}
                return response
            elif product_type != "" and product_type != None and product_type.lower() not in ["futures", "options", "furtureplus", "optionplus", "cash", "eatm", "margin"]:
                response = {"Success": "", "Status": 500, "Error": "Product-type should be either 'futures', 'options', 'furtureplus', 'optionplus', 'cash', 'eatm', or 'margin'"}
                return response
        ...
        try:
            if stock_code == "" or stock_code == None or exchange_code == "" or exchange_code == None or product == "" or product == None or action == "" or action == None or order_type == "" or order_type == None or quantity == "" or quantity == None or price == "" or price == None or action == "" or action == None:
                if stock_code == "" or stock_code == None:
                    ...
                    return response
            elif product.lower() not in ["futures", "options", "furtureplus", "optionplus", "cash", "eatm", "margin"]:
                response = {"Success": "", "Status": 500, "Error": "Product should be either 'futures', 'options', 'furtureplus', 'optionplus', 'cash', 'eatm', or 'margin'"}
                return response
