...
class Music(commands.Cog):
    ...
    async def start_nodes(self):
        ...
        for config in self.bot.lavalink_nodes:
            try:
                ...
                # print(f"[nextmusic] INFO - Created node: {node.identifier}")
                print(f'[Nextmusic] INFO - Node created: {node.identifier}\nWant help? Join Discord Server - https://araki.social/support')
            except Exception:
                print(
    ...
    #     vc: NextPlayer = ctx.voice_client
    #     embed = discord.Embed(description=f'[{vc.track.title}]({vc.track.uri})', colour=Commands.embed_colour())
    #     embed.set_author(name='Now playing')
    #     embed.add_field(name='Author', value=vc.track.author, inline=False)
