...
from datomizer.utils.constants import MANAGEMENT_PUT_TRAIN_FLOW
from datomizer.utils.general import ID, TRAIN_ITERATIONS


def create_train_request() -> dict:
    return {"sampleInputRatio": 1, "sampleOutputRatio": 1}


def train(dato_mapper: DatoMapper) -> int:
    dato_mapper.wait()
    response_json = dato_mapper.datomizer.get_response_json(requests.put,
                                                            ...
                                                            data=json.dumps(create_train_request()))
