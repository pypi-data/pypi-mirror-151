    ...
    for params in ets_params:
        ...
        try:
            ...
            model = ExponentialSmoothing(
                input_series, trend=trend, seasonal=seasonal, damped_trend=damped,
...
def ets_predict_monthly(df, ets_train, horizon=3, out_of_sample=3):
    ...
    try:
        model = ExponentialSmoothing(
                series, trend=trend, seasonal=seasonal, damped_trend=damped,
                seasonal_periods=seasonal_periods, use_boxcox=use_boxcox)
        fit = model.fit(optimized=True)
        if np.isnan(fit.sse) == True or fit.forecast(horizon)[0] < 0 or \
            ...
            raise Exception(
                ...
                + str(df['drug_id'].unique()[0]) + ' running model with default params')
    except Exception as error:
        model = ExponentialSmoothing(
                series, trend=None, seasonal=None, damped_trend=False,
