...
def main(debug_mode, db_read, db_write, read_schema, write_schema, logger):
    s3 = S3()
...
if __name__ == '__main__':
    ...
    main(debug_mode, rs_db_read, rs_db_write, read_schema,
         write_schema, logger)
...
def build_probability_matrix(dfi):
    ...
    df = df.groupby(['historical_bucket']).agg(selling_probability=('sold_analysis_month_flag', 'mean'),
                                    total_drugs_in_bucket=('drug_id', 'count')).reset_index()
