...
def main(debug_mode, weights_as, weights_pr, as_low_volume_cutoff,
    ...
    weights_as = literal_eval(weights_as)
    weights_pr = literal_eval(weights_pr)
    ...
    try:
        ...
        if debug_mode == 'N':
            ...
            try:
                q_index_increment = f"""select max(id) from "{write_schema}"."distributor-ranking-rules" """
                q_redundant_increment = f"""select max(id) from "{write_schema}"."distributor-ranking-rule-values" """
                index_increment = int(db_write.get_df(q_index_increment).values[0][0]) + 1
                redundant_increment = int(db_write.get_df(q_redundant_increment).values[0][0]) + 1

            except:
                index_increment = 1
                redundant_increment = 1

            logger.info("Incremented distributor-ranking-rules by ", index_increment)
            logger.info("Incremented distributor-ranking-rule-values by ", redundant_increment)
            ...
            db_write.execute(f"""UPDATE "{write_schema}"."distributor-ranking-rules" SET "is-active" = 0""")

            logger.info("Writing to table: distributor-ranking-rules")
            s3.write_df_to_db(df=distributor_ranking_rules,
                              table_name='distributor-ranking-rules',
                              db=db_write, schema=write_schema)
            logger.info("Writing to table: distributor-ranking-rule-values")
            s3.write_df_to_db(df=distributor_ranking_rule_values,
                              table_name='distributor-ranking-rule-values',
                              db=db_write, schema=write_schema)
...
if __name__ == '__main__':
    ...
    main(debug_mode, weights_as, weights_pr, as_low_volume_cutoff,
         pr_low_volume_cutoff, low_volume_cutoff_franchisee, volume_fraction,
         time_interval, time_interval_franchisee, rank_override_dc_active,
         rank_override_franchisee_active, rs_db_read, rs_db_write, read_schema,
         write_schema, logger)
