...
try:
    ...
    from
        "prod2-generico".fifo a
    right join "prod2-generico".item b on
        a.itemc = b.code
    left join "prod2-generico".Acm a2 on
        a2.code = a.Acno
        and a2.Slcd in ('SL', 'GL')
    left join "prod2-generico".Master m on
        b.ItemCat = m.code
    left join "prod2-generico".Salt s on
        b.Saltcode = s.Code
    left join "prod2-generico".SalePurchase2 sp on
    ...
    wh_inventory['snapshot_date'] = cur_date

    wh_inventory['created-at'] = datetime.datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S")
    wh_inventory['created-by'] = 'etl-automation'
    wh_inventory['updated-by'] = 'etl-automation'
    wh_inventory['updated-at'] = datetime.datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S")
    ...
    wh_inventory['order_upto_point'] = wh_inventory['order_upto_point'].astype(
        int)
    wh_inventory['total_quantity'] = wh_inventory['total_quantity'].astype(int)
    wh_inventory['balance_quantity'] = wh_inventory['balance_quantity'].astype(
        int)
    wh_inventory['locked_quantity'] = wh_inventory['locked_quantity'].astype(
        int)
