...
if start1=="NULL" and start2=="NULL":
    ...
    if datetime.now(tz=gettz('Asia/Kolkata')).strftime('%H:%M:%S') < '08:30:00':
        start1 = (datetime.now(tz=gettz('Asia/Kolkata')) -
                  timedelta(days=1)).strftime('%Y-%m-%d 20:30:00')
        start2 = (datetime.now(tz=gettz('Asia/Kolkata'))).strftime('%Y-%m-%d 08:00:00')
        ...
        start2 = ceil_dt(datetime.now(tz=gettz('Asia/Kolkata')), timedelta(minutes=-30)).strftime('%Y-%m-%d %H:%M:%S')
...
for cluster in cluster_list:
    ...
    for i in cluster_stores:
        ...
        orde = """
                ...
                where
                    pso."created-at" > '{start1}'
                    and pso."created-at" <= '{start2}'
        ...
        orders1 = rs_db.get_df(orde)

        # =============================================================================
        # Fetch open PSOs for day before selected time for which PSTM is not created yet
        # =============================================================================
        orde2 = """
                select
                    pso."order-number",
                ...
                join "prod2-generico"."stores" s on
                    s."id" = pso."store-id"
                left join "prod2-generico"."pso-stock-transfer-mapping" pstm on
                    pso.id = pstm."patient-store-order-id"
                where
                    pstm.id is null
                    and date(pso."created-at") = '{today_date}'
                    and pso."created-at" <= '{start1}'
                    and pso."store-id" = {analysis_store}
                    and pso.status not in ('billed', 'completed')
                order by
                    pso."created-at" desc;
         """.format(today_date=today_date, start1=start1, analysis_store=analysis_store)

        orders2 = rs_db.get_df(orde2)

        orders = pd.concat((orders1,orders2))
...
pso_cluster_fulfillment[['from-store-id', 'to-store-id', 'drug-id', 'item-quantity']] = pso_cluster_fulfillment[['from-store-id', 'to-store-id', 'drug-id', 'item-quantity']].apply(pd.to_numeric, errors='ignore').astype('Int64')
#
# =============================================================================
# writing to PG
# =============================================================================
...
pso_added_uri = s3.save_df_to_s3(df=check_first_final, file_name='pso_transfer_details_{}_{}.csv'.format(start1, start2))
