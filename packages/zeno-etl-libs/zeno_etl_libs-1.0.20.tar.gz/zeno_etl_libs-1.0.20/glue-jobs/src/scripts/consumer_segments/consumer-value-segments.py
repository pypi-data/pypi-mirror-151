...
parser.add_argument('-pe', '--period_end_d_plus1', default=0, type=str, required=False)
...
try:
    ...
    period_end_d_plus1 = dt.today().strftime('%Y-%m-%d')
...
s = f"""
    ...
    WHERE DATEDIFF('days', date("created-at"),'{period_end_d_plus1}') between -90 and -1
    GROUP BY "patient-id"
    """
