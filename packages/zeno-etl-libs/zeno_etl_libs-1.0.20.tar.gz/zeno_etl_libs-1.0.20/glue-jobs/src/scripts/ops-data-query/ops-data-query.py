# Script For Sale Purchase Ratio#
...
parser.add_argument('-sd', '--start_date', default='2022-03-01', type=str, required=False)
parser.add_argument('-ed', '--end_date', default='2022-04-30', type=str, required=False)
...
network_wh_purchase= pd.read_sql(q_7,cnxn)

network_wh_purchase[['entity-id', 'drug-id']]= network_wh_purchase[['entity-id','drug-id']].apply(pd.to_numeric, errors='ignore').astype('Int64')

network_wh_purchase=pd.merge(network_wh_purchase,drug_info,how='left',on='drug-id')



network_wh_purchase[['drug-type', 'goodaid-flag']]=network_wh_purchase[['drug-type', 'goodaid-flag']].fillna('NA')
...
network_wh_purchase.dtypes

network_wh_purchase=network_wh_purchase.groupby(['type-1','entity-id','entity-name','sub-type-1',
                                                 'sub-type-2','sub-type-3','distributor-id',
