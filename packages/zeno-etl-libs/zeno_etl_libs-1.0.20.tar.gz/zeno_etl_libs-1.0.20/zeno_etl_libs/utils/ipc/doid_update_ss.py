...
def doid_update(data, type_list, db, schema, logger=None, gaid_omit=True):
    ...
    for store_id in data['store_id'].unique():
        current_ss_query = """
            select doi.id, "store-id", "drug-id", min, doi."safe-stock", max
            from "{schema}"."drug-order-info-data" doi
            left join "{schema}".drugs d
            on d.id = doi."drug-id"
            where doi."store-id" = {store_id}
            ...
            """.format(store_id=store_id,
                       ...
                       schema=schema)
        current_ss = db.get_df(current_ss_query)
        ...
        if len(data_to_be_updated_list) > 0:
            ...
            update_test_query = """
                    ...
                    from "{schema}"."drug-order-info-data"
                    where "store-id" = {store_id}
                    and "drug-id" in {drug_list}
                    """.format(store_id=store_id,
                               drug_list=drug_list,
                               schema=schema)
            update_test = db.get_df(update_test_query)
