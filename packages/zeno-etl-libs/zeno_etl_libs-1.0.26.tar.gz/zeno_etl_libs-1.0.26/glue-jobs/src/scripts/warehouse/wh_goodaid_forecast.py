...
Purpose: To generate forecast for Goodaid drugs at warehouse
"""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="To generate forecast for Goodaid drugs at warehouse.")
    ...
    try:
        ...
        expected_new_stores = int(params_table.where(
            ...
            axis=0).dropna()['value'])
        wh_id = int(params_table.where(params_table['param'] == 'wh_id',
                                       axis=0).dropna()['value'])
        ...
        # get 30 days sales for active gaid drugs
        drug_sales_query = '''
            select
                "drug-id" as drug_id,
        ...
        composition_sales['composition_sales_quantity'] = composition_sales[
                                                              'composition_sales_quantity'] * 30 / days
        ...
        # getting last-to-last 30 day sales for calcuating growth factor
        last_to_last_sales_query = '''
            select
                "drug-id" as drug_id,
                sum(quantity) as last_to_last_30_day_sales
            from
                "prod2-generico".sales
            where
                "drug-id" in {drug_ids}
                and date("created-at") >= current_date - 60
                and date("created-at") < current_date - 30
            group by
                "drug-id"
        '''.format(drug_ids=tuple(drugs['drug_id']))
        last_to_last_sales = rs_db.get_df(last_to_last_sales_query)
        logger.info('last-to-last 30 day sales data pulled from rs')

        # getting last 30 day sales
        last_sales_query = '''
            select
                "drug-id" as drug_id,
                sum(quantity) as last_30_day_sales
            from
                "prod2-generico".sales
            where
                "drug-id" in {drug_ids}
        ...
        logger.info('last 30 day sales data pulled from rs')

        # merging to main_df
        main_df = main_df.merge(last_to_last_sales, on='drug_id', how='left')
        main_df['last_to_last_30_day_sales'].fillna(0, inplace=True)
        main_df = main_df.merge(last_sales, on='drug_id', how='left')
        main_df['last_30_day_sales'].fillna(0, inplace=True)
        main_df['growth_factor'] = main_df['last_30_day_sales'] / main_df[
            'last_to_last_30_day_sales']
        ...
        stores_cmp_query = '''
            ...
            from
                "prod2-generico".sales
            where
                date("created-at") >= current_date - 30
                and date("created-at") < current_date
            group by
                "store-id"
        '''
        ...
        # getting avg sales
        # add exception loop for no store found in future
        ...
        main_df['safety_stock'] = (main_df['forecast'] / 30 *
                                   main_df['safety_stock_doh']).round()
        main_df['reorder_point'] = (main_df['forecast'] / 30 *
                                    main_df['reorder_point_doh']).round()
        main_df['order_upto_point'] = (main_df['forecast'] / 30 *
        ...
        to_upload['demand_daily'] = main_df['forecast'] / 30
        ...
        to_upload['month'] = str(datetime.now().strftime("%m"))
        to_upload['year'] = str(datetime.now().strftime("%Y"))
        to_upload['month_begin_dt'] = str(
            datetime.now().date() - timedelta(days=datetime.now().day - 1))
        to_upload['created_at'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        to_upload['created_by'] = 'etl-automation'
        to_upload['updated_at'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        df_uri = s3.save_df_to_s3(df=main_df,
                                  file_name='wh_goodaid_forecast_{date}.csv'.format(date=str(run_date)))
        ...
        err_msg = str(e)
        logger.info('wh_goodaid_forecast job failed')
        logger.exception(e)
    ...
    if status:
        result = 'Success'
        email.send_email_file(subject=f"wh_goodaid_forecast ({env}): {result}",
                              mail_body=f"""
