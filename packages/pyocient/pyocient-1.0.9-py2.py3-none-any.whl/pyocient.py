...
class _SetParameterFactory(_OcientRequestFactory):
    def request(self, op: str, val: int):
        ...
        elif op == "PRIORITY_ADJUSTMENT_FACTOR":
            sp.priority_adjust_factor.priority_adjust_factor = val
        elif op == "PRIORITY_ADJUSTMENT_TIME":
            sp.priority_adjust_time.priority_adjust_time = val
        elif op == "MIN_PRIORITY":
            sp.min_priority.min_priority = val
        elif op == "MAX_PRIORITY":
            sp.max_priority.max_priority = val
        else:
...
class Connection:
    ...
    # places
    def __init__(self, dsn=None, user=None, password=None, host=None, database=None, tls=None, handshake=None, force=None, configfile=None, session=None):
    ...
    def _client_handshake_CBC_GCM(self, is_explicit_sso=False, force=False):
        while True:
            ...
            if self.handshake == self.HANDSHAKE_CBC:
                ...
                req.client_connection2.pubKey = public_key.public_bytes(encoding=serialization.Encoding.PEM, format=serialization.PublicFormat.SubjectPublicKeyInfo)
            else:
                ...
                req.client_connection_gcm2.pubKey = public_key.public_bytes(encoding=serialization.Encoding.PEM, format=serialization.PublicFormat.SubjectPublicKeyInfo)
            ...
            if rsp.response.type == proto.ConfirmationResponse.RESPONSE_WARN:
                ...
                if rsp.redirect:
                    ...
                    logger.debug(f"Redirecting connection to {rsp.redirectHost}:{rsp.redirectPort}, which maps to {mapped_host}:{mapped_port}")
    ...
    def _client_handshake_SSO(self):
        ...
        if not open_new(rsp.authUrl):
            logger.info("[pyocient] Could not open default browser with webbrowser library. Please authenticate at: " + rsp.authUrl)
            print("[pyocient] Could not open default browser with webbrowser library. Please authenticate at: ", rsp.authUrl)
    ...
    def _poll_database(self, requestId):
        ...
        self.session = Session(securityToken=SecurityToken(received_token.data, received_token.signature, received_token.issuerFingerprint))
    ...
    def _save_secondary_interfaces(self, new_secondary_interfaces):
        ...
        logger.debug(f"Saving secondary interfaces: index {self.secondary_index} interfaces: {self.secondary_interfaces}")
    ...
    def _encryption_routine(self, initialization_vector, peer_key):
        ...
        if self.handshake == self.HANDSHAKE_CBC:
            ...
            # we got from our KDF
            encryptor = Cipher(algorithms.AES(key), modes.CBC(initialization_vector), backend=default_backend()).encryptor()
            ...
            encryptor = Cipher(algorithms.AES(key), modes.GCM(initialization_vector), backend=default_backend()).encryptor()
            # We do not use AAD
    ...
    def _parse_args(self, dsn, user, password, host, database, tls, handshake, force, configfile):  # pylint: disable=too-many-arguments
        ...
        config = configparser.ConfigParser(
            defaults={"port": str(self.DEFAULT_PORT), "database": self.DEFAULT_DATABASE, "tls": "unverified", "force": "false", "handshake": ""}, interpolation=None
        )
        configvals = None
    ...
    def resolve_new_endpoint(self, new_host, new_port):
        ...
        """
        logger.debug(f"Resolving new endpoint {new_host}:{new_port} with secondary_index {self.secondary_index} and secondary_interface {self.secondary_interfaces}")
    ...
    def redirect(self, new_host, new_port):
        ...
        return Connection(
            user=self.user, password=self.password, host=new_endpoint, database=self.database, tls=self.tls, handshake=self.handshake, force=self.force, session=self.session
        )
...
class Cursor:
    ...
    def columns(self, schema="%", table="%", column="%"):
        ...
        # pylint: disable=no-member
        self._metadata_req(proto.FetchSystemMetadata.GET_COLUMNS, schema=schema, table=table, column=column)
        return self
    ...
    def _execute_query(self, operation, query_type: str = "SELECT"):
        ...
        if query_type in ["SELECT", "WITH"]:
            ...
            self.description = [("explain", TypeCodes.CHAR, None, None, None, None, None)]  # display_size  # internal_size  # precision  # scale  # null_ok
    ...
    def _execute_list(self, operation: str = "LIST ALL QUERIES"):
        ...
        for field in row._fields:
            self.description.append(
                (field, TypeCodes.cls_to_type(row._field_types[field]), None, None, None, None, None)  # display_size  # internal_size  # precision  # scale  # null_ok
            )
    ...
    def _get_result_metadata(self):
        ...
        for i in range(len(cols)):  # pylint: disable=consider-using-enumerate
            ...
            self.description.append((name, TypeCodes.to_type(rsp.cols2Types[name]), None, None, None, None, None))  # display_size  # internal_size  # precision  # scale  # null_ok
    ...
    def _execute_get_schema(self) -> None:
        ...
        self.description = [("schema", TypeCodes.CHAR, None, None, None, None, None)]  # display_size  # internal_size  # precision  # scale  # null_ok
    ...
    def _execute_get_server_session_id(self) -> None:
        self.description = [("server_session_id", TypeCodes.CHAR, None, None, None, None, None)]  # display_size  # internal_size  # precision  # scale  # null_ok
        self.list_result = [
    ...
    def _execute_get(self, params) -> None:
        if len(params) == 1 and params[0].upper() == "SCHEMA":
            self._execute_get_schema()
        elif len(params) == 3 and params[0].upper() == "SERVER" and params[1].upper() == "SESSION" and params[2].upper() == "ID":
...
def connect(dsn=None, user=None, password=None, host=None, database=None, tls=None, force=None, handshake="gcm", configfile=None):
...
""",
    ...
    outgroup.add_argument("-n", "--noout", action="store_const", const=None, dest="outfile", help="Do not output results")
    configgroup = parser.add_mutually_exclusive_group()
    configgroup.add_argument("-c", "--configfile", type=str, default=str(configfile), help="Configuration file")
    configgroup.add_argument("--noconfig", action="store_const", const=None, dest="configfile", help="No configuration file")
    parser.add_argument("-i", "--infile", type=FileType("r"), default=None, help="Input file containing SQL statements")
    parser.add_argument("-l", "--loglevel", type=str, default="critical", choices=["critical", "error", "warning", "info", "debug"], help="Logging level, defaults to critical")
    ...
    parser.add_argument("dsn", nargs="?", help="DSN of the form ocient://user:password@[host][:port][/database][?param1=value1&...]")
    parser.add_argument("sql", nargs="?", help="SQL statement")
    parser.add_argument("--format", "-f", type=str, choices=["json", "table", "csv"], default="json", help="Output format, defaults to json")
    parser.add_argument("--nocolor", action="store_true", help="When using pyocient interactively, do not color")
...
def main():
    ...
    logging.basicConfig(level=log_level, stream=args.logfile, format="[%(asctime)s][%(levelname)s] %(message)s")
    ...
    def _do_query(args, connection, query):
        ...
        tokens = [token for (token_type, token) in lexer.get_tokens(query) if token_type in (Token.Keyword, Token.Name, Token.Literal.String.Symbol, Token.Literal.String.Single)]
        ...
        try:
            ...
            if cursor.description and not cursor.explain_result:
        ...
        if cursor.description is None:
            ...
            if cursor.explain_result:
                ...
                print(json.dumps(result, indent=4, default=_custom_type_to_json), file=args.outfile)
            elif result:
                if args.format == "json":
                    ...
                    print(tabulate(result, headers=[c[0] for c in cursor.description], tablefmt="psql"), file=args.outfile)
    ...
    def _do_repl(args, connection):
        ...
        if args.nocolor:
            session = PromptSession(history=FileHistory(str(Path.home() / ".pyocient_history")))
        else:
            session = PromptSession(lexer=PygmentsLexer(SqlLexer), history=FileHistory(str(Path.home() / ".pyocient_history")))
