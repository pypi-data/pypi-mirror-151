...
from ..version import MHMup_version as ult_ver
...
def MHMup_cmd(
    pattern=None, manager=False, MHMup_bot=MHMup_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await MHMup_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await MHMup_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@Dev-MHM/MHMuserStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = (
                    "**MHMuser Client Error:** `Forward this to` @MHMuserSupport\n\n"
                )
                ftext += "**pyMHMuser Version:** `" + str(pyver)
                ftext += "`\n**MHMuser Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START MHMuser CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**MHMuser Client Error:** `Forward this to` @MHMuserSupport\n\n",
                        )
                else:
