...
class MongoDB:
    def __init__(self, key, dbname="MHMuserDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        self.re_cache()

    def __repr__(self):
        return f"<MHMuser.MonGoDB\n -total_keys: {len(self.keys())}\n>"
    ...
    def flushall(self):
        self.dB.drop_database("MHMuserDB")
        self._cache = {}
        return True
...
# for SQL Implementation in MHMuser.
#
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute("CREATE TABLE IF NOT EXISTS MHMuser ()")
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('MHMuser')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'MHMuser'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT (%s) FROM MHMuser", (str(variable),))
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(
                f"ALTER TABLE MHMuser DROP COLUMN IF EXISTS (%s)", (str(key),)
            )
            ...
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE MHMuser ADD (%s) TEXT", (str(key),))
        self._cursor.execute(
            f"INSERT INTO MHMuser (%s) values (%s)", (str(key), str(value))
        )
        return True
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE MHMuser DROP COLUMN (%s)", (str(key),))
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE MHMuser")
        self._cursor.execute("CREATE TABLE IF NOT EXISTS MHMuser ()")
        return True
