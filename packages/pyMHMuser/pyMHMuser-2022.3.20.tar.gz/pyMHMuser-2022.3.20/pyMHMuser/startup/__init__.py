...
if run_as_module:
    ...
    from ..version import __version__ as __pyMHMup__
    from ..version import MHMup_version

    file = f"MHMuser{sys.argv[6]}.log" if len(sys.argv) > 6 else "MHMuser.log"
    ...
    LOGS.info(f"pyMHMuser Version - {__pyMHMup__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"MHMuser Version - {MHMup_version} [{HOSTED_ON}]")
