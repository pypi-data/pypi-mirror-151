...
async def metadata(file):
    ...
    if info.get("AudioCount"):
        ...
        data["performer"] = (
            info.get("Performer") or udB.get_key("artist") or MHMup_bot.me.first_name
        )
...
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=MHMuserUSERBOT/MHMuser/{}/message={}"
    req_link = chatbot_base.format(
        MHMup_bot.me.first_name or "MHMuser user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = MHMup_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{MHMup_bot.me.username}" if MHMup_bot.me.username else None
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="MHMuseruser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
