...
class Solution:
    def __init__(self, components, compiler='gcc'):
        ...
        self.libpath = []
        self.dylib_path = []
        self.userspace_lib_path = []
        self.depend_libs = []
        self.depend_userspace_libs = []
        self.depend_dylibs = []
        self.org_defines = {}
        self.defines = {}
        self.ASFLAGS = []
                ...
                if self.solution_component.hw_info.board_name:
                    ...
                    if not self.chip_component:
                        put_string("Not found in `%s` components that the current project depends on. " %
                                   self.board_component.hw_info.chip_name)
                        exit(-1)
                    put_string("build toolchain checked = ", self.chip_component.hw_info.toolchain_prefix)
        ...
        def cpu_set_variable(cpu, arch):
            ...
            if arch == '' or arch.upper() == 'CSKY':
                ...
                self.variables.set('ARCH', arch, True)
            elif arch.upper() == 'MIPS':
                self.variables.set('cpu_num', mips_cpu_id(cpu))
                self.variables.set('ARCH', arch, True)
            else:
        ...
        def update_variables():
            ...
            for component in self.components:
                if component.type == 'userspace_common':
                    add_defines(component)
            for component in self.components:
            ...
            for component in [self.chip_component, self.board_component, self.solution_component]:
                if component:
                    ...
                    for c in component.build_config.appldflag.split():
                        if(check_duplicate_value(self.APPLINKFLAGS, c) == False):
                            self.APPLINKFLAGS.append(c)
            ...
            self.userspace_lib_path = os.path.join(self.yoc_path, 'userspace_lib') 
            self.libpath.append(self.userspace_lib_path)
            ...
            for component in self.components:
                if (len(component.source_files) > 0 and component.name not in self.libs
                    and component.type != "dynamic" and component.type != "userspace_common"):
                ...
                if (len(component.source_files) > 0 and component.name not in self.dylibs
                ...
                if (len(component.source_files) > 0 and component.name not in self.userspace_libs
                   and component.type == "userspace_common"):
                    self.userspace_libs.append(component.name)
                    if component.name.startswith('lib'):
                        self.depend_userspace_libs.append(os.path.join(
                            self.userspace_lib_path, component.name + '.a'))
                    else:
                        self.depend_userspace_libs.append(os.path.join(
                            self.userspace_lib_path, 'lib' + component.name + '.a'))
            ...
            self.uspace_app_ld_script = solution_hw_info.uspace_app_ld_script
            self.uspace_app_ld_script = self.variables.convert(self.uspace_app_ld_script)
            ...
            for component in self.components:
                if component.name == 'uspace_app':
                    component.uspace_app_ld_script = self.uspace_app_ld_script
