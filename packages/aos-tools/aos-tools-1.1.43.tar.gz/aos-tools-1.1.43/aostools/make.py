...
from .yoc import YoC
from .builder import *
from .package import set_specified_board
...
class Make(object):
    def __init__(self, elf=None, objcopy=None, objdump=None):
        ...
        set_specified_board(board_name)
        print_depends = False
        if not SCons.GetOption('verbose'):
            print_depends = True
        
        # do user board pre config
        self.yoc = YoC()
        result = self.yoc.doBoardPreConfig(board_name)
        if result == True:
            put_string("run user pre_config finish, rebuild compoments")
            self.yoc = YoC()
        
        solution = self.yoc.getSolution(board_name, print_depends=print_depends, exit_if_lost=False)
        if not solution:
            # retry if auto install missing component
            self.yoc = YoC()
            solution = self.yoc.getSolution(board_name, print_depends=print_depends)
        if solution:
            if not SCons.GetOption('verbose'):
                put_string("----------------------------------------------------------------")
                for comp in solution.cond_components:
                    if comp.conditions:
                        put_string(comp.name + ":", comp.conditions)
                put_string("----------------------------------------------------------------")
            filename = "%s@%s" % (solution.solution_component.name, solution.solution_component.hw_info.board_name)
            if not elf:
                elf = os.path.join("out", "%s.elf" % filename)
            if not objcopy:
                objcopy = os.path.join("out", "%s.bin" % filename)
    ...
    def library_yaml(self):
        ...
        if conf and 'name' in conf:
            ...
            component.export()
            if component.name in ['separate_app']:
