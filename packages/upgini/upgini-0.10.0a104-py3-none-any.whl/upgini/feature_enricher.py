...
class FeaturesEnricher(TransformerMixin):  # type: ignore
    ...
    def __init__(
        ...
        keep_input: Optional[bool] = False,
        accurate_model: bool = False,
        api_key: Optional[str] = None,
        endpoint: Optional[str] = None,
    ):
        if len(search_keys) == 0:
            raise Exception("Key columns should be marked up by search_keys")
    ...
    def _inner_fit(
        ...
        **fit_params,
        ...
        if not isinstance(X, pd.DataFrame):
            raise Exception(f"Only pandas.DataFrame supported for X, but {type(X)} was passed")
        if not isinstance(y, pd.Series) and not isinstance(y, np.ndarray) and not isinstance(y, list):
            raise Exception(f"Only pandas.Series or numpy.ndarray or list supported for y, but {type(y)} was passed")
        ...
        if eval_set is not None and len(eval_set) > 0:
            ...
            for idx, eval_pair in enumerate(eval_set):
                ...
                if not isinstance(eval_X, pd.DataFrame):
                    raise Exception(f"Only pandas.DataFrame supported for X in eval_set, but {type(eval_X)} was passed")
                if (
                    ...
                    raise Exception(
                        f"Only pandas.Series or numpy.ndarray or list supported for y in eval_set, \
                        but {type(eval_y)} was passed"
                    )
                eval_df = eval_X.copy()
    ...
    def fit_transform(
        ...
        **fit_params,
        ...
        if result_features is None:
            raise Exception("No provider supports feature enrichment")
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if self._search_task is None:
            raise Exception("`fit` or `fit_transform` should be called before `transform`")
        if not isinstance(X, pd.DataFrame):
            raise Exception(f"Only pandas.DataFrame supported for X, but {type(X)} was passed")
    ...
    def _prepare_search_keys(self, x: pd.DataFrame) -> Dict[str, FileColumnMeaningType]:
        ...
        for column_id, meaning_type in self.search_keys.items():
            if isinstance(column_id, str):
                ...
                raise Exception(f"Unsupported type of key in search_keys: {type(column_id)}")
            if meaning_type == SearchKey.CUSTOM_KEY:
                raise Exception("SearchKey.CUSTOM_KEY is not supported for FeaturesEnricher")
    ...
    def get_metrics(self) -> Optional[pd.DataFrame]:
        if self._search_task is None or self._search_task.summary is None:
            raise Exception("Run fit before get metrics")
