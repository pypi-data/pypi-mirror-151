...
class RegisterUser(QDialog):
    ...
    def save_data(self):
        ...
        if not self.client_name.text():
            self.messages.critical(self, 'Не указано имя пользователя.')
            return
        elif self.client_passwd.text() != self.client_conf.text():
            self.messages.critical(self, 'Введённые пароли не совпадают.')
            return
        elif self.database.check_user(self.client_name.text()):
            self.messages.critical(self, 'Пользователь уже существует.')
            return
        else:
