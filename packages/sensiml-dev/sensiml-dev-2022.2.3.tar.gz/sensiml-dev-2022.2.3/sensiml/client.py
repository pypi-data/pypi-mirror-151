...
def print_list(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if func.__name__ == "list_sandboxes":
            extra = [
                {"CPU Used (seconds)": item.last_cpu_clock_time}
                for name, item in result.items()
            ]
...
class Client(object):
    # Server Minimum Version For Compatibility
    ...
    def validate_client_version(self, skip_validate=True):
        ...
        if response_data["SensiML_Python_Library_Windows"] != self.__minversion__:
            print(
                "The SensiML Python SDK is out of date. Update before connecting to the server."
            )
            print(
                "In order to update the SensiML Python SDK use 'pip install sensiml -U'"
            )
            raise Exception("SensiML Python SDK is out of date.")
    ...
    def upload_project(self, name, dclprojpath, asynchronous=True):
        ...
        for capture in capture_files:
            print("Uploading Capture data, metadata, and labels for {}".format(capture))
            try:
