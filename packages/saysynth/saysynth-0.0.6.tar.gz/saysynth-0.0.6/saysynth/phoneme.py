...
def text_from_note(
    note: Union[int, str],  # accept a midi note or note name
    duration: float,
    velocity: int,
    phoneme: str,
    type: str = "note",
...
def drone_from_note(max_duration=1_000_000, phoneme_loops=None, **kwargs) -> str:
    """
    Generate drone text from `text_from_note` options
    """
    # set each phonemes duration to the max phoneme duration
    kwargs["duration"] = kwargs["phoneme_duration"]

    loops = 0
    duration = 0
    drone_text = f"{SAY_TUNE_TAG}\n"
    while True:
        drone_text += f"{text_from_note(**kwargs)}\n"
