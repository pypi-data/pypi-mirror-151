...
class E2EE():
    ...
    def encryptE2EEMessage(self, to, text, specVersion=2, isCompact=False, contentType=0):
        _from = self.mid
    ...
    def encryptE2EELocationMessage(self, senderKeyId, receiverKeyId, keyData, specVersion, location, to, _from, isCompact=False):
        salt = os.urandom(16)
        gcmKey = self.getSHA256Sum(keyData, salt, b'Key')
        aad = self.generateAAD(to, _from, senderKeyId,
                               receiverKeyId, specVersion, 15)
        sign = os.urandom(16)
        data = json.dumps({
            'location': location
        }).encode()
