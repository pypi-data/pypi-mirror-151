...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def autosegmentation(self):
        ...
        if self.imagePath != None:
            ...
            if self.imagePath.endswith((".png", ".tiff", "tif")):
                ...
                try:
                    FOV_m = int(self.currentModel.split("_")[-1])
                except:
                    FOV_m = 400
                # set FOV to model's FOV
                FOV = FOV_m

                FOV, ok = QtWidgets.QInputDialog().getInt(self,
                                                          "Please provide the field of view (FOV) diameter for this image (default: 500)",
                                                          "Field of View (µm)\n e.g. give '500' for a 500x500µm^2 FOV.",
                                                          FOV)
                if ok and FOV:
                    zoom_factor = (FOV_m/FOV)

                    self.minMaxNeuronValues = [i * (zoom_factor**2) for i in self.minMaxNeuronValues]

                    minArea, maxArea = self.minMaxNeuronValues

                    pred_main(im_path, model=self.currentModel, minimal_area_neuron_in_pixels=minArea,
                              maximal_area_neuron_in_pixels=maxArea, zoom_factor=zoom_factor)
    ...
    def loadFile(self, filename=None, justJSON=False):
    ...
    def start_CASCADE(self):
        ...
        # open dialog for frame rate
#        frameRate, ok = QtWidgets.QInputDialog().getDouble(self,
#                                                      "Provide FrameRate for experiment",
#                                                      "Frame Rate (FPS)",
#                                                      self.frameRate, 10.0, 50.0)
#        if ok and frameRate:
#            self.frameRate = frameRate
    ...
    def extractTraces(self, imagesArray):  # , frequency):
        #todo - check if it is save to assume self.filename as JSON file
        if self.filename:
            if self.filename.endswith(".json"):
                with open(self.filename) as f:
                    data = json.load(f)
        ...
        fps, ok = QtWidgets.QInputDialog().getDouble(self,
                                                     "Frames Per Second (FPS)",
                                                      "Please provide frames per second of the experiment for the baseline calculation",
                                                      30, 0, 500)
        if ok and fps:
            dff_calc(self.filename.replace(".json", "_traces.tsv"), fps)
            ...
            mbFormat = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, "Files Written",
                                             "No FPS were given. Only raw traces were written to {}.".format(
    ...
    def loadPolygons(self):
        ...
        if osp.exists(label_file):
            self.loadFile(label_file, justJSON=True)
    ...
    def refresh(self):
        ...
        # temporary change:
        #out.save(self.lastOpenDir + "/" + str(0) + "-" + str(len(imlist)) + "_mean.png")
        ...
        # temporary change:
        #self.imagePath = self.lastOpenDir + "/" + str(0) + "-" + str(len(imlist)) + "_mean.png"
