...
Veci_int ball_k_means_Ring(MatrixOur& dataset, MatrixOur& centroids, int ITER, bool detail) {
    ...
    if (detail == true) {
        ...
        cout << "Time per round:                     ||"
             <<  time_spend / iteration_counter << endl;
...
Veci_int ball_k_means_noRing(MatrixOur& dataset, MatrixOur& centroids, int ITER, bool detail) {
    ...
    if (detail == true) {
        ...
        cout << "The number of calculating distance: ||" << cal_dist_num << endl;
        cout << "The number of neighbors:            ||" << num_of_neighbour << endl;
        cout << "Time per round:                     ||" << time_spend / iteration_counter << endl;
...
inline MatrixOur initial_centroids(MatrixOur dataset, int k, int random_seed) {
...
Veci_int ball_k_means(MatrixOur& dataset, MatrixOur& centroids, int ITER, bool isRing, bool detail) {
    // MatrixOur centroids;
    // if (filename == "0") {
    ...
    // cout << "ITER = " << ITER << ", isRing = " << isRing << ", debug = " << detail << endl;

    Veci_int ret;

    if (isRing) {
        ret = ball_k_means_Ring(dataset, centroids, ITER, detail);
    }
    else {
        ret = ball_k_means_noRing(dataset, centroids, ITER, detail);
    }
    return ret;
}
...
//     double start_time, end_time;
//     start_time = clock();
//     MatrixOur dataset = load_data("/home/pei/BallKMeans/BKM_code/data/Epileptic.csv");
//     for (int i = 0; i < 5; i++){
//         for (int j = 0; j < dataset.row(i).size(); j++){
//             cout << dataset(i, j) << ", ";
//         }
//         cout << endl;
//     }
//     VectorXi labels = ball_k_means(dataset, 300, true, true);
//     end_time = clock();
